...
| C3 | View tab strip | `nav.snw-views[aria-label="Workspace views"]` (`:302`) | Static `VIEWS` constant (`:61`) | n/a | In-page `setView(...)`; `aria-current="page"` (`:308`) | `no harness` | `no spec` | `no harness` |

### Work view — one subview per phase state

| # | Row | RF | Data / read model | Empty or blocked state | Action destination | Desktop | Mobile | A11y |
|---|---|---|---|---|---|---|---|---|
| W1 | Current phase — "Confirm this request" / "Continue the governed event" | `section.snw-main-work` (`:317`) | `event.trigger`, `event.scope`, `event.decisionOwner` through `fact()` (`:100`) | `fact()` renders the literal **"Not recorded"** for a blank field — absence is shown, not hidden | Via W5 aside | `no harness` | `no spec` | `no harness` |
...
| W4 | Passed phase with a record — "Recorded earlier in this event" | `:398` | `phaseEvidence` returned true | n/a | "Viewing it does not mark it complete, approve any gate, or change the current stage." Conditional panels P1–P4 may render beneath | `no harness` | `no spec` | `no harness` |
...
| # | Row | RF | Data / read model | Empty or blocked state | Action destination | Desktop | Mobile | A11y |
|---|---|---|---|---|---|---|---|---|
| F1 | Files toolbar — search, "Older versions" | `.source-new-files__toolbar` (`:300`); search `input[aria-label="Search files"]` | Client-side over the `rows` prop | n/a | In-page state only | `no harness` | `no spec` | `no harness` |
| F2 | Folder rail | `nav[aria-label="File folders"]` (`:321`) | `PHASES` (`:77`); the `other` folder appears only when such files exist, and the comment at `:82` records why they are never hidden | n/a | In-page `setPhase` + clears selection; `aria-current="true"` | `no harness` | `no spec` | `no harness` |
...
| I3 | Industry reference requirements | `section[aria-label="Industry reference requirements"]` (`:1136`) | Conditional on the intelligence view carrying them | Section omitted when absent | none | `no harness` | `no spec` | `no harness` |

### Approvals view

| # | Row | RF | Data / read model | Empty or blocked state | Action destination | Desktop | Mobile | A11y |
|---|---|---|---|---|---|---|---|---|
| A1 | Governed decision panel | `section.snw-panel.snw-plain` (`:496`) | `awaitsIntakeReview(event.lifecycle)` | n/a | `Link` → approval or event href. Guardrail rendered: "This overview does not approve or advance anything… approving happens in the governed event flow" | `no harness` | `no spec` | `no harness` |
...
| # | Row | RF | Data / read model | Empty or blocked state | Action destination | Desktop | Mobile | A11y |
|---|---|---|---|---|---|---|---|---|
| D1 | aVa dock | `AgentDock surface="source/new-workspace"` (`:1260`, `surface` at `:1268`) | `useAtlasPageState()` conversation | Empty thread renders the dock with `initialQuote` only | `pageState?.ask(message)`; attachments are appended to the message text as `[Attached evidence: …]` | `no harness` | `no spec` — **mode switches to `collapsed` below 900px** (`:1242`), exercised by no spec | `no harness` |

---

## Rows that could not be filled, and why

Three of the seven columns are empty for every row. They look like three problems and are one:
**no harness in this repository can render a signed-in, tenant-scoped route.** The three sections
below separate what is genuinely missing from what already exists and is merely pointed elsewhere,
because that difference decides how much work closing each column actually is.

### Desktop: the capture harness exists and is used, but no spec points it here

A screenshot harness exists and is in active use — `tests/e2e/source/_audit-harness.ts`, consumed by
three Source specs, capturing before/after through its `step()` helper. It does not reach this
surface. Every `page.goto(...)` in `tests/e2e/source/*.spec.ts` targets
...
### Mobile: the device matrix exists and runs on every PR, but cannot reach a signed-in route

**Correction to an earlier revision of this document, which claimed no mobile project exists. It
does.** `playwright.browser-matrix.config.ts` declares five projects (`:37`), two of them real
device descriptors — `mobile-chrome` / Pixel 5 (`:41`) and `mobile-safari` / iPhone 13 (`:42`) — and
the `Chrome Firefox Safari mobile smoke` job runs it on every pull request
(`.github/workflows/browser-matrix-smoke.yml:12`). The earlier claim came from reading
`playwright.config.ts` alone and treating it as the repository's Playwright configuration. There are
three configs, and the default one is the smallest.

What is true, and it is a different and cheaper problem: **that harness does not reach this
surface, and in its present form cannot.**

- Its `testDir` is `./tests/browser-matrix` (`:14`), which holds one spec.
- That spec covers three public HTML routes — `/`, `/sign-in`, `/signed-out`
  (`public-surface-smoke.spec.ts:3`) — plus `/api/health`, `/robots.txt` and `/sitemap.xml`.
- It **captures no screenshot at all.** It asserts response status, a non-empty body, a measurable
  viewport, and no console or page errors. So even for the routes it covers, the screenshot cell
  would be empty.
- Its test-mode auth bypass cannot be pointed at a signed-in route. `ACCESSIBILITY_AXE_DISABLE_CLERK=1`
  is honoured only when the request is already public — `shouldBypassClerkForAxe` returns
  `env === "1" && isPublicRoute(request)` (`src/proxy.ts:294`). Setting it does not open
  `/source/new/{eventId}`; and even past the proxy, the page requires both an active client row and
  a tenancy that matches it (`[eventId]/page.tsx:37`).

*What would be needed:* not a runner configuration — that is already paid for. A way to render a
signed-in, tenant-scoped route under a harness, plus a spec that captures. The first half is the
same missing piece the desktop and accessibility rows need, which is why the three gaps below are
one root cause rather than three.

Two rows change behaviour below a breakpoint, so this is load-bearing rather than cosmetic:
...
- **F4** — at `max-width: 760px` the Files pane becomes a single-column push/pop flow. The detail
  ...
  (`:209`, `useLayoutEffect`). That is a distinct interaction model, with focus management, that
  exists only below the breakpoint.
...
### Accessibility: the axe harness covers two public routes, for the same reason

`tests/accessibility/public-axe.spec.ts` is the only axe harness in the repository, run by
`playwright.accessibility.config.ts` under the `Public axe accessibility` job
(`.github/workflows/accessibility-axe.yml:12`). Its route list is literally two entries — `/` and
`/sign-in` (`:4`) — tagged `wcag2a wcag2aa wcag21a wcag21aa`. Every row in this matrix is behind a
signed-in tenancy check, so **no row is covered.**

The name of that spec is the honest description of its scope, and the blocker is the same one the
mobile row hits: the Clerk bypass it depends on is public-route-only (`src/proxy.ts:294`). Extending
axe to Source New is not a matter of adding two strings to the route list.

Marked `no harness` rather than `owed to Anand` throughout, and the difference is deliberate: an
a11y verdict here is not waiting on someone to sign in and look, it is waiting on a way for any
harness to reach a signed-in route.
...
|---|---|---|
| No spec navigates to `/source/new/{eventId}`, in any config | R3 and all 23 rows inside it — **24 of 26** | Missing spec |
| The mobile device projects exist and run per PR, but their one spec is public-route-only and captures nothing | **26 of 26**; F4 and D1 materially | Missing spec + no capture |
| Axe covers two public routes; none is signed in | **26 of 26** | Missing spec |

**These are one gap, not three.** Each traces to the same root: **no harness in this repository can
render a signed-in, tenant-scoped route.** The device matrix is already configured and already runs;
the axe harness is already wired; both are fenced to public routes by the same
`isPublicRoute` condition on the test-mode Clerk bypass (`src/proxy.ts:294`). Build a signed-in
harness path once and all three columns become writable. That is a materially smaller and
better-targeted piece of work than the three separate harness build-outs an earlier revision of this
document implied.
...
| Playwright configs in the repository | `ls playwright*.config.ts` | **3** — default, browser-matrix, accessibility |
| Projects in the default config | `projects:` in `playwright.config.ts:14` | **1** — `chromium` / `Desktop Chrome` |
| Projects in the browser-matrix config, run on every PR | `projects:` in `playwright.browser-matrix.config.ts:37` | **5**, incl. `mobile-chrome` / Pixel 5 and `mobile-safari` / iPhone 13 |
| Specs in the browser-matrix `testDir` | `ls tests/browser-matrix/` | **1** — `public-surface-smoke.spec.ts` |
| HTML routes that spec covers | `htmlRoutes` at `public-surface-smoke.spec.ts:3` | **3**, all public — `/`, `/sign-in`, `/signed-out` |
| Screenshots that spec captures | `grep -c "screenshot" tests/browser-matrix/` | **0** |
...
| The test-mode Clerk bypass is public-route-only | `shouldBypassClerkForAxe` at `src/proxy.ts:294` | `env === "1" && isPublicRoute(request)` |
...
**A note on how the mobile rows were first got wrong**, because it bears on how to read the rest.
An earlier revision of this document reported "no mobile project exists", having read
`playwright.config.ts` and treated it as *the* Playwright configuration. There are three, and the
