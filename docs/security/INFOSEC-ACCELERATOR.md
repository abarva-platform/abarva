...
| CCC-01 | Change Control | Are all production changes managed via pull request with code review? | In place | GitHub PR-based workflow on `anandsundaram-hash/abarva`; merge requires green CI | Every cited PR (#1923–#1940) is publicly auditable in repo history. |
...
| EKM-02 | Encryption & Key Management | Are customer-managed keys (CMK / BYOK) supported? | Planned | Roadmap; default today is Microsoft-managed keys | In-VPC deployment (B4) gives customer full key custody by definition. Managed-SaaS BYOK is a Q+1 target. |
...
| STA-01 | Supply Chain | Is there a dependency-review process for third-party packages? | Planned | GitHub Dependabot enabled on `anandsundaram-hash/abarva`; Snyk evaluation pending | Honest gap. Adding automated SAST/DAST is a pre-production target. |
...
| Customer-managed keys (BYOK) in managed-SaaS | CAIQ EKM-02 | Microsoft-managed keys today. In-VPC option gives full key custody. | BYOK in managed SaaS as a Q+1 target. |
