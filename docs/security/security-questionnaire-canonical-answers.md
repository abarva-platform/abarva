...
- Describe Vercel as the shared SaaS control plane and Azure/Postgres as the
  data-plane adapter target. Do not describe Vercel as the client data plane.
...
| Control-plane hosting | The shared SaaS control plane is a Next.js application deployed through Vercel. It owns routing, rendering, Clerk sessions, previews, production deploys, release behavior, and orchestration. | In place | `vercel.ts`, `docs/deployment/migrations.md`, `docs/architecture/adr/ADR-0007-vercel-control-plane-posture.md` |
...
| Encryption in transit | Vercel-hosted control-plane TLS is platform inherited. Azure/Postgres code paths configure SSL for non-local Postgres connection strings unless SSL is explicitly disabled. | In place with hardening item | `docs/security/encryption-posture.md`, `src/lib/data-plane/postgresCompat.ts`, `src/lib/data-plane/read-adapters/azurePostgresReadAdapter.ts` |
...
| Business continuity | Vercel deployments are immutable and rollback-capable. Database PITR and backup posture depend on the selected data-plane deployment and require customer-lane evidence. | Partial / deployment-specific | `docs/pilot/SECURITY_POSTURE.md`, `docs/security/encryption-posture.md`, `docs/architecture/azure/AZURE-FULL-STACK-TEST-LAYERS.md` |
| Change management | Production changes are managed through GitHub PRs, CI, release records for release-relevant changes, and Vercel deployment checks. | In place | `AGENTS.md`, `docs/releases/RELEASE_CONTROL_POLICY.md`, `.github/workflows/` |
...
For shared SaaS pilots, the control plane is Vercel-hosted and client-scoped
persistence should route through data-plane adapters. For customer private
lanes, Azure is the target data plane and each customer lane requires its own
resource, network, key, retention, and smoke-test evidence pack.
