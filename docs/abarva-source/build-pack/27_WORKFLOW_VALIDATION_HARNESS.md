...
Artifact review validation asks: is this document ready for release, approval, lock, issue/publish, or rework?

All three are required before production.
...
- evaluates document readiness for review, approval, lock, release, reopen, and rework
- checks artifact version state, comments, approvals, evidence/citations, and waiver rules
- catches premature release, unsafe overwrite, and unsupported approval state

Source needs all three:

- context validation prevents generic advice
- workflow validation prevents unsafe execution
- artifact review validation prevents unreliable documents from becoming operational records
