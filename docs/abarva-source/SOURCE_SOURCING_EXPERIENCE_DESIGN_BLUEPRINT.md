...
Current Apex setup-data contract: Apex Retail now has a seeded enterprise setup layer that Source must consume before asking the user for company facts. The loaded layer includes 14 segment rollups, 403 data records, 257 graph nodes, 275 graph edges, 415 context chunks, 14 expected baselines, 14 audit log entries, and one ingestion run. Segment families include enterprise profile, org structure, IT landscape, KPI dictionary, program inventory, evidence ledger, vendor contracts, and cross-program signals. This means Source should not ask the user "who is your CIO?" if the signed-in tenant context exposes an org or executive bench record. It may ask for event-specific confirmation: "I see the CIO role in setup context; confirm whether this person is the decision owner for this AMS event." The difference matters. Setup data answers who the enterprise knows; Source still needs the sourcing event's trigger, owner confirmation, scope boundary, value target, and baseline owner.

Current Meridian setup-data contract: Meridian Health is the second flagship tenant and raises the same rule at larger depth. The loaded layer includes 30 source files across 14 data families, 698 structured records, 423 graph nodes, 584 graph edges, 715 context chunks, 14 baselines, 14 audit rows, and one ingestion run. Segment families include enterprise profile, org structure, IT landscape, IT financials, KPI dictionary, active programs, sourcing artifacts, program deliverables, evidence ledger, operating telemetry, vendor contracts, compliance, industry context, and cross-program signals. Source should not ask a Meridian user to recreate known health-system leadership, Epic/HealthEdge/Innovaccer system context, program inventory, contract posture, or evidence history. It should ask for the event-specific motion: which technology sourcing event, which decision owner, which scope boundary, which value target, which baseline owner, and which constraints or waivers apply.

Loaded is not embedded. The latest Apex setup-load report describes 415 context chunks as retrieval-ready with embedding still pending. Source can disclose and use deterministic setup facts passed through the setup data broker or Source context bundle, but it must not claim vector search, semantic neighborhood retrieval, or embedding-backed recall until embeddings are created, exposed through a runtime field or index, and tenant filters are proven. The right product language is: "Apex setup evidence is loaded for Source; embeddings are pending or not yet available to this surface; this response uses deterministic setup context." The wrong product language is: "I searched all Apex enterprise knowledge" when the only available basis is seeded records, graph rows, and pending chunks.

Source consumption model for setup data:
- Segment rollups drive the data readiness summary, coverage, stale count, missing count, and health posture shown on `/source`, `/source/events`, `/source/new`, and event canvases.
- Data records drive row-level context: systems, KPIs, programs, risks, evidence, vendors, contracts, and cross-program signals that can appear in tables, drawers, and context-used chips.
- Graph nodes and edges drive deterministic relationship context, such as `SPONSORED_BY`, `OWNED_BY`, `LICENSED_FROM`, `MEASURED_FROM`, and `HAS_EXECUTIVE`, but not graph reasoning claims beyond the stored edge set.
- Context chunks drive future retrieval and current provenance previews only when exposed by the context bundle; until embedded, they are not vector-searchable.
- Expected baselines define what complete/fresh looks like per segment family and should feed Steward readiness language.
- Audit log and ingestion run rows support provenance: when data was loaded, by which run, and which segment the row came from.
...
- Source consumes Apex and Meridian setup facts before asking for enterprise facts, but still asks for event-specific confirmation and boundaries.
- Source explicitly labels setup chunks as not vector-searchable when embeddings are pending or unavailable to the surface, and does not imply vector retrieval until embeddings and tenant-filter tests are live.
...
Apex setup-data volumetrics are part of the storyline and should be visible when relevant: 14 segment rollups, 403 tenant records, 257 graph nodes, 275 graph edges, 415 retrieval-ready context chunks reported as not yet embedded or not yet available for vector retrieval, 14 expected baselines, 14 import audit entries, and one ingestion run. The user experience should make these numbers useful, not decorative. A Source intake page can say, "Apex setup evidence is loaded: org structure, IT landscape, KPI dictionary, programs, evidence ledger, vendor contracts, and cross-program signals. I will not ask you to recreate those. I need five event-specific facts to stand up this AMS sourcing event." An event canvas can show the setup context strip near the stage workspace: "Apex setup context loaded; vector retrieval not active; deterministic context only." A drawer can show which segment families fed the current recommendation.

Concrete Apex records should shape the demo. The SAP S/4HANA system record, owned by Diana Lopez and licensed from SAP, can support AMS scope, application criticality, renewal posture, data sensitivity, and run-cost questions. KPI dictionary rows such as revenue sourced from SAP S/4HANA and owned by James Wright can support value context without claiming that sourcing savings are realized. Program inventory rows such as CDP Activation 2026, sponsored by Jennifer Park and led by Priya Iyer, can support linked-program and cross-program-load warnings. Evidence ledger rows such as the identity match-rate claim can support confidence and caveat behavior only when the evidence state is usable for the current claim. Cross-program signals such as Priya Iyer leading two critical-path programs should appear as staffing and dependency cautions, not as generic "risk" prose.
...
What is demo-ready: deterministic dashboard/event canvas, seeded event posture, setup-data disclosure, scope/data readiness panels, RFP readiness, vendor response completeness, pricing normalization, BAFO panel, executive decision summary, vendor selection readiness panel, stage gate panel, artifact strip, and value ledger shell where present. What remains seeded or thin: persistence beyond the loaded setup layer, live upload/parsing, full approval engine, live model responses, live vector embeddings, artifact versioning, document export/import, and live market benchmarks. What not to claim: production-ready sourcing engine, live procurement write-back, final vendor selection automation, realized savings, citable uploaded documents unless evidence state supports it, vector retrieval while context chunks are pending, or live benchmark coverage.

Meridian Health should be treated as the second flagship tenant, not as a small sample. It demonstrates how the same Source design handles integrated delivery network and health-plan technology sourcing. Meridian's loaded substrate includes 698 records, 423 nodes, 584 edges, and 715 context chunks. Example Source-relevant facts include four active programs, Epic touched by all four programs, Cohere linked to both DENIALS-2024 and prior authorization automation, Patricia Okafor overloaded across prior-auth and RCM modernization, Wexler involved in three of four active programs, and vendor contract/compliance data that is rich enough for readiness posture but still must be checked for evidence usability. The demo should show Meridian's context as tenant-specific and healthcare-specific, not as a generic replay of Apex.

Pinecone role in the product architecture: Pinecone should become the semantic memory layer over tenant context chunks, not the source of truth. Postgres remains the record and provenance layer. Graph remains the relationship and multi-hop context layer. Pinecone retrieves semantically relevant chunks when the user asks fuzzy questions such as "Why is prior auth risky?" or "What should we ask the CIO?" The LLM synthesizes only after the broker assembles Postgres facts, graph paths, Pinecone hits, evidence warnings, and allowed actions into a typed context bundle. The visible product moment is a Context Assembled panel: Postgres facts, graph paths, semantic hits, evidence warnings, and answer guardrails shown beside the answer. Until Pinecone embeddings and tenant filters are live, Source should show deterministic setup context and should not claim semantic memory.
...
- Meridian storyline uses healthcare-specific tenant facts honestly and does not collapse provider-plan complexity into generic sourcing copy.
- Demo-ready and seeded/thin areas are separated.
- The story does not claim production capabilities that are not implemented.
- Apex setup-data volumetrics and pending embedding state are disclosed wherever the demo relies on loaded enterprise context.
- Pinecone is positioned as semantic retrieval over chunks, not as the system of record, and remains a next-layer capability until embeddings and tenant filters are proven.
...
- Production claims, live benchmarks, approval engine, upload/parsing, vector retrieval, and model calls stay out of scope.
