...
|---|---:|---:|---|---|---|
| Lakeshore | Buyer brief + detailed reading page + MP4 ready for review; live app proof refreshed after PR #3187 deploy | 99.5% | `docs/build/lakeshore-success-brief/LAKESHORE_AI_SUCCESS_PLATFORM_DETAILED_READING_2026-06-05.html` | `docs/build/lakeshore-proof/live-walkthrough/post-3181-authenticated/post-3187-qa/app-demo-readiness.md` | Final human listen/read-through before sending externally; Azure/private-plane-specific health endpoints still need proof. |
...
| Post-3187 Source/Moves retrieval QA | `docs/build/lakeshore-proof/live-walkthrough/post-3181-authenticated/post-3187-qa/source-moves-retrieval.md` | Latest deployed Source/Moves retrieval proof after PR #3187 deploy: 8 checks, 8 pass, 0 fail when the retrieval harness uses broker client scope `lakeshore-holdings` for Move attachment APIs. |
...
- Live post-3187 deployed Source/Moves retrieval: 8/8 checks pass on `dpl_HRG1qvhL63nvVqzpsJU5kfnN9b4x` when using active client `lakeshore-holdings` for broker-scoped Move attachment APIs. A control rerun with active client `lakeshore` returned 5/8 because Move attachment list routes are broker-tenant gated.
