...
|---|---:|---:|---|---|---|
| Lakeshore | Buyer brief + detailed reading page + MP4 ready for review; live app proof refreshed after PR #3190 deploy and Azure private-plane proof refreshed after PR #3193 deploy | 99.8% | `docs/build/lakeshore-success-brief/LAKESHORE_AI_SUCCESS_PLATFORM_DETAILED_READING_2026-06-05.html` | `docs/build/lakeshore-proof/live-walkthrough/post-3181-authenticated/post-3193-azure-private-plane/azure-connectivity-smoke.md` | Final human listen/read-through before sending externally; public Vercel operator health route still needs token/env provisioning if we want route-level positive proof from `app.abarva.ai`. |
...
| Detailed MP4 V3 render | `docs/build/lakeshore-success-brief/video-storyboard/detailed-render/LAKESHORE_AI_SUCCESS_PLATFORM_DETAILED_V3.mp4` | Current detailed MP4: 10:45, 1600x900, H.264/AAC, no zoom, correct nav logo, product screenshots, expanded context graph, 90-day AI strategy, IT/data modernization, captured-interaction QA proof, six-month AI strategy/use-case roadmap, and regenerated OpenAI voiceover with AbarVa pronunciation guidance. |
...
| Post-3187 app readiness QA | `docs/build/lakeshore-proof/live-walkthrough/post-3181-authenticated/post-3187-qa/app-demo-readiness.md` | Latest deployed production app readiness after PR #3187 deploy `dpl_HRG1qvhL63nvVqzpsJU5kfnN9b4x`: 26 checks, 26 pass, 0 watch, 0 fail across Admin, Setup, Source, Moves, Tower, and Intelligence. |
| Post-3187 Source/Moves retrieval QA | `docs/build/lakeshore-proof/live-walkthrough/post-3181-authenticated/post-3187-qa/source-moves-retrieval.md` | Historical PR #3187 retrieval proof: 8 checks, 8 pass, 0 fail. Superseded by post-3190 proof using active client `lakeshore`, the correct app-client key for locked Lakeshore personas. |
| Post-3190 app readiness QA | `docs/build/lakeshore-proof/live-walkthrough/post-3181-authenticated/post-3190-qa/app-demo-readiness.md` | Latest deployed production app readiness after PR #3190 deploy `dpl_95PtNH1wp3XXbBY94BYkpGWjjRJr`: 26 checks, 26 pass, 0 watch, 0 fail across Admin, Setup, Source, Moves, Tower, and Intelligence. A first rerun hit Clerk 429 on admin/setup routes; delayed rerun cleared all checks. |
| Post-3190 Source/Moves retrieval QA | `docs/build/lakeshore-proof/live-walkthrough/post-3181-authenticated/post-3190-qa/source-moves-retrieval.md` | Latest deployed Source/Moves retrieval proof after PR #3190 deploy: 8 checks, 8 pass, 0 fail with active client `lakeshore`, which is the correct app-client key for locked Lakeshore personas. |
| Post-3193 Azure private-plane connectivity proof | `docs/build/lakeshore-proof/live-walkthrough/post-3181-authenticated/post-3193-azure-private-plane/azure-connectivity-smoke.md` | Fresh Azure Container Apps Job execution `job-azure-connectivity-smoke-eus-fti8q63` succeeded on 2026-06-06: Postgres, Blob, Service Bus, Key Vault, and Azure AI Search all passed from the private lab runtime lane. |
...
- Live post-3187 deployed app readiness: 26/26 checks pass on `dpl_HRG1qvhL63nvVqzpsJU5kfnN9b4x`, aliased to `https://app.abarva.ai`.
- Live post-3187 deployed Source/Moves retrieval: 8/8 checks pass on `dpl_HRG1qvhL63nvVqzpsJU5kfnN9b4x`; this historical note is superseded by the post-3190 rerun that confirms `lakeshore` is the correct active app-client key for locked Lakeshore personas.
- Live post-3190 deployed app readiness: 26/26 checks pass on `dpl_95PtNH1wp3XXbBY94BYkpGWjjRJr`, aliased to `https://app.abarva.ai`.
- Live post-3190 deployed Source/Moves retrieval: 8/8 checks pass on `dpl_95PtNH1wp3XXbBY94BYkpGWjjRJr` using active client `lakeshore`. The earlier post-3190 admin/setup 500s were Clerk 429 rate-limit noise during a too-tight auth/crawl burst; focused delayed checks returned 200 for all five admin/setup routes.
- Live post-3193 proof-index deploy: PR #3193 merged with all 19 checks passing and deployed as `dpl_G2NWxCwBseLMAmTP3YFhfb7Gw9qS`, aliased to `https://app.abarva.ai`.
- Current Azure private-plane positive proof: Container Apps Job execution `job-azure-connectivity-smoke-eus-fti8q63` succeeded on 2026-06-06 with pass results for Postgres, Blob Storage, Service Bus, Key Vault, and Azure AI Search.
...
- Azure/private-plane truth boundary: the main `/api/health` endpoint is green for Postgres/direct Postgres, and the Azure Container Apps private-plane smoke is green. The public Vercel `/api/health/azure-connectivity` and `/api/health/postgres-disruption` routes still return guarded 404 without operator auth/env; keep Azure below 100% until route-level operator proof is provisioned or explicitly waived.
