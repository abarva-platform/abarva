...
| Source / Outsourcing | `/source`, `/source/events/[eventId]` | Nexus / Sentinel / Atlas / Steward | `production-readiness.json` component `source` | `src/components/source/AbarVaSourceDashboard.tsx`, `src/components/source/SentinelEngagementCanvas.tsx`, `src/components/source/SourceDataReadinessPanel.tsx`, `src/lib/source/nexus-api.ts`, `src/lib/source/context-builder.ts`, `src/lib/source/agent-validation-report.ts`, `src/lib/source/workflow-validation-report.ts`, `src/lib/source/agent-missions.ts`, `src/lib/source/agent-mission-report.ts` | `docs/abarva-source/SOURCE_PRODUCTION_READINESS_TRACKER.md`, `docs/abarva-source/SOURCE_LAYERED_PROGRESS_TRACKER.md`, Source pattern packs, Source implementation reviews | No production upload/evidence pipeline, no model-assisted Source Nexus runtime, no full workflow engine, no production persistence, production-domain visual QA incomplete | Use the unified manifest as the Source readiness spine; local Source trackers are supporting detail only |
