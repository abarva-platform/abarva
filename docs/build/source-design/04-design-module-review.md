...
**Companion artifacts:** `05-wireframe-atlas.html` (all 19 specs in one map), `06-strategy-screen.html` and `07-executive-decision-screen.html` (full-fidelity pattern setters)
...
**Partial-scope exception (added per Anand's feedback):** The CXO Report HTML and Deal Pack are read on phones today (CXOs open them in email previews mid-meeting). Spec 15 (Exports) adds to acceptance: *"HTML exports render legibly at 375px width."* CSS responsive sweep only — not full mobile redesign.
...
// Wireframe reference: 05-wireframe-atlas.html §1 Decision Queue
```
...
// Open with Codex before ship: no
// Wireframe reference: 05-wireframe-atlas.html §2 Intake footer
```
...
// Wireframe reference: 05-wireframe-atlas.html §3 Approval page
```
...
// Wireframe reference: state machine in 02-end-to-end-wireframes.html Part 1
```
...
// Wireframe reference: 06-strategy-screen.html (full-fidelity pattern setter)
```
...
// Open with Codex before ship: no
// Wireframe reference: 06-strategy-screen.html shows the 30% rail in context
```
...
// Wireframe reference: 06-strategy-screen.html and 07-executive-decision-screen.html both reflect humanized labels
```
...
// Wireframe reference: 06-strategy-screen.html (the canonical screen — match pixel-for-discipline, not pixel-for-pixel)
```
...
// Open with Codex before ship: no
// Wireframe reference: 05-wireframe-atlas.html §7 Composed stages (Scope/RFP card)
```
...
// Open with Codex before ship: no
// Wireframe reference: 05-wireframe-atlas.html §7 Composed stages (Responses/Eval card)
```
...
// Wireframe reference: 05-wireframe-atlas.html §4 Pricing/BAFO
```
...
//   - Dark charcoal header: KEEP — but enforce "only dark moment in the lifecycle" (cross-spec Q6). Document this in 03-build-specs.html Part 3 (Design Tokens) as a usage constraint, not just prose buried here.
...
//   - Squint test is the acceptance gate here above all. Build to 07-executive-decision-screen.html fidelity.
// Open with Codex before ship: yes
// Wireframe reference: 07-executive-decision-screen.html (full-fidelity pattern setter)
```
...
// Open with Codex before ship: no
// Wireframe reference: 05-wireframe-atlas.html §5 Transition
```
...
// Open with Codex before ship: no
// Wireframe reference: 05-wireframe-atlas.html §7 Composed stages (Value card)
```
...
//   - PARTIAL MOBILE (per Anand): HTML exports render legibly at 375px width. CSS responsive sweep only — not full mobile redesign.
// Open with Codex before ship: no
// Wireframe reference: target structure in 02-end-to-end-wireframes.html Part 16
```
...
// Open with Codex before ship: no
// Wireframe reference: 05-wireframe-atlas.html §6 Evidence drawer + Attention bell
```
...
//   - SELF-ACTION FLAG (per Anand): when actor of a gate_met action equals the event creator, render the same amber "Self-approval notice" + tag the audit log row 'self_action'. Same risk surfaces at gate-met as at intake approval.
// Open with Codex before ship: no
// Wireframe reference: 05-wireframe-atlas.html §7 Composed stages (Audit card)
```
...
// Wireframe reference: 02-end-to-end-wireframes.html Part 18 (renewal loop)
```
...
3. **Spec 12 + Q6 dark-header rule** — keep the dark Exec Decision header, but document "only dark moment in the lifecycle" in Part 3 Design Tokens as a usage constraint, not just prose.
...
**Wave 1 PR sequencing (per Anand):**
- **Phase 1 (parallel):** Spec 4 (routing guard) · Spec 2 (intake footer) · Spec 7 (humanization sweep)
- **Phase 2 (after Spec 4 merges):** Spec 3 (approval page) — depends on the routing guard existing

**Next from the design module:** mock the remaining load-bearing screens — **Executive Decision page-1** (Spec 12 → `07-executive-decision-screen.html`), the **Approval page** (Spec 3), and **Pricing/BAFO** (Spec 11) — so each high-risk spec has a target to build against, not just prose. The atlas (`05-wireframe-atlas.html`) is the single map; the two full-fidelity screens are the fidelity bar.
