...
- Follow-up: kept governed confidence values on one line so `not established` reads as a single status value.
...
- FOLLOW-UP: Production signed-in Chrome proof also identified narrow confidence-cell wrapping in Top contracts and a fixed launcher overlap on action controls; this candidate includes both visual polish items plus single-line confidence values.
