...
The current correction keeps the frozen package unchanged and fixes the blocked Layer 1 execution substrate: the Meridian Health loader now targets only `foundation_v2_meridian_health_demo`, uses Meridian Health-specific writer/reader roles, requires the frozen source release ID, writes 54 source-file routing metadata rows and rejects local/non-ACA apply even if the old bypass env is present.
The latest operator-readiness patch adds an exact Meridian Health schema/RLS migration runner and lets the source-volume loader consume the approved proof ZIP by URL inside ACA, with SHA-256 verification before extraction.
The continuous lab execution update adds isolated Layer 2 adapter/candidate staging for the loaded Meridian Health source volume. It stages normalized source-record objects and review candidates only; it does not publish canonical objects, activate baselines, refresh Cube or bind product runtime surfaces.
The latest lab execution pass applies and independently verifies Layer 2 inside `foundation_v2_meridian_health_demo`: 54,967 normalized objects, 54,967 knowledge candidates, three adapter gates and 1,640,131 field dispositions now reconcile exactly to the verified Layer 1 source volume.
This update also adds the governed expert narrative and architecture quality gate to the Meridian Health Phase B execution plan. Home, architecture, Source, Intelligence, Moves and aVa prose must wait until the data, relationships, findings, evidence and reconciliations pass; accepted artifacts must come from the shared advisory-packet path with deterministic lint, independent critique, revision and validation.
...
The latest Layer 3 lab execution applies and independently verifies canonical promotion inside `foundation_v2_meridian_health_demo`: 54,967 promotion decisions resolve into 794 canonical master entities, 47,941 transactional observations, 2,390 relationships, 16,000 evidence records and 4,370 event-native records, with exact match and no defects.
...
The Layer 5 continuation builds and proves a private internal Cube canary for the Meridian Health demo tenant and the existing synthetic airline tenant. It verifies Postgres/Cube reconciliation, dimensions, measures, hierarchy, drill members, missing-tenant rejection, tenant isolation and zero regression for the airline baseline without shifting shared Cube traffic.
...
Layer 1 client intake lane: creates source-owner-shaped workbooks and source-system-shaped synthetic extracts for audit. Layer 2 adapter lane: stages normalized objects and knowledge candidates. Layer 3 canonical lane: promotes verified candidates into the isolated Meridian Health canonical-promotion substrate only. Layer 4 projection lane: publishes typed isolated projections for product consumption. Layer 5 semantic lane: proves a private Cube canary. Layer 6 product-binding lane: binds product modules to typed projections and Cube proof without shared traffic shift.
The latest update adds an executable Layer 1 source-volume loader with `self-test`, `plan`, `preflight`, `apply` and `verify` modes. It remains gated: plan and self-test are local only, while apply requires an approved proof SHA, exact Meridian Health schema/tenant/namespace/release/role contract and an ACA data-build job context. Application, CMDB, vendor and contract rows are tenant enterprise-context candidates; the sourcing event references selected entity IDs and later pins an immutable event-context snapshot through a separate governed action.
The continuous lab execution update adds Layer 2 migration, self-test, preflight, apply and verify commands. Layer 2 writes only isolated normalized-object and knowledge-candidate staging rows in `foundation_v2_meridian_health_demo`; canonical promotion and product projections remain out of scope.
...
The current Layer 3 update remains in the isolated Meridian Health schema. It applies and verifies the Meridian Health-only canonical-promotion substrate, but does not publish shared canonical objects or activate a tenant.
...
- `scripts/source/build-meridian-health-demo-package.mjs`
- `scripts/source/validate-meridian-health-demo-package.mjs`
- `scripts/source/plan-meridian-health-demo-data-layers.mjs`
- `scripts/foundation-v2/load-meridian-health-demo-source-volume-db.mjs`
- `scripts/foundation-v2/apply-meridian-health-demo-schema.mjs`
- `scripts/foundation-v2/apply-meridian-health-demo-layer2-schema.mjs`
- `scripts/foundation-v2/normalize-meridian-health-demo-source-volume-db.mjs`
- `scripts/foundation-v2/apply-meridian-health-demo-layer3-schema.mjs`
- `scripts/foundation-v2/promote-meridian-health-demo-canonical-db.mjs`
- `scripts/foundation-v2/apply-meridian-health-demo-layer4-schema.mjs`
- `scripts/foundation-v2/project-meridian-health-demo-consumption-db.mjs`
- `scripts/foundation-v2/apply-meridian-health-demo-layer5-cube-canary-schema.mjs`
- `scripts/source/verify-meridian-health-demo-cube-canary.mjs`
- `scripts/foundation-v2/bind-meridian-health-demo-layer6-products.mjs`
- `cube/model/meridian_health_demo.yml`
- `cube/cube.py`
- `Dockerfile.cube`
- `scripts/source/fixtures/meridian-health-demo/canary-defects.json`
- `docs/source/MERIDIAN_HEALTH_DEMO_PHASE_A_PACKAGE.md`
- `docs/source/MERIDIAN_HEALTH_DEMO_MODEL_FIT_AUDIT.md`
- `docs/source/MERIDIAN_HEALTH_DEMO_PHASE_B_TEST_LOAD_PLAN.md`
- `docs/source/MERIDIAN_HEALTH_DEMO_ONE_CLICK_MIGRATION_SPEC.md`
- `supabase/migrations/20260805230000_foundation_v2_meridian_health_demo_source_volume.sql`
- `supabase/migrations/20260806002000_foundation_v2_meridian_health_demo_adapter_candidates.sql`
- `supabase/migrations/20260806041000_foundation_v2_meridian_health_demo_canonical_promotion.sql`
- `supabase/migrations/20260806125500_foundation_v2_meridian_health_demo_layer3_repair_delete_grants.sql`
- `supabase/migrations/20260806132000_foundation_v2_meridian_health_demo_layer4_projections.sql`
- `supabase/migrations/20260806161000_foundation_v2_meridian_health_demo_layer6_product_bindings.sql`
- `supabase/migrations/20260806162500_foundation_v2_meridian_health_demo_layer6_canary_read_grants.sql`
- `supabase/migrations/20260806165500_foundation_v2_meridian_health_demo_layer6_gate_update_grant.sql`
- `package.json` npm scripts

## QA / Validation

Passed: `node --check scripts/source/build-meridian-health-demo-package.mjs && node --check scripts/source/validate-meridian-health-demo-package.mjs`.
Passed: `node scripts/source/build-meridian-health-demo-package.mjs --out-dir /Users/anand/Downloads`.
Passed: `node scripts/source/validate-meridian-health-demo-package.mjs --package-dir /Users/anand/Downloads/meridian_health_demo_phase_a_20260805T223224Z`.
Passed: `unzip -t /Users/anand/Downloads/Meridian_Health_Demo_Audit_Proof_20260805T223224Z.zip`.
...
Passed: SHA-256 attestation matches `/Users/anand/Downloads/Meridian_Health_Demo_Audit_Proof_20260805T223224Z.zip`.
...
Passed: `node --check scripts/source/plan-meridian-health-demo-data-layers.mjs`.
Passed: `npm run source:meridian-health-demo:data-layer-plan -- --package-dir /Users/anand/Downloads/meridian_health_demo_phase_a_20260805T223224Z --out-dir /Users/anand/Downloads`.
Passed: data-layer plan ZIP SHA-256 attestation matches `/Users/anand/Downloads/Meridian_Health_Demo_Data_Layer_Plan_20260805T230818Z.zip`.
Passed: `node --check scripts/foundation-v2/load-meridian-health-demo-source-volume-db.mjs`.
...
Passed: negative local apply check still rejected execution without `ACA_JOB_NAME` when the removed `MERIDIAN_HEALTH_DEMO_ALLOW_NON_ACA_APPLY=true` bypass env was present.
Passed: ACA Layer 1 schema apply, source-volume preflight, apply and independent reader verify against `foundation_v2_meridian_health_demo`; final readback matched 54 source files, 54 source-file context rows, 54,967 source records and 1,640,131 source field values.
Passed: `node --check scripts/foundation-v2/apply-meridian-health-demo-layer2-schema.mjs && node --check scripts/foundation-v2/normalize-meridian-health-demo-source-volume-db.mjs`.
Passed: `npm run source:meridian-health-demo:layer2:self-test -- --out-dir /tmp/meridian-health-layer2-self-test`.
Passed: ACA Layer 2 schema apply against `foundation_v2_meridian_health_demo`; migration `20260806002000_foundation_v2_meridian_health_demo_adapter_candidates.sql` created the isolated candidate-staging tables with forced RLS and no data rows.
...
Passed: ACA Layer 2 apply through the approved data-build job path; proof status `MERIDIAN_HEALTH_DEMO_NORMALIZATION_VERIFIED` recorded 54,967 normalized objects, 54,967 knowledge candidates, three adapter gates, `exact_match=true` and no broken transition.
Passed: ACA Layer 2 independent reader verify; proof status `MERIDIAN_HEALTH_DEMO_NORMALIZATION_VERIFIED` repeated the same counts with `mutation_executed=false`.
Passed: Meridian Health Phase B plan updated to require the shared governed advisory packet, module-boundary rules, role-specific lenses, generic-language control, critique/revision loop and side-by-side claim-to-evidence artifact audit before any Home, architecture, Source, Intelligence, Moves or aVa narrative is accepted.
Passed: `node --check scripts/foundation-v2/apply-meridian-health-demo-layer3-schema.mjs && node --check scripts/foundation-v2/promote-meridian-health-demo-canonical-db.mjs`.
Passed: `npm run source:meridian-health-demo:layer3:self-test -- --out-dir /tmp/meridian-health-layer3-self-test`.
...
Passed: ACA Layer 3 schema apply against `foundation_v2_meridian_health_demo`; migration `20260806041000_foundation_v2_meridian_health_demo_canonical_promotion.sql` created the isolated canonical-promotion tables with forced RLS and no Layer 3 rows.
...
Passed: `node --check scripts/foundation-v2/apply-meridian-health-demo-layer4-schema.mjs && node --check scripts/foundation-v2/project-meridian-health-demo-consumption-db.mjs`.
...
Passed: `node --check scripts/foundation-v2/apply-meridian-health-demo-layer5-cube-canary-schema.mjs && node --check scripts/source/verify-meridian-health-demo-cube-canary.mjs`.
Passed: `npm run source:meridian-health-demo:cube-canary:model`; model proof recorded 18 cubes, 66 measures, 18 hierarchies and no failures.
Passed: private internal Cube canary runtime verification returned health 200, missing-tenant rejection 403, tenant-isolation cross-check counts of 0, 18 typed Meridian Health canary tables reconciled to Postgres, drill-member readback succeeded and the airline regression set matched expected counts and amounts.
Passed: `node --check scripts/foundation-v2/bind-meridian-health-demo-layer6-products.mjs`.
Passed: `npm run source:meridian-health-demo:layer6:self-test -- --out-dir /tmp/meridian-health-layer6-self-test-scan-fix`.
...
Revert the generator, validator, fixtures, migrations, scripts, Cube canary files, docs and npm scripts. Generated Downloads artifacts are local audit outputs. Private-lab data rollback requires an approved tenant-scoped operator action against only the isolated Meridian Health schemas and canary resources; do not run shared-runtime rollback for this record because no shared runtime traffic was changed.
...
Layer 2 ACA proof paths: `/Users/anand/Downloads/meridian-health-demo-layer2-aca-20260806T003650Z/20-layer2-preflight-after-lock-cleanup/proof/MERIDIAN_HEALTH_NORMALIZATION_CANDIDATES.json`, `/Users/anand/Downloads/meridian-health-demo-layer2-aca-20260806T003650Z/21-layer2-apply-after-lock-cleanup/proof/MERIDIAN_HEALTH_NORMALIZATION_CANDIDATES.json`, `/Users/anand/Downloads/meridian-health-demo-layer2-aca-20260806T003650Z/22-layer2-verify-after-lock-cleanup/proof/MERIDIAN_HEALTH_NORMALIZATION_CANDIDATES.json`.
Layer 3 local self-test proof path: `/tmp/meridian-health-layer3-self-test/proof/MERIDIAN_HEALTH_CANONICAL_PROMOTION_SELF_TEST.json`.
...
Layer 3 ACA proof paths: `/Users/anand/Downloads/meridian-health-demo-layer3-aca-20260806T113500Z/01-layer3-schema-apply/proof/MERIDIAN_HEALTH_DEMO_LAYER3_SCHEMA_PROOF.json`, `/Users/anand/Downloads/meridian-health-demo-layer3-aca-20260806T113500Z/04-layer3-preflight-after-stopped-apply/proof/MERIDIAN_HEALTH_CANONICAL_PROMOTION.json`, `/Users/anand/Downloads/meridian-health-demo-layer3-aca-20260806T113500Z/05-layer3-apply-optimized/proof/MERIDIAN_HEALTH_CANONICAL_PROMOTION.json`, `/Users/anand/Downloads/meridian-health-demo-layer3-aca-20260806T113500Z/06-layer3-verify/proof/MERIDIAN_HEALTH_CANONICAL_PROMOTION.json`.
Layer 3 ACA proof ZIP: `/Users/anand/Downloads/Meridian_Health_Demo_Layer3_ACA_Proof_20260806T122300Z.zip`.
Layer 3 ACA proof ZIP SHA-256: `83934b64a809b153ccc52e25812d4c0c84d6561c08bc8d801cfd760637bee47f`.
Source-to-Layer 3 reconciliation packet: `/Users/anand/Downloads/Meridian_Health_Demo_Source_Reconciliation_20260806T123700Z.zip`.
Source-to-Layer 3 reconciliation packet SHA-256: `67f9fc4df82f56c4cf7affbb3d638e3bb00bda0ddd8f67cb52117e88489c39e0`.
Layer 3 initiative repair proof ZIP: `/Users/anand/Downloads/Meridian_Health_Demo_Layer3_Initiative_Repair_ACA_Proof_20260806T131100Z.zip`.
...
Layer 4 proof ZIP: `/Users/anand/Downloads/Meridian_Health_Demo_Layer4_ACA_Proof_20260806T141800Z.zip`.
...
Final Cube canary runtime proof returned `MERIDIAN_HEALTH_DEMO_CUBE_CANARY_VERIFIED`, 18 cubes, 18 hierarchies, 66 measures, health 200, missing-tenant rejection 403, zero tenant-isolation cross-read counts and zero SkyHarbor reconciliation failures.
Final Layer 6 apply and independent verify both returned `MERIDIAN_HEALTH_DEMO_LAYER6_PRODUCT_BINDING_VERIFIED`, `exact_match=true`, `structural_exact_match=true`, `semantic_plausibility_pass=true`, `narrative_quality_pass=true`, `unsupported_claim_hits=0`, zero cross-tenant defects, zero SkyHarbor regression defects and zero defects.
...
The governed artifact set is the final six requested Meridian Health executive artifacts: Enterprise Current-State Brief, Data & Analytics Current-State Assessment, Technology Architecture Current-State Assessment, Procurement & Vendor Landscape Brief, BPO Sourcing Decision Brief and AWS / Databricks Decision Brief. The latest correction blocks identical critic-score vectors, requires a human executive-read result, keeps internal mechanics out of the model-visible and executive prose surfaces, and records artifact-specific critique, zero unsupported claims and zero contradictions.

### Meridian Boundary Repair Addendum
...
- Passed: `node --check scripts/source/build-meridian-health-demo-package.mjs && node --check scripts/foundation-v2/project-meridian-health-demo-consumption-db.mjs && node --check scripts/foundation-v2/apply-meridian-health-demo-layer5-cube-canary-schema.mjs && node --check scripts/foundation-v2/bind-meridian-health-demo-layer6-products.mjs && node --check scripts/source/verify-meridian-health-demo-cube-canary.mjs`.
- Passed: `npx eslint scripts/foundation-v2/project-meridian-health-demo-consumption-db.mjs scripts/foundation-v2/apply-meridian-health-demo-layer5-cube-canary-schema.mjs scripts/foundation-v2/bind-meridian-health-demo-layer6-products.mjs scripts/source/verify-meridian-health-demo-cube-canary.mjs scripts/source/build-meridian-health-demo-package.mjs`.
- Passed: `npm run source:meridian-health-demo:layer4:self-test -- --out-dir /tmp/meridian-layer4-self-test`.
- Passed: `npm run source:meridian-health-demo:layer5:self-test -- --out-dir /tmp/meridian-layer5-self-test`.
- Passed: `npm run source:meridian-health-demo:cube-canary:model -- --out-dir /tmp/meridian-cube-model-self-test`; model proof now records 18 cubes, 73 measures, 18 hierarchies and no failures after explicit BPO economics and workforce-denominator measures were added.
