...
Chart collision guard replaces plotted initiative-name annotations with numbered chart markers and a compact key below the value/readiness plot. Full initiative names remain in the key, tooltip, accessibility label, funding sequence, and detail cards, avoiding dense-label overlap in real-world executive charts.
...
- `src/components/intelligence-v2/IntelligenceV2Surface.tsx`: replaces value/readiness plotted text labels with numbered markers and a full-name chart key to reduce real-world overlap risk without losing initiative names.
