...
- Follow-up correction: the Tower Claude story timeout is raised to 120 seconds after live ACA proof showed the bounded Sonnet JSON response still exceeded the previous 45-second guardrail.
...
- Partial live proof after model-latency correction: ACA served the Tower Claude code path using `claude-sonnet-4-6`, but the DOM marker still showed `claude_fallback` because the response exceeded the 45-second Tower story guardrail (`tower_cxo_claude_story_timeout`).
- Not run yet: signed-in ACA browser proof after the egress artifact-id, request-parameter, model-latency, and timeout corrections.
...
Candidate is not yet deployed or browser-proven after the egress artifact-id, request-parameter, model-latency, and timeout corrections. This PR does not migrate additional Tower routes, change value calculations, or promote any data.
