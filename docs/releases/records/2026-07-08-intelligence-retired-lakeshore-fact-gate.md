...
- Azure-only access-check repair and model-output callback type alignment for CI readiness
...
- `scripts/check-access.mjs`: Removes stale Supabase/Pinecone live-check residue, restores parsing, and keeps live access checks focused on GitHub CLI plus Azure Container Apps.
...
| Touched Intelligence typecheck scan | `(npx tsc --noEmit --pretty false 2>&1 || true) \| rg "src/app/api/intelligence/ask/route.ts\|src/lib/intelligence/ask/index.ts" \|\| true` | Local repo | No remaining typecheck errors in touched Intelligence ask files. | Passed; no matching typecheck errors after fixing GET `answerOnlyStreaming`. Repo-wide typecheck still has unrelated pre-existing errors in Source, Atlas, context ingestion, and other areas. | `PASS` | Console output from local run. |
...
| Access check and ask callback lint | `node --check scripts/check-access.mjs && npx eslint scripts/check-access.mjs src/lib/intelligence/ask/index.ts --quiet` | Local repo | Access helper parses/lints and ask callback types lint cleanly. | Passed. | `PASS` | Console output from local run. |
