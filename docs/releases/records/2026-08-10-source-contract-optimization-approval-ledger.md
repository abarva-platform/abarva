...
- `src/lib/source/source-event-shell-v2.ts` normalizes current-stage approval items through the resolved journey before rendering the per-event approvals workspace, so skipped competitive-sourcing stages cannot leak into contract-optimization approval copy.
