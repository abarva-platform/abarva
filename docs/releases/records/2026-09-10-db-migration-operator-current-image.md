...
The private operator wrapper now restores the operator job to the same digest-pinned image used for the execution unless a caller explicitly provides another idle image. Operator job update calls also retry bounded ACA provisioning conflicts instead of failing on transient platform lag. The governed lab migration workflow runs the exact Moves current-state schema verifier after applying migrations, so the run reports table-level readiness for the upload/review path instead of relying only on a broad database summary.

## Layer Impact

Release lane: `internal-admin`.

Operations layer: hardens the private operator job restore and retry behavior used by governed migration and verification runs.
...
- PASS: `ruby -e "require 'yaml'; YAML.load_file('.github/workflows/db-migration-lab.yml'); puts 'yaml ok'"`
...
- Worker image invariant: operator restore now preserves the execution digest by default and records update retry attempts.
...
- Operator wrapper plan-only test output, including update retry settings.
