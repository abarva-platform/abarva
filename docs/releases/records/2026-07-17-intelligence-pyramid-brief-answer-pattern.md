...
- `src/app/api/intelligence/ask/route.ts`: Preserves leading/trailing whitespace on streamed display chunks after public-answer scrubbing so adjacent chunks do not glue words together.
...
- `src/app/api/intelligence/ask/__tests__/route.telemetry.test.ts`: Adds regression coverage for streamed chunk-boundary spacing.
...
- Second deployed pass on SHA `2d7698f0166f32add35e4ec56488b16cb779eb0e` fixed the table/follow-up behavior but exposed a live streamed-display spacing defect (`work.Payment`, `pilot.The`, `Day 1.Decision`). This record keeps that as release evidence for the third-pass spacing hardening.
- Second-pass local validation passed:
  - `npx jest src/app/api/intelligence/ask/__tests__/route.telemetry.test.ts src/lib/intelligence/ask/response-policy.test.ts src/lib/intelligence/ask/__tests__/strategy-to-moves-contract.test.ts src/lib/intelligence/ask/__tests__/decision-table-gate.test.ts src/lib/agent/product-truth/__tests__/runtime-guard.test.ts --runInBand`: 5 suites, 45 tests.
  - `npx eslint src/app/api/intelligence/ask/route.ts src/app/api/intelligence/ask/__tests__/route.telemetry.test.ts src/lib/intelligence/ask/response-policy.ts src/lib/intelligence/ask/synthesizer.ts src/lib/intelligence/ask/response-policy.test.ts src/lib/intelligence/ask/__tests__/decision-table-gate.test.ts src/lib/agent/product-truth/runtime-guard.ts src/lib/agent/product-truth/__tests__/runtime-guard.test.ts` passed with one pre-existing warning in the route telemetry test fixture.
