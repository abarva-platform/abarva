...
Scoring lane: the verifier still caps broad data-unavailable admissions, but no longer fails an answer that has SkyHarbor tenant sources, concrete identifiers, and a narrow "field to confirm" caveat.
...
- `scripts/skyharbor/stages/07_verify/ground_truth_runner.mjs` adds a `partial_detail_gap` scoring distinction for grounded answers with narrow missing sub-fields.
