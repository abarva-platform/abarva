...
Sixth follow-up live proof used a freshly minted `agent-lakeshore` Clerk state and confirmed the Source shell was signed in, but the Lakeshore event page and aVa ask route still returned `not_found`. Private ACA/VNet inspection showed the parent `source_events` row exists under client key `lakeshore`, while artifacts are tenant-keyed as `lakeshore-holdings`; the automation agent is a tenant-rostered proof user with a viewer membership, not a participant on the specific event. This release now grants known same-tenant agent roster logins Source proof scope for their own tenant alias family only, preserving the cross-tenant fence while allowing signed-in browser proof to read same-tenant Source events.
...
- `src/lib/auth/source-access-policy.ts`: grants known same-tenant Clerk agent roster logins all-client Source proof scope for their own tenant alias family only.
...
- `src/lib/auth/__tests__/source-access-policy.test.ts`: regression proving same-tenant agent roster Source access is allowed and cross-tenant agent access remains denied.
