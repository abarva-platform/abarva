...
- `src/lib/deliverables/golden-bar.ts`: golden-bar exhibit detection now enriches detected visuals/tables from the per-artifact visual contract using normalized required-exhibit phrase matching.
- `src/lib/deliverables/__tests__/golden-bar.test.ts`: regression coverage for live-style P0 output with value tree, stakeholder map, KPI scorecard, scope boundary table, and proceed/hold/stop gate sections.
...
- `npx jest src/lib/deliverables/__tests__/golden-bar.test.ts --runInBand` — pass, 5 tests.
- `npx eslint src/lib/deliverables/golden-bar.ts src/lib/deliverables/__tests__/golden-bar.test.ts` — pass.
...
- Post-deploy diagnosis after the live-schema fix showed generation no longer 500s; P0 reached the golden-bar evaluator and returned `golden_bar_failed` because the evaluator did not recognize required contract exhibits rendered as titled sections and tables.

## Rollout Plan

Merge to `main`; the repo-owned `aca-main-deploy` workflow builds and deploys the Azure Container Apps image. After deploy, create a fresh Meridian Move, approve the P0 brief through the live UI, confirm an approved phase-0 snapshot exists, then generate the P0/P1 artifacts. For the golden-bar detector follow-up, retry live P0 generation on the Meridian proof Move to confirm it no longer fails solely on missing exhibit detection.
...
- Focused Jest output for `golden-bar.test.ts`.
...
- Live P0 generation proof on Move `2dbed99d-cca5-4f80-978a-c1175cc1714f` showing the post-schema-fix failure advanced from HTTP 500 to HTTP 422 `golden_bar_failed`, isolating the remaining issue to exhibit detection.
