...
This release adds a static audit package for existing Moves, Source, and Tower persisted objects. It inventories module tables and code-referenced objects, classifies each item as operational, canonical-promotion candidate, shared-consumption projection, archive, or replacement candidate, and documents the target integration pattern for later controlled migration planning.
...
- Internal audit tooling: adds repeatable scripts that generate CSV, XLSX, JSON, and Markdown audit outputs from repository migrations and focused module code references.
