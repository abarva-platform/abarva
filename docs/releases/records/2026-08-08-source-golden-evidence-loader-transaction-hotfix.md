# 2026-08-08 Source Golden Evidence Loader Transaction Hotfix
...
The golden contract evidence loader now creates optional helper indexes only when
the loaded source table contains the indexed column. This keeps a non-applicable
index from aborting the whole PostgreSQL transaction during an operator load.
Operator failures also print structured PostgreSQL diagnostic fields.

## Layer Impact

- Release lane: `client-data-lane`.
- Client intake: No change.
- Source adapters: The contract evidence package loader is safer for flexible
  source tables that do not all share the same columns.
- Canonical model: No schema or semantic model change.
- Products: No direct UI change; Contract 360 continues to consume the same
  loaded evidence when the data job succeeds.

## Client Applicability

- All clients: Loader behavior is tenant-agnostic.
- Specific clients: None.
- Internal only: Operator diagnostics are internal.
- Public/demo only: None.
- Feature flag: None.

## Changes Included

- PR: `https://github.com/abarva-platform/abarva/pull/6043`
...
guards optional index creation and improves error logging.
