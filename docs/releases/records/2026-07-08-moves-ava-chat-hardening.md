...
- ~~**Blocking discovery from live proof: `programId` does not appear to reach this chat route...**~~ **RESOLVED** by `docs/releases/records/2026-07-08-moves-ava-chat-programid-fix.md` (PR #4599, merged and deployed). Root cause: `StrategicMovePhaseClient.tsx` sent the Move id under `surfaceContext.moveId`, but `canonicalizeSurface`/`canonicalizeFromBody` only checked `surfaceContext.programId`. Live re-proof (`proof/moves-ava-chat-programid-fix-live-2026-07-08/README.md`) confirms all 6 specified prompts now ground correctly in the active Move — real gate-criterion IDs, real evidence ledger, appropriate Source/Tower mentions, no workflow-bypass claims, zero cross-program contamination.
