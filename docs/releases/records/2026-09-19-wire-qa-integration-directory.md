...
- PASS: the wired command runs 31 suites and 763 assertions green.
...
- PASS: the existing directory-coverage behavior suite passes 13 of 13 with the new wiring, so the directory now counts as reached.
...
**The 763 assertions this makes visible are not new coverage; they are coverage that existed and was never collected.** Saying otherwise would be the overstating this work exists to refuse. The 6 excluded suites run nowhere, and their passing assertions are not counted either.
