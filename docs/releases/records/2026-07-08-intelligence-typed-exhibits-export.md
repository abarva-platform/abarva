...
Second follow-up hardening adds the CXO answer-quality contract above the typed exhibit renderer. Intelligence now classifies answer mode, blocks model-deflection language, warns on missing requested visuals/evidence/caveats/next moves, stamps the validated packet with CXO quality metadata, and exposes HTML/PDF export actions in the shared answer renderer.
...
- Governance: Keeps rendering behind the existing validated answer packet; export does not re-query Claude or use model prose scraping as the source of truth. Adds deterministic CXO quality findings to the packet validation path.
...
- Adds compact HTML/PDF export actions to the shared `AgentAnswerRenderer`.
- Adds the CXO answer-quality contract to Intelligence prompt policy: direct executive read, 3-5 grounded insights, evidence/caveats, next moves, and typed table/chart/graph/matrix output when the question calls for it.
- Adds deterministic CXO packet scoring and mode classification: `direct_fact`, `strategy_insight`, `industry_trend`, `tenant_diagnosis`, `investment_case`, `operating_model`, `sourcing_decision`, `risk_control`, `roadmap`, and `portfolio_comparison`.
- Blocks final-answer model deflection such as telling users to ask Claude/ChatGPT/an LLM; warnings identify missing visuals, evidence boundary, caveats, or next moves without misclassifying those as tenant-safety failures.
- Adds focused tests for routing, structured exhibit extraction, agent answer rendering/export UI, HTML export, response policy, and CXO quality validation.

## QA / Validation

- Pass: `npx jest src/lib/ava-answer/__tests__/cxo-quality-gate.test.ts src/lib/intelligence/ask/response-policy.test.ts src/components/agent-answer/__tests__/AgentAnswerRenderer.test.tsx src/lib/ava-answer/export/__tests__/render-answer-html.test.ts src/lib/intelligence/answer/__tests__/structured-exhibits.test.ts src/lib/intelligence/answer/__tests__/router.test.ts --runInBand` — 6 suites, 63 tests passed. Jest still prints the repo's pre-existing duplicate manual mock warnings.
- Pass: `npx eslint <changed source and test files>` — no findings.
- Pass: Focused TypeScript check over touched production files with a temporary `noEmit` tsconfig — no findings.
...
- Blocked: repo-wide `npx tsc --noEmit --pretty false` remains blocked by the existing repo-wide TypeScript baseline outside this PR's changed files. The separate baseline TypeScript repair lane remains the owner for repo-wide TS health; this release does not claim repo-wide TS green.
...
- PR URL: https://github.com/abarva-platform/abarva/pull/4582
- Focused test output: 6 suites / 63 tests passed.
