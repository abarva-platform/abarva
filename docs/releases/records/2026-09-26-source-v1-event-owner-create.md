...
- API and agent event creation use the tenant-selected participant writer; the agent refuses a missing client context instead of falling back to a generic tenant key.
...
- Pass: API, adapter, and agent creation suites (53 tests); focused event-scope authorization regression.
