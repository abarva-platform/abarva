...
- API and agent event creation use the tenant-selected participant writer; the agent refuses a missing client context instead of falling back to a generic tenant key, and both surfaces require a canonical person ID that the access policy can read back.
...
- Pass: API, adapter, and agent creation suites (55 tests); focused event-scope authorization regression.
