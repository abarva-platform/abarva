...
The live schema preflight also found the setup relationship graph and `data_ingestion_runs` ledger tables absent in the product database. This release includes an additive compatibility migration so the loader can write the same graph and ledger evidence that the repo expects.
...
4. Run `npm run db:seed:northstar-clinical-setup-data:postgres` inside the private VNet.
5. Run `npm run scb:truth-gates -- --require-live` inside the private VNet.
6. Capture counts proving Northstar has `enterprise_context_records`, graph rows, ingestion-run evidence, and no embedded chunks missing `embedding_vector`.
...
- Live VNet migration/apply: pending.
