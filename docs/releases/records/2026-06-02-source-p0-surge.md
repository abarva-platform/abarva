...
This release closes the highest-severity Source findings from the rigorous L6 audit. Source AI artifact generation now sends the resolved event UUID into the AI egress preflight instead of the human-readable slug, preventing UUID parse failures on Apex AMS events. Source exports now fail with structured JSON fallback responses instead of opaque platform 503s, the Strategy Memo is wired through DOCX/HTML/PDF export routing, approval decisions require a rationale before either approve or reject, and the Source Decision Queue collapses duplicate vendor-contract records into one row per real vendor/product contract.
...
- PASS — `npx jest src/components/admin/__tests__/ApprovalDecisionPanel.test.tsx src/lib/source/decision-queue/__tests__/projection.test.ts src/lib/source/exports/__tests__/format-router.test.ts src/lib/source/exports/__tests__/dispatch.test.ts --runInBand` — 4 suites / 64 tests.
