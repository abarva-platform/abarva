...
This release hardens tenancy resolution so only real UUID-shaped person ids are passed as database actor ids. Legacy display-name values now fall through to the existing Clerk-backed actor path.
...
- `src/lib/auth/tenancy.ts`: validates `person.id` and `user.personId` before treating either value as a database UUID actor id.
- `src/app/api/v1/programs/__tests__/_auth.test.ts`: adds regression coverage for a legacy display-name person id.
