...
- **Schema layer:** Adds `context_explorer_answer_audit` with tenant-key RLS.
...
- **API layer:** Adds `POST /api/intelligence/qa` as an NDJSON streaming route.
- **UI layer:** Updates `SentinelExplorerRail` to call the route and render the grounded chip from live route metadata.
