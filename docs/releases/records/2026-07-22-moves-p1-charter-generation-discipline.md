...
The Moves P1 Charter generator is being tightened so a Charter remains a concise approval instrument for funding discovery and design. Live FS Demo proof showed the generator could produce an 11,374-word P1 Charter, which the quality gate correctly blocked as too long. Later fixes reduced the issue but did not close it: a 3,000-word ceiling run still produced 4,045 words, and the first 1,600-word ceiling deployment still produced 2,724 words. After section-level brevity deployed, the Charter stopped failing on length and exposed a synthesis completeness gap instead: missing recommendation and risk/dependency table. This release fixes both live paths: per-section word budgets/token discipline plus deterministic conservative synthesis fallbacks.
...
- `src/lib/deliverables/orchestrator/section-generation.ts`: adds conservative fallback recommendation and risk/issues/dependencies table assembly for concise Moves Charters when the synthesis model omits required fields.
...
- Pass: `npx jest src/lib/ai/__tests__/document-generation-policy.test.ts src/lib/deliverables/orchestrator/__tests__/generation-plan.test.ts src/lib/deliverables/orchestrator/__tests__/quality-bar-registry.test.ts src/lib/deliverables/orchestrator/__tests__/orchestrator.test.ts --runInBand` — 56/56 tests passed before the synthesis fallback patch.
- Pass: `npx jest src/lib/deliverables/orchestrator/__tests__/section-generation.test.ts src/lib/ai/__tests__/document-generation-policy.test.ts src/lib/deliverables/orchestrator/__tests__/orchestrator.test.ts --runInBand` — 49/49 tests passed after the synthesis fallback patch.
- Pass: `npx eslint src/lib/deliverables/orchestrator/section-generation.ts src/lib/deliverables/orchestrator/__tests__/section-generation.test.ts`.
...
- Live blocker found after section-level brevity deployment: FS Demo sandbox run `5b31cd54-0426-4643-8128-3b8a5a9473e5` generated exactly 7 sections and retrieved 12 evidence items, no longer failed on length, and then blocked on `no clear recommendation` plus `no risk/issues/dependencies table`. The quality gate worked; the synthesis fields needed deterministic fallbacks.
...
- Follow-up live proof after section-level brevity deployment: FS Demo sandbox P1 generation run `5b31cd54-0426-4643-8128-3b8a5a9473e5` produced 7 sections, retrieved 12 evidence items, and blocked on missing synthesis artifacts rather than length.
- PR URL: Pending.
