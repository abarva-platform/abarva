...
- `scripts/ops/purge-retired-tenant-residue.mjs`: focused final-residue purge for the five audited direct `tenant_key` tables, with explicit retired-key predicates, per-table expression indexes, chunk-level progress output, dry-run/apply modes, budget controls, and before/after proof.
