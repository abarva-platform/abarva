...
- `src/components/source/canvas/analytics/SourceAnalyticsCanvas.tsx` adds an active-step "What Continue needs" strip, compact "Run this step" guide card, Intelligence-workspace readiness brief, and a Files-workspace file-use readiness map with gate/evidence role, parse, search, graph, readiness, and next-action states.
...
- `src/components/source/canvas/analytics/__tests__/SourceAnalyticsCanvas.chat.test.tsx` covers the active-step requirement strip, Intelligence-workspace readiness brief, Files-workspace readiness map, client-readable parse/search/graph labels, next-action copy, and current workspace navigation label.
...
- This release does not implement long-form proposal parsing, vendor-isolated citation extraction, automated scoring math, or fully automated negotiation optimization. It makes the active-step requirement, active-step guide, Intelligence-workspace readiness, package readiness, file readiness, uploaded-file use readiness, produced intelligence, evidence use, missing inputs, negotiation leverage, executive decision posture, forward movement, and scoring gate honest in the workflow while those deeper capabilities are implemented.
