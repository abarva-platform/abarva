...
- This is Slice 1 of the larger Home Dimension Visualization Contract effort. Not yet built: the Enterprise Brief landing's six-visual-question rebuild, and the Context Horizon ghost-node concept.
- The relationship/topology graph is built from already-loaded evidenced fields (apps/infra.integrations, vendors.linked_systems, data.systems), not the V6 graph substrate. Wiring V6 `graph_nodes`/`graph_edges` in shadow mode per the standing architecture rule remains a real follow-on (it would give tenant-scoped, quality-scored edges instead of ad hoc field-parsing), but was not a prerequisite for closing the immediate gap — the underlying data for that gap was already present and unparsed.
