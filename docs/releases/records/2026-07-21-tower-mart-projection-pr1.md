...
Addressed before merge, from the PR #5262 review:

- **RLS on `cio_tower.tool_identity_aliases`**: the migration now enables row-level security with `svc_all` (service_role full) + `auth_read` fenced to the caller's own tenant via `can_read_tenant_by_key(tenant_key)` — stricter than the mart tables' permissive read, because this is tenant configuration data. One client can never read another's tool→program mappings.
- **Fail-closed on unresolved `client_id`**: the `--write` path now hard-aborts (both in the CLI and defensively in the write module) if the tenant does not resolve to a `clients.id`, instead of silently skipping `ai_control_refresh_runs` tracking. An untracked mutating build is exactly what the ACA job rule forbids.
- **Meridian alias seed** (`20260721161000_..._meridian_tool_aliases_seed.sql`): seeds the tool→program crosswalk so real coding-assistant telemetry (GitHub Copilot/Codex, Claude Code, Cursor) rolls up to `PROG-DEV-PRODUCTIVITY`, with forward-ready `abarva_proposed` rows for M365 Copilot / ServiceNow / Workday programs whose telemetry sources are not yet ingested. A regression test locks the seed's `canonical_program_key` slugs to `programKeyFromCode()`.

Still deferred (genuinely cannot be done from the dev shell — VNet / operator only): the governed ACA write job, Azure volumetrics export, and the signed-in Tower browser proof. **Tower is NOT claimed live-correct until those three are complete.**
