...
This change removes a Home ECL narrative cleanup path that replaced visible chapter text with a generated terminal-state headline. The operator job now keeps generated chapter prose intact after identifier scrubbing, treats raw object IDs as uppercase machine identifiers, narrows projection vocabulary checks to implementation context, and lets the visible-quality gate refuse any remaining unsupported or implementation-facing language.
...
- PASS: projection detector smoke confirms ordinary business projection language is not blocked while implementation-context projection terms still are
