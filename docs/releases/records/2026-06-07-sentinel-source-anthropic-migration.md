...
Completes the Sentinel Ask / Source provider cutover by moving the production
answer paths and Ask utility model calls that were still on OpenAI onto the
...
- **Sentinel Ask utility calls** (intent classification and follow-up
  suggestions) — were OpenAI `gpt-4o-mini`; now Claude Haiku via the same
  audited runtime.
- **Source Sentinel chat** — was OpenAI direct; now the audited Claude client.

Nexus already used Claude (unchanged). Other OpenAI-dependent surfaces outside
Sentinel Ask / Source chat remain out of scope for this release record.

## Layer Impact

- Lane: `global-control-lane`. Changes the LLM provider for Sentinel Ask
  synthesis, Sentinel Ask utility model calls, and Source chat for all tenants.
...
- All clients: yes — Sentinel Ask answers, Ask model-assisted utility calls, and
  Source chat answers are now Claude-generated.
...
- `src/lib/intelligence/ask/classifier.ts`: intent classification now uses the
  Anthropic small-model runtime.
- `src/lib/intelligence/ask/followups.ts`: model-assisted follow-up suggestions
  now use the Anthropic small-model runtime; deterministic concise follow-ups are
  unchanged.
...
- Tests: `provider-audit.test.ts` (now asserts Sentinel/Source on Anthropic,
  Sentinel Ask utility calls on Anthropic, Sentinel Ask audit identity payload,
  and Anthropic preflight provider/route fields), `openai-runtime-contract.test.ts`
  (now asserts Ask utilities are Anthropic),
...
passes. Requires `ANTHROPIC_API_KEY` in the target runtime for the Sentinel Ask
and Source chat provider check. Ticket-mint QA from an agent VM requires
...
The legacy `OPENAI_API_KEY` is no longer used by Sentinel Ask or Source chat paths
in scope.

## Rollback Plan

Revert the PR (restores the OpenAI synthesis/utility runtime + Source OpenAI
path). No data/schema change. Per-path revert is possible (`synthesizer.ts`,
`classifier.ts`, `followups.ts`, and `sentinel-chat-llm.ts` are independent).

## Audit Evidence

- `provider-audit.test.ts` wiring and audit-envelope assertions; jest outputs
  on the branch.
- `ai_egress_audit` rows for `intelligence-ask-synthesis`,
  `intelligence-ask-intent-classifier`, `intelligence-ask-followups`, and
