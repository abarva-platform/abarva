...
- Follow-up: `scripts/tenant-canonical-cleanup.ts` temporarily disables only the known Responsible AI immutable-ledger maintenance triggers while canonicalizing their denormalized `client_key` values, then re-enables them inside the same transaction.
- Follow-up: `scripts/tenant-canonical-cleanup.ts` discovers live semantic2 dossier invalidation triggers, disables them transactionally during the bulk tenant-key rewrite, and invalidates `semantic2_dossiers` once per affected canonical tenant before commit to avoid row-by-row timeout failures.
...
- pass: seventh dry-run succeeded inside the private VNet against image digest `sha256:ba923957804a4d1304ed4844913ada32d1d26402db467189ae906418e85bafb7`, auditing 212 active tenant columns and reporting 132,351 alias rows plus 456 duplicate alias rows before apply.
- pass: second apply attempt failed safely inside a transaction before commit on `responsible_ai_acknowledgments.client_key` because the live operator role is not allowed through that table's immutable-ledger trigger; the cleanup now uses a scoped maintenance-trigger override for the Responsible AI ledger tables only.
- pass: eighth dry-run succeeded inside the private VNet against image digest `sha256:71b52b5def003bcd2c257d9c96a204945866b942dccecf824cfbbb55c087dc34`, auditing 212 active tenant columns and reporting 132,351 alias rows plus 456 duplicate alias rows before apply.
- pass: third apply attempt progressed past Responsible AI ledger canonicalization and failed safely before commit when semantic2 row-level dossier invalidation repeatedly updated `semantic2_dossiers` until statement timeout; the cleanup now disables those live invalidation triggers during the bulk rewrite and performs one explicit semantic2 dossier invalidation by canonical tenant.
