...
| 7 | Azure AI Foundry / Azure OpenAI | Lets Azure-native enterprises consume model services through their approved cloud lane. | Claude/LLM routing through approved Foundry/provider lane where available. |
| 8 | Defender for Cloud / Microsoft Sentinel | SOC/SecOps readiness. | Add when budgets and logs are stable. |
| 9 | Azure Policy | Prevents drift and enforces private-data-lane posture. | Required before first customer VPC/private subscription. |
| 10 | Data Factory / Fabric | Better for mature Day-2 ingestion than pilot loading. | Defer; Service Bus/Event Grid + worker is enough first. |
...
5. Can we route model calls through an enterprise-approved lane?
