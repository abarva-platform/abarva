...
**Current state.** AZLAB40 adds the first L9 operator drill: produce a malformed ingestion message, run the A2b worker once, then verify the message is in the Service Bus dead-letter subqueue with a non-empty worker rejection reason. AZLAB50 adds the mixed good+poison batch path; the remaining L9 work is live execution, LLM overload/fallback, Postgres disruption, and PITR restore timing.
