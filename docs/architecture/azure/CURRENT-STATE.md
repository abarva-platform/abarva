...
| Agent quality corpus | JSONL corpus + validator + GitHub Actions workflow | `tests/agent-quality/golden/*.jsonl`, `npm run qa:agent-quality:corpus`, `.github/workflows/agent-quality-corpus.yml` | L7 deterministic corpus contract: 50 golden/adversarial prompts across Sentinel, Atlas, Nexus, Source, and Steward with required terms, forbidden terms, tenant/citation/dissent expectations, and coverage validation. |
...
| 7 | L7 live agent-quality runner | Agent APIs + corpus scorer | AZLAB37 wired the deterministic corpus contract. Next step is executing the 50 prompts against live/staging agents and scoring generated answers. |
