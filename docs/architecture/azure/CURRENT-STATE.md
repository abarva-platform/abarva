...
| Ingestion E2E smoke harness | Container Apps Jobs | `job-a2b-smoke-send-eus`, `job-a2b-smoke-verify-eus` | Synthetic producer/verifier jobs for the A2b lane. Passing run `azlab22-20260515133350` verified safe upload = `allow` and sensitive fake PHI/PII sample = `quarantine` in Azure Postgres. |
...
| 7 | Event Grid normalizer | Event Grid + Service Bus + Function/Job | AZLAB22 proved canonical queue messages. Raw BlobCreated events still need a normalizer before they can be treated as context updates. |
...
| Raw Event Grid messages are not canonicalized | Medium | AZLAB22 proved direct canonical queue messages. Raw storage events generated by Blob uploads were rejected by the worker, so add a normalizer or route raw events to a separate queue. |
