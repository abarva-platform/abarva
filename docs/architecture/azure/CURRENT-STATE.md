...
| 7 | L7 live agent-quality baseline run | Agent APIs + corpus scorer | AZLAB37 wired the deterministic corpus contract and AZLAB43 added the live runner/scorer. Next step is running the 50 prompts against Azure/prod with a short-lived authenticated cookie and storing the first scored baseline artifact. |
