# Architecture Decision Records

This directory stores durable architecture decisions. New ADRs should use `ADR-template.md`, keep facts tied to verified repo paths, and avoid recording aspirations as shipped implementation.

## Index

| ADR                                                                                                            | Title                                             | Status   | Date       |
| -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | -------- | ---------- |
| [0001-canonical-pattern-storage.md](0001-canonical-pattern-storage.md)                                         | Canonical Pattern Storage                         | Accepted | 2026-05-29 |
| [ADR-0001-control-plane-vs-data-plane.md](ADR-0001-control-plane-vs-data-plane.md)                             | Control Plane vs Data Plane                       | Accepted | 2026-06-01 |
| [ADR-0002-agent-context-broker-boundary.md](ADR-0002-agent-context-broker-boundary.md)                         | Agent Context Broker Boundary                     | Accepted | 2026-06-01 |
| [ADR-0003-release-lanes.md](ADR-0003-release-lanes.md)                                                         | Release Lanes                                     | Accepted | 2026-06-01 |
| [ADR-0004-per-user-rls.md](ADR-0004-per-user-rls.md)                                                           | Per-User RLS                                      | Accepted | 2026-06-01 |
| [ADR-0005-ai-tool-governance.md](ADR-0005-ai-tool-governance.md)                                               | AI Tool Governance                                | Accepted | 2026-06-01 |
| [ADR-0006-ai-as-advisor.md](ADR-0006-ai-as-advisor.md)                                                         | AI as Advisor, Never Decision-Maker               | Accepted | 2026-06-01 |
| [ADR-0007-vercel-control-plane-posture.md](ADR-0007-vercel-control-plane-posture.md)                           | Vercel Control Plane with Azure Client Data Plane | Accepted | 2026-06-01 |
| [ADR-0008-context-ingestion-guardrails.md](ADR-0008-context-ingestion-guardrails.md)                           | Context Ingestion Guardrails                      | Accepted | 2026-06-01 |
| [ADR-0009-release-environments-and-pilot-production.md](ADR-0009-release-environments-and-pilot-production.md) | Release Environments and Pilot Production         | Accepted | 2026-06-03 |
| [ADR-0010-software-delivery-ai-led-dev-corpus-wave.md](ADR-0010-software-delivery-ai-led-dev-corpus-wave.md)   | Software Delivery and AI-Led Dev Corpus Wave      | Accepted | 2026-06-03 |
| [ADR-0011-pdf-parser-routing-and-claude-fallback.md](ADR-0011-pdf-parser-routing-and-claude-fallback.md)       | PDF Parser Routing and Claude Fallback            | Accepted | 2026-06-03 |

Note: the legacy `0001-canonical-pattern-storage.md` predates this framework. The `ADR-0001-*` series starts the governance ADR set requested on 2026-06-01.
