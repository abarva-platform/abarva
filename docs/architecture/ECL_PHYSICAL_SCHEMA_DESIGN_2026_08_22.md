...
Commercial Compare update on 2026-08-23: the same projection table now admits `workspace_tab = compare` and `row_type = vendor_response_compare`. The commercial proof builder adds three client-owned response-pack extracts (`source_vendor_response_tracker.csv`, `source_pricing_response_lines.csv`, and `source_evaluation_scorecard.csv`), promotes three sourcing-review events, creates candidate vendor objects for non-incumbent bidders, and projects three open Compare rows for the weak contract proof case. Compare rankings come from submitted response and scorecard facts only; synthetic directional benchmarks are not allowed to rank vendors or recommend award. DB proof confirms 13 review events, 13 event-workspace rows, 5 Events rows, 5 Approvals rows, 3 Compare rows, 10 gated rows, 3 open rows, 0 review-event drift, and 0 candidate-vendor drift. Source page fact supply is now 14 of 14 rows. Product route repointing and browser QA remain closed gates.
