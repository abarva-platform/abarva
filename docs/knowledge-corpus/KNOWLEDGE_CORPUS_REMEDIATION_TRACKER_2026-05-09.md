...
| PR-C1 Corpus Validation Suite | Pending PR | `knowledge/corpus-validation-suite-2026-05-09` / `/tmp/nexus-kc-c1` | QA | `src/scripts/intelligence/validate-canonical-corpus.ts`; `src/scripts/intelligence/validate-canonical-corpus.test.ts`; `docs/knowledge-corpus/CANONICAL_CORPUS_VALIDATION_REPORT_2026-05-09.md`; `package.json`; tracker | `npm run intel:canonical-corpus:validate`; focused Jest; scoped ESLint; `npx tsc --noEmit --pretty false`; `npm run build`; CI pending | No DB writes. Validator reads generated dry-run preview JSON and does not mutate source or database content. | Current corpus fails strict quality gates by design: 2,297 error issues and 40 warning issues across 271 preview rows. Strict failure mode should be enabled after Wave 3 content remediation closes the known gaps. | PR-C2 Sample Retrieval QA and PR-C3 Source/Confidence Display | amber |
...
| Pattern QA validation scripts | In PR-C1 | Validator checks canonical fields, KPI minimums, data-domain minimums, failure modes, mitigations, artifacts, workshops, provenance, phase coverage, and unsupported quantitative claim flags. | Merge PR-C1, then use C2 for sample retrieval QA and Wave 3 for content remediation. |
