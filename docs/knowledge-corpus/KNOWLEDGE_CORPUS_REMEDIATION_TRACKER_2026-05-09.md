...
- Canonical corpus backfill executor: In progress in PR-E1 / PR #1867; dry-run execution report shows 301 non-colliding rows eligible for persistence and 11 canonical-id collision groups blocking write mode.
- Remaining completion blockers: resolve canonical-id collisions, execute persisted canonical corpus backfill, remediate legacy strict-validator errors, make keyword fallback runtime-real when vector is unavailable, and add response-shape regression tests across surfaces.
...
| DB mutation/content migration | Partial | Additive persistence schema and deterministic dry-run preview exist. PR-E1 adds an idempotent dry-run-first write executor and execution report. No DB writes have been performed. | Resolve 11 canonical-id collisions, rerun dry-run with credentials for hash comparison, then execute write mode only after preview validates. |
...
9. Corpus-wide validation passes for required fields, KPI minimums, data requirements, artifacts/workshops, provenance, and unsupported quantitative claims. Current strict baseline after Wave 3 packs: `patterns=323 errors=1800 warnings=0`; the remaining errors are concentrated in legacy/generated/cross-industry rows, not the three new industry packs.
...
1. `knowledge/canonical-duplicate-resolution-2026-05-09`
   - Resolve the 11 canonical-id collisions and high-risk duplicate clusters from the crosswalk report.
   - Document keep/merge/deprecate decisions.
   - Rerun backfill dry-run until write mode is unblocked.
2. `knowledge/canonical-corpus-write-execution-2026-05-09`
...
3. `knowledge/canonical-corpus-quality-remediation-2026-05-09`
   - Close the 1,800 strict-validator errors in legacy/generated/cross-industry rows without duplicating the new industry packs.
   - Preserve source provenance and unsupported-claim flags rather than inventing citations or outcomes.
4. `knowledge/vector-unavailable-keyword-fallback-2026-05-09`
...
5. `knowledge/agent-response-shape-regression-2026-05-09`
