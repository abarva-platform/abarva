...
- All-agent training and response doctrine: In progress; shared runtime doctrine and Nexus phase training framework are in PR-D5.
...
| PR-D5 All-Agent Training Framework | Pending PR | `knowledge/all-agent-training-framework-2026-05-09` / `/tmp/nexus-kc-d4` | Nexus training / Agent runtime | `docs/design/agent-coordination/ALL_AGENT_KNOWLEDGE_GROUNDING_AND_RESPONSE_DOCTRINE_2026-05-09.md`; `docs/design/nexus/NEXUS_PHASE_TRAINING_FRAMEWORK_2026-05-09.md`; `src/lib/agent/all-agent-doctrine.ts`; `src/app/api/chat/agent/route.ts`; tests; tracker | Pending local and CI validation | No DB writes. Runtime prompt doctrine only; no data mutation. | Needs CI and merge. | Follow-up hardening: response-shape tests by surface and optional UI display refinements. | amber |
...
| All-agent training and response doctrine | In progress | PR-D5 adds shared runtime doctrine for Setup/Admin, Intelligence, Strategic Moves, Source, Tower, and general chat, plus the report-specific Nexus phase training framework. | Merge PR-D5, then add response-shape regression tests by surface. |
| Phase-specific Nexus training framework | In progress | PR-D5 adds `docs/design/nexus/NEXUS_PHASE_TRAINING_FRAMEWORK_2026-05-09.md`. | Merge PR-D5 and wire additional phase-specific hardening as needed. |
