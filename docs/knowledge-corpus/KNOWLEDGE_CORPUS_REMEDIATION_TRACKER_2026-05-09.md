...
- Large industry corpus expansion: In progress; Retail pack is in PR-D2 after PR-D1 coverage matrix merge.
...
| PR-D2 Retail Pattern Pack | [#1855](https://github.com/anandsundaram-hash/abarva/pull/1855) Add retail AI pattern pack | `knowledge/retail-pattern-pack-2026-05-09` / `/tmp/nexus-kc-d2` | Domain corpus | `src/lib/intelligence/seed-patterns-retail-ai.ts`; `src/lib/intelligence/seed-patterns-industry.ts`; `src/lib/intelligence/seed-types.ts`; `src/lib/intelligence/canonical/build-canonical-pattern.ts`; `src/lib/intelligence/canonical/build-canonical-pattern.test.ts`; generated canonical preview/validation/sample retrieval reports; tracker | `npm test -- --runInBand src/lib/intelligence/canonical/build-canonical-pattern.test.ts`; scoped ESLint; `npx tsc --noEmit --pretty false`; `npm run intel:canonical-corpus:preview`; `npm run intel:canonical-corpus:validate`; `npm run intel:canonical-corpus:sample-retrieval-qa`; direct retail row QA; `npm run build`; banned-name scan clean | No DB writes. Adds 40 source-code-seeded Retail AI patterns that become deterministic canonical backfill preview rows. Preview rows increase to 243; strict validator still reports legacy/non-retail gaps. | Sample retrieval improves to 4/6; remaining failures are Financial Services AML and Healthcare back-office coverage. | PR-D3 Financial Services Pattern Pack and PR-D4 Healthcare Pattern Pack | amber |
...
| Runtime canonical pattern index | PR-A3 complete | PR #1841 adds a persisted canonical read/index layer with explicit empty/no-match/error statuses and TTL-only cache. | Merge PR-A3, then wire the index into context broker and agents. |
| Nexus pattern-first retrieval | Done in PR-B2 | Nexus ask route retrieves persisted canonical patterns after program context and before synthesis; free-text synthesis surfaces source basis, confidence, missing provenance, and no-match states. | Verify user-visible response/source display in PR-C3. |
| Sentinel evidence/pattern gap checks | Done in PR-B3 | Sentinel orchestration reads canonical patterns and surfaces missing evidence, artifacts, KPIs, guardrails, failure-mode mitigation, phase mismatch, and tenant-pattern contradiction gaps. | Include source/confidence visibility in PR-C3. |
| Atlas value/KPI grounding | Done in PR-B4 | Atlas value guidance separates projected/tracked/verified framing and grounds KPI, baseline, measurement, value lever, confidence, and quantitative claim handling in canonical patterns. | Include unified source/confidence visibility in PR-C3. |
| `corpusPatterns` hydration | Done | PR #1843 maps persisted canonical hits into `corpusPatterns`, provenance, and retrieval trace shared-corpus ids. | Ensure Nexus/Sentinel/Atlas consume canonical patterns in B2/B3/B4 and expose source/confidence in C3. |
| Warning handling | In PR-B1 | `WARNING_CORPUS_PENDING` is replaced on context-broker corpus/full paths by canonical empty/no-match/error warnings; vector/worldview warnings remain explicit. | Route-level agent warning display and refusal behavior remain for B2/B3/B4/C3. |
...
| Large industry/function pattern expansion | In progress | Retail pack PR-D2 adds 40 canonical-complete Retail AI patterns and improves sample retrieval QA to 4/6. | Add Financial Services and Healthcare packs, then phase training framework. |
...
| Runtime retrieval sample-query QA | Done in PR-C2 | Sample retrieval QA exercises six executive queries plus no-match behavior against the deterministic canonical preview fallback. | Use Wave 3 content remediation to close failing sample queries. |
