...
| 9   | Artifact prompt/workflow maturity by phase (d06, d08, d10, d12–d33 partial)          | **Candidate — Pricing slice in PR.** This branch adds governed prompt/workflow support for the first priority family: Pricing d19–d21. The new sequence is `d21_assumption_set` → `d19_pricing_workbook` → `d20_trap_log`, with missing-upstream guards so pricing normalization cannot be fabricated before assumptions are locked.                                                                                                                                                         | Responses, Evaluation, Transition, and Value prompt/workflow coverage gaps remain open after this pricing slice.                                                                                             | Current PR scope only: Pricing d19–d21 prompt templates, evidence binding, cost-section controls, trap-category controls, and workflow sequencing. Do not bundle the remaining artifact families.                                                                                                                                                                             | New prompt-registry tests cover d19/d20/d21 availability, upstream blocking, pricing evidence binding, and trap-log dependency on the normalized workbook.                                                                                                                                                                                                                                       | Signed-in proof after deploy should confirm Pricing artifact generation support or expected missing-upstream behavior where authenticated Source access is available.                                                                                                                                      |
