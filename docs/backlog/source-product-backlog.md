...
| 4   | Render/export authority slice — format-mismatch honesty                              | **Candidate — in PR.** `codex/source-render-authority-001c` changes the render route so a client-final format mismatch returns explicit `409 client_final_format_mismatch` instead of silently falling through to generated rendering. Generated fallback responses now carry `x-source-artifact-authoritative: generated-fallback`; matching client-final streams retain `x-source-artifact-authoritative: client-final`.                                                                                                                                                                                     | Pending PR/merge/deploy/live proof for this slice.                                                                                                                                                                                                              | Current PR scope only: explicit mismatch branch and response headers. No transcoding/conversion and no legacy per-format route deletion.                                                                                                                                                                                                                                                                                                                                   | New route test: client-final exists as DOCX, request `?format=pdf` returns the explicit conflict and does not call the renderer; no-client-final fallback renders with `generated-fallback` header.                                                                             | Live signed-in: request a client-final artifact in its stored format and mismatched format; mismatch must return explicit conflict rather than generated draft.                |
...
`app.abarva.ai` as of 2026-07-22. `SOURCE-ARTIFACT-AUTHORITY-001a` and `001b` are merged,
deployed, ACA-invariant-checked, and signed-in live-proven. `SOURCE-ARTIFACT-AUTHORITY-001c`
(render/export format-mismatch honesty) is the active slice. After that, pick item #5
(direct artifact download authority), then #6 (Deal Pack authority), then the remaining
