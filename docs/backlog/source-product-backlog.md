...
| 9   | Artifact prompt/workflow maturity by phase (d06, d08, d10, d12–d33 partial)          | **Candidate — Responses slice in PR.** Pricing d19–d21 prompt workflow and lifecycle projection are merged, deployed, ACA-invariant-checked, and signed-in Apex proven. This branch adds the next priority family: Responses d13–d15. The sequence is `d09_rfp_pack` + `d11_response_checklist` → `d13_vendor_responses`, `d09_rfp_pack` → `d14_qa_log`, and `d11_response_checklist` + `d13_vendor_responses` → `d15_response_completeness`, so evaluation cannot inherit fabricated or unverified response completeness. | Evaluation, Transition, and Value prompt/workflow coverage gaps remain open after this Responses slice. | Current PR scope only: Responses d13–d15 prompt templates, response-intake controls, Q&A parity controls, response-completeness dimensions, upstream blocking, and lifecycle/standards prompt-backed projection. Do not bundle Evaluation, Transition, or Value.                                                                                                                                 | Prompt-registry tests cover d13/d14/d15 availability, upstream blocking, uploaded response evidence binding, and completeness dependency on the response pack. Lifecycle-matrix regression asserts d13/d14/d15 export as prompt-backed and no longer show "No dedicated prompt."                                                                                                                                 | Signed-in proof after deploy should confirm the Responses Files matrix shows d13/d14/d15 as prompt-backed / Claude model labels instead of "No dedicated prompt"; no generation or data mutation required. |
