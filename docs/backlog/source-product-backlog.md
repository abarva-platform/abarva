...
| 9   | Artifact prompt/workflow maturity by phase (d06, d08, d10, d12–d33 partial)          | **Follow-up candidate — Pricing lifecycle projection.** PR #5392 added governed prompt/workflow support for Pricing d19–d21 and deployed cleanly, but signed-in Apex proof showed the Files/standards matrix still labeled those artifacts as "No dedicated prompt" because `artifact-lifecycle-matrix.ts` kept a duplicate prompt-contract table. This follow-up derives lifecycle prompt labels from the authoritative generation registry. | Responses, Evaluation, Transition, and Value prompt/workflow coverage gaps remain open after the pricing slice. The pricing generation path itself is deployed; this follow-up corrects the UI/export standards projection for prompt-backed status. | Current PR scope only: align the lifecycle/standards prompt-backed projection with the prompt registry for d19/d20/d21 and future registry-backed artifacts. Do not bundle the remaining artifact families.                                                                                                                                 | Prompt-registry tests cover d19/d20/d21 availability, upstream blocking, pricing evidence binding, and trap-log dependency. New lifecycle-matrix regression asserts d19/d20/d21 export as prompt-backed and no longer show "No dedicated prompt."                                                                                                                                                 | Signed-in Apex proof after deploy should confirm the Pricing Files matrix shows d19/d20/d21 as prompt-backed / Claude Opus instead of "No dedicated prompt"; no generation or data mutation required.                                                                 |
