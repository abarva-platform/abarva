...
| 9   | Artifact prompt/workflow maturity by phase (d06, d08, d10, d12–d33 partial)          | **Candidate — Evaluation slice in branch.** Pricing d19–d21 prompt workflow and lifecycle projection are merged, deployed, ACA-invariant-checked, and signed-in Apex proven. Responses d13–d15 prompt workflow and lifecycle projection are also merged, deployed, ACA-invariant-checked, and signed-in Apex proven. This branch adds the next priority family: Evaluation d16–d18. The sequence is `d09_rfp_pack` → `d17_weight_log`, `d17_weight_log` + `d13_vendor_responses` + `d15_response_completeness` → `d16_scorecard`, and `d15_response_completeness` → `d18_disqualification_log`, so scoring cannot inherit invented weights, unverified responses, or fabricated exclusions. | Transition and Value prompt/workflow coverage gaps remain open after this Evaluation slice. | Current PR scope only: Evaluation d16–d18 prompt templates, weight-governance controls, evidence-cited scorecard controls, disqualification rationale controls, upstream blocking, and lifecycle/standards prompt-backed projection. Do not bundle Transition or Value.                                                                                                                                 | Prompt-registry tests cover d16/d17/d18 availability, upstream blocking, uploaded evaluation evidence binding, and disqualification controls that do not invent eliminated vendors. Lifecycle-matrix regression asserts d16/d17/d18 export as prompt-backed and no longer show "No dedicated prompt."                                                                                                                                 | Signed-in proof after deploy should confirm the Evaluation Files matrix shows d16/d17/d18 as prompt-backed / Claude model labels instead of "No dedicated prompt"; no generation or data mutation required. |
...
`app.abarva.ai` as of 2026-07-22. `SOURCE-ARTIFACT-AUTHORITY-001a`, `001b`, `001c`, `001d`, and `001e`
are merged, deployed, ACA-invariant-checked, and signed-in proven to the extent current production data allows. `SOURCE-ARTIFACT-AUTHORITY-001f`
(Governance label unification) is the active slice. After that, pick remaining
opportunistic authority item #8 before moving to `SOURCE-ARTIFACT-QUALITY-001`,
