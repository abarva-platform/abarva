...
| 5   | Direct artifact download authority                                                   | **Candidate — in PR.** `codex/source-download-authority-001d` wires the direct download route through `resolveAuthoritativeArtifactSlots()` for File Cabinet artifacts before bytes are streamed.                                                                                                                                                                                                                                                                                                                                                                                                              | Pending PR/merge/deploy/live proof for this slice.                                                                                                                                                                                                              | Current PR scope only: resolve the requested id's slot; if a more-authoritative sibling exists and the request is not explicit history mode, serve the authoritative sibling with substitution headers. Preserve `includeHistory=1` as the escape hatch for retrieving the originally-requested stale/draft id.                                                                                                                                                            | New route tests cover generated-draft id + client-final sibling in the same slot → streams the final with substitution headers, and `includeHistory=1` → streams the originally-requested draft id.                                                                             | Live signed-in: old draft download link for an artifact that has since gone client-final resolves to the final, not the stale draft; `includeHistory=1` still preserves the old id. |
...
`app.abarva.ai` as of 2026-07-22. `SOURCE-ARTIFACT-AUTHORITY-001a`, `001b`, and `001c` are
merged, deployed, ACA-invariant-checked, and signed-in live-proven. `SOURCE-ARTIFACT-AUTHORITY-001d`
(direct artifact download authority) is the active slice. After that, pick item #6
(Deal Pack authority), then the remaining
opportunistic authority items #7/#8 before moving to `SOURCE-ARTIFACT-QUALITY-001`,
