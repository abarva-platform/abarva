...
| 6   | Deal Pack authority                                                                  | **Candidate — in PR.** `codex/source-deal-pack-authority-001e` threads File Cabinet authority into Deal Pack assembly so accepted client-final artifacts can replace stale generated `artifactStates` before payload binding and rendering.                                                                                                                                                                                                                                                                                                                                                                    | Pending PR/merge/deploy/live proof for this slice.                                                                                                                                                                                                              | Current PR scope only: resolve File Cabinet artifacts into one authoritative artifact per slot, load the authoritative body into the Deal Pack context, render an explicit authority/audit note, and preserve stale generated bodies only as labeled audit history.                                                                                                                                                                                                        | New test: Deal Pack output for a slot with both a superseded AI draft and an accepted client-final uses the final's content, and the draft appears only as labeled audit history.                                                                                               | Live signed-in: generate a Deal Pack for an event with a mixed final/draft artifact set; confirm the final's content appears and the draft doesn't silently substitute.        |
...
`app.abarva.ai` as of 2026-07-22. `SOURCE-ARTIFACT-AUTHORITY-001a`, `001b`, `001c`, and `001d`
are merged, deployed, ACA-invariant-checked, and signed-in live-proven. `SOURCE-ARTIFACT-AUTHORITY-001e`
(Deal Pack authority) is the active slice. After that, pick the remaining
