...
| 8   | Safe repair/regenerate old persisted drafts (`d01_strategy_memo` CONTENT BLOCKERS 3) | **Candidate — in PR.** This branch adds an explicit safe-repair route for already-persisted bodies. It reuses the current client-facing sanitizer/content-blocker scanner, returns a dry-run receipt first, and applies only to one selected terminal artifact row with exact confirmation phrase + body hash.                                                                                                                                               | Pending PR/merge/deploy/live proof. Production apply remains a real data mutation and must not be run silently; dry-run proof can be collected without changing client data. | Current PR scope only: deterministic safe repair for old persisted client-facing bodies; no Claude regeneration, no automatic batch cleanup, no unlock of locked/superseded status. The route records the before/after content-QA receipt in `body_generation_metadata` and the Source activity log when explicitly applied.                                                                                                                                                                                              | New route test: dry-run captures before/after blockers without writing; locked artifact apply requires explicit confirmation; confirmed apply keeps terminal status and writes metadata + activity audit receipt.                                                                                                                                  | Live signed-in: first run dry-run on the affected `d01` artifact and capture receipt + clean after-scan; apply proof requires explicit production data-repair approval or an approved test event.                             |
