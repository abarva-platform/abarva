# Visual Acceptance Criteria

Before a UI PR is approved, the review must confirm:

- Primary canvas is warm off-white unless explicitly justified.
- Dark panels are sparse and purposeful.
- Page answers the primary user question in the first viewport.
- Next action is visible.
- Value, risk, status, and owner are visible.
- Journey stage is visible wherever workflow matters.
- Agent guidance is contextual, not generic.
- Agent response shape matches the user situation.
- Three choices plus custom appears only when it helps the user move forward.
- Context used, missing context, and confidence are visible when relevant.
- Tables are readable and scannable.
- Icons and symbols are restrained.
- Badges do not overwhelm the page.
- The page does not feel like a generic dashboard.
- The page does not feel like a generic chatbot.
- Responsive fit is acceptable.
- Screenshot or manual review is included.

## Review Output

Every visual review should produce:

- Decision: approve as baseline, approve with polish, or hold for redesign.
- What works.
- What feels weak.
- Must-fix items.
- Recommended next slice.
- Screenshot/auth blocker status.
