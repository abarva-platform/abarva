...
- Next recommended item: define the Admin/Setup readiness contract that will eventually replace seeded Source readiness rows, then capture authenticated `/source` and `/source/events/[eventId]` screenshot/manual review with the panel visible.
- Next planning artifact: Admin/Setup-to-Source readiness contract plan.
