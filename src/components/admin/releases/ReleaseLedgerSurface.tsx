...
export function ReleaseLedgerSurface({ view, generatedAt }: ReleaseLedgerSurfaceProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING.lg }}>
        ...
        >
          <div style={{ minWidth: 0, flex: '1 1 520px' }}>
            ...
            <p style={{ ...bodyStyle, margin: '8px 0 0', maxWidth: 620 }}>
              Each entry is a release record with a plain-English impact note, layer mapping,
              rollout and rollback path, and validation evidence.
            </p>
          </div>
          <div
            style={{
        ...
        >
          <div style={{ minWidth: 0 }}>
            ...
            <p style={{ ...bodyStyle, margin: '6px 0 0' }}>
              ...
              recorded change in the deployed repo. It is not live GitHub, Vercel, or database
              telemetry, and it cannot display an unrecorded change.
            </p>
          </div>
      ...
      <section aria-label="Release records" style={{ display: 'flex', flexDirection: 'column', gap: SPACING.md }}>
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: SPACING.md, flexWrap: 'wrap' }}>
        <div style={{ minWidth: 0, maxWidth: 660 }}>
          ...
          <p style={{ ...bodyStyle, margin: 0 }}>{record.summary || 'No summary captured.'}</p>
        </div>
        <div
          style={{
            display: 'flex',
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING.md, minWidth: 0 }}>
          <RecordSection title="Layer impact" items={record.layerImpact} empty="No layer impact captured." />
          <RecordSection
            title="Client applicability"
            items={record.clientApplicability}
            empty="No applicability captured."
          />
          <RecordSection title="Changes included" items={record.changesIncluded} empty="No changes captured." />
        </div>
        ...
        >
          <RecordSection title="QA / validation" items={record.qaValidation} empty="No QA note." compact />
          <RecordSection title="Audit evidence" items={record.auditEvidence} empty="No evidence note." compact />
...
function RecordSection({
  ...
  return (
    <section style={{ minWidth: 0 }}>
      ...
      {items.length ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: compact ? 6 : 8, marginTop: 8 }}>
          {items.map((item) => (
...
function TextSection({ title, value }: { title: string; value: string }) {
  return (
    <section style={{ minWidth: 0 }}>
      <div style={blockTitleStyle}>{title}</div>
      <p style={{ ...bodyStyle, margin: '8px 0 0' }}>{value || 'Not captured.'}</p>
    </section>
  );
}
