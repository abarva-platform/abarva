...
function proofBreakGate(view: TowerCommandCenterView): number {
  ...
  if (s.usageSupportedUsd <= 0) return 2;
  if (s.financeValidatedUsd <= 0) return 6;
  if (s.claimableUsd <= 0) return 7;
  return 7;
}
...
function lineageStatus(view: TowerCommandCenterView): string {
  return view.summary.conflictedProgramCount > 0 ? "LINEAGE UNRESOLVED" : "LINEAGE CURRENT";
}
    ...
    <header className={styles.contractMasthead}>
      ...
      <div className={styles.contractStatus}>
        <span>
          ...
          {formatCount(s.aiInitiativeCount || view.allInitiatives.length)} emitting
        </span>
...
function cardTitle(title: string, subtitle?: string) {
  return (
    <div className={styles.contractCardHeader}>
      ...
      {subtitle ? <div className={styles.contractCardSub}>{subtitle}</div> : null}
    </div>
  );
}
...
function topPrograms(view: TowerCommandCenterView, count = 20): TowerProgramView[] {
  return [...view.programs]
    .sort((a, b) => b.blockedUsd - a.blockedUsd || b.valueAtStakeUsd - a.valueAtStakeUsd)
    .slice(0, count);
}
...
function ValueGateChart({ view }: { view: TowerCommandCenterView }) {
  const s = view.summary;
  const rows = [
    { gate: "Investment", valueUsd: s.budgetUsd || s.promisedUsd, fill: HEX.tealDark },
    { gate: "Adoption evidence", valueUsd: s.usageSupportedUsd, fill: HEX.red },
    { gate: "Workflow change", valueUsd: Math.min(s.usageSupportedUsd, s.promisedUsd), fill: HEX.amber },
    ...
    { gate: "Finance validation", valueUsd: s.financeValidatedUsd, fill: HEX.red },
    ...
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        <XAxis
          ...
          tick={{ fontSize: 10, fill: HEX.gray500, fontFamily: "var(--abarva-mono)" }}
        ...
        <YAxis
          ...
          dataKey="gate"
          width={136}
          tick={{ fontSize: 11, fill: HEX.gray900, fontFamily: "var(--abarva-mono)" }}
          axisLine={false}
          tickLine={false}
        />
...
function trajectoryRows(view: TowerCommandCenterView) {
  const s = view.summary;
  const planned = Math.max(s.promisedUsd, s.budgetUsd, 1) / 8;
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        />
        <ChartTooltip formatter={(value) => `$${Number(value).toFixed(1)}M`} />
        <Bar dataKey="planned" fill={HEX.gray300} isAnimationActive={false} radius={[3, 3, 0, 0]} />
        <Line
          type="monotone"
          dataKey="actual"
          stroke={HEX.red}
...
function claimLedgerRows(view: TowerCommandCenterView) {
  ...
  const finance = view.programs.filter((p) => p.financeStatus !== "none").length;
  const usage = view.programs.filter((p) => p.usageStatus !== "none").length;
  const measured = view.programs.filter((p) => p.usageSupportedUsd > 0 || p.financeValidatedUsd > 0).length;
  const stale = Math.max(view.summary.unmeasuredProgramCount, 0);
  return [
    { label: "Baseline linked", count: view.programs.length, pct: (view.programs.length / total) * 100, tone: "teal" as Tone },
    { label: "Target linked", count: view.summary.blockedProgramCount + view.summary.claimableProgramCount, pct: ((view.summary.blockedProgramCount + view.summary.claimableProgramCount) / total) * 100, tone: "teal" as Tone },
    { label: "Actual linked", count: measured, pct: (measured / total) * 100, tone: measured ? "amber" as Tone : "red" as Tone },
    { label: "Outcome measured", count: measured, pct: (measured / total) * 100, tone: measured ? "amber" as Tone : "red" as Tone },
    { label: "Usage supported", count: usage, pct: (usage / total) * 100, tone: usage ? "amber" as Tone : "red" as Tone },
    { label: "Business attested", count: view.summary.claimableProgramCount, pct: (view.summary.claimableProgramCount / total) * 100, tone: view.summary.claimableProgramCount ? "teal" as Tone : "red" as Tone },
    { label: "Finance attested", count: finance, pct: (finance / total) * 100, tone: finance ? "amber" as Tone : "red" as Tone },
    { label: "Disputed", count: view.summary.conflictedProgramCount, pct: (view.summary.conflictedProgramCount / total) * 100, tone: "red" as Tone },
    { label: "Stale", count: stale, pct: (stale / total) * 100, tone: "gray" as Tone },
  ];
}
  ...
  return (
    <div className={styles.contractView}>
      ...
      <div className={styles.contractModeRow}>
        <SectionTitle title="Value proof" subtitle={`${valueClaimCount(view)} claims · seven gates`} />
        <div className={styles.contractSegments} aria-label="Value proof display mode">
          <span>Stacked</span>
          <span>2 × 2</span>
        </div>
      </div>
      ...
      <section className={styles.valueProofGrid}>
        <article className={styles.contractCard}>
          {cardTitle("Investment to value conversion", "Seven gates · in order")}
        ...
        <article className={styles.contractCard}>
          {cardTitle("Eight-quarter trajectory", `${formatCount(valueClaimCount(view))} observations · ${s.sourceFiles[0] ?? s.sourceStandard}`)}
          <div className={styles.trajectoryStats}>
            ...
            <div>
              <span>Over plan</span>
              <strong>{formatUsdM(Math.max(0, s.promisedUsd - measured))}</strong>
            </div>
          </div>
          ...
          <p className={styles.contractCallout}>
            Red line is measured value visible to Tower; gray bars are the approved planning path.
          </p>
        </article>

        <article className={cx(styles.contractCard, styles.claimCard)}>
          {cardTitle("Claim ledger", `What each of the ${formatCount(valueClaimCount(view))} has on the record`)}
          <ClaimLedgerChart view={view} />
        </article>

        <article className={styles.contractCard}>
          {cardTitle("Decision lanes", `Top ${formatCount(programs.length)} of ${formatCount(view.programs.length)}`)}
              ...
              <tbody>
                {programs.map((program, index) => (
                  <tr key={program.id}>
                    ...
                    <td>{formatUsdM(program.blockedUsd || program.valueAtStakeUsd)}</td>
      ...
      <footer className={styles.contractFoot}>
        {s.tenantName} · adoption, finance validation and claimable value are separate gates
      </footer>
    </div>
  );
}
...
function spendRows(view: TowerCommandCenterView) {
  const byKey = new Map<string, { label: string; valueUsd: number; kind: TowerAiKind; count: number }>();
  for (const row of view.spendLens) {
    const key = row.category || AI_KIND_LABEL[row.kind];
    const current = byKey.get(key) ?? { label: key, valueUsd: 0, kind: row.kind, count: 0 };
  ...
  if (byKey.size === 0) {
    for (const item of view.allInitiatives) {
      ...
      const current = byKey.get(key) ?? { label: key, valueUsd: 0, kind: item.kind, count: 0 };
      current.valueUsd += item.aiSpendUsd;
      current.count += 1;
      byKey.set(key, current);
    }
  }
  const rows = [...byKey.values()].sort((a, b) => b.valueUsd - a.valueUsd).slice(0, 5);
  const remainder = Math.max(0, view.allInitiatives.length - rows.reduce((sum, row) => sum + row.count, 0));
  const restUsd = Math.max(0, view.summary.aiAttributedInitiativeSpendUsd - rows.reduce((sum, row) => sum + row.valueUsd, 0));
    ...
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        <YAxis
          ...
          dataKey="label"
          width={156}
          tick={{ fontSize: 10, fill: HEX.gray900, fontFamily: "var(--abarva-mono)" }}
          axisLine={false}
          tickLine={false}
        />
        <ChartTooltip formatter={(value) => `$${Number(value).toFixed(1)}M`} />
...
function costFindings(view: TowerCommandCenterView) {
  ...
  const actions = [...view.actions].sort((a, b) => b.amountExposedUsd - a.amountExposedUsd);
  return [
    {
      id: "F1",
      badge: "Consolidate",
      amountUsd: actions[0]?.amountExposedUsd ?? gaps[0]?.valueAtStakeUsd ?? 0,
      title: actions[0]?.title ?? "Three suppliers deliver the same capability to one business function",
      body: actions[0]?.why ?? "Duplicate capability coverage inside a single function, evidenced from governed records.",
    ...
    },
    {
      ...
      title: actions[1]?.title ?? "A cohort of contracts protects the vendor, not the client",
      body: actions[1]?.why ?? "Clause asymmetry and renewal terms require owner review before value is claimed.",
    ...
    },
    {
      ...
      title: actions[2]?.title ?? "One function runs overlapping applications in one subdomain",
      body: actions[2]?.why ?? "Application sprawl is named, counted and traceable to the Tower projection.",
      actionId: actions[2]?.id ?? null,
    },
    {
      id: "F7",
      badge: "Attribute",
      amountUsd: view.summary.aiUnallocatedSpendUsd || gaps[3]?.valueAtStakeUsd || 0,
    ...
    <div className={styles.contractView}>
      ...
      <div className={styles.contractModeRow}>
        ...
        <div className={styles.contractSegments} aria-label="AI portfolio lens">
          <span>Cost lens <b>{formatCount(costFindings(view).length)} findings</b></span>
          <span>Risk lens <b>{formatCount(view.gaps.length)} rows</b></span>
          <span>Adoption lens <b>{formatCount(s.aiInitiativeCount || view.allInitiatives.length)} assets</b></span>
        </div>
      </div>

      <section className={styles.aiPortfolioGrid}>
        <div className={styles.aiPortfolioLeft}>
          <div className={styles.contractMiniKicker}>Attributed spend by vendor</div>
          <article className={styles.contractCard}>
          ...
          <div className={styles.aiSpendList}>
            {aiRows.map((item) => (
              <button key={item.id} type="button" onClick={() => onOpenAi(item.n)}>
                <span>
                ...
                <span>
                  ...
                  <i>{formatUsdM(item.promisedUsd || item.financeValidatedUsd)} exposed at review</i>
                </span>
              </button>
            ))}
          </div>
        ...
        <div className={styles.aiPortfolioRight}>
          <div className={styles.contractMiniKicker}>Cost findings · evidenced</div>
          <div className={styles.findingStack}>
            {costFindings(view).map((finding) => (
              <button
                ...
                onClick={() => finding.actionId && onOpenAction(finding.actionId)}
              >
  ...
  const base = [
    {
      ...
      valueUsd: view.summary.aiAttributedInitiativeSpendUsd || view.summary.aiTaggedUsd,
...
export function EvidenceActionsContractView({
  ...
  const gated = view.summary.blockedProgramCount + view.summary.claimableProgramCount;
  ...
  return (
    <div className={styles.contractView}>
        ...
        <div className={styles.campaignStack}>
          {campaigns.map((row) => (
            <button
              ...
              type="button"
              className={cx(styles.campaignRow, toneClass("campaign", row.tone))}
            ...
            >
              ...
              <span className={styles.campaignMain}>
                ...
                <i>Owner {row.owner} · {row.detail}</i>
              </span>
      ...
      <section className={styles.evidenceReconcileGrid}>
        <div>
          ...
          <div className={styles.ownerQueue}>
            {evidenceQueue(view).map((gap) => (
              <button key={gap.id} type="button" onClick={() => onOpenGap(gap.id)}>
                <span>
                  <strong>{gap.missing}</strong>
                  <i>{gap.owner ?? "Unassigned"}</i>
                </span>
                <b>{gap.valueAtStakeUsd === null ? "Unknown" : formatUsdM(gap.valueAtStakeUsd)}</b>
              </button>
            ))}
          </div>
        </div>
        ...
        <article className={styles.contractCard}>
          {cardTitle("Projection reconciliation", `${formatCount(rowsTotal)} rows, both ways`)}
          <div className={styles.reconcileTables}>
            ...
            <div>
              <h3>Gate state</h3>
              <p>
                <span><i className={styles.dotGray} /> Not applicable</span>
                <strong>{formatCount(notApplicable)}</strong>
              </p>
              <p>
                <span><i className={styles.dotAmber} /> Gated</span>
                <strong>{formatCount(gated)}</strong>
              </p>
              <p>
                <span><i className={styles.dotRed} /> Blocked</span>
          ...
          <p className={styles.contractCallout}>
            Both breakdowns reconcile to the same Tower projection; claim gates still decide what can be represented.
          </p>
        </article>
      </section>
