...
function mapTowerMissionToPanel(
...
export function ProgramPressureCards({
  tenant,
  view,
  topN,
}: ProgramPressureCardsProps) {
  ...
  return (
    ...
    >
      ...
      >
        Composed from Tower program-pressure signals. No live agent or model
        call.
      </footer>
    </section>
  );
...
function AtlasExecutiveBriefPanel({
  brief,
}: {
  brief: AtlasProgramPressureBrief;
}) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Evidence basis ·{" "}
          {brief.accountability.evidenceBasis.slice(0, 3).join(" · ")}
...
function BriefLine({
  ...
  return (
    ...
    >
      ...
      <span
        style={{ fontSize: 13, color: tone ?? COLORS.ink, lineHeight: 1.55 }}
      >
        {value}
      </span>
    </div>
...
function atlasBriefAccent(
  label: AtlasProgramPressureBrief["confidenceLabel"],
): string {
  switch (label) {
    case "high":
...
function ExecutiveStrip({
  ...
  return (
    ...
    >
      <StripMetric
        label="Program pressure signals"
        value={strip.totalSignals}
      />
      <StripMetric
        label="Top severity"
      ...
      >
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {(
            ...
            <TypeChip key={type} label={short} count={summary.byType[type]} />
          ))}
        </div>
      </div>
    </div>
...
function PressureCard({ signal }: { signal: ProgramControlTowerSignal }) {
  ...
  return (
    ...
    >
      ...
      <p
        style={{ margin: 0, fontSize: 13, color: COLORS.ink, lineHeight: 1.55 }}
      >
        {signal.summary}
      </p>
...
function EmptyState() {
  return (
    ...
    >
      No program-pressure signals for this tenant yet. Future data updates will
      surface value gaps and delivery pressure here.
    </div>
  );
}
