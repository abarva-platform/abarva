...
import {
  applicationTables, applicationFindings,
  vendorTables, vendorFindings,
  infrastructureTables, infrastructureFindings,
  dataTables, dataFindings,
  type EstateRow, type Finding, type TableSpec,
} from "./page-tables";
 ...
 */
const CHAPTER_SOURCES: Partial<Record<ChapterId, Array<"applications" | "vendors" | "infrastructure" | "data">>> = {
...
const BUILDERS = {
  ...
  infrastructure: { tables: infrastructureTables, findings: infrastructureFindings },
  data: { tables: dataTables, findings: dataFindings },
} as const;

export function chapterDepth(chapterId: ChapterId, estate: EstateRecordTypes): ChapterDepth {
  ...
  return { tables, findings: findings.filter((f) => (seen.has(f.claim) ? false : (seen.add(f.claim), true))) };
}
