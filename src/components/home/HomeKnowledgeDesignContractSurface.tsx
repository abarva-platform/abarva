...
function coveragePosture(count: unknown): string {
  const value = typeof count === "number" ? count : Number(count);
  if (!Number.isFinite(value) || value <= 0) return "Not loaded";
  if (value >= 200) return "Broad coverage";
  if (value >= 50) return "Good coverage";
  if (value >= 10) return "Focused coverage";
  return "Starter coverage";
}

function dimensionNavLabel(dimension: HomeKnowledgeDimension): string {
  const covers = (dimension.covers ?? []).map(asText).filter(Boolean);
  if (covers.length) return covers.slice(0, 2).join(" · ");
  return sourceStatus(dimension.status);
}

const DIMENSION_NAV_GROUPS: Array<{
  title: string;
  keys: string[];
}> = [
  {
    title: "Enterprise",
    keys: ["profile", "functions", "org", "workforce"],
  },
  {
    title: "Technology Landscape",
    keys: ["apps", "data", "infra", "vendors", "ms"],
  },
  {
    title: "Change Agenda",
    keys: ["programs", "ai", "budget", "metrics"],
  },
  {
    title: "Risk And Evidence",
    keys: ["risks", "rel", "evidence", "industry", "lenses", "opev"],
  },
];

function evidencePosture(count: unknown): string {
  const value = typeof count === "number" ? count : Number(count);
  if (!Number.isFinite(value) || value <= 0) return "Needs sources";
  if (value >= 5) return "Well supported";
  if (value >= 2) return "Supported";
  return "Limited support";
}

function decisionGradePosture(decisionGrade: number, total: number): string {
  if (!total) return "Not assessed";
  if (decisionGrade === total) return "All supporting areas decision-grade";
  if (decisionGrade >= Math.ceil(total / 2)) {
    return "Most supporting areas decision-grade";
  }
  if (decisionGrade > 0) return "Some supporting areas decision-grade";
  return "Needs evidence before decision use";
}
...
export function HomeKnowledgeDesignContractSurface({
  ...
  const filteredDimensionGroups = useMemo(() => {
    const byKey = new Map(
      filteredDimensions.map((dimension) => [dimension.key, dimension]),
    );
    const used = new Set<string>();
    const groups = DIMENSION_NAV_GROUPS.map((group) => {
    ...
    const uncategorized = filteredDimensions.filter(
      (dimension) => !used.has(dimension.key),
    );
    return uncategorized.length
      ? [...groups, { title: "Other", keys: [], items: uncategorized }]
      : groups;
  }, [filteredDimensions]);
        ...
        <div className="nkh-rail-section">
          ...
          <div className="nkh-dim-list">
            {filteredDimensionGroups.map((group) => (
              <div className="nkh-dim-group" key={group.title}>
                <div className="nkh-dim-group-label">{group.title}</div>
                {group.items.map((dimension) => (
                  ...
                  >
                    <span
                      className="nkh-dot"
                      style={{ color: statusColor(dimension.status) }}
                    >
                      ●
                    </span>
                    <span>
                      <strong>{dimension.name}</strong>
                      <small>{dimensionNavLabel(dimension)}</small>
                    </span>
                  </button>
                ))}
              </div>
...
function DimensionView({
  ...
  return (
    <div className="nkh-section">
      <nav className="nkh-subtabs" aria-label={`${dimension.name} views`}>
      ...
      {tab === "summary" ? (
        <div>
          ...
          <div className="nkh-dashboard-grid">
            <MetricTile
              label="Coverage"
              value={coveragePosture(dimension.count ?? rows.length)}
            />
            <MetricTile
              label="Confidence"
            ...
            <MetricTile
              label="Evidence"
              value={evidencePosture(dimension.evCount ?? evidence.length)}
            />
        ...
        <div className="nkh-data-card">
          <div className="nkh-data-tools">
            ...
            <span className="nkh-chip">
              ...
              facetValue !== "all" ||
              confidenceValue !== "all" ||
              Object.values(crossFilterValues).some((value) => value !== "all")
      ...
      {tab === "evidence" ? (
        <div>
          <p className="nkh-tab-intro">
            Source inventory behind this dimension: file, load date, loader
            metadata, and what each source supports.
          </p>
    ...
    <div className="nkh-section nkh-enterprise-overview">
      ...
      <section className="nkh-at-glance" aria-label="Enterprise at a glance">
        <div className="nkh-inline-head">
          ...
          <span>selected signals · every metric traceable to evidence</span>
        </div>
      ...
      <section className="nkh-intel-canvas-card">
        <div className="nkh-canvas-copy">
          ...
          <p>
            The strongest covered domains show where Nexus can already orient a
...
function SixQuestionsLanding({
  ...
  const cards = SIX_LANDING_QUESTIONS.map((question) => {
    ...
    const totalCoverage = matched.reduce(
    ...
    return { ...question, matched, totalCoverage, decisionGrade };
      ...
      <div className="nkh-six-questions-grid">
        {cards.map((card) => (
          ...
          >
            ...
            <strong>{coveragePosture(card.totalCoverage)}</strong>
            <span className="nkh-six-question-meta">
              {decisionGradePosture(card.decisionGrade, card.matched.length)} ·{" "}
  ...
  return (
    <section className="nkh-brief-signal-band">
      <div className="nkh-inline-head">
        ...
        <span>Approved context · evidence boundaries preserved</span>
      </div>
      {pairs ? (
  ...
  return (
    <section className="nkh-strategic-narratives">
      <div className="nkh-inline-head">
        ...
        <span>Approved strategic narrative set</span>
      </div>
  ...
  return (
    <section className="nkh-context-horizon" aria-label="Context horizon">
      <div className="nkh-inline-head">
        ...
        <span>
          Priority evidence requests would sharpen this picture once closed —
          not loaded yet.
        </span>
      </div>
...
function AiSuccessThesis({
  ...
  const evidenceItems = evidencePosture(
      ...
      <div className="nkh-ai-thesis-lead">
        ...
        >
          {[
            [
              "Context map",
              dimensions.length >= 19 ? "Complete" : "In progress",
            ],
            ["Evidence support", evidenceItems],
            ["Technology estate", coveragePosture(apps)],
            ["AI agenda", coveragePosture(aiCandidates)],
            ["Risk/control view", coveragePosture(risks)],
          ].map(([label, value]) => (
            <div key={label}>
              <strong>{value}</strong>
              <em>{label}</em>
      ...
      <div className="nkh-ai-thesis-grid">
        <article>
          ...
          <p>
            ...
            which missing evidence would make a recommendation unsafe.
          </p>
        </article>
        <article>
...
function RowDetail({
  ...
  return (
    <aside className="nkh-drawer" aria-label="Selected context detail">
      <div className="nkh-drawer-head">
        <div>
          <span>Selected context item</span>
          <h3>{rowTitle(row)}</h3>
        </div>
    ...
    <div className="nkh-section">
      ...
      <div className="nkh-proof-split">
        <section className="nkh-proof-table">
          ...
          <p>
            Documents, system exports, and interviews behind the brief. Every
            visible claim above remains traceable.
          </p>
...
function DimensionVolumeChart({
  ...
  return (
    <div
      ...
      aria-label="Context coverage by dimension"
      ...
      <ResponsiveContainer width="100%" height={314}>
        ...
        >
          ...
          <YAxis
            ...
            tickLine={false}
          />
          <Tooltip content={<HomeChartTooltip valueLabel="Context signal" />} />
          <Bar
            dataKey="count"
            name="Coverage"
            radius={[0, 8, 8, 0]}
  ...
  return (
    ...
    >
      <div className="nkh-crosstab-head">
        <strong>{title}</strong>
        <span>
          Coverage shown where both {fieldA.label.toLowerCase()} and{" "}
          {fieldB.label.toLowerCase()} are supported
          {pending > 0 ? ` · some items still need evidence` : ""}
        </span>
      </div>
      <div
            ...
            {bValues.map((b) => {
              ...
              return (
                <div
                  ...
                  title={`${a.replaceAll("_", " ")} × ${b.replaceAll("_", " ")}: ${
                    count ? "supported" : "not yet supported"
                  }`}
                >
                  {count ? "✓" : ""}
                </div>
    ...
    <section className="nkh-relationship-graph-section">
      {edges.length ? (
        ...
        <EmptyState
          ...
          body="The Relationships dimension has source material, but the validated connection pairs are not ready for a graph view yet."
        />
      )}
  ...
  return (
    <section className="nkh-relationship-possible">
      <div className="nkh-crosstab-head">
        ...
        <span>
          {edges.length
            ? "Validated connection families are visible today."
            : `${coveragePosture(rowCount)} exists, but connection-ready evidence still needs projection.`}
        </span>
      </div>
      <p>
        A board-grade context layer should not just inventory the enterprise. It
        should show how ownership, systems, data, vendors, risks, initiatives,
        and outcomes move together. That is what lets Intelligence answer better
        than a generic LLM, Moves scope change realistically, Source target the
        right evidence, and Tower explain value realization.
      </p>
  ...
  return (
    ...
    >
      <div className="nkh-crosstab-head">
        ...
        <span>
          Evidenced connections derived from integration, vendor, and use-case
          material
          {showingSubset ? " · showing the most material path first" : ""}
        </span>
      </div>
  ...
  return (
    <div className="nkh-chart-tooltip">
      ...
      {typeof row.share === "number" ? (
        <em>{row.share}% of covered material</em>
      ) : null}
    </div>
  );
}
  ...
  if (!data.length) {
    return (
      <EmptyState
        ...
        body="The approved pack has use cases, but not enough scored evidence to render the value/readiness matrix."
      />
    );
  }
      ...
      <div className="nkh-proof-visual-copy">
        ...
        <p>
          {relationshipVisual?.caption ||
            "The proof shows how evidence becomes context, where caveats remain, and which product action should come next."}
        </p>
      </div>
            ...
            <em>
              {index === 0
                ...
                : index === 1
                  ...
                  : index === 2
                    ? "evidence · gaps · relationships"
                    : index === 3
                      ? "Home · Intelligence · Moves · Source · Tower"
                      : nextEvidence.length
                        ? "Evidence requests queued"
                        : "Evidence requests to define"}
            </em>
          </div>
        ))}
      </div>
...
function ProofStageRechart({
  ...
  const data = [
    { name: "Evidence", value: evidence.length, detail: "source evidence" },
      ...
      <ResponsiveContainer width="100%" height={260}>
        ...
        >
          ...
          <YAxis
            ...
            tickLine={false}
          />
          <Tooltip content={<HomeChartTooltip valueLabel="Signal" />} />
          <Bar
            dataKey="value"
            name="Signal"
            radius={[8, 8, 0, 0]}
    ...
    <div className={`nkh-evidence-card ${compact ? "is-compact" : ""}`}>
      ...
      <dl>
        <div>
          ...
          <dd>{evidence.rows ? "Captured" : "—"}</dd>
        </div>
        <div>
          <dt>Business support</dt>
          <dd>{evidence.facts ? "Mapped" : "—"}</dd>
        </div>
        <div>
          <dt>Status</dt>
  ...
  if (!rows.length) {
    return (
      <EmptyState
        ...
        body="The approved pack includes business context, but it did not include file-level metadata for this dimension."
      />
    );
  }
...
function formatBytes(value?: string): string {
  ...
  if (!Number.isFinite(bytes) || bytes <= 0) return value || "Not captured";
...
.nkh-dim-group + .nkh-dim-group { margin-top: 18px; }
.nkh-rail-label {
...
.nkh-subtabs button.is-active {
  color: #071426;
  background: #fff;
  box-shadow: 0 1px 2px rgba(10, 16, 28, .08);
}
.nkh-section {
  margin-top: 34px;
}
