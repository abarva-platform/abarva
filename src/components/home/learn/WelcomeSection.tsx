...
const TENANTS: Record<TenantKey, TenantConfig> = {
  apex: {
    ...
    scaleStats: [
      { label: 'Revenue', value: '$108B', detail: 'Mass + specialty omnichannel' },
      { label: 'Footprint', value: '1,976 stores', detail: '12 DCs across 3 regions (W/E/Central)' },
      { label: 'Headcount', value: '~250k', detail: 'Largest single workforce: store associates' },
      { label: 'IT operating budget', value: '$1.62B', detail: '1.5% of revenue (retail benchmark)' },
    ],
    leadership: [
      {
        title: 'CIO',
    ...
    dataLoaded: [
      ...
      { kind: 'Capital plan lines', count: '15', example: 'Store tech $78M, Merch platform $32.5M, AI program $12.5M, DC automation $385M' },
      { kind: 'Funding authority bands', count: '9', example: 'Director $250K → VP $1M → SVP $5M → CIO $25M → CFO+CIO $75M → CEO $200M → Board' },
      { kind: 'Systems inventory', count: '65', example: 'Salesforce Einstein, JDA, Oracle ERP, Snowflake, Adobe Experience Platform' },
      { kind: 'Vendor scorecards', count: '28', example: 'Performance + risk + financial health per vendor with renewal-calendar flags' },
    ],
    ...
    agentSamples: [
      ...
      },
      {
        ...
        citesFrom: 'vendor_scorecards.csv + renewal_calendar.csv + Salesforce Einstein license audit',
      },
    ],
    walkStart: {
  ...
  meridian: {
    ...
    scaleStats: [
      { label: 'Revenue', value: '$16.8B', detail: 'Integrated provider + plan + research' },
      { label: 'Footprint', value: '30 hospitals', detail: '280 clinics across CA + HI; Plan covers 1.6M members' },
      { label: 'Headcount', value: '~58k', detail: '~14,800 RNs · 7,400 employed MDs' },
      { label: 'IT operating budget', value: '$384M', detail: '2.3% of revenue (healthcare benchmark)' },
    ],
    leadership: [
      {
        title: 'CDIO',
    ...
    dataLoaded: [
      ...
      { kind: 'Capital plan lines', count: '18', example: 'Cyber $22M, Epic $38M, RCM mod $18.5M, ambient doc $8.5M, AI Gov $2.2M, Hawaii DEFERRED' },
      { kind: 'Funding authority bands', count: '10', example: 'Director $100K → VP $500K → SVP $2M → CDIO $10M → CFO+CDIO $25M → CEO $50M → Board' },
      { kind: 'Systems inventory', count: '114', example: 'Epic Hyperspace + Ambulatory + Cogito + Cosmos · Snowflake · Atlas custom · Workday' },
      { kind: 'Vendor scorecards', count: '45', example: 'Performance + risk + financial health; Cohere Health + Abridge + Paige AI + Arcadia' },
    ],
    ...
    agentSamples: [
      ...
      },
      {
        ...
        citesFrom: 'vendor_scorecards.csv + program inventory + renewal_calendar.csv',
      },
    ],
    walkStart: {
  ...
  firstcapital: {
    ...
    scaleStats: [
      { label: 'Revenue', value: '$18.2B', detail: '$362B assets · $420B wealth AUM' },
      { label: 'Footprint', value: '480 branches', detail: '720 ATMs · 4 LOBs · OCC examination cycle' },
      { label: 'Headcount', value: '~46k', detail: '~1,180 in compliance/BSA-AML alone (2.5%)' },
      { label: 'IT operating budget', value: '$1.67B', detail: '9.2% of revenue · 34% compliance share (highest in peer)' },
    ],
    leadership: [
      {
        title: 'CIO',
    ...
    dataLoaded: [
      ...
      { kind: 'Capital plan lines', count: '18', example: 'FedNow $32.5M, core banking discovery $12M, cyber $38.5M, AI under MRM $8.5M, CRA $145M' },
      { kind: 'Funding authority bands', count: '11', example: 'Director $100K → VP $500K → SVP $2M → CIO $5M → CFO+CIO+CRO three-sig $25M → Board' },
      { kind: 'Systems inventory', count: '84', example: 'FIS Profile core, Volante FedNow, FIS Charlotte wealth, BNY Pershing, Murex MX.3, AxiomSL' },
      { kind: 'Vendor scorecards', count: '51', example: 'FIS Tier 1 ($100M+), Microsoft Tier 1, Adenza renegotiation flag, NICE Actimize expansion' },
    ],
    ...
    agentSamples: [
      ...
      },
      {
        ...
        citesFrom: 'renewal_calendar.csv + vendor_scorecards.csv (3 renegotiation flags)',
      },
    ],
    walkStart: {
...
export function WelcomeSection() {
  ...
  return (
    <>
      ...
      <HeroBand color="navy">
        <Eyebrow light><InlineAbarvaLogo light heightEm={1.35} /> · Five-minute orientation</Eyebrow>
        ...
        >
          {TENANT_KEYS.map((key) => {
            ...
            return (
              ...
              >
                <span style={{ display: 'block', fontFamily: T.fBody, fontSize: 14, fontWeight: 800 }}>
                  {t.shortLabel}
                </span>
                <span style={{ display: 'block', fontFamily: T.fMono, fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: 4, opacity: 0.9 }}>
                  {t.tabDetail}
                </span>
              </button>
            );
      ...
      <Section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20, margin: '24px 0 8px' }}>
          {AGENT_TRIO.map((a) => (
            <div key={a.name} style={{ border: `1px solid ${T.border}`, borderRadius: 10, padding: 22, background: T.surface, borderLeft: `4px solid ${a.color}` }}>
              <div style={{ fontFamily: T.fMono, fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: a.color, marginBottom: 8 }}>
                {a.surface}
              </div>
              <div style={{ fontFamily: T.fDisp, fontSize: 24, color: T.ink, fontWeight: 500, letterSpacing: '-0.01em', marginBottom: 4 }}>
                {a.name}
              </div>
              <div style={{ fontFamily: T.fBody, fontSize: 12, color: T.faint, fontStyle: 'italic', marginBottom: 12 }}>
                {a.archetype}
              </div>
              <div style={{ fontFamily: T.fBody, fontSize: 13, color: T.body, lineHeight: 1.65 }}>
                {a.job}
              </div>
            </div>
          ))}
      ...
      <Section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, margin: '20px 0 28px' }}>
          {active.scaleStats.map((s) => (
            <div key={s.label} style={{ borderTop: `3px solid ${active.accent}`, paddingTop: 12 }}>
              <div style={{ fontFamily: T.fMono, fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: T.faint }}>{s.label}</div>
              <div style={{ fontFamily: T.fDisp, fontSize: 28, color: T.ink, letterSpacing: '-0.01em', marginTop: 6, fontWeight: 500 }}>{s.value}</div>
              <div style={{ fontFamily: T.fBody, fontSize: 12, color: T.muted, lineHeight: 1.5, marginTop: 4 }}>{s.detail}</div>
            </div>
          ))}
        </div>

        <SubHead>Leadership · The 3 CXO test logins for this tenant</SubHead>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14, margin: '14px 0 8px' }}>
          {active.leadership.map((cx) => (
            <div key={cx.email} style={{ border: `1px solid ${T.borderLt}`, background: active.accentSoft, borderRadius: 8, padding: 18 }}>
              <div style={{ fontFamily: T.fMono, fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: active.accent, marginBottom: 6 }}>{cx.title}</div>
              <div style={{ fontFamily: T.fBody, fontSize: 16, fontWeight: 700, color: T.ink, marginBottom: 4 }}>{cx.name}</div>
              <div style={{ fontFamily: T.fMono, fontSize: 11, color: T.muted, marginBottom: 8 }}>{cx.email}</div>
              <div style={{ fontFamily: T.fBody, fontSize: 12, color: T.body, lineHeight: 1.55 }}>{cx.responsibility}</div>
            </div>
          ))}
        </div>
        <div style={{ fontFamily: T.fBody, fontSize: 12, color: T.muted, marginTop: 14 }}>
          Shared password <code style={{ fontFamily: T.fMono, padding: '2px 6px', background: T.surface3, borderRadius: 3 }}>Demo2026!</code> · OTP code <code style={{ fontFamily: T.fMono, padding: '2px 6px', background: T.surface3, borderRadius: 3 }}>424242</code> · provisioned via <code style={{ fontFamily: T.fMono, padding: '2px 6px', background: T.surface3, borderRadius: 3 }}>npx tsx scripts/provision-cxo-personas.ts --apply</code>
        </div>
      </Section>
      ...
      <Section>
        ...
        <ul style={{ margin: '12px 0 0', paddingLeft: 20, fontFamily: T.fBody, fontSize: 14, lineHeight: 1.75, color: T.body }}>
          {active.whatsAtStakeBullets.map((b) => <li key={b} style={{ marginBottom: 6 }}>{b}</li>)}
        </ul>
      </Section>
      ...
      <Section>
        ...
        <SectionTitle>{"What's loaded into the database for this tenant"}</SectionTitle>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: T.fBody, fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: `2px solid ${T.border}` }}>
                {['Record kind', 'Count', 'What\'s in it (examples)'].map((h) => (
                  <th key={h} style={{ textAlign: 'left', padding: '10px 12px', fontFamily: T.fMono, fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: T.muted }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {active.dataLoaded.map((d) => (
                <tr key={d.kind} style={{ borderBottom: `1px solid ${T.borderLt}` }}>
                  <td style={{ padding: '12px', fontFamily: T.fBody, fontWeight: 700, color: T.ink, whiteSpace: 'nowrap' }}>{d.kind}</td>
                  <td style={{ padding: '12px', fontFamily: T.fDisp, fontSize: 22, color: active.accent, fontWeight: 500 }}>{d.count}</td>
                  <td style={{ padding: '12px', color: T.muted, lineHeight: 1.55 }}>{d.example}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {active.agentSamples.map((qa, idx) => (
            <div key={idx} style={{ border: `1px solid ${T.border}`, borderRadius: 10, overflow: 'hidden', background: T.surface }}>
              ...
              <div style={{ padding: '18px 20px' }}>
                <div style={{ fontFamily: T.fBody, fontSize: 13, color: T.body, lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{qa.groundedAnswer}</div>
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px dashed ${T.borderLt}`, fontFamily: T.fMono, fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: T.faint }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 0, border: `1px solid ${T.border}`, borderRadius: 8, overflow: 'hidden', margin: '24px 0 8px' }}>
          {[
            ...
            <div key={s.step} style={{ padding: 22, borderRight: `1px solid ${T.borderLt}`, background: i === 0 ? active.accentSoft : T.surface }}>
              ...
              <div style={{ fontFamily: T.fBody, fontSize: 15, fontWeight: 700, color: T.ink, marginBottom: 8 }}>{s.step}</div>
              <div style={{ fontFamily: T.fBody, fontSize: 13, color: T.muted, lineHeight: 1.6 }}>{s.body}</div>
            </div>
          ))}
        </div>
      </Section>
