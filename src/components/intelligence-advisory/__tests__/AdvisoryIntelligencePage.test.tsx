...
describe('AdvisoryIntelligencePage', () => {
  ...
  it('renders Intelligence as a chat-only advisor surface', () => {
    ...
    expect(
      screen.queryByLabelText('Intelligence briefing'),
    ).toBeNull();
    expect(screen.queryByRole('button', { name: /Industry Outlook/i })).toBeNull();
  ...
  it('keeps vertical starter questions visible in chat-only mode', () => {
    ...
    expect(
      screen.getByText('What are the top AI opportunities for Airline Demo, grounded only in the loaded context?'),
    ).toBeTruthy();
    expect(
      screen.getByText('Which current-state technology, data, or operating-model gaps should a CXO care about first?'),
    ).toBeTruthy();
  });
});
