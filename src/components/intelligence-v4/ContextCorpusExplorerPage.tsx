...
function deriveSignals(overview: EnterpriseContextOverview | null, tower: AiControlTowerReadModel): IntelligenceSignal[] {
  ...
  return [...contextSignals, ...riskSignals, ...spendSignals, ...adoptionSignals].slice(0, 10);
}
...
function buildSentinelAnswer(question: string, tower: AiControlTowerReadModel, overview: EnterpriseContextOverview | null): SentinelAnswer {
  ...
  const derivedRead = overview?.derivedEnterpriseRead ?? tower.derivedEnterpriseRead ?? null;
  if (
    derivedRead &&
    (
      ...
      lower.includes('kyriba') ||
      lower.includes('what is') ||
      lower.includes('telling')
    )
  ) {
    const topMoves = derivedRead.recommendedMoves.slice(0, 3);
    return {
      headline: derivedRead.headline,
      columns: ['Read', 'What it means', 'Evidence path'],
      rows: [
        ['Current state', derivedRead.architecturePattern || derivedRead.executiveSummary, derivedRead.source.path],
        ['Maturity', derivedRead.maturityRead || derivedRead.whatThisMeans, derivedRead.source.kind],
        ['North star', derivedRead.northStar || derivedRead.peerImplication, `${derivedRead.matchedPatterns.length} corpus pattern matches`],
        ...topMoves.map((move) => [move.title, move.decision, move.expectedImpact]),
      ],
      citations: [
        derivedRead.source.path,
        ...derivedRead.insights.flatMap((insight) => insight.evidence).slice(0, 5),
      ],
      next: ['Browse the context dimensions', 'Show corpus pattern matches', 'Shape the top move'],
    };
  }
    ...
    <main style={styles.page}>
      ...
      <section style={styles.hero}>
        <div>
          ...
          <p style={styles.lede}>{overview?.derivedEnterpriseRead?.executiveSummary.split('\n')[0] ?? 'A reading of the loaded enterprise context and corpus — in plain English, cross-domain, with the supporting evidence and source citations one click away.'}</p>
        </div>
      ...
      <footer style={styles.footer}>
        <span>{tenantName} context intelligence</span>
