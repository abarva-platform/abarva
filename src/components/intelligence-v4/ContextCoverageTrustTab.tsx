...
const SOURCES = [
  { nm: 'Application Inventory', ty: 'CSV', fr: 'loaded' as DimStatus, rec: 142, reached: 8, upd: 'Jun 14' },
  { nm: 'AI Initiative Register', ty: 'XLSX', fr: 'loaded' as DimStatus, rec: 38, reached: 8, upd: 'Jun 12' },
  { nm: 'Vendor Contracts', ty: 'XLSX', fr: 'attention' as DimStatus, rec: 67, reached: 7, upd: 'May 28' },
  { nm: 'Operating telemetry (ITSM)', ty: 'CSV', fr: 'stale' as DimStatus, rec: 500, reached: 8, upd: 'Apr 30' },
  { nm: 'Org Structure', ty: 'Source artifact', fr: 'loaded' as DimStatus, rec: 19, reached: 8, upd: 'Jun 11' },
  { nm: 'AMS Board Memo', ty: 'PDF', fr: 'review' as DimStatus, rec: 0, reached: 4, upd: 'Jun 15' },
];
...
function verdictForReqs(req: number[]): 'lit' | 'degraded' | 'blocked' {
  ...
  if (statuses.some((s) => s === 'stale' || s === 'attention' || s === 'review')) return 'degraded';
  return 'lit';
}
...
function dimHeatClass(st: DimStatus): { bg: string; border: string; textColor: string } {
  const map: Record<DimStatus, { bg: string; border: string; textColor: string }> = {
    loaded: { bg: C.freshBg, border: `${C.fresh}40`, textColor: C.fresh },
    attention: { bg: C.attentionBg, border: `${C.attention}44`, textColor: C.attention },
    stale: { bg: C.staleBg, border: `${C.stale}44`, textColor: C.stale },
    review: { bg: C.reviewBg, border: `${C.review}44`, textColor: C.review },
    missing: { bg: 'repeating-linear-gradient(45deg, #fbfaf7, #fbfaf7 6px, #f3f1ec 6px, #f3f1ec 12px)', border: C.line, textColor: C.unknown },
  };
  return map[st];
}
...
export function ContextCoverageTrustTab() {
  return (
    <div>
      <h2
        style={{
      ...
      <p style={{ color: C.muted, fontSize: 12.5, margin: '0 0 14px', maxWidth: 720 }}>
        The plumbing lens: which dimensions fed the context, how fresh, and which insights they unlock. Support for the meaning above — not the headline.
      </p>
      ...
      >
        {[
          { v: '14', k: 'Sources', d: '9 fresh · 3 attention · 2 stale' },
          { v: '1,284', k: 'Active facts', d: '96 superseded · 71% cited' },
          { v: '71%', k: 'Evidence coverage', bar: 71, barColor: C.attention },
          { v: '78%', k: 'Answerability', bar: 78, barColor: C.fresh },
        ].map((kpi) => (
          <div
            key={kpi.k}
            style={{
          ...
          >
            ...
            <div style={{ fontSize: 11, color: C.muted, marginTop: 6 }}>{kpi.k}</div>
            {kpi.d && <div style={{ fontSize: 10.5, color: C.muted, marginTop: 3 }}>{kpi.d}</div>}
            {kpi.bar !== undefined && (
              <div
                style={{
                  height: 4,
      ...
      >
        ...
        {/* Legend */}
        <div style={{ display: 'flex', gap: 13, flexWrap: 'wrap' as const, fontSize: 11, color: C.muted, marginBottom: 11 }}>
          {(['loaded', 'attention', 'stale', 'review', 'missing'] as DimStatus[]).map((st) => (
      ...
      >
        ...
        <div style={{ fontSize: 11.5, color: C.muted, marginTop: 11 }}>
          ...
          <strong>Renewal-without-benchmark</strong> and <strong>Compliance exposure</strong> need dimensions 13 &amp; 12.
        </div>
      </div>
      ...
      >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
            ...
            <tbody>
              {SOURCES.map((s, idx) => {
                ...
                return (
                  <tr key={idx}>
                    <td style={{ padding: '9px 10px', borderBottom: idx < SOURCES.length - 1 ? `1px solid ${C.line2}` : 'none' }}>
                      <span
                        style={{
                          width: 8,
                          height: 8,
                      ...
                      {s.nm}
                      <br />
                      <span style={{ fontSize: 10.5, color: C.muted }}>{s.ty} · {s.upd}</span>
                    </td>
                    <td style={{ padding: '9px 10px', borderBottom: idx < SOURCES.length - 1 ? `1px solid ${C.line2}` : 'none' }}>
                      <span
                        style={{
                          ...
                          padding: '2px 7px',
                          borderRadius: 20,
                          background: s.fr === 'loaded' ? C.freshBg : s.fr === 'attention' ? C.attentionBg : s.fr === 'stale' ? C.staleBg : C.reviewBg,
                          color: col,
                        }}
                      >
                        {s.fr}
                    ...
                    <td style={{ padding: '9px 10px', borderBottom: idx < SOURCES.length - 1 ? `1px solid ${C.line2}` : 'none', color: C.muted, fontVariantNumeric: 'tabular-nums' }}>
                      {s.reached}/8
                    </td>
                  </tr>
                );
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {TRUTH_FUNNEL.map((row) => {
              const barW = 34 + (row.n / 6) * 160;
              const barColor = row.phase === 0 ? C.fresh : row.phase === 1 ? C.attention : C.stale;
              return (
