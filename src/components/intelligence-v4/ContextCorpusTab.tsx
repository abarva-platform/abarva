...
// Relevant: stub patterns matched to live insights
...
const CORPUS_PATTERNS: CorpusPattern[] = [
  { h: 'AMS Vendor Consolidation Playbook', c: 'Sourcing', conf: 0.86, depth: 9, x: 'Consolidating fragmented AMS contracts; leverage windows and benchmarks.' },
  { h: 'AMS Renewal Negotiation Canon', c: 'Sourcing', conf: 0.82, depth: 8, x: 'Levers for incumbent AMS renewals — benchmark anchoring, exit credibility.' },
  { h: 'Copilot Value Realization Curve', c: 'AI Value', conf: 0.78, depth: 7, x: 'Adoption→productivity lag for code-assist tools.' },
  { h: 'AI Initiative Stage-Gate Model', c: 'Operating Model', conf: 0.80, depth: 8, x: 'Pilot→Scale→Embed gates and the evidence each requires.' },
  { h: 'ITSM AI Triage Pattern', c: 'AI Value', conf: 0.74, depth: 6, x: 'Where ML-assisted routing cuts MTTR.' },
];
...
function CorpusPatternCard({ p, showCategory = false, fit }: { p: CorpusPattern; showCategory?: boolean; fit?: 'high' | 'medium' }) {
  return (
    <div
      style={{
        background: C.panel,
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 11, flexWrap: 'wrap' as const }}>
        {fit && (
          <span
            style={{
              fontSize: 10.5,
          ...
          >
            applies to → {RELEVANT_MATCHES.find((m) => m.p.h === p.h)?.to ?? 'insight'}
          </span>
        )}
        <span
          style={{
  ...
  return (
    <div>
      ...
      <p style={{ color: C.muted, fontSize: 12.5, margin: '0 0 14px', maxWidth: 720 }}>
        The global knowledge library. At 10k+ patterns it&apos;s never a flat list — enter by relevance to your insights, by taxonomy, or by search.
      </p>
        ...
        {SUBTABS.map((t) => (
          <button
            ...
            style={{
              ...
              borderBottom: mode === t.key ? `2px solid ${C.ink}` : '2px solid transparent',
              marginBottom: -1,
              cursor: 'pointer',
              fontFamily: 'inherit',
      ...
      {mode === 'relevant' && (
        <div>
          <p style={{ fontSize: 12, color: C.muted, margin: '-4px 0 12px' }}>
            Patterns matched to your 6 live insights, ranked by applicability.
          </p>
          {RELEVANT_MATCHES.map((r, idx) => (
            <CorpusPatternCard key={idx} p={r.p} fit={r.fit as 'high' | 'medium'} />
          ))}
        </div>
      )}
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 11, color: C.muted, marginTop: 10 }}>
              Real library: 10,000+ patterns reached by category → sub-topic → cluster (via pattern relationships). Full list never rendered.
            </div>
          </div>
            ...
            <div style={{ fontSize: 12.5, color: C.muted }}>
              {CORPUS_PATTERNS.map((p) => (
                ...
                >
                  ...
                  <span>
                    <span style={{ color: C.blue, fontWeight: 600 }}>{p.conf}</span>
