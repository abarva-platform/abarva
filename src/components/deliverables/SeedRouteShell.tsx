...
export function SeedTenantDashboard({ tenant }: { tenant: TenantSeedPlan }) {
  ...
  return (
    <SeedPageFrame
      eyebrow={`${tenant.displayName} · tenant route`}
      title="Seeded program surface"
      summary="Every canonical tenant route resolves to a scoped page before the final production UI is layered on top."
    >
      <MetricGrid
        metrics={[
      ...
      <CardGrid>
        <LinkCard
          href={tenantProgramsPath(tenant)}
          label="Programs"
          title={`${tenant.displayName} programs`}
          description="Tenant-scoped program index and deliverable paths."
        />
        <LinkCard
          href={`/tenant/${tenant.routeSlug}/tower`}
          label="Tower"
          title="Control Tower placeholder"
          description="Canonical tower URL for this tenant."
        />
      </CardGrid>
    </SeedPageFrame>
  );
...
export function SeedProgramsIndex({ tenant }: { tenant: TenantSeedPlan }) {
  return (
    <SeedPageFrame
      eyebrow={`${tenant.displayName} · programs`}
      title="Programs"
      summary={`${tenant.programs.length} seeded programs with canonical phase and deliverable routes.`}
    >
      <CardGrid>
...
export function SeedProgramOverview({
  tenant,
  program,
}: {
  ...
  return (
    <SeedPageFrame
      eyebrow={`${tenant.displayName} · ${program.code}`}
      title={program.name}
      summary={`${program.roleInDemo}. Canonical program route is wired with phase, deliverable, and source-pattern links.`}
    >
      <MetricGrid
        metrics={[
          [
            "Current phase",
            `P${program.currentPhaseSpec}`,
            phaseMeta(program.currentPhaseSpec).name,
          ],
          ["Deliverables", program.deliverables.length.toString(), "all tiers"],
          ["Rich", countTier(program, "rich").toString(), "full render"],
          ["Outline", countTier(program, "outline").toString(), "draft render"],
          ["Stub", countTier(program, "stub").toString(), "future-safe"],
        ]}
      />
...
export function SeedPhaseOverview({
  tenant,
  program,
  phase,
}: {
  ...
  const deliverables = program.deliverables.filter(
    (deliverable) => deliverable.phaseSpec === phase,
  );
  const meta = phaseMeta(phase);
  return (
    <SeedPageFrame
      eyebrow={`${tenant.displayName} · ${program.code} · phase ${phase}`}
      title={meta.name}
      summary={meta.gateCriterion}
    >
      <MetricGrid
        metrics={[
...
export function SeedTenantPattern({
  tenant,
  patternSlug,
}: {
  tenant: TenantSeedPlan;
  ...
  const matchingPrograms = tenant.programs.filter(
    (program) => program.patternSlug === patternSlug,
  );
  const pattern = getPatternManifestEntryWithMetrics(
    patternSlug,
    tenant.routeSlug,
  );
  const evidenceSources = getPatternBrowsableEvidence(
    patternSlug,
    tenant.routeSlug,
  ).slice(0, 6);
  ...
  const highestPhase = applicablePrograms.reduce(
    (max, program) => Math.max(max, program.currentPhaseSpec),
    0,
  );
  const overlayState =
  ...
  const traceableDeliverables = applicablePrograms
    ...
    .sort(
      (a, b) =>
        Number(b.renderTier === "rich") - Number(a.renderTier === "rich") ||
  ...
  return (
    <SeedPageFrame
      ...
      summary={
        ...
        "Canonical tenant-scoped pattern route. It exists so source-pattern cross-links never leak to localhost or 404."
      }
    >
      <MetricGrid
        metrics={[
          ...
          [
            "Freshness",
            pattern ? formatSeedFreshness(pattern.lastUpdatedAt) : "unknown",
            "from source file",
          ],
        ]}
      />
      <div
        className="del-panel"
        style={{
          borderColor: "rgba(169, 111, 0, 0.35)",
          background: "rgba(169, 111, 0, 0.08)",
        }}
      >
        <div className="del-eyebrow" style={{ color: "var(--del-amber)" }}>
      ...
      {pattern ? (
        <CardGrid>
          <LinkCard
            ...
            title="Open full pattern page"
            description={`${pattern.sections.length} authored sections · ${pattern.diagnosticQuestions.length} diagnostic probes`}
          />
        </CardGrid>
      ) : null}
      {applicablePrograms.length > 0 ? (
        ...
        <div className="del-panel">
          ...
          <p style={{ color: "var(--del-muted)", lineHeight: 1.65 }}>
            No seeded program currently applies this pattern for{" "}
            {tenant.displayName}. The route still renders to preserve link
            integrity.
          </p>
        </div>
      )}
      ...
      <div className="del-panel">
        ...
        >
          {COMPOSITE_DISCLAIMER} Demo rendering: tenant-specific pattern state
          is generated from seeded program and deliverable links and must be
          sponsor-validated before production use.
        </p>
      </div>
    </SeedPageFrame>
  );
...
export function SeedGlobalPattern({ patternSlug }: { patternSlug: string }) {
  ...
  const matchingPrograms = plan.programs.filter(
    (program) => program.patternSlug === patternSlug,
  );
  ...
  return (
    <SeedPageFrame
      ...
      summary={
        pattern?.shortDescription ??
        "Canonical global pattern route. Tenant-scoped variants are available for each applying program."
      }
    >
      {matchingPrograms.length > 0 ? (
        <CardGrid>
          {matchingPrograms.map((program) => (
            <LinkCard
              key={program.code}
              href={tenantProgramPath(
                { routeSlug: program.tenantRouteSlug },
                program,
              )}
              label={`${program.clientDisplayName} · ${program.code}`}
              title={program.name}
              description={`${program.deliverables.length} deliverables apply this pattern.`}
            />
          ))}
        ...
        <div className="del-panel">
          <div className="del-eyebrow">Pattern placeholder</div>
          <p style={{ color: "var(--del-muted)", lineHeight: 1.65 }}>
            This pattern route renders as a safe placeholder until detailed
            pattern content is authored.
          </p>
        </div>
      )}
      {evidenceSources.length > 0 ? (
...
export function SeedTenantTower({ tenant }: { tenant: TenantSeedPlan }) {
  ...
  return (
    <SeedPageFrame
      eyebrow={`${tenant.displayName} · control tower`}
      title="Tenant tower route"
      summary="Canonical tower URL is ready for tenant-scoped cockpit integration."
    >
      <MetricGrid
        metrics={[
...
export function SeedTenantTowerSubsurface({
  tenant,
  surface,
}: {
  tenant: TenantSeedPlan;
  surface: TowerSubsurfaceSlug;
}) {
  const definition = TOWER_SUBSURFACE_DEFINITIONS.find(
    (entry) => entry.slug === surface,
  )!;
    ...
    >
      ...
      >
        ...
        <p style={{ color: "var(--del-muted)", lineHeight: 1.65, margin: 0 }}>
          The route intentionally renders a Stub-style scheduled state so demo
          navigation never dead-ends or invents unsupported Tower functionality.
        </p>
      </div>
      <CardGrid>
        <LinkCard
...
export function SeedOperationsPortfolio() {
  ...
  return (
    <SeedPageFrame
      ...
      title="Seed portfolio route"
      summary={`${plan.summary.programCount} programs across ${plan.summary.tenantCount} tenants, with ${plan.summary.deliverableCount} deliverable routes.`}
    >
      <MetricGrid
        metrics={[
          ...
          [
            "Deliverables",
            plan.summary.deliverableCount.toString(),
            "canonical routes",
          ],
          ["Rich", plan.summary.richDeliverableCount.toString(), "full render"],
          [
            "Outline",
            plan.summary.outlineDeliverableCount.toString(),
            "draft render",
          ],
...
function PatternObservationsPipelinePanel() {
  return (
    <div
      className="del-panel"
      style={{
        borderColor: "rgba(14, 159, 140, 0.24)",
    ...
    >
      ...
      >
        This pattern receives observations from completed Phase 5 programs. When
        Morrison reaches Phase 5 outcome attestation, observations will be
        anonymized, composite-tagged, and contributed back to this pattern.
      </p>
      <div
        style={{
          marginTop: 12,
      ...
      >
        <span className="del-eyebrow" style={{ color: "var(--del-teal)" }}>
          Zero state
        </span>
        <span
          style={{ color: "var(--del-ink)", fontSize: 13, lineHeight: 1.4 }}
        >
    ...
    <main className="del-page">
      ...
      <div className="del-shell">
        ...
        <footer className="del-footer">
          {COMPOSITE_DISCLAIMER} Canonical seed route scaffold for demo
          integrity.
        </footer>
      </div>
    </main>
...
function LinkCard({
  ...
  return (
    <Link className="del-link-card" href={href} style={{ minHeight: 122 }}>
      <span>{label}</span>
      <strong style={{ display: "block", fontSize: 19, lineHeight: 1.25 }}>
        {title}
      </strong>
...
function SectionTitle({ label }: { label: string }) {
  return (
    <div className="del-eyebrow" style={{ marginTop: 10 }}>
      {label}
    </div>
  );
}
...
function summarizeTenant(tenant: TenantSeedPlan) {
  const deliverables = tenant.programs.flatMap(
    (program) => program.deliverables,
  );
  return {
    deliverables: deliverables.length,
    rich: deliverables.filter(
      (deliverable) => deliverable.renderTier === "rich",
    ).length,
    outline: deliverables.filter(
      (deliverable) => deliverable.renderTier === "outline",
    ).length,
    stub: deliverables.filter(
...
function countTier(
  ...
  return program.deliverables.filter(
    (deliverable) => deliverable.renderTier === tier,
  ).length;
}
