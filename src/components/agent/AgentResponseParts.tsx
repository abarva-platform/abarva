      ...
      <div style={METRIC_GRID_STYLE}>
        {part.metrics.map((metric) => (
          <div key={metric.label} style={METRIC_CARD_STYLE}>
            ...
            <strong
              style={{ ...METRIC_VALUE_STYLE, color: toneColor(metric.tone) }}
            >
              {metric.value}
            </strong>
          </div>
...
function NextActionPart({ part }: { part: AgentNextActionPart }) {
  return (
    <section style={NEXT_ACTION_STYLE} data-testid="agent-response-next-action">
      ...
      {part.confidence ? (
        <p style={CAPTION_STYLE}>Confidence: {part.confidence}</p>
      ) : null}
    </section>
  );
}
...
const PARTS_STACK_STYLE: CSSProperties = {
  display: "grid",
  gap: 12,
  width: "100%",
};
...
const METRIC_CARD_STYLE: CSSProperties = {
  border: "1px solid rgba(38,48,64,0.14)",
  borderRadius: 7,
  padding: "7px 8px",
...
const TH_STYLE: CSSProperties = {
  ...
  fontSize: 10,
  color: "rgba(38,48,64,0.72)",
  background: "rgba(38,48,64,0.05)",
  borderBottom: "1px solid rgba(38,48,64,0.12)",
};
const TD_STYLE: CSSProperties = {
  ...
  lineHeight: 1.45,
  color: "#263040",
  borderTop: "1px solid rgba(38,48,64,0.08)",
  verticalAlign: "top",
};
const BAR_STACK_STYLE: CSSProperties = { display: "grid", gap: 8 };
const BAR_ROW_STYLE: CSSProperties = {
  display: "grid",
  gridTemplateColumns: "92px minmax(96px, 1fr) auto",
  alignItems: "center",
  gap: 8,
};
...
const CITATION_STYLE: CSSProperties = {
  borderLeft: "3px solid rgba(49,95,134,0.48)",
  paddingLeft: 8,
};
const CITATION_LABEL_STYLE: CSSProperties = {
  fontSize: 11,
  fontWeight: 700,
  color: "#263040",
};
const CITATION_META_STYLE: CSSProperties = {
  fontSize: 10,
  color: "rgba(38,48,64,0.58)",
};
