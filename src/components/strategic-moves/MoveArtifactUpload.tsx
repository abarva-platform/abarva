...
import { DiscoveryReceiptCard } from "@/components/programs/discovery/DiscoveryReceiptCard";
...
type UploadState =
  ...
  | {
      ...
      receipt?: ExtractionReceipt | null;
    }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {uploadState.status === "success" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <div style={S.successChip}>
            <span>✓</span>
            {uploadState.record.originalName}
            <span style={{ opacity: 0.6 }}>
              · {formatBytes(uploadState.record.sizeBytes)}
            </span>
          </div>
          {/* Discovery Intake (Tier B): name-every-stage extraction receipt, when the
              upload route routed evidence into the discovery shape (flag-gated). */}
          {uploadState.receipt && (
            <DiscoveryReceiptCard receipt={uploadState.receipt} />
          )}
          <button style={S.smallBtn} onClick={reset} type="button">
            Upload another file
          </button>
        </div>
      )}
      ...
      {uploadState.status === "idle" && (
        ...
        >
          ...
          <div style={S.sub}>
            PDF, Word, Excel, PowerPoint, image — max 100 MB
          </div>
        </div>
      )}
    </div>
