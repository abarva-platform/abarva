...
import type { ByFunctionData, MyStrategyData, PeerActivityData } from '@/lib/intelligence-v3/stages-display';
  ...
  themes: [
    {
      ...
      strategicContext: 'Reduce incident frequency while preserving delivery speed.',
      initiativeCount: 1,
...
describe('IntelligenceV3Page tenant corpus rendering', () => {
  ...
  it('uses live vendor data for non-Meridian tenants', async () => {
    ...
    expect(await screen.findByText('AeroEdge Cloud')).toBeInTheDocument();
    expect(document.body.textContent ?? '').not.toMatch(/No tenant-specific vendor spend is loaded yet/i);
  });
  ...
  it('uses live my-strategy bullets for non-Meridian tenants', async () => {
    ...
    expect(await screen.findByText('Platform reliability play')).toBeInTheDocument();
