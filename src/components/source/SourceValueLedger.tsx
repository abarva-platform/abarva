...
function deriveRows(snapshot: SourceValueLedgerSnapshot): {
  ...
  const committed = projected.filter(
    (entry) => entry.confidence === "high" && entry.evidenceCount > 0,
  );
  const measuring = projected.filter(
    (entry) => !(entry.confidence === "high" && entry.evidenceCount > 0),
  );
...
export function SourceValueLedger({
  ...
  const { rows, projected, committed, measuring, realized } =
    deriveRows(snapshot);
  ...
  const lowConfidenceRows = rows.filter(
    ({ entry }) => entry.confidence === "low" || entry.evidenceCount === 0,
  );
  const assumptionCount = assumptions.length;
  const totalLowConfidenceAmount = lowConfidenceRows.reduce(
    (sum, row) => sum + Math.abs(row.entry.amountUsd),
    0,
  );
    ...
    <section style={SECTION_GRID}>
      <section style={SOURCE_CARD} aria-label="Atlas value ledger shell">
        ...
        >
          <div>
            ...
            >
              Atlas is monitoring value intent, measurability, and risk posture
              for the seeded Source program set.
            </p>
          </div>
        ...
        {isDegraded ? (
          ...
          >
            Source value data is temporarily unavailable. Atlas is showing a
            degraded empty ledger rather than inventing value.
          </div>
        ) : null}
        ...
        <div style={SUMMARY_PANEL}>
          <article style={PANEL_CARD}>
            ...
            <div style={{ ...TEXT_SMALL, color: "#576074" }}>
              {projected.length} line items from seeded event contracts.
            </div>
          </article>
          <article style={PANEL_CARD}>
            ...
            <div style={{ ...TEXT_SMALL, color: "#576074" }}>
              Evidence-backed high-confidence line items.
            </div>
          </article>
          <article style={PANEL_CARD}>
            ...
            <div style={{ ...TEXT_SMALL, color: "#576074" }}>
              {measuring.length} items not yet confirmed.
            </div>
          </article>
          <article style={PANEL_CARD}>
            ...
            <div style={{ ...TEXT_SMALL, color: "#576074" }}>
              {realized.length} realized entries with seeded status only.
            </div>
          </article>
        </div>
      </section>
      ...
      <div style={TWO_COL}>
        <section style={SOURCE_CARD}>
          <div style={{ ...PERSPECTIVE_LABEL, color: "#1D4F8C" }}>
            Value ledger line items
          </div>
          <div style={{ overflowX: "auto", marginTop: 8 }}>
            <table
              style={{ width: "100%", borderCollapse: "collapse" }}
              aria-label="Source value line item table"
            >
              <thead>
                <tr>
                  ...
                  <tr>
                    <td style={SOURCE_TABLE_CELL} colSpan={7}>
                      <span style={{ ...TEXT_SMALL, color: "#576074" }}>
                        No value ledger rows are available for this tenant right
                        now.
                      </span>
                    </td>
                  </tr>
                  ...
                  rows.map((row, index) => (
                    <tr key={`${row.entry.id}-${row.perspective}-${index}`}>
                      <td style={SOURCE_TABLE_CELL}>
                        {toPerspectiveLabel(row.perspective)}
                      </td>
                      ...
                      <td style={SOURCE_TABLE_CELL}>
                        {row.entry.evidenceCount}
                      </td>
                      <td style={SOURCE_TABLE_CELL}>
                        <span
                          style={{
                            color: confidenceColor(row.entry.confidence),
                          }}
                        >
        ...
        <aside style={SECTION_GRID}>
          ...
          <section id="value-confidence" style={SOURCE_CARD}>
            ...
            <p style={{ ...TEXT_SMALL, marginTop: 8 }}>
              Confidence is deterministic from seeded evidence depth and stage
              assumptions. We treat low confidence and zero-evidence items as
              ...
              {lowConfidenceRows.length > 0 ? (
                lowConfidenceRows.slice(0, 5).map((row) => (
                  <li key={`${row.entry.id}-conf`} style={TEXT_SMALL}>
                    {toPerspectiveLabel(row.perspective)} item &quot;
                    {row.entry.label}&quot; remains at {row.entry.confidence}{" "}
                    confidence.
                  </li>
                ))
              ) : (
                <li style={TEXT_SMALL}>
                  No low-confidence or missing-evidence items are seeded.
                </li>
              )}
            </ul>
          </section>
          ...
          <section id="value-assumptions" style={SOURCE_CARD}>
            ...
            <div style={{ ...TEXT_SMALL, color: "#9B5F17", marginTop: 8 }}>
              Variance watch: {assumptionCount} assumption anchors,{" "}
              {formatSourceFinancialValue(
                totalLowConfidenceAmount,
                canViewFinancialValues,
              )}{" "}
              currently in low-confidence posture.
            </div>
          </section>
          ...
          <section id="value-actions" style={SOURCE_CARD}>
            ...
            <div style={{ display: "grid", gap: 8, marginTop: 8 }}>
              <a href="#value-assumptions" style={ACTION_LINK}>
                Show assumptions
              </a>
              <a href="#value-confidence" style={ACTION_LINK}>
                Show evidence gaps
              </a>
              <a href="#value-caveat" style={ACTION_LINK}>
                Explain value confidence
              </a>
              <label
                htmlFor="value-custom-input"
                style={{
                  ...TEXT_SMALL,
                  color: "#3B4453",
                  display: "grid",
      ...
      <div
        style={{ ...SOURCE_CARD, padding: "12px 16px", background: "#F8FAFF" }}
        id="value-caveat"
      >
        <div style={{ ...TEXT_SMALL, color: "#576074" }}>
          Atlas ledger values are deterministic, seed-backed, and for execution
          planning. This is not a live realized-savings claim unless downstream
          evidence and measurement gates are explicitly marked as usable.
        </div>
      </div>
    </section>
  );
