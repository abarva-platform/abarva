...
import {
  SOURCE_INDEX_VIEW,
  AMS_SOURCE_EVENT,
} from "@/lib/source/shell-source-fixture";
...
function VendorResponseModal({ onClose }: VendorResponseModalProps) {
  ...
  function handleSubmit() {
    if (!canSubmit) return;
    console.log("BAFO response recorded", {
      vendor,
      responseType,
      price,
      priceUnit,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Divider */}
        <div
          style={{ height: 1, background: SHELL.CARD_LINE, marginBottom: 20 }}
        />
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {vendors.map(({ key, label }) => (
              ...
              >
                {label}
                {key === "B" && vendor === "B" && (
                  <span style={{ marginLeft: 5, color: SHELL.PEACH_TEXT }}>
                    ⚠
                  </span>
                )}
              </button>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          {isWithdrawal && vendor && (
            ...
            >
              Marking {vendorName} as withdrawn from BAFO. This cannot be
              undone.
            </div>
          )}
        </div>
        ...
        >
          ...
          >
            Ava will extract scoring signals from this response and update the
        ...
        {submitted && (
          ...
          >
            ✓ {vendorName} response recorded · Ava will process and update
            rankings
          </div>
        )}
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 11,
              color:
                !canSubmit || submitted ? SHELL.INK_MUTED : SHELL.CARD_WHITE,
    ...
    >
      <AgentColumn
        ...
        actions={[
          {
            letter: "A",
            text: "Originate a source event",
            detail: "Start a new vendor consolidation or procurement exercise",
          },
          {
            letter: "B",
            text: "Browse event templates",
            detail: "Pre-configured event structures for common source types",
          },
          {
            letter: "C",
            text: "Import from prior system",
            detail: "Link existing procurement records from ServiceNow or SAP",
          },
        ]}
        surface="source"
      />
    ...
    >
      <SentinelAgentColumn
        ...
        onActionClick={(letter) => {
          if (letter === "A") {
            firstEventRowRef.current?.scrollIntoView({
              behavior: "smooth",
              block: "start",
            });
          } else if (letter === "B") {
      ...
      >
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          >
            AMS Vendor Consolidation award confirms Vendor C as the managed CDP
            layer. This unlocks the P3 Design architecture sprint and is a
            required criterion for the P3→P4 Build gate.
          </p>
          <LinkedProgramChip
            direction="source-to-program"
            linkedId="APX-CDP-2026"
        ...
        >
          ...
          >
            ⚠ Steward flag: Vendor B SOC-2 Type II attestation gap · must
            resolve before Award stage
          </span>
        </div>
