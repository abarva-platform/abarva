...
function readinessColor(
  status: SourceVendorSelectionReadiness["readinessStatus"],
): string {
...
function readinessLabel(
  ...
  switch (status) {
    case "ready_for_selection_review":
      return "Ready for selection review";
    case "proceed_to_bafo":
      return "Proceed to BAFO";
    case "defer_pending_clarifications":
      return "Clarifications needed";
    case "blocked_missing_pricing":
      return "Pricing incomplete";
    case "blocked_low_evidence":
      return "Evidence incomplete";
    case "blocked_gate_not_ready":
...
function sourceLabel(source: string): string {
  switch (source) {
    case "commercial-signals":
      return "Commercial signals";
    case "commercial-mission-adapter":
      return "Commercial mission adapter";
    case "executive-decision-summary":
      return "Executive decision summary";
    case "source-stage-gates":
      return "Stage gates";
    default:
      return source;
  }
    ...
    <section style={PANEL} aria-label="Vendor selection readiness panel">
      <div style={HEADER}>
        <div>
          ...
          >
            Deterministic synthesis for steering review. This panel does not
        ...
        <div style={POSTURE_BADGE}>
          ...
          <div
            style={{
              ...
              fontSize: 12,
              lineHeight: 1.4,
              color: readinessColor(readiness.readinessStatus),
            }}
          >
        ...
        <Section title="Readiness status">
          ...
          <ul style={LIST}>
            {readiness.requiredApprovals.length > 0 ? (
              readiness.requiredApprovals.map((approval) => (
          ...
          <ul style={LIST}>
            {readiness.unresolvedGateIssues.length > 0 ? (
              readiness.unresolvedGateIssues.map((issue) => (
        ...
        <Section title="Required artifacts / next action">
          ...
          <ul style={LIST}>
            {readiness.unresolvedCommercialIssues.length > 0 ? (
              readiness.unresolvedCommercialIssues.map((issue) => (
                <li key={issue}>{issue}</li>
              ))
            ) : (
          ...
          <ul style={LIST}>
            {readiness.unresolvedEvidenceIssues.length > 0 ? (
              readiness.unresolvedEvidenceIssues.map((issue) => (
                <li key={issue}>{issue}</li>
              ))
            ) : (
      ...
      <div style={GRID_THREE}>
        <Section title="Vendor outcomes">
          ...
          <ul style={LIST}>
            {readiness.viableVendors.length > 0 ? (
              readiness.viableVendors.map((vendor) => (
          ...
          <ul style={LIST}>
            {readiness.blockedVendors.length > 0 ? (
              readiness.blockedVendors.map((vendor) => (
                <li key={vendor}>{vendor}</li>
              ))
            ) : (
        ...
        <Section title="Guardrails">
          ...
          <ul style={LIST}>
            {readiness.sentinelCautions.length > 0 ? (
              readiness.sentinelCautions.map((note) => (
                <li key={note}>{note}</li>
              ))
            ) : (
        ...
        <Section title="aVa guidance">
          ...
          <div style={sourceSectionLabel}>Evidence used</div>
          <div
            style={{
              fontFamily: SHELL.SANS,
              fontSize: 12,
          ...
          >
            {readiness.sourceModulesUsed.map(sourceLabel).join(", ")}
          </div>
        </Section>
      </div>
    </section>
