...
export function SourcePortfolioBookPage({
  ...
  return (
    ...
    >
      <main
        data-testid="source-portfolio-book"
        style={MAIN_STYLE}
      >
  ...
  return (
    <header data-testid="source-book-header" style={HEADER_STYLE}>
      ...
      <div style={HEADER_ACTIONS_STYLE}>
        <Link href="/source/new" style={GHOST_BTN_STYLE} data-testid="source-book-optimize">
          Optimize a contract
        </Link>
        <Link href="/source/new" style={SOLID_BTN_STYLE} data-testid="source-book-new-event">
          New event
        </Link>
      </div>
    </header>
...
function BookBody({ view }: { view: PortfolioBookView }) {
  return (
    <div style={BODY_GRID_STYLE}>
      ...
      <aside style={RAIL_STYLE} aria-label="Renewals and proactive intelligence">
        {view.proactiveNudge ? (
      ...
      <div style={FLIGHT_TOP_STYLE}>
        ...
        {card.value ? (
          <div style={FLIGHT_VALUE_STYLE}>
            ...
            <div style={FLIGHT_VALUE_SECONDARY_STYLE}>{card.value.secondary}</div>
          </div>
        ) : null}
      </div>
      <StageRail
        activeIndex={card.stageIndex}
...
function StageRail({
  ...
  return (
    <div style={RAIL_TRACK_STYLE} role="img" aria-label={`Stage ${activeIndex + 1} of ${stepCount}`}>
      {dots.map((i) => {
        const state =
...
function RenewalsOnTheClock({ view }: { view: PortfolioBookView }) {
  return (
    <section aria-label="Renewals on the clock" data-testid="source-book-renewals">
      <div style={SECTION_HEAD_STYLE}>
