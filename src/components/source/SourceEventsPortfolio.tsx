...
export function SourceEventsPortfolio({
  ...
  const waitingEvents = filteredEvents.filter((event) =>
    event.status.startsWith("waiting_on_"),
  ).length;
  const openAlerts = filteredEvents.reduce(
    (sum, event) => sum + event.openAlerts,
    0,
  );
  const valueAtStake = filteredEvents.reduce(
  ...
  const topEvent =
    ...
    events[0] ??
    null;
  const linkedProgramCount = filteredEvents.filter((event) =>
      ...
      <div style={TWO_COL}>
        <article style={BRIEF_CARD}>
          ...
          <div style={{ marginTop: 12, ...CHIP_ROW }}>
            <ChoiceChip
              href={buildChoiceHref({ stage: "bafo" })}
              label="Review BAFO events"
            />
            <ChoiceChip
              href={buildChoiceHref({ stage: "evaluation" })}
              label="Review evaluation queue"
            />
            <ChoiceChip
              href={buildChoiceHref({ status: "at_risk" })}
              label="Review at-risk events"
            />
          </div>
          <div
            style={{
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div style={FILTER_ROW}>
            ...
            <div style={FILTER_GROUP}>
              ...
              <FilterLink
                href={buildChoiceHref({ stage: activeStage ?? undefined })}
                active={!activeStatus}
              >
                All
              </FilterLink>
...
function buildBriefBody(
  ...
  const stageLabel = activeStage
    ? (STAGE_LABELS.find((stage) => stage.key === activeStage)?.label ??
      activeStage)
    : "all stages";
  const statusLabel = activeStatus
    ? (STATUS_LABELS.find((status) => status.key === activeStatus)?.label ??
      activeStatus)
...
function MissingItem({ title, detail }: { title: string; detail: string }) {
  return (
    ...
    >
      <div style={{ fontSize: "13px", fontWeight: 700, color: SHELL.INK }}>
        {title}
      </div>
...
function MetricLine({ label, detail }: { label: string; detail: string }) {
  return (
    <div style={{ display: "grid", gap: 4 }}>
      ...
      <div style={{ ...SOURCE_MUTED, fontSize: "13px", color: SHELL.INK }}>
        {detail}
      </div>
    </div>
  );
