...
function hasNamedImport(
  ...
  return sourceFile.statements.some((statement) => {
    ...
    if (statement.moduleSpecifier.text !== modulePath) return false;

    const clause = statement.importClause;
    if (!clause) return false;
    if (importKind === "type" && !clause.isTypeOnly) return false;
    if (importKind === "value" && clause.isTypeOnly) return false;
...
describe("PROG21 · ProgramDetailPage · gate drawer imports", () => {
  it("imports buildGateApprovalDrawerView from gate-approval-drawer-view", () => {
    expect(
      hasNamedImport(
        detailAst,
        "@/lib/programs/gate-approval-drawer-view",
        "buildGateApprovalDrawerView",
        "value",
      ),
    ).toBe(true);
  });
...
const pendingView: ProgramDetailView = {
  programId: "fixture-program-2026",
  displayId: "FIXTURE-PROGRAM-2026",
