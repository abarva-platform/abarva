...
describe("Programs index linked-state and filters", () => {
  it("anchors the active healthcare flagship at P3 Design", () => {
    const view = buildProgramsIndexView("meridian-health");
    const flagship = view.programs.find(
      (program) => program.id === "mh-prog-agentic-care-data-accelerator",
    );
    ...
    expect(flagship?.linkedSourceEvent).toBe(
      "Meridian simulation handoff · 5 source artifacts · 9 corpus entries",
    );
    expect(flagship?.linkedSourceEventState).toBe(
      "Published to corpus index · not yet app-wired retrieval",
    );
  });
  ...
  it("builds the Meridian tenant view without Apex fixture bleed", () => {
    ...
    expect(view.tenant).toBe("Meridian Health");
    ...
    expect(
      view.programs.some((program) => program.displayId.startsWith("APX-")),
    ).toBe(false);
    expect(view.agentRail.map((agent) => agent.job).join(" ")).not.toContain(
      "APX-",
    );
  });

  it("builds the Meridian simulation detail fixture without Apex fallback copy", () => {
    const view = buildProgramDetailView(
      "mh-prog-agentic-care-data-accelerator",
    );
  ...
  it("returns a fresh program array per tenant view build", () => {
    ...
    expect(
      second.programs.some((program) => program.id === "test-only-row"),
    ).toBe(false);
  });
  ...
  it("filters active, idle, and gated rows deterministically", () => {
    ...
    expect(filterProgramRowsForIndex(view.programs, "all")).toHaveLength(
  ...
  it("locks filter-specific empty-state copy", () => {
    expect(getProgramsIndexEmptyStateTitle("all")).toBe(
      "No strategic moves yet",
    );
  ...
  it("locks canonical filter hrefs and visible-count summaries", () => {
    ...
    expect(getProgramsIndexFilterHref("active")).toBe(
