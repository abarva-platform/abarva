...
describe('Wave-15 anchor', () => {
  const waves = loadWaves().waves;
  type WaveEntry = WaveRecord;
  const w15 = waves.find((w: WaveEntry) => w.waveId === 'wave-15');
  it('wave-15 exists', () => expect(w15).toBeDefined());
  it('wave-15 has 10 plannedSlices', () => expect(w15?.plannedSlices).toHaveLength(10));
  it('wave-15 completedSlices contains all 10 lane IDs', () => {
    const ids = w15?.completedSlices ?? [];
    ['SRC19','SRC20','SRC21','SRC22','SRC23','SRC24','SRC25','SRC26','ARCH4','QA22'].forEach(id => {
...
describe('Wave-17 anchor', () => {
  const waves = loadWaves().waves;
  type WaveEntry = WaveRecord;
  const w17 = waves.find((w: WaveEntry) => w.waveId === 'wave-17');
  it('wave-17 exists', () => expect(w17).toBeDefined());
  it('wave-17 has 7 plannedSlices', () => expect(w17?.plannedSlices).toHaveLength(7));
  it('wave-17 completedSlices contains all 7 lane IDs', () => {
    const ids = w17?.completedSlices ?? [];
    ['DES7','DES8','ARCH5','PROD8','SRC31','QA24','DEMO5'].forEach(id => {
      expect(ids).toContain(id);
    });
  });
  it('wave-17 percentComplete is 100', () => expect(w17?.percentComplete).toBe(100));
  it('wave-17 status is merged', () => expect(w17?.status).toBe('merged'));
  it('wave-17 mergedPrs contains 362', () => expect(w17?.mergedPrs).toContain(362));
});
...
describe('Wave-18 anchor', () => {
  const waves = loadWaves().waves;
  type WaveEntry = WaveRecord;
  const w18 = waves.find((w: WaveEntry) => w.waveId === 'wave-18');
  it('wave-18 exists', () => expect(w18).toBeDefined());
  it('wave-18 has 7 plannedSlices', () => expect(w18?.plannedSlices).toHaveLength(7));
  it('wave-18 completedSlices contains all 7 lane IDs', () => {
    const ids = w18?.completedSlices ?? [];
    ['PROG10','PROG11','PROG12','PROG13','PROG14','QA25','DEMO6'].forEach(id => {
      expect(ids).toContain(id);
    });
  });
