...
describe("Tower invariants — Apple-grade polish slice", () => {
  describe("design system (locked AbarVa tokens on the index)", () => {
    ...
    it("uses #F8F7F4 cream as the index page background, not #ffffff", () => {
      ...
      expect(indexSource).toMatch(/PAGE_BG:\s*["']#F8F7F4["']/);
      expect(indexSource).not.toMatch(/PAGE_BG:\s*["']#ffffff["']/i);
    });
  ...
  describe("broker-bound portfolio cards", () => {
    const towerPageSource = readFileSync(
      `${TOWER_ROUTES_DIR}/page.tsx`,
      "utf8",
    );
    const brokerSource = readFileSync(
      "src/lib/tower/move-portfolio-broker.ts",
      "utf8",
    );
    ...
    it('does not surface a hardcoded "Apex Contact Center" string on any non-fixture Tower render-path file', () => {
      ...
      const files = [...walk(TOWER_ROUTES_DIR), ...walk(TOWER_COMPONENTS_DIR)];
      const offenders = files.filter((f) => {
        if (f.endsWith("apex-contact-center-portfolio-fixture.ts"))
          return false;
  ...
  describe("redirect-shell cleanup (audit §5.4)", () => {
    ...
    it("keeps Tower program detail dynamic segments aligned for Next.js 16", () => {
      expect(
        existsSync("src/app/(maestro)/tower/programs/[programId]/page.tsx"),
      ).toBe(true);
      expect(
        existsSync(
      ...
      expect(
        existsSync("src/app/(maestro)/tower/programs/[moveId]/value/page.tsx"),
      ).toBe(false);
    });
