...
import { buildProgrammeGateStatusView } from "@/lib/tower/programme-gate-status-view";
...
describe("TOWER8 · buildProgrammeGateStatusView · runtime contract", () => {
  ...
  it("summary counts match programmes array", () => {
    const blocked = view.programmes.filter(
      (p) => p.activeGate.status === "blocked",
    ).length;
    const atRisk = view.programmes.filter(
      (p) => p.activeGate.status === "at_risk",
    ).length;
    const pending = view.programmes.filter(
      (p) => p.activeGate.status === "pending",
    ).length;
