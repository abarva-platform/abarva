...
describe('TOWER_TABS', () => {
  it('declares exactly four canonical tabs', () => {
    expect(TOWER_TABS).toHaveLength(4);
  });

  it('contains portfolio, scorecards, pressure, executive_brief in that order', () => {
...
describe('resolveTowerTab', () => {
  ...
  const VALID: TowerTabKey[] = ['portfolio', 'scorecards', 'pressure', 'executive_brief'];
...
describe('buildTowerLensTabsView', () => {
  ...
  it('echoes the active tab', () => {
    const tabs: TowerTabKey[] = ['portfolio', 'scorecards', 'pressure', 'executive_brief'];
...
describe('TOWER4 component file probe', () => {
  ...
  it('references all four tab keys in the component source', () => {
    const src = fs.readFileSync(componentPath, 'utf8');
    for (const key of ['portfolio', 'scorecards', 'pressure', 'executive_brief']) {
