...
function initiative(
  partial: Partial<AIInitiative> &
    Pick<AIInitiative, "initiativeId" | "displayId" | "name">,
): AIInitiative {
  return {
...
function vendor(
  partial: Partial<AIInitiativeVendorRow> &
    Pick<AIInitiativeVendorRow, "vendorId" | "initiativeId" | "vendorName">,
): AIInitiativeVendorRow {
  return {
...
describe("Atlas reasoning traces", () => {
  it("summarizes Tower right-rail reasoning without persisting full substrate", () => {
    ...
    const initiatives = [
      initiative({
        ...
        statusSummary:
          "Measured value is 400000 against 1000000 annual committed.",
    ...
    const reasoningInput = {
      ...
      todayIso,
      lens: "value" as const,
      bandMetrics: buildTowerBandMetrics(
        initiatives,
        vendors,
        todayIso,
        "value",
      ),
      pressuresView: buildTowerPressuresView(
        initiatives,
        vendors,
        todayIso,
        "value",
    ...
    expect(
      interpretation.accountabilityDisclosure.humanDecisionBoundary,
    ).toContain("does not approve");
    expect(interpretation.accountabilityDisclosure.citationSummary).toContain(
      "citation",
    );
    expect(
      interpretation.accountabilityDisclosure.assumptionDisclosure,
    ).toContain("Tower registry");
    ...
    expect(trace.inputSummary).toEqual(
      expect.objectContaining({
        initiativesCount: 2,
        vendorsCount: 1,
        pressuresCount: 2,
    ...
    expect(trace.observations[0]).toEqual(
      expect.objectContaining({
        number: 1,
  ...
  it("defines the atlas_reasoning_traces table and indexes", () => {
    const migration = fs.readFileSync(
      path.join(
        process.cwd(),
        "supabase/migrations/20260513140000_atlas_reasoning_traces.sql",
      ),
      "utf8",
    );
    ...
    expect(migration).toContain(
      "trigger IN ('tower_right_rail_render','atlas_chat_turn','metric_explanation')",
    );
