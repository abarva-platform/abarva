...
const VALID_STATUSES: PageVerificationStatus[] = [
  "present",
  "missing",
  "deferred",
  "not_checked",
...
describe("allCriticalPagesPresent", () => {
  it("is true only when admin page and API route are both present", () => {
    const adminPage = report.pages.find(
      (p: PageVerificationRecord) =>
        p.routeId === "admin-production-readiness-page",
    );
    const apiRoute = report.pages.find(
      (p: PageVerificationRecord) =>
        p.routeId === "api-admin-production-readiness",
    );
...
describe("verificationCaveats content", () => {
  it("includes a caveat about live CI/Azure status", () => {
    const match = report.verificationCaveats.some(
      (c: string) =>
  ...
  it("includes a caveat about the static manifest state", () => {
    const match = report.verificationCaveats.some(
      (c: string) =>
        c.toLowerCase().includes("manifest") ||
        c.toLowerCase().includes("static"),
    );
    expect(match).toBe(true);
  });
});
