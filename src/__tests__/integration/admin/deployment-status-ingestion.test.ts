...
describe("PROD4 deployment status ingestion: env-driven availability", () => {
  it("reports unavailable when neither GITHUB_STATUS_TOKEN nor AZURE_STATUS_TOKEN is set", () => {
    delete process.env.GITHUB_STATUS_TOKEN;
    delete process.env.AZURE_STATUS_TOKEN;
    ...
    for (const result of results) {
      ...
      expect(result.source).toBe("github_azure_optional");
    ...
    expect(signal.source).toBe("github_azure_optional");
  ...
  it("reports configured (never live) when tokens are present, without surfacing token values", () => {
    ...
    process.env.AZURE_STATUS_TOKEN = TEST_TOKEN_VALUE_AZURE;
    ...
    expect(serialized).not.toContain(TEST_TOKEN_VALUE_AZURE);
  });
  ...
  it("reports configured for one provider and unavailable for the other when only one token is set", () => {
    ...
    const azureSource = sources.find((source) => source.provider === "azure");
    expect(githubSource?.configured).toBe(true);
    expect(azureSource?.configured).toBe(false);

    const results = getDeploymentStatusResults();
    const githubResult = results.find((result) => result.provider === "github");
    const azureResult = results.find((result) => result.provider === "azure");
    expect(githubResult?.liveStatus).toBe("configured");
    expect(azureResult?.liveStatus).toBe("unavailable");
...
describe("PROD4 deployment status ingestion: determinism and merge", () => {
  ...
  it("exposes the buildDeploymentStatusUnavailable helper for both providers", () => {
    const githubResult = buildDeploymentStatusUnavailable(
      "github",
      FIXED_GENERATED_AT,
    );
    const azureResult = buildDeploymentStatusUnavailable(
      "azure",
      FIXED_GENERATED_AT,
    );

    for (const result of [githubResult, azureResult]) {
      expect(result.liveStatus).toBe("unavailable");
    ...
    expect(azureResult.message).toMatch(/Azure/);
  });
  ...
  it("mergeDeploymentStatusIntoReadinessView attaches signal without mutating input", () => {
    ...
    expect(
      (view as { deploymentStatus?: unknown }).deploymentStatus,
    ).toBeUndefined();
  });
});
...
describe("PROD4 deployment status ingestion: V1 honesty and no fake live status", () => {
  ...
  it("does not expose any test token value in any serialized output", () => {
    ...
    for (const tokenValue of [TEST_TOKEN_VALUE_GH, TEST_TOKEN_VALUE_AZURE]) {
...
describe("PROD4 route response shape", () => {
  ...
  it("route source does not call any external GitHub or Azure API", () => {
    expect(routeSource).not.toMatch(/api\.github\.com/);
    expect(routeSource).not.toMatch(/management\.azure\.com/);
  });
});
...
describe("PROD4 module hygiene", () => {
  ...
  it("does not fetch from GitHub or Azure and does not call any external host", () => {
    expect(combinedSource).not.toMatch(/api\.github\.com/);
    expect(combinedSource).not.toMatch(/management\.azure\.com/);
  ...
  it("does not name common token-leak placeholders or embed token values", () => {
    ...
    expect(combinedSource).not.toMatch(
      /process\.env\.GITHUB_STATUS_TOKEN\s*\)/,
    );
    expect(combinedSource).not.toMatch(/JSON\.stringify\(\s*process\.env/);
    expect(combinedSource).not.toMatch(
      /console\.(log|info|warn|error)\([^)]*GITHUB_STATUS_TOKEN/,
    );
    expect(combinedSource).not.toMatch(
      /console\.(log|info|warn|error)\([^)]*AZURE_STATUS_TOKEN/,
    );
  });
