...
import { getProductionReadinessRefreshMetadata } from "@/lib/admin/production-readiness";
import {
...
const trackerSource = readCode(
  "src/components/admin/ProductionReadinessTracker.tsx",
);
// ADMIN8 — canonical Production Readiness page is /admin/production-readiness.
const pageSource = readCode(
...
describe("PROD3 production readiness live refresh API response", () => {
  it("includes request-time refresh metadata without changing readiness scoring", () => {
    ...
    expect(response.view.overallReadinessPercent).toBe(
      response.manifest.overallReadinessPercent,
    );
  });

  it("does not claim true GitHub or Azure status polling is configured", () => {
    ...
    expect(metadata.note.toLowerCase()).toContain("azure deployments");
  ...
  it("keeps the read model deterministic except for explicit refresh metadata", () => {
    expect(buildProductionReadinessView()).toEqual(
      buildProductionReadinessView(),
    );

    const first = buildProductionReadinessApiResponse(
      "2026-04-26T00:00:00.000Z",
    );
    const second = buildProductionReadinessApiResponse(
...
describe("PROD3 safety and module hygiene", () => {
  ...
  it("does not call external GitHub or Azure APIs and does not expose token-like fields", () => {
    expect(combinedSource).not.toMatch(/api\.github\.com/);
    expect(combinedSource).not.toMatch(/management\.azure\.com/);
    expect(combinedSource).not.toMatch(
      /GITHUB_TOKEN|AZURE_TOKEN|SECRET|SERVICE_ROLE/,
    );

    const response = JSON.stringify(
      buildProductionReadinessApiResponse(fixedGeneratedAt),
    );
    ...
    // actual token VALUE: a real GitHub/Azure/etc. token shape, or a
    ...
    // api.github.com / management.azure.com URLs in the application code. The
