...
describe("buildLiveCiStatusStubReport", () => {
  ...
  it('unifiedControlPlaneNote contains "unified control plane"', () => {
    expect(report.unifiedControlPlaneNote.toLowerCase()).toContain(
  ...
  it('unifiedControlPlaneNote contains "azure deployments"', () => {
    expect(report.unifiedControlPlaneNote.toLowerCase()).toContain(
      "azure deployments",
    );
  });
  ...
  it("azure provider has a non-empty wouldPollEndpoint", () => {
    const v = report.providers.find((p) => p.provider === "azure")!;
  ...
  it("azure provider requiredEnvVar is set", () => {
    const v = report.providers.find((p) => p.provider === "azure")!;
