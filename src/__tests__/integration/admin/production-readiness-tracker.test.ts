...
const componentMapPath = resolve(
  __dirname,
  "../../../../docs/build/PRODUCTION_READINESS_COMPONENT_MAP.md",
);
const buildSlicesPath = resolve(
  __dirname,
...
describe("production-readiness.json manifest", () => {
  ...
  it("uses only valid readiness and gate statuses", () => {
    const manifest = loadProductionReadinessManifest();
    expect(manifest.stewardBrief.title).toBe(
      "Unified Production Readiness Control Plane",
    );
    expect(manifest.stewardBrief.summary).toContain(
      "one canonical production-readiness spine",
    );
    expect(PRODUCTION_READINESS_STATUSES).toContain(manifest.overallStatus);
    expect(PRODUCTION_READINESS_STATUSES).toContain(
      manifest.stewardBrief.fullFlowTestingReadiness,
    );
    expect(PRODUCTION_READINESS_STATUSES).toContain(
      manifest.stewardBrief.pilotReadinessStatus,
    );
    expect(PRODUCTION_READINESS_STATUSES).toContain(
      manifest.stewardBrief.productionReadinessStatus,
    );
    ...
    for (const component of manifest.components) {
      ...
      for (const gate of PRODUCTION_READINESS_GATES) {
        expect(PRODUCTION_READINESS_GATE_STATUSES).toContain(
          component.testingGates[gate].status,
        );
      }
    }
  });
  ...
  it("records the founder maturity snapshot as deterministic ranges", () => {
    ...
    expect(manifest.maturitySnapshot.source.toLowerCase()).toContain(
      "deterministic planning snapshot",
    );
...
describe("production readiness read model", () => {
  it("is deterministic across repeated calls", () => {
    expect(buildProductionReadinessViewFromDisk()).toEqual(
      buildProductionReadinessViewFromDisk(),
    );
  });
  ...
  it("exposes freshness metadata without changing deterministic scoring", () => {
    ...
    const apiResponse = buildProductionReadinessApiResponse(
      "2026-04-26T12:00:00.000Z",
    );
    ...
    expect(apiResponse.view.overallReadinessPercent).toBe(
      view.overallReadinessPercent,
    );
    expect(apiResponse.view.stewardBrief.title).toBe(
      "Unified Production Readiness Control Plane",
    );
  });
  ...
  it("classifies fresh, aging, stale, and unknown manifests deterministically", () => {
    ...
    expect(
      getProductionReadinessFreshnessMetadata(
        withLastUpdated("2026-04-26"),
        "2026-04-26T12:00:00.000Z",
      ).freshnessStatus,
    ).toBe("fresh");
    expect(
      getProductionReadinessFreshnessMetadata(
        withLastUpdated("2026-04-23"),
        "2026-04-26T12:00:00.000Z",
      ).freshnessStatus,
    ).toBe("aging");
    expect(
      getProductionReadinessFreshnessMetadata(
        withLastUpdated("2026-04-01"),
        "2026-04-26T12:00:00.000Z",
      ).freshnessStatus,
    ).toBe("stale");
    expect(
      getProductionReadinessFreshnessMetadata(
        withLastUpdated("not-a-date"),
        "2026-04-26T12:00:00.000Z",
      ).freshnessStatus,
    ).toBe("unknown");
  });
  ...
  it("does not claim true live monitoring when the tracker is repository-backed", () => {
    ...
    expect(apiResponse.note.toLowerCase()).toContain(
      "not true live monitoring",
    );
    expect(apiResponse.note.toLowerCase()).not.toMatch(
      /github.*enabled|vercel.*enabled|live monitoring enabled/,
    );
  });
  ...
  it("overall summary reconciles to components", () => {
    ...
    for (const status of PRODUCTION_READINESS_STATUSES) {
      expect(summary.byStatus[status]).toBe(
        manifest.components.filter((component) => component.status === status)
          .length,
      );
    }
    ...
    expect(summary.overallReadinessPercent).toBe(
      manifest.overallReadinessPercent,
    );
  });

  it("exposes component progress with started and percent pending math", () => {
    const manifest = loadProductionReadinessManifest();
    const progress = getProductionReadinessComponentProgress(
      manifest.components,
    );

    expect(progress).toHaveLength(manifest.components.length);
    expect(progress.map((component) => component.componentId)).toEqual([
      ...PRODUCTION_READINESS_COMPONENT_IDS,
    ]);
    ...
    expect(
      progress.find((component) => component.componentId === "model_gateway")
        ?.started,
    ).toBe(false);
    expect(
      progress.find((component) => component.componentId === "source")
        ?.percentComplete,
    ).toBeGreaterThan(0);
  });
  ...
  it("groups readiness by operator-friendly segments", () => {
    ...
    expect(segments.map((segment) => segment.id)).toEqual(
      PRODUCTION_READINESS_SEGMENTS.map((segment) => segment.id),
    );
    expect(
      segments.reduce((sum, segment) => sum + segment.totalComponents, 0),
    ).toBe(manifest.components.length);
    expect(
      segments
        .flatMap((segment) =>
          segment.components.map((component) => component.componentId),
        )
        .sort(),
    ).toEqual([...PRODUCTION_READINESS_COMPONENT_IDS].sort());

    for (const segment of segments) {
      expect(segment.percentComplete + segment.percentPending).toBe(100);
      expect(segment.startedCount + segment.notStartedCount).toBe(
        segment.totalComponents,
      );
...
describe("production readiness component map and PROD4B slice", () => {
  it("maps every canonical component through one readiness spine", () => {
    ...
    for (const name of loadProductionReadinessManifest().components.map(
  ...
  it("records PROD4B without duplicating PROD4 or claiming live monitoring", () => {
    ...
    expect(prod4b?.notes?.toLowerCase()).toContain(
      "does not add live monitoring",
    );
  });
});

describe("module hygiene", () => {
  const readModelSource = readCode(
    "../../../lib/admin/production-readiness.ts",
  );
  const componentSource = readCode(
    "../../../components/admin/ProductionReadinessTracker.tsx",
  );
  const livePanelSource = readCode(
    "../../../components/admin/ProductionReadinessLivePanel.tsx",
  );
  // ADMIN8 — admin tree consolidated under /admin/* (legacy /platform/admin/* now redirects).
  const routeSource = readCode(
    "../../../app/(maestro)/admin/production-readiness/page.tsx",
  );
  const apiRouteSource = readCode(
    "../../../app/api/admin/production-readiness/route.ts",
  );
  ...
  it("component and route wire the deterministic read model", () => {
    ...
    expect(componentSource).toMatch(
      /unified readiness spine, not live monitoring/,
    );
