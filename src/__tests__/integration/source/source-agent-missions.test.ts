...
function findMission(
  ...
  return missions.find(
    (mission) =>
      mission.agentName === agentName && mission.missionType === missionType,
  );
}
...
describe("Source deterministic agent mission read model", () => {
  it("creates a Nexus mission for the seeded Data and AI event", () => {
    ...
    expect(nexusMission?.sourceEventId).toBe(
    ...
    expect(
      nexusMission?.suggestedActions.some(
        (action) => action.actionType === "askCustomQuestion",
      ),
    ).toBe(true);
  });

  it("turns missing Source inputs into a Sentinel data request mission", () => {
    const missions = buildSourceAgentMissions(buildMissionInput());
    const dataMission = findMission(missions, "sentinel", "data_readiness");
  ...
  it("turns context validation defers into a Sentinel mission", () => {
    const missions = buildSourceAgentMissions(buildMissionInput());
    const sentinelMission = findMission(
      missions,
      "sentinel",
      "validation_defer",
    );
  ...
  it("turns value at stake into an Atlas mission", () => {
    ...
    expect(atlasMission?.recommendedAction).toContain(
      "executive value/risk brief",
    );
  });
  ...
  it("prioritizes missions deterministically", () => {
    ...
    const priorityRanks = prioritizeSourceAgentMissions(missions).map(
      (mission) => priorities[mission.priority],
    );
    const sortedRanks = [...priorityRanks].sort((a, b) => a - b);

    expect(priorityRanks).toEqual(sortedRanks);
    expect(prioritizeSourceAgentMissions(missions)[0]?.priority).toBe(
      "critical",
    );
  });
  ...
  it("does not import model providers, persistence, UI, schedulers, or program runtime", () => {
    ...
    expect(files).not.toMatch(
      /from ['"][^'"]*(supabase|prisma|database|repository|ProgramSurface|programs\/mock)[^'"]*['"]/i,
    );
    expect(files).not.toMatch(
      /from ['"][^'"]*(react|components\/|app\/api)[^'"]*['"]/i,
    );
