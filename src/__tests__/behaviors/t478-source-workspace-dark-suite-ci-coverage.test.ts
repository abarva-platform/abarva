 ...
 * `tenant_scoped_read`, over 31 product sources — held seven of its 35 files
 * unwired, in two distinct shapes. Three are wired; four are held unwired.
 * This file holds both halves of that disposition in place.
 *
 ...
 *
 * **Why the negative half is asserted at all.** Four of the seven are
 * source-text scanners: they read source files at module scope and assert
 ...
 * case here and quietly make all four merge-blocking. The negative cases are
 * what make the positive ones mean "named individually" rather than "reached".
 *
...
describe("T-478 Source workspace dark-suite disposition", () => {
  it("reads both verdicts out of the record, so no case below iterates an empty list", () => {
    ...
    expect({ wired: WIRED.length, held: HELD.length }).toEqual({
      wired: 3,
      held: 4,
    });
  });
  ...
  it("holds every scanner in the draw, so a later sweep cannot wire one silently", () => {
    // Stated as the set rather than a count: four of the seven are scanners and
    // all four must be on the held list, not merely four things.
