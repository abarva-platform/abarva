...
describe('Board-grade Move model files — P1-3 end-to-end', () => {
  ...
  const RENDERERS: ReadonlyArray<{ name: string; build: () => unknown }> = [
    { name: 'discover-brief', build: () => buildMoveDiscoverBrief(APEX_MOVE, GENERATED_ON) },
    { name: 'cfo-pack', build: () => buildMoveCfoPack(APEX_MOVE, GENERATED_ON) },
    { name: 'charter-skeleton', build: () => buildMoveCharterSkeleton(APEX_MOVE, GENERATED_ON) },
    { name: 'estimate-model', build: () => buildMoveEstimateModel(APEX_MOVE, GENERATED_ON) },
    { name: 'mobilize-packet', build: () => buildMoveMobilizePacket(APEX_MOVE, GENERATED_ON) },
    { name: 'master-dossier', build: () => buildMoveMasterDossier(APEX_MOVE, 'apex-test-move', GENERATED_ON) },
    { name: 'move-pack', build: () => buildMoveCostedBusinessCasePack(APEX_MOVE, GENERATED_ON) },
    { name: 'solution-architecture', build: () => buildMoveSolutionArchitecture(APEX_MOVE, GENERATED_ON) },
  ];
  ...
  it.each(RENDERERS.map(({ name }) => ({
    name,
    build: () => {
      switch (name) {
        case 'discover-brief': return buildMoveDiscoverBrief(NORTHWIND_MOVE, GENERATED_ON);
        case 'cfo-pack': return buildMoveCfoPack(NORTHWIND_MOVE, GENERATED_ON);
        case 'charter-skeleton': return buildMoveCharterSkeleton(NORTHWIND_MOVE, GENERATED_ON);
        case 'estimate-model': return buildMoveEstimateModel(NORTHWIND_MOVE, GENERATED_ON);
        case 'mobilize-packet': return buildMoveMobilizePacket(NORTHWIND_MOVE, GENERATED_ON);
        case 'master-dossier': return buildMoveMasterDossier(NORTHWIND_MOVE, 'northwind-test-move', GENERATED_ON);
        case 'move-pack': return buildMoveCostedBusinessCasePack(NORTHWIND_MOVE, GENERATED_ON);
        case 'solution-architecture': return buildMoveSolutionArchitecture(NORTHWIND_MOVE, GENERATED_ON);
        default: throw new Error(`unknown renderer ${name}`);
      }
    },
  })))(
  ...
  it.each(RENDERERS.map(({ name }) => ({
    ...
    build: () => {
      switch (name) {
        case 'discover-brief': return buildMoveDiscoverBrief(NO_TENANT_MOVE, GENERATED_ON);
        case 'cfo-pack': return buildMoveCfoPack(NO_TENANT_MOVE, GENERATED_ON);
        case 'charter-skeleton': return buildMoveCharterSkeleton(NO_TENANT_MOVE, GENERATED_ON);
        case 'estimate-model': return buildMoveEstimateModel(NO_TENANT_MOVE, GENERATED_ON);
        case 'mobilize-packet': return buildMoveMobilizePacket(NO_TENANT_MOVE, GENERATED_ON);
        case 'master-dossier': return buildMoveMasterDossier(NO_TENANT_MOVE, 'no-tenant-test-move', GENERATED_ON);
        case 'move-pack': return buildMoveCostedBusinessCasePack(NO_TENANT_MOVE, GENERATED_ON);
        case 'solution-architecture': return buildMoveSolutionArchitecture(NO_TENANT_MOVE, GENERATED_ON);
        default: throw new Error(`unknown renderer ${name}`);
      }
    },
  })))(
