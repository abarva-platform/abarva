...
describe('golden suites · structure', () => {
  ...
  it('includes SkyHarbor and Lakeshore', () => {
    const keys = suites.map((s) => s.tenantKey);
    expect(keys).toContain('skyharbor-air');
    expect(keys).toContain('lakeshore-holdings');
  });
...
function traceFor(tenantKey: string, sources: RawAskSource[], citations: string[]) {
...
function questionOf(tenantKey: string, category: GoldenQuestion['category']): GoldenQuestion {
...
describe('assertGoldenQuestion', () => {
  it('passes a well-grounded leadership answer', () => {
    const q = questionOf('apex-retail', 'leadership');
    const trace = traceFor(
      'apex-retail',
      [{ id: 'f1', type: 'TENANT', name: 'Apex CIO', confidence: 0.8 }],
      ['f1'],
    );
    const result = assertGoldenQuestion(q, {
      trace,
      answerText: 'Apex Retail is led by its CIO Carlos Rivera and CDO Lynne Stratham.',
  ...
  it('fails when the wrong tenant is resolved', () => {
    ...
    const result = assertGoldenQuestion(q, { trace, answerText: 'answer', tenantIsolationOk: true });
  ...
  it('passes a negative/unsupported question only when it caveats missing data', () => {
    const q = questionOf('lakeshore-holdings', 'missing_unsupported');
    const trace = traceFor('lakeshore-holdings', [], []);
    const withCaveat = assertGoldenQuestion(q, {
      trace,
      answerText: 'I do not have that figure loaded for this tenant; it is not available to the dollar.',
      tenantIsolationOk: true,
    });
  ...
  it('fails when cross-tenant leakage is reported by the live runner', () => {
    const q = questionOf('apex-retail', 'company_scale');
    const trace = traceFor(
      'apex-retail',
      [{ id: 'f1', type: 'TENANT', name: 'x', confidence: 0.8 }],
      ['f1'],
    ...
    const result = assertGoldenQuestion(q, {
      trace,
      answerText: 'Apex is comparable to Meridian Health in scale.',
