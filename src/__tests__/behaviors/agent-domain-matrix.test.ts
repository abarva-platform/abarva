...
describe('expert matrix · structure', () => {
  ...
  it('includes the brief-required tenants (Apex, Meridian, SkyHarbor, Lakeshore)', () => {
    const keys = new Set(matrix.map((q) => q.tenantKey));
    expect(keys.has('apex-retail')).toBe(true);
    expect(keys.has('meridian-health')).toBe(true);
    expect(keys.has('skyharbor-air')).toBe(true);
    expect(keys.has('lakeshore-holdings')).toBe(true);
  });

  it('emits all 10 archetypes for every (tenant, domain, subdomain)', () => {
    const apex = buildMatrixForTenant({ key: 'apex-retail', name: 'Apex Retail', industry: 'retail' });
  ...
  it('gives at least 10 questions per subdomain selected for the tenant', () => {
    ...
    for (const [, count] of perSubdomain) expect(count).toBeGreaterThanOrEqual(10);
  });

  it('has a missing-evidence negative test in every subdomain', () => {
    const apex = buildMatrixForTenant({ key: 'apex-retail', name: 'Apex Retail', industry: 'retail' });
    const subdomains = new Set(apex.map((q) => `${q.domain}:${q.subdomain}`));
    for (const sd of subdomains) {
      const hasNegative = apex.some(
  ...
  it('applies industry filtering (supply_chain excluded for healthcare_provider)', () => {
    ...
    const apex = buildMatrixForTenant({ key: 'apex-retail', name: 'Apex Retail', industry: 'retail' });
    expect(apex.some((q) => q.domain === 'supply_chain')).toBe(true);
  });
