...
const tree = "src/lib/data-plane/write-adapters/__tests__";
...
describe("data-plane write-adapter CI ownership", () => {
  it("names the directory literally in a Jest workflow command", () => {
  ...
  it("is fully covered according to the execution census", () => {
    const census = runCensus();
    expect(census.counts.indeterminateInvocations).toBe(0);
    expect(
      census.uncoveredDirectories.some((row) => row.directory === tree),
    ).toBe(false);
    expect(
      census.partiallyCoveredDirectories.some((row) => row.directory === tree),
    ).toBe(false);
  });

  it("has no sibling whose name the Jest path would also prefix-match", () => {
