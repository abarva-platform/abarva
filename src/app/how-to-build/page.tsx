...
const S = {
  page: { minHeight: '100vh', background: '#F8F7F4', fontFamily: "'IBM Plex Sans', Inter, sans-serif", color: '#0C0C0C' } as React.CSSProperties,
  mono: { fontFamily: "'IBM Plex Mono', monospace" } as React.CSSProperties,
  card: { background: '#FFFFFF', border: '1px solid #E2E1DC', borderRadius: '10px', padding: '20px' } as React.CSSProperties,
}
...
function HowToBuildContent() {
  ...
  return (
    <div style={S.page}>
      ...
      {/* Client selector */}
      <div style={{ background: '#F8F7F4', borderBottom: '1px solid #E2E1DC', padding: '0 32px', display: 'flex', gap: '4px' }}>
        {Object.entries(CLIENT_META).map(([id, m]) => (
          <button key={id} onClick={() => setSelectedClient(id)}
            style={{ padding: '10px 20px', ...S.mono, fontSize: '11px', fontWeight: 600, cursor: 'pointer', border: 'none', borderBottom: selectedClient === id ? '2px solid ' + m.accent : '2px solid transparent', background: 'transparent', color: selectedClient === id ? m.accent : '#3C3C3C', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '9px', padding: '1px 6px', borderRadius: '3px', background: selectedClient === id ? m.cloudBg + 'aa' : '#E2E1DC', color: selectedClient === id ? 'white' : '#3C3C3C' }}>{m.cloud}</span>
      ...
      <div style={{ maxWidth: '1480px', margin: '0 auto', padding: '32px' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <h1 style={{ fontSize: '28px', fontWeight: 700, color: '#0C0C0C', marginBottom: '8px' }}>How to Build This</h1>
          <p style={{ fontSize: '14px', color: '#3C3C3C', lineHeight: 1.6 }}>A phased, incremental approach using a lean team of Maestros and agents.<br />No large SI engagement required. First value in 90 days.</p>
        </div>
          ...
          {[
            {
              ...
              invest: '$2–4M', accentColor: '#3C3C3C',
            ...
            <div key={i} style={{ ...S.card, borderTop: '3px solid ' + phase.accentColor }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                <div>
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 700, color: '#0C0C0C' }}>{phase.title}</div>
                </div>
                <div style={{ ...S.mono, fontSize: '9px', color: '#888888', textAlign: 'right' as const }}>
              ...
              <div style={{ marginBottom: '10px' }}>
                <div style={{ ...S.mono, fontSize: '9px', color: '#3C3C3C', letterSpacing: '0.08em', marginBottom: '5px' }}>TEAM</div>
                {phase.team.map((t, ti) => (
                  <div key={ti} style={{ fontSize: '10px', color: '#888888', marginBottom: '3px', paddingLeft: '8px', borderLeft: '2px solid #E2E1DC' }}>· {t}</div>
                ))}
              </div>
              <div style={{ marginBottom: '10px' }}>
                <div style={{ ...S.mono, fontSize: '9px', color: '#3C3C3C', letterSpacing: '0.08em', marginBottom: '5px' }}>WHAT GETS BUILT</div>
                {phase.builds.map((b, bi) => (
                  <div key={bi} style={{ fontSize: '10px', color: '#0C0C0C', marginBottom: '3px' }}>✓ {b}</div>
                ))}
              </div>
        ...
        <div style={{ ...S.card, marginBottom: '32px' }}>
          <div style={{ ...S.mono, fontSize: '10px', color: '#3C3C3C', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '16px' }}>COST COMPARISON — {meta.name.toUpperCase()} · {meta.cloud}</div>
          <div style={{ overflowX: 'auto' as const }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' as const, fontSize: '12px' }}>
              <thead>
                <tr>
                  {['', 'Traditional SI', 'AbarVa + Lean Team', 'Hybrid'].map((h, i) => (
                    <th key={i} style={{ padding: '10px 16px', textAlign: i === 0 ? 'left' as const : 'center' as const, ...S.mono, fontSize: '9px', color: i === 2 ? '#2DD4C8' : '#3C3C3C', letterSpacing: '0.08em', textTransform: 'uppercase' as const, borderBottom: '1px solid #E2E1DC', background: i === 2 ? 'rgba(45,212,200,0.06)' : 'transparent' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} style={{ borderBottom: '1px solid #E2E1DC' }}>
                    {row.map((cell, ci) => (
                      <td key={ci} style={{ padding: '10px 16px', color: ci === 0 ? '#888888' : ci === 2 ? '#2DD4C8' : ci === 1 ? '#EF4444' : '#3C3C3C', textAlign: ci === 0 ? 'left' as const : 'center' as const, background: ci === 2 ? 'rgba(45,212,200,0.06)' : 'transparent', fontSize: '12px' }}>{cell}</td>
                    ))}
                  </tr>
                ))}
                {[
                  ...
                  <tr key={i} style={{ borderBottom: '1px solid #E2E1DC' }}>
                    <td style={{ padding: '10px 16px', color: '#888888', fontWeight: row[4] ? 700 : 400 }}>{row[0]}</td>
                    ...
                    <td style={{ padding: '10px 16px', color: row[4] ? '#2DD4C8' : '#2DD4C8', textAlign: 'center' as const, fontWeight: row[4] ? 700 : 600, background: 'rgba(45,212,200,0.06)' }}>{row[2]}</td>
                    <td style={{ padding: '10px 16px', color: '#3C3C3C', textAlign: 'center' as const, fontWeight: row[4] ? 700 : 400 }}>{row[3]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ ...S.card, marginBottom: '32px' }}>
          <div style={{ ...S.mono, fontSize: '10px', color: '#3C3C3C', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '4px' }}>AGENT VS HUMAN DECISION MATRIX</div>
          <div style={{ fontSize: '12px', color: '#3C3C3C', marginBottom: '16px' }}>When to trust the agent · when to keep the human · and what the human checkpoint looks like</div>
          <div style={{ overflowX: 'auto' as const }}>
              ...
              <thead>
                <tr>
                  {['Task', 'Recommended', 'Why', 'Cost per Unit', 'Volume', 'Human Checkpoint'].map(h => (
                    <th key={h} style={{ padding: '8px 12px', textAlign: 'left' as const, ...S.mono, fontSize: '9px', color: '#3C3C3C', letterSpacing: '0.06em', textTransform: 'uppercase' as const, borderBottom: '1px solid #E2E1DC', whiteSpace: 'nowrap' as const }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {tasks.map((row, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: '1px solid #E2E1DC' }}>
                      <td style={{ padding: '9px 12px', color: '#0C0C0C', fontWeight: 500 }}>{row.task}</td>
                      ...
                      <td style={{ padding: '9px 12px', color: '#3C3C3C', fontSize: '10px', maxWidth: '200px' }}>{row.why}</td>
                      <td style={{ padding: '9px 12px', ...S.mono, fontSize: '9px', color: '#059669' }}>{row.cost}</td>
                      <td style={{ padding: '9px 12px', ...S.mono, fontSize: '9px', color: '#888888' }}>{row.volume}</td>
                      <td style={{ padding: '9px 12px', fontSize: '10px', color: row.checkpoint.toLowerCase().includes('every') ? '#F59E0B' : '#3C3C3C' }}>{row.checkpoint}</td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        <div style={{ ...S.card, borderLeft: '4px solid #2DD4C8', background: '#FFFFFF', padding: '24px 28px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
            <div>
              ...
              {['Tickets closed', 'Sprints completed', 'Story points burned', 'Deployment frequency', 'Vendor meeting hours'].map((item, i) => (
                <div key={i} style={{ fontSize: '12px', color: '#3C3C3C', marginBottom: '4px', textDecoration: 'line-through' }}>· {item}</div>
              ))}
            </div>
            <div>
              ...
              {(selectedClient === 'meridian' ? [
                ...
                <div key={i} style={{ fontSize: '12px', color: '#0C0C0C', marginBottom: '4px' }}>✓ {item}</div>
              ))}
            </div>
          </div>
          <div style={{ marginTop: '20px', padding: '14px 18px', background: 'rgba(45,212,200,0.06)', borderRadius: '8px', border: '1px solid rgba(45,212,200,0.15)' }}>
            <div style={{ fontSize: '13px', color: '#0C0C0C', lineHeight: 1.7 }}>
              ...
              <span style={{ fontSize: '12px', color: '#3C3C3C' }}>This is how AbarVa stays aligned with your goals — not your project plan.</span>
            </div>
          </div>
        </div>
...
export default function HowToBuildPage() {
  return (
    <Suspense fallback={
      <div style={{ background: '#F8F7F4', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2DD4C8', fontFamily: 'IBM Plex Mono, monospace', fontSize: '13px' }}>
        Loading build plan...
      </div>
    }>
      <HowToBuildContent />
