...
export default function Home() {
  ...
  const currentGreenField = greenFieldClients.find(c => c.id === activeClient)
  const currentClient = currentBuiltIn || { id: activeClient, name: currentGreenField?.name || 'Client', industry: 'Unknown', confidence: 23 }
  ...
  const statusDot = (s: 'red' | 'yellow' | 'green') => (
    <span className={`inline-block w-2 h-2 rounded-full flex-shrink-0 ${s === 'red' ? 'bg-red-500' : s === 'yellow' ? 'bg-yellow-500' : 'bg-green-500'}`} />
  )
  ...
  function getSuggestions(r: string) {
    ...
    if (activeClient === 'apexretail') {
      const s: Record<string, string[]> = {
        ...
        CMO: ["18 million loyalty members — 42% active vs 68% benchmark — what is wrong?", "Personalization engine — Dynamic Yield vs Bloomreach vs Salesforce Einstein we already own?", "Email unsubscribe rate 2.8% vs 0.8% benchmark — what do we do?"],
  ...
  function getPrescribedLoads() {
    if (activeClient === 'firstcapital') return [
      '1. FIS HORIZON architecture docs — unlocks migration path analysis',
      '2. Full vendor contracts — unlocks negotiation intelligence',
      '3. Customer analytics — unlocks churn and growth strategy',
    ]
    if (activeClient === 'apexretail') return [
      '1. SAP ECC custom code inventory — unlocks migration complexity score',
      '2. Store-level P&L data — unlocks portfolio optimization',
      '3. Loyalty program analytics — unlocks $1.24B activation strategy',
    ]
    return [
      '1. Ensemble contract — unlocks SLA penalty analysis ($8M)',
      '2. RCM denial data by payer — unlocks payer-specific strategy',
      '3. Epic optimization assessment — unlocks module prioritization',
    ]
  }

  function getRoleTabs() {
    if (activeClient === 'firstcapital') return ['CIO', 'CFO', 'COO', 'CMO', 'CEO']
    if (activeClient === 'apexretail') return ['CIO', 'CFO', 'COO', 'CMO', 'CEO']
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            {greenFieldClients.map(c => (
              <button key={c.id} onClick={() => switchClient(c.id)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${activeClient === c.id ? 'bg-purple-600 text-white' : 'bg-gray-900 text-gray-400 hover:text-white border border-gray-800'}`}>
                {c.name}
                <span className="ml-1 text-yellow-400">●</span>
              </button>
            ))}
            <button onClick={() => window.location.href = '/search'}
              className="px-3 py-1.5 rounded-lg text-xs font-medium bg-green-900 text-green-300 hover:bg-green-800 transition border border-green-800">
              + New Client
            </button>
          </div>
        </div>
        ...
        {sidebarOpen && (
          <aside className="w-48 border-r border-gray-800 flex-shrink-0 flex flex-col bg-gray-950">
            <div className="p-4 flex-1">
              ...
              <p className="text-xs text-gray-600 mb-1">{currentClient.industry}</p>
            ...
            <div className="p-4 border-t border-gray-800">
              ...
              <div className="w-full bg-gray-800 rounded-full h-1.5 mb-1">
                <div className={`h-1.5 rounded-full transition-all ${currentClient.confidence >= 80 ? 'bg-green-500' : currentClient.confidence >= 50 ? 'bg-blue-500' : 'bg-yellow-500'}`}
                  style={{width: `${currentClient.confidence}%`}} />
              </div>
              <p className="text-xs text-gray-400">{currentClient.confidence}% confidence</p>
            </div>
          </aside>
        )}
        ...
        <main className="flex-1 overflow-y-auto p-8">
          ...
          {isGreenField && (
            <div>
              <div className="mb-6">
                ...
                <p className="text-gray-400">{currentClient.name} — Greenfield Client Workspace</p>
              </div>
              <div className="bg-yellow-950 border border-yellow-800 rounded-xl p-6 mb-6">
                <h3 className="font-semibold text-yellow-300 mb-2">Public Intelligence Only — 23% Confidence</h3>
                <p className="text-sm text-gray-300 mb-4">Abarva has gathered public intelligence on this organization. Internal data has not been loaded. Complete the guided data load to unlock full platform capabilities.</p>
                <button onClick={() => setActiveTab('data')} className="bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-2 px-6 rounded-lg transition text-sm">Start guided data load →</button>
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">What Abarva Found</h3>
                  <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-2 text-sm text-gray-300">
                    <p>✓ Financial performance from public filings</p>
                    <p>✓ Leadership team identified</p>
                    <p>✓ Strategic priorities from earnings calls</p>
                    <p>✓ Recent news and announcements</p>
                    <p className="text-gray-500">✗ Internal technology inventory</p>
                    <p className="text-gray-500">✗ Vendor contracts and SLAs</p>
                    <p className="text-gray-500">✗ Clinical or operational metrics</p>
                    <p className="text-gray-500">✗ Leadership interview insights</p>
                  </div>
                </div>
                <div>
                  <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Products Available</h3>
                  <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-2 text-sm">
                    <p className="text-yellow-400">⚡ Diagnose — Limited (public data only)</p>
                    <p className="text-gray-500">→ Transform — Needs more data</p>
                    <p className="text-gray-500">$ Justify — Needs more data</p>
                    <p className="text-gray-500">◈ Select — Needs more data</p>
                    <p className="text-gray-500">◎ Track — No baseline established</p>
                  </div>
                </div>
              </div>
              <div className="mt-6">
                <button onClick={() => setActiveTab('diagnose')} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-xl transition text-sm mr-3">
          ...
          {!isGreenField && activeTab === 'overview' && (
            <div>
              <div className="mb-6">
                <h2 className="text-2xl font-bold mb-1">Good morning, {user?.firstName || 'Maestro'}.</h2>
                <p className="text-gray-400">{currentClient.name} — {currentClient.industry} · {currentClient.confidence}% data confidence</p>
              </div>
          ...
          {!isGreenField && activeTab === 'diagnose' && (
            <div className="flex flex-col" style={{height: 'calc(100vh - 120px)'}}>
              <div className="mb-4">
                ...
                <p className="text-gray-400 text-sm">{currentClient.confidence}% confidence · {currentClient.industry}</p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-3 mb-4 pr-2">
                ...
                {loading && !streamingResponse && (
                  <div className="flex justify-start">
                    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 text-sm text-gray-400">Analyzing {currentClient.name}...</div>
                  </div>
                )}
                {streamingResponse && (
                  <div className="flex justify-start">
              ...
              <div className="flex gap-3">
                <input className="flex-1 bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 text-sm"
                  placeholder={`Ask Abarva anything as ${role} at ${currentClient.name}...`}
                  value={inputMessage}
                  onChange={e => setInputMessage(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage() } }} />
                <button onClick={() => sendMessage()} disabled={loading || !inputMessage.trim()}
                  className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold px-6 py-3 rounded-xl transition text-sm">Send</button>
              </div>
            </div>
          )}

          {/* DIAGNOSE FOR GREENFIELD */}
          {isGreenField && activeTab === 'diagnose' && (
            <div className="flex flex-col" style={{height: 'calc(100vh - 120px)'}}>
              <div className="mb-4">
                <h2 className="text-xl font-bold mb-1">Diagnose — {currentClient.name}</h2>
                <p className="text-yellow-400 text-sm">Public data only — 23% confidence · Load internal data to improve</p>
              </div>
              <div className="flex-1 overflow-y-auto space-y-3 mb-4 pr-2">
                {messages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                {loading && !streamingResponse && <div className="flex justify-start"><div className="bg-gray-900 border border-gray-800 rounded-xl p-4 text-sm text-gray-400">Analyzing...</div></div>}
                {streamingResponse && <div className="flex justify-start"><div className="bg-gray-900 border border-gray-800 rounded-xl p-4 text-sm text-gray-200 max-w-3xl leading-relaxed whitespace-pre-wrap">{streamingResponse}</div></div>}
              </div>
              <div className="flex gap-3">
                <input className="flex-1 bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 text-sm"
                  placeholder="Ask anything — Abarva will answer from public intelligence..."
                  value={inputMessage}
                  onChange={e => setInputMessage(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage() } }} />
                <button onClick={() => sendMessage()} disabled={loading || !inputMessage.trim()}
          ...
          {!isGreenField && activeTab === 'transform' && (
            <div>
              <div className="mb-6">
                <h2 className="text-2xl font-bold mb-1">Transformation Simulator — {currentClient.name}</h2>
                <p className="text-gray-400">Model the financial impact of transformation initiatives</p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    ...
                    <div className="space-y-8">
                      <div>
                        <div className="flex justify-between mb-2"><label className="text-sm font-medium">RCM Denial Rate</label><span className="text-sm font-bold text-blue-400">{denialRate.toFixed(1)}%</span></div>
                        ...
                        <div className="flex justify-between text-xs text-gray-500 mt-1"><span>8.2% top quartile</span><span>Current: 18.2%</span><span>22% critical</span></div>
                      </div>
                      <div>
                        <div className="flex justify-between mb-2"><label className="text-sm font-medium">Epic Optimization Score</label><span className="text-sm font-bold text-blue-400">{epicScore}/100</span></div>
                        ...
                        <div className="flex justify-between text-xs text-gray-500 mt-1"><span>Current: 58</span><span>Target: 85</span><span>Best: 100</span></div>
                      </div>
                      <div>
                        <div className="flex justify-between mb-2"><label className="text-sm font-medium">Medicare Advantage Stars</label><span className="text-sm font-bold text-blue-400">{maStars.toFixed(1)}</span></div>
                        ...
                        <div className="flex justify-between text-xs text-gray-500 mt-1"><span>Current: 3.5</span><span>Bonus: 4.0</span><span>Max: 5.0</span></div>
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-4">
                      ...
                      <div className="w-full bg-gray-800 rounded-full h-2 mb-2">
                        <div className={`h-2 rounded-full transition-all ${parseFloat(projectedMargin) >= 4.0 ? 'bg-green-500' : parseFloat(projectedMargin) >= 3.0 ? 'bg-yellow-500' : 'bg-red-500'}`}
                          style={{width: `${Math.min((parseFloat(projectedMargin) / 4.0) * 100, 100)}%`}} />
                      </div>
                    ...
                    <div className="space-y-3">
                      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                        <div className="flex justify-between mb-1"><span className="text-sm text-gray-400">RCM revenue recovered</span><span className="text-sm font-bold text-green-400">${revenueRecovered}M</span></div>
                      ...
                      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                        <div className="flex justify-between mb-1"><span className="text-sm text-gray-400">MA bonus revenue</span><span className="text-sm font-bold text-green-400">{maStars >= 4.0 ? '$34M' : '$0'}</span></div>
                        <p className="text-xs text-gray-500">{maStars >= 4.0 ? 'Bonus threshold achieved' : 'Raise stars to 4.0 to unlock $34M'}</p>
                      </div>
                      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                        <div className="flex justify-between mb-1"><span className="text-sm text-gray-400">Epic efficiency gains</span><span className="text-sm font-bold text-green-400">${Math.round((epicScore - 58) * 0.7)}M</span></div>
                  ...
                  {(activeClient === 'firstcapital' ? firstCapital.aiOpportunities : apexRetail.aiOpportunities).map((o, i) => (
                    <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                      ...
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        <div><p className="text-gray-500 text-xs mb-1">Time to Value</p><p className="font-medium">{o.timeToValue}</p></div>
                        <div><p className="text-gray-500 text-xs mb-1">ROI</p><p className="font-medium text-blue-400">{o.roi}x</p></div>
                        <div><p className="text-gray-500 text-xs mb-1">Investment</p><p className="font-medium">${((o.implementationCost || 0) / 1000000).toFixed(1)}M</p></div>
                      </div>
                    </div>
                  ))}
                  <button onClick={() => setActiveTab('diagnose')} className="w-full mt-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl transition text-sm">Ask Abarva how to prioritize →</button>
                </div>
              )}
            </div>
          )}
                ...
                <>
                  <div className="grid grid-cols-3 gap-4 mb-8">
                    {[
                      ...
                      { label: 'IT Budget', value: `$${meridianFinancials.itBudget.total2024}M`, sub: `${meridianFinancials.itBudget.asPercentRevenue}% vs ${meridianFinancials.itBudget.peerBenchmark}% peer`, status: 'yellow' as const },
                    ].map(m => (
              ...
              {activeClient === 'apexretail' && (
                <div className="grid grid-cols-3 gap-4">
                  {[
                    ...
                    { label: 'Digital Revenue', value: `${apexRetail.org.ecommercePercent}%`, sub: 'Target: 45% | Benchmark: 38%', status: 'yellow' as const },
                  ].map(m => (
                    <div key={m.label} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                      <div className="flex justify-between mb-2"><span className="text-sm text-gray-400">{m.label}</span>{statusDot(m.status)}</div>
                      <div className="text-2xl font-bold mb-1">{m.value}</div>
                    ...
                    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-3">
                      {meridianClinical.aiOpportunities.map((o, i) => (
                        <div key={i} className="border-b border-gray-800 last:border-0 pb-3 last:pb-0">
                          <div className="flex justify-between text-sm mb-1"><span className="font-medium">{o.useCase}</span><span className="text-green-400">${o.annualSavings}M</span></div>
              ...
              {activeClient !== 'meridian' && (
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                  <p className="text-gray-400 text-sm mb-4">
                    {activeClient === 'firstcapital' ? 'Detailed compliance and risk data available in the Diagnose product. Ask Abarva about specific compliance challenges.' : 'Detailed supply chain and operations data available. Ask Abarva about specific operational challenges.'}
                  </p>
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                {activeClient === 'firstcapital' && Object.entries(firstCapital.leadership).map(([key, exec]: [string, any], i) => (
                  <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                    <span className="text-xs font-semibold text-blue-400 uppercase tracking-wider">{key.toUpperCase()}</span>
                    <h3 className="text-lg font-bold mt-0.5">{exec.name}</h3>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-4">
                    ...
                    <div className="text-5xl font-bold text-blue-400 mb-2">{currentClient.confidence}%</div>
                    <div className="w-full bg-gray-800 rounded-full h-2 mb-3">
                      <div className={`h-2 rounded-full ${currentClient.confidence >= 80 ? 'bg-green-500' : currentClient.confidence >= 50 ? 'bg-blue-500' : 'bg-yellow-500'}`}
                        style={{width: `${currentClient.confidence}%`}} />
                    </div>
                    <p className="text-xs text-gray-500">Loading additional data increases diagnosis precision and unlocks deeper capabilities.</p>
                  </div>
                  <div className="bg-blue-950 border border-blue-800 rounded-xl p-5">
                    ...
                    <div className="space-y-2 text-xs text-gray-300">
                      {getPrescribedLoads().map((p, i) => <p key={i}>{p}</p>)}
                    </div>
