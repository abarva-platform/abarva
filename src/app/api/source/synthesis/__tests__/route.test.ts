...
describe('POST /api/source/synthesis', () => {
  ...
  it('uses the active Airline Demo V6 Source pack instead of defaulting to the Apex AMS fixture', async () => {
    ...
    expect(streamArgs.messages[0].content).toContain('OCC Modernization vendor and commercial readiness');
    expect(streamArgs.messages[0].content).toContain('vendor-commercial-packet');
    expect(streamArgs.messages[0].content).not.toContain('apex-retail-ams-outsourcing-2026');
  });

  it('uses the active Industrial Demo V6 Source pack with loaded commercial facts', async () => {
    mockAnthropicStream.mockReturnValue(claudeTextStream('Industrial Source V6 answer.'));
    mockGetActiveClientRow.mockResolvedValue({
      id: 'client-lakeshore',
    ...
    const streamArgs = mockAnthropicStream.mock.calls[0]?.[0];
    expect(streamArgs.messages[0].content).toContain('Kyriba global cash and payments rollout vendor and commercial readiness');
    expect(streamArgs.messages[0].content).toContain('vendor-commercial-packet');
  });
