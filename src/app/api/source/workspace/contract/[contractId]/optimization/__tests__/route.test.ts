...
describe('Source workspace contract optimization route helpers', () => {
  ...
  it('requires the selected opportunity identity when an opportunity is passed', () => {
    const matchingEvent = event({
      ...
      event_name: 'Northstar Software - Sales Platform Agreement 3 Rate variance Optimization',
      trigger_description:
    ...
    expect(__test__.optimizationEventMatchesContract(matchingEvent, selectedContract, 'CTR-090:rate-variance')).toBe(true);
    expect(__test__.optimizationEventMatchesContract(genericEvent, selectedContract, 'CTR-090:rate-variance')).toBe(false);
  });
  ...
  it('writes selected opportunity context into the optimization event brief fields', () => {
    ...
    })).toBe('Northstar Software - Sales Platform Agreement 3 Rate variance Optimization');
    expect(__test__.optimizationCreationRequestId({
      contractId: 'CTR-090',
      vendorName: 'Northstar Software',
      selectedOpportunity: opportunity,
    })).toBe('CTR090NOVARIANCE');
    expect(__test__.optimizationTriggerDescription({
      contractId: 'CTR-090',
      contractName: 'Sales Platform Agreement 3',
      vendorName: 'Northstar Software',
      selectedOpportunity: opportunity,
    })).toContain('Selected opportunity CTR-090:rate-variance');
    expect(__test__.optimizationScopeDescription({
      contractId: 'CTR-090',
      selectedOpportunity: opportunity,
