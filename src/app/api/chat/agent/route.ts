  ...
  const systemPrompt = [
    ...
    ...(isSourceSurface(surface)
      ? [
          ...
          "- SOURCE EVENT ANSWER SHAPE: event-stage asks about required files/templates, workshops, data to collect, vendor unsupported claims, or gate readiness are operational decisions. Use a compact markdown table with columns such as Item | Owner | Evidence needed | Next action, then add one sentence on the decision. Do not answer those asks as prose-only paragraphs.",
          "- SOURCE STAGE STATUS DISCIPLINE: when describing where a sourcing event is in the workflow, distinguish stage/task completion from approved value or realized savings. Say complete only for the gate/checklist state the page proves. Do not imply guaranteed, booked, approved, or realized savings unless the grounding explicitly says Finance/Tower approval is complete.",
