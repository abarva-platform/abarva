...
jest.mock(
  "@/lib/context-ingestion/meridian-stage-readiness-read-model",
  () => ({
    getMeridianStageReadinessInputForClient: (...args: unknown[]) =>
      mockGetReadinessInput(...args),
  }),
);

jest.mock("@/lib/context-ingestion/meridian-command-center-generation", () => ({
  generateMeridianCommandCenterArtifact: (...args: unknown[]) =>
    mockGenerate(...args),
}));

function request(body: unknown) {
  return new NextRequest(
    "http://localhost/api/v1/moves/meridian-command-center/generate",
    {
      method: "POST",
...
describe("/api/v1/moves/meridian-command-center/generate", () => {
  beforeEach(() => {
    ...
    mockGenerate.mockResolvedValue({
      status: "generated",
      text: "Generated strategy [Meridian-STARS-2026] [dom54-stars-measure-uplift]",
      auditId: "audit-1",
      model: "gpt-5.1",
      tokensIn: 100,
      tokensOut: 30,
      stopReason: "completed",
      evidenceIds: ["Meridian-STARS-2026"],
  ...
  it("uses the authenticated client id for readiness lookup and generation", async () => {
    ...
    expect(mockGetReadinessInput).toHaveBeenCalledWith(
    ...
    expect(mockGenerate).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        evidenceRefs: expect.arrayContaining([
          expect.objectContaining({ evidenceId: "Meridian-STARS-2026" }),
        ]),
      }),
    );
  });
