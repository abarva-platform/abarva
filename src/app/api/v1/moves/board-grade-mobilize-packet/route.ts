...
//
// Streams the board-grade Mobilize & Go-Decision Packet for the Apex "Contact
// Center AI Routing" Move as one self-contained HTML deck — the artifact
// called for by the Moves Board-Grade Artifact Blueprint (§11 / Appendix A.7).
//
...
//
// The renderer is deterministic and pure. Apex's honest go-decision is
// `no_go` — two Mobilize kill triggers fired — and is rendered exactly as the
// kernel returns it; that is a valid rendered outcome, never an error.
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  let html: string;
  try {
    html = renderApexMobilizePacketHtml(generatedOn);
  } catch (err) {
    console.error(
