...
import { buildStageCompletion, type StageSignals } from '@/lib/source/stage-gate/completion-feeder';
...
const ACTIONS: MaestroAction[] = ['approve', 'approve_with_gaps', 'reject', 'mark_preliminary', 'defer', 'assign_follow_up', 'mark_client_to_complete', 'force_advance'];
...
export async function POST(req: NextRequest, ctxParam: { params: Promise<{ eventId: string }> }) {
  try {
    const ctx = await requireTenancy();
    if (!ctx.clientKey) return Response.json({ error: 'no_tenant_key' }, { status: 409 });
    ...
    let body: Body;
    try { body = (await req.json()) as Body; } catch { return Response.json({ error: 'bad_request', detail: 'Body was not valid JSON.' }, { status: 400 }); }

    const def = body.stageKey ? getAmsStageGate(body.stageKey) : undefined;
    if (!def) return Response.json({ error: 'bad_request', detail: 'Unknown or missing stageKey (AMS).' }, { status: 400 });
    if (!body.action || !ACTIONS.includes(body.action)) return Response.json({ error: 'bad_request', detail: `action must be one of ${ACTIONS.join(', ')}.` }, { status: 400 });
    ...
    const decisionResult = applyMaestroDecision(assessment, {
      ...
      ...(body.gapsAcknowledged ? { gapsAcknowledged: body.gapsAcknowledged } : {}),
    ...
    if (!decisionResult.ok) {
      ...
      return Response.json({ error: 'invalid_decision', detail: decisionResult.error, assessment, guidance: buildGateGuidance(assessment) }, { status: 422 });
    }
