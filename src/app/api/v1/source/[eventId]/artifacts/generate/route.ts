...
import { verifyArtifactSections } from "@/lib/source/agent-generation/section-conformance";
...
function parseDataClassification(
  ...
  throw new Error(
    `dataClassification must be Public, Internal, Confidential, or Restricted; got ${parsed}`,
  );
}
...
function seedEventMatchesClient(
  ...
  if (clientKey === "firstcapital")
    return normalized.includes("first") || normalized.includes("capital");
  return false;
}
...
export async function POST(request: Request, { params }: GenerateRouteContext) {
  ...
  if (!accessPolicy?.canGenerateSourcingArtifacts) {
    return jsonError(
      403,
      "forbidden_source_artifact_generate_required",
      "Source artifact generation rights are required.",
    );
  }
  ...
  try {
    ...
    artifactCode = parseRequiredString(body.artifactCode, "artifactCode");
  } catch (error) {
    return jsonError(
      400,
      "invalid_metadata",
  ...
  // Source integrity fix, 2026-07-23: this route previously accepted any
  // free-text artifactKind (defaulting to a generic label when absent),
  // which meant chat-authored content could become a registered Source
  // artifact with no resolvable contract — section-conformance and
  // banned-term checks have nothing to check against without a real,
  // registered artifact code. See
  // docs/audits/SOURCE-VS-MOVES-STANDARD-AUDIT-2026-07-23.md (Pipeline
  // Drift Report, item D1). A caller must now name a real artifact type.
  // source-artifact-profiles.ts keys profiles by the short id ("d01"), while
  // prompt-registry.ts and callers use the full code ("d01_strategy_memo") —
  // same split every other consumer of both modules already needs.
  const shortCode = shortSourceArtifactCode(artifactCode);
  const artifactProfile = getSourceArtifactProfile(shortCode);
  if (!artifactProfile) {
    return jsonError(
      400,
      "unknown_artifact_code",
      `artifactCode "${artifactCode}" is not a registered Source artifact type.`,
    );
  }
  ...
  if (content.length > MAX_GENERATED_ARTIFACT_CHARS) {
    return jsonError(
      ...
      `content exceeds ${MAX_GENERATED_ARTIFACT_CHARS} characters`,
    );
  }

  // Run the same two unconditional, non-blocking checks the primary
  // [artifactCode]/generate pipeline runs on every draft — a visibility
  // signal for human review, not a generation failure. Chat-authored
  // content previously ran neither.
  const sectionVerification = verifyArtifactSections(artifactCode, content);
  const bannedTermMatches = scanForBannedTerms(content, shortCode);
  ...
  // (section-conformance, banned-term profile, export format routing) can
  // resolve the same contract this route just validated it against.
  const artifactKind = artifactCode;
  ...
  try {
    ...
    console.error(
      "[POST /api/v1/source/:eventId/artifacts/generate] file cabinet prior-version read failed",
      {
  ...
  try {
    ...
    console.error(
      "[POST /api/v1/source/:eventId/artifacts/generate] storage_upload_failed",
      {
        blobUri,
        message:
  ...
  try {
    const artifact = await registerSourceArtifactUpload({
      ...
      fileCabinet: {
        ...
        description: (() => {
          const base =
            "Generated Source draft preserved for client review, downstream lineage, and client-final supersession.";
        ...
        missingInputs: sectionVerification?.missingSections ?? [],
        clientCompleteItems: [],
        assumptions: [],
        supersedesArtifactId,
        blobSha256: sha256,
    ...
    return Response.json(
      {
        ...
        receipt: {
          ...
          sectionVerification: sectionVerification?.status ?? "not_applicable",
          missingSections: sectionVerification?.missingSections ?? [],
          complianceReviewFlagged: bannedTermMatches.length > 0,
        },
      },
      { status: 200 },
    );
    ...
    console.error(
      "[POST /api/v1/source/:eventId/artifacts/generate] metadata_insert_failed",
      error,
    );
    return jsonError(
      500,
      ...
      error instanceof Error
        ? error.message
        : "failed to persist generated Source artifact metadata",
    );
  }
}
