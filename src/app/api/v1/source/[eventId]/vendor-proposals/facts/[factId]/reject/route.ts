// POST /api/v1/source/:eventId/vendor-proposals/facts/:factId/reject
//
// Body: { rationale: string }
//
// Reject one candidate VendorProposalFact. Never mutates the fact's own
// row — writes a 'rejected' review row to the append-only ledger. A
// rejected fact is permanently excluded from
// getAuthoritativeVendorProposalFacts, but remains fully readable for audit.
//
// Auth: requireTenancy + canApproveSourceStages — rejecting is the other
// side of the same review decision as accepting, so it uses the identical
// stronger permission.
...
import { rejectVendorProposalFact } from "@/lib/source/vendor-proposals/vendor-proposal-facts";
  ...
  try {
    ...
    if (!canMutate) {
      return jsonError(
        403,
        "forbidden",
        "Stage-approval rights are required to reject a vendor proposal fact.",
      );
    }
