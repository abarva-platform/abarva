...
export async function GET(req: NextRequest, ctxParam: { params: Promise<{ artifactId: string }> }) {
  try {
...
function parseRequestedFormat(req: NextRequest): ArtifactFileFormat | 'invalid' | null {
...
async function resolveFormatRecord(
  ...
  return (
    siblings.find(
      (candidate) =>
        candidate.artifactType === targetType && candidate.fileFormat === format,
    ) ?? null
  );
}

async function streamFileCabinetRecord(record: SourceArtifactRecord): Promise<Response> {
