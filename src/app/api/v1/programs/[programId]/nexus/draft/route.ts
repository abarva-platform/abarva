  ...
  try {
    ...
    if (
      ...
      return Response.json(
        {
          error: "bad_request",
          detail: "moduleKey + deliverableTypeKey + title + prompt required",
        },
        { status: 400 },
      );
    }
    ...
    const plan = describePendingComposerCall({
      mode: "module_drafting",
      context,
      prompt: body.prompt,
    });
    ...
    const expectedShape =
      body.deliverableTypeKey === "charter"
        ? "charter"
        : body.deliverableTypeKey === "outcome_report"
          ? "outcome"
          : body.deliverableTypeKey === "design_spec"
            ? "design"
            : body.deliverableTypeKey === "execution_plan"
              ? "free"
              : "free";
