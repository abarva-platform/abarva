...
import { createEngagement, getEngagementByGraphId } from "@/lib/db/engagement";
...
const INDUSTRIES = [
  ...
  { code: "DIVERSIFIED", name: "Diversified Holdco" },
];
const FUNCTIONS = [
...
export async function POST(req: NextRequest) {
  ...
  const activeOrgKeyword =
    activeClientDisplayName?.split(/\s+/)[0]?.toLowerCase() ?? null;
  const scopedPersons = activeOrgKeyword
    ? persons.filter((p) =>
        (p.organization ?? "").toLowerCase().includes(activeOrgKeyword),
      )
    : persons;
  ...
  const system = assembleEngagementCreateSystemPrompt({
    ...
    activeClient: activeClient
      ? {
          name: activeClientDisplayName ?? activeClient.name,
      ...
      try {
        ...
        if (parsed) {
          ...
          const existing = persons.find(
            (p) => p.graph_node_id === parsed.sponsor_graph_node_id,
          );
          if (existing) {
            ...
            sponsorGraphId =
              newPerson.graph_node_id ?? parsed.sponsor_graph_node_id;
            const personSync = await Promise.allSettled([
              syncPersonToGraph({
                ...
                organization:
                  newPerson.organization ?? parsed.sponsor_payload.organization,
              }),
            ]);
            if (personSync[0]?.status === "rejected") {
              console.warn(
                "[program.create.person_graph_sync_failed]",
                personSync[0].reason,
              );
            }
          } else {
            throw new Error(
              `sponsor ${parsed.sponsor_graph_node_id} not found and no creation payload supplied`,
            );
          }
          ...
          const hydrated =
            (await getEngagementByGraphId(engagement.graph_node_id)) ??
            engagement;
          ...
          const sponsorRow = sponsorRowId
            ? (persons.find((p) => p.id === sponsorRowId) ?? null)
            : null;
          ...
          const labels = {
            industry:
              INDUSTRIES.find((i) => i.code === parsed.industry_code)?.name ??
              parsed.industry_code,
            function:
              FUNCTIONS.find((f) => f.code === parsed.function_code)?.name ??
              parsed.function_code,
            objective:
              OBJECTIVES.find((o) => o.code === parsed.objective_code)?.name ??
              parsed.objective_code,
          };
          ...
          for (const effect of sideEffects) {
            if (effect.status === "rejected") {
              console.warn(
                "[program.create.side_effect_failed]",
                effect.reason,
              );
            }
          }
