...
describe("Tower Atlas synthesis prompt", () => {
  it("inherits the shared agent output contract", () => {
    ...
    expect(prompt).toContain(
      "lead-bullets, lead-table, stat-stack, sequential-steps, or brief-narrative",
    );
    expect(prompt).toContain("do not emit raw markdown emphasis markers");
    expect(prompt).toContain(
      "do not show raw pattern, use-case, vendor, database field, or artifact IDs",
    );
    expect(prompt).toContain("Prefer lead-bullets for the Tower quote");
    expect(prompt).not.toContain("CONSULTANT ANSWER SHAPE");
    expect(prompt).toContain("You are a senior CXO advisor for AbarVa.");
    expect(prompt).toContain("Claude owns the answer");
    expect(prompt).toContain("No visible scaffolding labels");
    expect(prompt).toContain(
      "Reference named programs and source events by their display names, never raw IDs.",
    );
    expect(prompt).not.toContain("Read: the direct recommendation or judgment");
    expect(prompt).not.toContain("Evidence: the specific tenant facts");
    expect(prompt).not.toContain(
      "Implication: what this means for the executive decision",
    );
    expect(prompt).not.toContain("Next move: the owner, artifact, gate");
  });
});
...
describe("composeAtlasSynthesisUserMessage — P0 cross-tenant invariants", () => {
  ...
  it("uses the tenant display name in the message header — First Capital", () => {
    const msg = composeAtlasSynthesisUserMessage(
  ...
  it("snapshot test: unknown tenant gets honest empty — never Apex content as silent default", () => {
    const portfolio = loadTenantTowerPortfolio({
      clientKey: "some-future-tenant",
    });
    const ctx = buildTowerSynthesisContext(
      portfolio.programInstances,
      portfolio.sourceEventInstances,
