      ...
      <div style={{ display: 'flex', height: 'calc(100vh - 113px)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '32px' }}>
          <div style={{ maxWidth: '1200px' }}>
            ...
            {/* decision chooser — shown when no decision is selected */}
            {!selectedId && (
              <div>
                <div style={{ marginBottom: '24px' }}>
                  <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#0F172A', marginBottom: '6px', marginTop: 0 }}>Which decision do you need to make?</h1>
                  <p style={{ fontSize: '14px', color: '#6B7280', margin: 0 }}>Select a decision to see scored vendor shortlists, negotiation playbooks, and AbarVa recommendations.</p>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {(['URGENT', 'HIGH', 'MEDIUM'] as const).map(status => (
                    <div key={status}>
                      <div style={{ fontSize: '10px', fontWeight: 700, color: STATUS_COLOR[status], letterSpacing: '0.1em', marginBottom: '8px', marginTop: status !== 'URGENT' ? '16px' : 0 }}>{status}</div>
                      {DECISIONS.filter(d => d.status === status).map(d => (
                        <button key={d.id} onClick={() => setSelectedId(d.id)}
                          style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '14px 18px', background: '#FFFFFF', border: '1px solid #E2E8F0', borderLeft: '3px solid ' + STATUS_COLOR[status], borderRadius: '10px', marginBottom: '8px', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' as const }}
                          onMouseEnter={e => (e.currentTarget as HTMLElement).style.borderColor = '#0F172A'}
                          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderLeftColor = STATUS_COLOR[d.status]; (e.currentTarget as HTMLElement).style.borderTopColor = '#E2E8F0'; (e.currentTarget as HTMLElement).style.borderRightColor = '#E2E8F0'; (e.currentTarget as HTMLElement).style.borderBottomColor = '#E2E8F0'; }}>
                          <div>
                            <div style={{ fontSize: '14px', fontWeight: 600, color: '#0F172A', marginBottom: '2px' }}>{d.label}</div>
                            <div style={{ fontSize: '12px', color: '#6B7280' }}>{d.deadline}</div>
                          </div>
