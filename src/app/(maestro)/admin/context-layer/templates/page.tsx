  ...
  return (
    <main style={{ background: '#F8F7F4', minHeight: '100%', padding: 32 }}>
      <section style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gap: 18 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(92px, 1fr))', gap: 8, fontFamily: 'DM Sans, sans-serif' }}>
          {SUPPORTED_CONTEXT_UPLOAD_FORMATS.map((format) => (
            <div key={format} style={{ background: '#fffdf8', border: '1px solid #d8d2c4', borderRadius: 8, padding: 10 }}>
              <div style={{ fontSize: 12, color: '#6f6a60' }}>{FORMAT_LABELS[format]}</div>
              <div style={{ fontSize: 18, fontWeight: 700 }}>{coverage[format]}</div>
            </div>
          ))}
        </div>

        <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', minWidth: 1120, borderCollapse: 'collapse', background: '#fffdf8', fontFamily: 'DM Sans, sans-serif' }}>
          <thead>
            <tr>
              {[
                'Dimension',
          ...
          <tbody>
            {NORTHSTAR_CONTEXT_TEMPLATES.map((template) => (
              <tr key={template.id}>
                ...
                <td style={{ borderBottom: '1px solid #eee7d8', padding: 10 }}>{formatList(template.acceptedFormats)}</td>
                <td style={{ borderBottom: '1px solid #eee7d8', padding: 10 }}>
        ...
        <section style={{ fontFamily: 'DM Sans, sans-serif', lineHeight: 1.55 }}>
          <h2 style={{ fontSize: 20, margin: '8px 0' }}>Exception intake rule</h2>
          <p style={{ maxWidth: 920, margin: 0 }}>
            Files outside the canonical template are accepted only as controlled exceptions. The client must
            provide source ownership, sensitivity declaration, field mapping, parse instructions, and format-specific
            anchors such as workbook sheets, JSON paths, PDF pages, document headings, slide numbers, or archive manifest.
            Processing pauses until the mapping and metadata are complete.
          </p>
        </section>
      </section>
    </main>
