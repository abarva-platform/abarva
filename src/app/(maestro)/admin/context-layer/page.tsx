  ...
  return (
    <main style={shellStyle}>
      <section style={{ maxWidth: 960, margin: '0 auto', display: 'grid', gap: 16 }}>
        <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 42, margin: 0 }}>Context layer</h1>
    ...
    <main style={shellStyle}>
      <section style={{ maxWidth: 1180, margin: '0 auto', display: 'grid', gap: 24 }}>
        <div>
          ...
          <p style={{ maxWidth: 860, fontFamily: 'DM Sans, sans-serif', fontSize: 16, lineHeight: 1.6 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 12 }}>
          {[
            ...
            ['Pending or failed', numberLabel(summary.chunksPending + summary.chunksFailed)],
          ].map(([label, value]) => (
        ...
        <section style={panelStyle}>
          <h2 style={{ fontFamily: 'Georgia, serif', marginTop: 0 }}>Upload to agent availability</h2>
            ...
            {stages.map((stage) => (
              <div key={stage.stage} style={{ border: '1px solid #e3decf', borderRadius: 6, padding: 12 }}>
                ...
                <p style={{ margin: '8px 0 0', fontFamily: 'DM Sans, sans-serif', color: '#514c43' }}>
                  {numberLabel(stage.files)} files · {numberLabel(stage.facts)} chunks · {numberLabel(stage.issues)} issues
                </p>
              </div>
            ))}
          </div>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={panelStyle}>
            <h2 style={{ fontFamily: 'Georgia, serif', marginTop: 0 }}>Embedding coverage</h2>
            <dl style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 10, margin: 0, fontFamily: 'DM Sans, sans-serif' }}>
              <dt>Tenant key</dt><dd style={{ margin: 0 }}>{summary.tenantKey}</dd>
              <dt>Providers</dt><dd style={{ margin: 0 }}>{summary.embeddingProviders.join(', ') || 'None recorded'}</dd>
              <dt>Models</dt><dd style={{ margin: 0 }}>{summary.embeddingModels.join(', ') || 'None recorded'}</dd>
              <dt>Last embedded</dt><dd style={{ margin: 0 }}>{summary.lastEmbeddedAt ?? 'Not embedded yet'}</dd>
              <dt>Egress calls</dt><dd style={{ margin: 0 }}>{numberLabel(summary.totalEmbeddingCalls)}</dd>
            </dl>
          </div>
          <div style={panelStyle}>
