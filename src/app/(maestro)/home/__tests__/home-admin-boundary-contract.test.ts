...
describe('Home/Admin boundary contract', () => {
  it('keeps the canonical /home entry on the insight surface', () => {
    ...
    const homeSource = readRepoFile('src/components/home/ImpactInsightsHome.tsx');

    expect(pageSource).toContain("import { ImpactInsightsHome }");
    expect(pageSource).not.toMatch(/HomeOverviewV2|HomeIndexPage|StewardOrientationBlock/);
  ...
  it('keeps legacy setup-ish Home URLs redirected into Admin', () => {
    ...
    expect(proxySource).toContain('request.nextUrl.pathname.startsWith("/home/connectors/")');
    expect(proxySource).toContain('"/admin/connectors/" + sub + request.nextUrl.search');
    ...
    expect(proxySource).toContain('const target = homeToAdminMap[homeCandidate] ?? homeCandidate;');
  });
});
