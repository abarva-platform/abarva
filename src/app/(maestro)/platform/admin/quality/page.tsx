...
const QUALITY_ACTIONS: QualityAction[] = [
  {
    ...
    detail:
      "Current intelligence packets still contain 12 claims without direct Evidence Ledger anchors.",
    delta: 8,
    owner: "Steward",
  ...
  },
  {
    ...
    detail:
      "Finance and operations disagree on prior-auth accountability; CMO and CIO disagree on deployment readiness.",
    delta: 6,
    owner: "Sentinel",
  ...
  },
  {
    ...
    detail:
      "Signals are strong enough to promote, but legal anonymization sign-off is still missing.",
    delta: 5,
    owner: "Steward",
  ...
  },
  {
    ...
    detail:
      "Performance scorecards are being cited in strategy output without a documented system owner and export timestamp.",
    delta: 7,
    owner: "Steward",
  ...
  },
  {
    ...
    detail:
      "Current MAS and FCA materials are old enough to weaken readiness for live board discussion.",
    delta: 5,
    owner: "Nexus",
  ...
  },
  {
    ...
    detail:
      "Signals exist, but the n-count and intervention history are still too thin for canonical promotion.",
    delta: 4,
    owner: "Sentinel",
  ...
  },
  {
    ...
    detail:
      "Drop-off and retention claims are directionally right, but the customer event layer is incomplete.",
    delta: 6,
    owner: "Nexus",
  ...
  },
  {
    ...
    detail:
      "Some recommendations are good ideas but not yet justified by the current evidence base.",
    delta: 7,
    owner: "Sentinel",
  ...
  },
  {
    ...
    detail:
      "The signals are valuable but cannot enter the reusable layer until contribution packaging is complete.",
    delta: 4,
    owner: "Steward",
...
const BACKLOG_ITEMS: BacklogItem[] = [
  ...
  },
  {
    ...
    outcome:
      "Composers now wrap, support Shift+Enter, and enable spellcheck/autocorrect across agent surfaces.",
    nextStep:
      "Keep an eye on any remaining hardcoded input fields outside the shared composer.",
  },
  {
    id: "admin-nav",
    track: "admin",
    ...
    outcome:
      "Admin now routes to /platform/admin instead of dropping users back on the generic platform page.",
  ...
  },
  {
    ...
    outcome:
      "The design language is live on several priority pages, but page-to-page consistency is still incomplete.",
    nextStep:
      "Apply the same light-canvas grammar to Intelligence, Data, Investor, and remaining admin surfaces.",
  },
  {
    id: "phase-chat",
    track: "chat",
    ...
    title:
      "Reset program chat by phase and preserve prior-phase summary in the rail",
    status: "in_progress",
    progress: 72,
    counter: "3 interaction fixes / 4 shipped",
    outcome:
      "Phase handoff logic and right-rail summaries are in code, but the end-to-end live UX still needs smoke-testing.",
    nextStep:
      "Confirm Phase 0 approval cleanly opens a fresh Phase 1 thread in the deployed app.",
  },
  {
    id: "intel-left-rail",
    track: "intelligence",
    ...
    title:
      "Replace the current Intelligence shell with a real left-rail navigation layout",
    status: "in_progress",
    progress: 38,
    counter: "1 new browser component / 1 full-page IA rewrite pending",
    outcome:
      "Foundation browser building blocks exist, but the actual landing page still reads as the old workspace.",
    nextStep:
      "Promote the browser into the primary layout, add the persistent left rail, and retire the duplicated library path.",
  },
  {
    id: "intel-links",
    track: "intelligence",
    ...
    title:
      "Make every Intelligence section and card navigate to real supporting content",
    status: "queued",
    progress: 18,
    counter: "0 dead-link pages acceptable / content audit still required",
    outcome:
      "The IA is not trustworthy until every top box and content card either opens a real page or is removed.",
    nextStep:
      "Audit patterns, benchmarks, vendors, frameworks, and regulations; generate content where the route is empty.",
  },
  {
    id: "backlog-tracker",
    track: "admin",
    ...
    title:
      "Display a live backlog tracker with item-by-item status and counters",
    status: "done",
    progress: 100,
    counter: "1 operator tracker / now live on this page",
    outcome:
      "Progress is now visible in-product instead of being trapped in chat history and PR archaeology.",
    nextStep:
      "Keep this table current as each backlog item is merged and verified.",
  },
];
...
function readinessColor(label: Readiness) {
  if (label === "Settlement-ready")
    return { bg: "#ECFDF5", text: "#166534", border: "#A7F3D0" };
  if (label === "Board-ready")
...
function backlogStatusTheme(status: BacklogStatus) {
  if (status === "done")
    return { bg: "#ECFDF5", text: "#166534", border: "#A7F3D0", label: "Done" };
...
export default function AdminQualityOps() {
  const [selectedTenant, setSelectedTenant] = useState<TenantKey | "all">(
    "all",
  );
  const [resolvedIds, setResolvedIds] = useState<string[]>([]);
  const [selectedTrack, setSelectedTrack] = useState<BacklogTrack | "all">(
    "all",
  );

  const scopedActions = QUALITY_ACTIONS.filter(
    (action) => selectedTenant === "all" || action.tenant === selectedTenant,
  );

  const tenantScores = useMemo(() => {
    return TENANTS.map((tenant) => {
      const tenantActions = QUALITY_ACTIONS.filter(
        (action) => action.tenant === tenant.key,
      );
      const resolved = tenantActions.filter((action) =>
        resolvedIds.includes(action.id),
      );
      const unresolved = tenantActions.filter(
        (action) => !resolvedIds.includes(action.id),
      );
      ...
      const score = {
        data: Math.min(
          ...
          tenant.baseData +
            resolved
              .filter((action) => action.pillar === "data")
              .reduce((sum, action) => sum + action.delta, 0),
        ),
        evidence: Math.min(
          100,
          tenant.baseEvidence +
            resolved
              .filter((action) => action.pillar === "evidence")
              .reduce((sum, action) => sum + action.delta, 0),
        ),
        intelligence: Math.min(
          100,
          tenant.baseIntelligence +
            resolved
              .filter((action) => action.pillar === "intelligence")
              .reduce((sum, action) => sum + action.delta, 0),
        ),
        knowledge: Math.min(
          100,
          tenant.baseKnowledge +
            resolved
              .filter((action) => action.pillar === "knowledge")
              .reduce((sum, action) => sum + action.delta, 0),
        ),
      };
      ...
      const totalLiftRemaining = unresolved.reduce(
  ...
  const scopedScoreRows = tenantScores.filter(
    (row) => selectedTenant === "all" || row.tenant.key === selectedTenant,
  );
  ...
  const averageEvidence = average(
    scopedScoreRows.map((row) => row.score.evidence),
  );
  const averageIntelligence = average(
    scopedScoreRows.map((row) => row.score.intelligence),
  );
  const averageKnowledge = average(
    scopedScoreRows.map((row) => row.score.knowledge),
  );
  const topActions = [...scopedActions]
    .filter((action) => !resolvedIds.includes(action.id))
    .sort((a, b) => b.delta - a.delta);
  const totalPotentialLift = topActions
    .slice(0, 5)
    .reduce((sum, action) => sum + action.delta, 0);
  ...
  const doneCount = backlogItems.filter(
  ...
  const blockedCount = backlogItems.filter(
    (item) => item.status === "blocked",
  ).length;
  const backlogCompletion = average(backlogItems.map((item) => item.progress));
  const nextBacklogItem =
    backlogItems.find((item) => item.status === "in_progress") ??
    backlogItems.find((item) => item.status === "queued") ??
    backlogItems[0];
        ...
        >
          ...
          >
            <div style={{ maxWidth: "880px" }}>
              <div
                style={{ ...S.label, color: "#14B8A6", marginBottom: "10px" }}
              >
                Quality Ops
              </div>
              <h1
              ...
              >
                Score the corpus. Show the gaps. Move the score with operator
                work.
              </h1>
              <p
                style={{
              ...
              >
                This is the internal quality surface for AbarVa operators. It
                separates data quality, evidence integrity, intelligence
                quality, and knowledge health, then ties each weakness to a
                concrete action that can improve the score.
              </p>
            </div>
            ...
            >
              ...
              >
                Resolve the top five actions below and this cohort can gain
                about <strong>+{totalPotentialLift}</strong> points.
              </div>
            </div>
          </div>
        </section>
        ...
        >
          {[
            ...
            },
            {
              label: "Data Quality",
              value: averageData,
              sub: "Coverage · freshness · structure",
              accent: "#2563EB",
            },
            {
              label: "Evidence Integrity",
              value: averageEvidence,
              sub: "Provenance · chain-of-custody",
              accent: "#059669",
            },
            {
              label: "Intelligence Quality",
              value: averageIntelligence,
              sub: "Grounding · specificity · actionability",
              accent: "#14B8A6",
            },
            {
              label: "Knowledge Health",
              value: averageKnowledge,
              sub: "Promotion discipline · reusable patterns",
              accent: "#7C3AED",
            },
          ].map((card) => (
            <div
              key={card.label}
              style={{ ...S.card, padding: "18px 18px 16px" }}
            >
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <div style={{ maxWidth: "760px" }}>
                  ...
                  >
                    Execute the backlog one item at a time, with status always
                    visible.
                  </div>
                  <div
                    style={{
                  ...
                  >
                    This is the operator view for Intelligence and backlog
                    rollout. Every item below carries a status, percent
                    complete, live counter, current outcome, and next action so
                    we always know what shipped and what is still pending.
                  </div>
                </div>
                ...
                >
                  ...
                  >
                    ...
                    {queuedCount} queued · {blockedCount} blocked
                  </div>
                </div>
              </div>
            ...
            <div style={{ padding: "24px", background: "#FFFEFC" }}>
              ...
              >
                {nextBacklogItem?.nextStep ??
                  "Everything in the current filter is complete."}
              </div>
              <div
                style={{
                  display: "flex",
                ...
                {(
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid #E8E6E3",
                      background:
                        selectedTrack === option.key ? "#0F172A" : "#FFFFFF",
                      color:
                        selectedTrack === option.key ? "#FFFFFF" : "#374151",
                      fontSize: "12px",
                      fontWeight: 700,
                      cursor: "pointer",
                    }}
              ...
              >
                Use this strip as the single source of truth during rollout. If
                an item is done, mark it as done here. If it is still debated,
                it stays in progress or queued until the shipped experience
                changes.
              </div>
            </div>
          </div>
                    ...
                    >
                      ...
                      <td style={{ padding: "16px", minWidth: "150px" }}>
                        ...
                        >
                          ...
                          <span>
                            {item.progress === 100 ? "Complete" : "Active"}
                          </span>
                        </div>
                        <div
                          style={{
          ...
          <div style={{ ...S.card, padding: "22px" }}>
            ...
            >
              <div>
                ...
                >
                  Resolve these next to move the score fastest.
                </div>
              </div>
                ...
                {(
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid #E8E6E3",
                      background:
                        selectedTenant === option.key ? "#0F172A" : "#FFFFFF",
                      color:
                        selectedTenant === option.key ? "#FFFFFF" : "#374151",
                      fontSize: "12px",
                      fontWeight: 700,
                      cursor: "pointer",
                    }}
                    ...
                    >
                      <div style={{ flex: 1, minWidth: "280px" }}>
                        ...
                        >
                          ...
                          <span
                            style={{
                              ...
                              padding: "3px 8px",
                              borderRadius: "999px",
                              background: `${pillarAccent(action.pillar)}14`,
                              color: pillarAccent(action.pillar),
                            }}
                          >
                        ...
                        >
                          ...
                          · Estimated lift:{" "}
                          <strong
                            style={{ color: pillarAccent(action.pillar) }}
                          >
                            +{action.delta}
                          </strong>
                        </div>
          ...
          >
            <div style={{ ...S.card, padding: "22px" }}>
              ...
              {[
                {
                  title: "Data Quality",
                  body: "Coverage, freshness, structure, and completeness of the operating corpus.",
                },
                {
                  title: "Evidence Integrity",
                  body: "Provenance, source-of-record links, chain-of-custody, and owner accountability.",
                },
                {
                  title: "Intelligence Quality",
                  body: "Grounding, specificity, contradiction handling, and decision usefulness.",
                },
                {
                  title: "Knowledge Health",
                  body: "Pattern confidence, promotion discipline, anonymization integrity, and reuse value.",
                },
              ].map((item) => (
                <div
                  key={item.title}
                  style={{ padding: "12px 0", borderTop: "1px solid #F0EEEA" }}
                >
                  <div
                    style={{
                      fontSize: "14px",
            ...
            >
              ...
              >
                A better score must come from better evidence, not more words.
              </div>
              <div
                style={{
                  marginTop: "12px",
              ...
              >
                This page should never reward longer decks, prettier summaries,
                or more generated content unless coverage, provenance,
                contradiction handling, or promotion discipline actually
                improved.
              </div>
            </div>
          </div>
        ...
        <section style={{ ...S.card, padding: "22px", marginBottom: "24px" }}>
          ...
          >
            <div>
              ...
              >
                Which tenants are board-ready and which are still exploratory?
              </div>
            </div>
            <div
              style={{
            ...
            >
              Click “Resolve” above and watch these scores move. This is
              intentionally operator-facing.
            </div>
          </div>
                ...
                >
                  ...
                  >
                    {[
                      {
                        label: "Data",
                        value: row.score.data,
                        color: "#2563EB",
                      },
                      {
                        label: "Evidence",
                        value: row.score.evidence,
                        color: "#059669",
                      },
                      {
                        label: "Intelligence",
                        value: row.score.intelligence,
                        color: "#14B8A6",
                      },
                      {
                        label: "Knowledge",
                        value: row.score.knowledge,
                        color: "#7C3AED",
                      },
                    ].map((metric) => (
                      <div
                  ...
                  >
                    ...
                    >
                      {row.nextBest?.title ?? "No open blockers"}
                    </div>
                    <div
                      style={{
                        marginTop: "4px",
                    ...
                    >
                      {row.nextBest
                        ? `Estimated lift +${row.nextBest.delta}. ${row.nextBest.detail}`
                        : "This tenant has no remaining queued quality actions."}
                    </div>
                    <ul
                      style={{
                        margin: "10px 0 0",
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ fontSize: "13px", color: "#6B7280" }}>
                      ...
                      · potential remaining lift{" "}
                      <strong style={{ color: "#111827" }}>
                        +{row.totalLiftRemaining}
                      </strong>
                    </div>
                  </div>
                  {(
                    ...
                    <div key={pillar.key} style={{ marginBottom: "10px" }}>
                      ...
                      >
                        ...
                        <strong style={{ color: pillarAccent(pillar.key) }}>
                          {pillar.value}
                        </strong>
                      </div>
                      <div
                      ...
                      >
                        <div
                          style={{
                            width: `${pillar.value}%`,
                            height: "100%",
                            background: pillarAccent(pillar.key),
                            borderRadius: "999px",
                          }}
                        />
                      </div>
