...
export function buildViewModel(vm: WorkspaceViewModel) {
  ...
  const avaSurfaceContext = {
    ...
    sourceV4: {
      ...
      selectedContract: kind === 'contract' && c ? {
        ...
        contractedToActualVarianceUsd:
          c.annual_value != null && c.actual_annual_spend != null
            ? (numberFromDb(c.annual_value) ?? 0) - (numberFromDb(c.actual_annual_spend) ?? 0)
            : null,
        endDate: fmtDate(c.end_date),
        noticeDate: contract?.noticeDate ? fmtDate(contract.noticeDate.toISOString()) : 'Not established',
      ...
      optimizationSpine: optSpineView ? {
        ...
        sourceConnections: optSpineView.sourceConnections.map((connection) => ({
          id: connection.id,
          sourceSystem: connection.sourceSystem,
      ...
      categoryQuality: {
        ...
        categoryConflictedContractCount: categoryQuality.categoryConflictedContractCount,
        categoryUnclassifiedContractCount: categoryQuality.categoryUnclassifiedContractCount,
