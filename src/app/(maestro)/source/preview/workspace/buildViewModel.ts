...
export function buildViewModel(vm: WorkspaceViewModel) {
    ...
    (
      [
        ...
        [
          ...
          "Concentration",
          "",
          money(summary.totalAnnualValue),
        ],
        ["Explore", "Explore", "", ""],
        [
          "Renewals",
          "Renewals",
          String(rec180Fixed.expiringWithinWindow.length),
          money(rec180Fixed.expiringWithinWindowAnnualValue),
        ],
        [
          "Leverage",
          "Leverage",
          String(rows.filter((c) => c.leverage.weakSignalCount >= 2).length),
          money(
            rows
              .filter((c) => c.leverage.weakSignalCount >= 2)
              .reduce(addRowAnnualValue, 0),
          ),
        ],
        [
          "Opportunities",
          ...
          money(
            opportunities.reduce(
              (t, o) => t + (numberFromDb(o.annualValue) ?? 0),
              0,
            ),
          ),
        ],
      ...
      T.push(
        node({
          ...
          badgeCount: x[2],
          badgeVal: x[3],
          active: kind === "portfolio" && S.tabs.portfolio === x[1],
          onClick: () => vm.select("portfolio", null, x[1]),
        }),
      ),
    );
  }
  ...
  if (S.open.vendors) {
    const cats = Array.from(
      new Set(
        vm.portfolio.vendors.map((v) => v.vendor_category ?? "Unresolved"),
      ),
    );
    cats.forEach((cat) => {
      const vs = vm.portfolio.vendors.filter(
        (v) => (v.vendor_category ?? "Unresolved") === cat,
      );
      T.push(
        node({
          id: "v." + cat,
          label: cat,
          depth: 1,
          badgeCount: String(vs.length),
          badgeVal: money(vs.reduce(addAnnualValue, 0)),
          onClick: () => vm.select("vendorList", cat),
        }),
      );
    });
    T.push(
  ...
  if (S.open.opps) {
    opportunities.forEach((o) =>
      T.push(
        node({
          id: "o." + o.contractId,
          label: o.vendorName + " · " + REASON_LABEL[o.reasons[0]],
          depth: 1,
          size: "12px",
          badgeVal: money(o.annualValue),
          active: kind === "opportunity" && sel.id === o.contractId,
  ...
  if (kind === "portfolio") {
    title = (
      {
        Context: "What does the governed Source data plane show?",
        Explore: "Slice the portfolio any way the question demands",
        Concentration: "Where is spend concentrated?",
        Renewals: "Which decisions are already live?",
        Leverage: "Where is leverage weak?",
        Opportunities: "Where should leadership intervene?",
        Agenda: "The sourcing agenda for this quarter",
    ...
    thesis = (
      {
        Context:
          ...
          " contracts directly from source.contract_360, as of " +
          fmtDate(vm.portfolio.asOfDateIso) +
          ". Where a read returned no rows, or the schema has no column for something, the page says so rather than guessing.",
        Explore:
          "One governed query, re-grouped live against source.contract_360.",
        Concentration:
          "The top ten vendors hold " +
        ...
        Leverage:
          rows.filter((c) => c.leverage.weakSignalCount >= 2).length +
          " contracts carry two or more weak leverage signals. Every position is a countable signal returned by computeContractLeverageSignals, not a score.",
        Opportunities:
          opportunities.length +
          " deterministic opportunities, computed from weak-leverage signals, missed notice deadlines, and vendor concentration — never a fabricated priority score.",
        Agenda:
          "What the governed numbers say, generated live from the same aggregates the lenses show.",
      } as Record<string, string>
    )[activeTab];
    ...
    thesis =
      money(
        vendorPortfolioRow?.annual_value ??
          vendorContracts.reduce(addRowAnnualValue, 0),
      ) +
      " of annual contract value across " +
      (vendorPortfolioRow?.contract_count ?? vendorContracts.length) +
      " governed contracts" +
  ...
  if (kind === "contract" && contract) {
    ...
    valueStrip = [
      ...
      vsItem(
        "Source confidence",
        c.source_confidence != null ? pct(c.source_confidence) : null,
    ...
    valueStrip = [
      vsItem(
        "Annual contract value",
        money(vendorPortfolioRow?.annual_value ?? null),
        (vendorPortfolioRow?.contract_count ?? vendorContracts.length) +
          " governed contracts",
      ),
      vsItem(
    ...
    valueStrip = [
      ...
      vsItem(
        "Actual annual spend",
        money(summary.totalActualAnnualSpend),
      ...
      vsItem(
        "Total committed value",
        money(summary.totalCommittedValue),
      ...
      vsItem(
        "Auto-renewing",
        String(summary.autoRenewCount),
        "of " + summary.contractCount + " contracts",
      ),
    ];
  }
  ...
  const findings = [
    {
      ...
      headline:
        ...
        " active contract" +
        (passedN === 1 ? "" : "s") +
        " have passed their notice window, " +
        autoN +
        " auto-renewing",
      observed:
        ...
        money(rec180Fixed.noticeDeadlinePassedAnnualValue) +
        " in annual value are past the contractual notice window.",
    ...
    },
    {
      ...
      observed:
        ...
        " of annual value, computed by computeContractLeverageSignals from benchmarking_clause, alternatives_available, and concentration_note.",
      why: "Every position on the leverage axis is a countable signal, not a score.",
      response:
        "Prioritise the leverage matrix’s top-right quadrant for renegotiation.",
    },
    {
      ref: "F-3",
      dot: COL.blue,
    ...
    },
    {
      ...
      observed:
        "Computed from weak-leverage signals, missed notice deadlines, and top-concentration vendor status — never a fabricated priority score.",
  ...
  const journeys = [
    {
      id: "A",
      eyebrow: "Path A · optimise an existing contract",
      title: "Select a contract and build a fact-based renewal strategy",
      narrative:
        "Use the governed register to build a renewal, renegotiation or optimisation strategy on a contract already held.",
      cta: "Select a contract to optimise",
  ...
  const vendorStats = [
    ...
    },
    {
      ...
      value: vendorPortfolioRow
        ? pct(
            (vendorPortfolioRow.annual_value ?? 0) /
              (conc.totalAnnualValue || 1),
          )
        : "—",
    },
    {
      label: "Governed contracts",
      value: String(
        vendorPortfolioRow?.contract_count ?? vendorContracts.length,
      ),
    },
  ...
  const econBars = c
    ...
    : [];
  const scopeRows: DataTableRow[] = c
    ? vm
    ...
    : [];
  const scopeCols: DataTableColumn[] = [
    { label: "Application" },
  ...
  return {
    ...
    headerActions:
      kind === "contract" && contract
        ? [
            {
              label: "Build optimisation strategy",
              bg: "#0a0a0b",
              fg: "#fff",
              border: "#0a0a0b",
              onClick: () => vm.setTab("contract", "Optimization"),
            },
          ]
        : kind === "portfolio"
          ? [
              {
                label: "Select a contract to optimise",
                bg: "#0a0a0b",
                fg: "#fff",
                border: "#0a0a0b",
                onClick: () => vm.select("contractList", "weak"),
    ...
    contextTableCols,
    contextTableRows,
    availDot: vm.portfolio.isEmpty ? COL.gray : COL.amber,
    availLabel: vm.portfolio.isEmpty
      ? "Availability: no rows returned for this tenant"
      : "Availability: live · source.contract_360",
    isPortfolioContext: kind === "portfolio" && activeTab === "Context",
    leadershipPosition,
    coverage,
    ...
    isConc: kind === "portfolio" && activeTab === "Concentration",
    pareto,
    ...
    passedCols,
    passedRows,
    isLeverage: kind === "portfolio" && activeTab === "Leverage",
    mx,
    quadPanel,
    signalDefs,
    isOpps: kind === "portfolio" && activeTab === "Opportunities",
    oppGroups,
    oppCols,
    oppRows,
    isAgenda: kind === "portfolio" && activeTab === "Agenda",
    findings,
    journeys,
