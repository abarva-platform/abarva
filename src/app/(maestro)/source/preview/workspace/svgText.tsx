...
export function measureText(t: string, size: number, weight?: number, mono?: boolean): number {
...
export function fitText(text: string, budget: number, size: number, weight?: number, mono?: boolean): string {
  if (measureText(text, size, weight, mono) <= budget) return text;
  let t = String(text);
  while (t.length > 1 && measureText(t + '…', size, weight, mono) > budget) t = t.slice(0, -1);
  return t.replace(/[\s,&]+$/, '') + '…';
}
...
export function SVGT(key: string, x: number, y: number, text: string, o?: SVGTextOpts): ReactElement {
  return (
    <text
      key={key}
      x={x}
