...
const CONTRACTS: SourceContract360Row[] = [
  contractRow({ contract_id: "c1", vendor_ref: "v1", vendor_name: "Vendor One", annual_value: "50000000.00" as unknown as number }),
  contractRow({ contract_id: "c2", vendor_ref: "v2", vendor_name: "Vendor Two", annual_value: "35000000.00" as unknown as number }),
  contractRow({ contract_id: "c3", vendor_ref: "v3", vendor_name: "Vendor Three", annual_value: "42000000.00" as unknown as number }),
];

const VENDORS: SourceVendorContractPortfolioRow[] = [
  vendorRow({ vendor_ref: "v1", vendor_name: "Vendor One", annual_value: "50000000.00" as unknown as number }),
  vendorRow({ vendor_ref: "v2", vendor_name: "Vendor Two", annual_value: "35000000.00" as unknown as number }),
  vendorRow({ vendor_ref: "v3", vendor_name: "Vendor Three", annual_value: "42000000.00" as unknown as number }),
];
...
describe("buildViewModel numeric coercion", () => {
  it("sums explorer-tree badges without string-concatenating NUMERIC-as-string fields", () => {
    const vm = buildVm();
    const built = buildViewModel(vm) as { tree: Array<{ id: string; badgeVal: string }> };
