  ...
  if (shape === "circle-fill")
    return (
      <svg width={s} height={s} style={{ flexShrink: 0 }}>
        <circle cx={c} cy={c} r={c - 2} fill={color} />
      </svg>
    );
  if (shape === "square-fill")
    return (
      <svg width={s} height={s} style={{ flexShrink: 0 }}>
  ...
  if (shape === "check")
    return (
      <svg width={s} height={s} style={{ flexShrink: 0 }} viewBox="0 0 14 14">
        <path
          d="M2.5,7.2 L5.8,10.5 L11.5,3.5"
          fill="none"
          stroke={color}
          strokeWidth="2"
          strokeLinecap="round"
...
export function EvidenceLineageGraph({ vm }: { vm: SourceWorkspaceVM }) {
  ...
  const groups = LEDGER_ORDER.map((kind) => ({
    kind,
    items: lines.filter((l) => l.kind === kind),
  ...
  return (
    ...
    >
      ...
      >
        Trace each opportunity to its contract, transaction, operational, and
        validation evidence. Marker color shows evidence strength, not whether
        the finding is good or bad news.
      </div>
      ...
      >
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        textAlign: "left",
                        border: "none",
                        borderLeft: `2px solid ${isSel ? item.evidenceTone : "rgba(10,10,11,.1)"}`,
                        background: isSel ? "#fbfaf7" : "transparent",
                        cursor: "pointer",
                        padding: "8px 10px",
                        borderRadius: 4,
                        marginBottom: 2,
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          fontWeight: 700,
                          color:
                            item.state === "Quantified" ? "#1d9e75" : "#5f5e5a",
                          whiteSpace: "nowrap",
                        }}
                      >
                        {item.amount}
        ...
        {selected ? (
          <div
            style={{
              background: "#fbfaf7",
              border: `1px solid ${selected.evidenceTone}`,
              borderRadius: 8,
              padding: "18px 20px",
            }}
          >
            ...
            >
              <Marker
                shape={MARKER[selected.evidenceClass] ?? "circle-hollow"}
                color={selected.evidenceTone}
              />
              <span
                style={{
                  fontFamily: "'JetBrains Mono', monospace",
            ...
            {selected.sourceRefs.length > 0 ? (
              <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                <div
                  style={{
                    background: selected.evidenceTone + "1a",
                    border: `1px solid ${selected.evidenceTone}`,
                    borderRadius: 6,
                    padding: "10px 12px",
                    display: "flex",
              ...
              >
                No source record is linked yet — this is exactly why the
                evidence class reads {selected.evidenceClass}.
              </div>
            )}
            ...
            >
              <b style={{ color: "#2c2c2a" }}>Next.</b> {selected.nextAction}
            </div>
            ...
            {selected.lineageFields.length > 0 ? (
              <>
                ...
                >
                  Lineage fields required for full trace
                </div>
