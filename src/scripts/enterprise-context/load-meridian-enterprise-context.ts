...
const IMPORTED_BY = 'Meridian Enterprise Context synthetic Day One import';
...
function parseArgs(): Args {
  ...
  return {
    ...
    sourceRoot: sourceIdx >= 0 ? path.resolve(args[sourceIdx + 1]) : DEFAULT_SOURCE_ROOT,
  };
}
...
function sourceRows(plan: EnterpriseContextIngestionPlan, clientId: string, now: string): DbRow[] {
  return plan.sources.map((source) => ({
    ...
    tenant_aliases: ['meridian-health', 'meridian-health-system'],
    ...
    metadata: { imported_by: IMPORTED_BY },
    updated_at: now,
  }));
}
