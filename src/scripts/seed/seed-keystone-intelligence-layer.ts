...
import { getScopeId, loadKeystoneOverlay } from './intelligence-layer-lib';
...
async function resolveKeystoneClient(sb: SupabaseClient): Promise<ClientRow> {
  for (const field of [
    { column: 'name', value: TENANTS.keystone.shortName },
    { column: 'name', value: TENANTS.keystone.canonicalName },
  ...
  throw new Error('Keystone client missing. Run `npm run db:seed:wave -- --tenant keystone` first.');
}
...
async function assertBaseSeedPresent(sb: SupabaseClient, clientId: string): Promise<void> {
  const { data, error } = await sb
    ...
    .select('category')
    .eq('org_id', clientId)
    .in('category', ['subsidiary_structure', 'regulatory_environment', 'benchmark_data', 'active_patterns'])
    .limit(10);
  if (error) throw error;
  const categories = new Set(((data ?? []) as Array<{ category: string }>).map((row) => row.category));
  const missing = ['subsidiary_structure', 'regulatory_environment', 'benchmark_data', 'active_patterns']
    .filter((category) => !categories.has(category));
  if (missing.length > 0) {
    throw new Error(`Keystone base seed incomplete. Missing categories: ${missing.join(', ')}`);
  }
}
...
async function loadPeopleByName(sb: SupabaseClient): Promise<Map<string, PersonRow>> {
  ...
  if (error) throw error;
  return new Map(((data ?? []) as PersonRow[]).map((row) => [row.name, row]));
}
...
async function main() {
  ...
  const kpiRows = overlay.kpis.map((kpi) => ({
    ...
    owner_person_id: kpi.ownerName ? peopleByName.get(kpi.ownerName)?.id ?? null : null,
    owner_person_name: kpi.ownerName,
