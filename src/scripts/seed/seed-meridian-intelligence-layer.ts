...
async function assertBaseSeedPresent(sb: SupabaseClient, clientId: string): Promise<void> {
  ...
  if (missing.length > 0) {
    throw new Error(`Meridian base seed incomplete. Missing categories: ${missing.join(', ')}`);
  }
}
...
async function main() {
  ...
  const client = await resolveMeridianClient(sb);
  ...
  console.log('\nMeridian intelligence layer seeded');
