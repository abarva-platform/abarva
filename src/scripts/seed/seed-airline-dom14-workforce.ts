...
const AIRLINE_WORKFORCE_PATTERNS: AirlineWorkforcePatternSeed[] = [
  {
    code: 'A4200',
...
function graphEdgesFor(pattern: AirlineWorkforcePatternSeed): Array<Record<string, unknown>> {
  ...
  return [
    {
      ...
      vertical: 'airline',
      weight: 1,
      evidence: { seeded_by: 'seed-airline-dom14-workforce', office_category: pattern.officeCategory },
    ...
    },
    {
      ...
      vertical: 'airline',
      weight: 0.82,
      evidence: { seeded_by: 'seed-airline-dom14-workforce', keywords: pattern.keywords },
      source_key: 'skyharbor-air',
    },
  ];
}
...
async function main() {
  ...
  const patternRows = AIRLINE_WORKFORCE_PATTERNS.map((pattern) => ({
  ...
  const graphEdges = AIRLINE_WORKFORCE_PATTERNS.flatMap(graphEdgesFor);
  ...
  const { count: edgeCount, error: edgeCountError } = await sb
    ...
    .select('id', { count: 'exact', head: true })
    .eq('vertical', 'airline')
    .in('from_node_id', AIRLINE_WORKFORCE_PATTERNS.map((pattern) => pattern.code));
  if (edgeCountError) throw edgeCountError;

  console.log(`Seeded airline workforce Genome patterns: ${patternCount ?? 0}`);
  console.log(`Seeded airline workforce Genome graph edges: ${edgeCount ?? 0}`);
}
