...
import { loadFirstCapitalOverlay } from './first-capital-intelligence-layer-lib';
...
async function countLegalContextsWithScope(clientId: string): Promise<number> {
  const sb = createSeedClient();
  const { data, error } = await sb
    .from('legal_privileged_contexts')
    .select('id, access_scope_id')
    .eq('client_id', clientId);
  if (error) throw error;
  return ((data ?? []) as Array<{ access_scope_id: string | null }>)
    .filter((row) => Boolean(row.access_scope_id))
    .length;
}
...
async function main() {
  ...
  const [
    ...
    count('legal_privileged_contexts', clientId),
    ...
    countLegalContextsWithScope(clientId),
  ]);

  const expectedLegalContexts = 4;
  const checks = [
    ...
    { label: `legal privileged contexts = ${expectedLegalContexts}`, passed: legalContextCount === expectedLegalContexts, actual: legalContextCount },
    ...
    { label: `legal privileged contexts scoped = ${expectedLegalContexts}`, passed: legalContextsWithScope === expectedLegalContexts, actual: legalContextsWithScope },
  ];

  console.log('\nFirst Capital intelligence layer verification');
