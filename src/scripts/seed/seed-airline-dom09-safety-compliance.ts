...
const AIRLINE_SAFETY_PATTERNS: AirlineSafetyPatternSeed[] = [
  {
    code: 'A2700',
  ...
  return [
    {
      ...
      vertical: 'airline',
      weight: 1,
      evidence: { seeded_by: 'seed-airline-dom09-safety-compliance', office_category: pattern.officeCategory },
    ...
    },
    {
      ...
      vertical: 'airline',
      weight: 0.82,
      evidence: { seeded_by: 'seed-airline-dom09-safety-compliance', keywords: pattern.keywords },
      source_key: 'skyharbor-air',
    },
  ];
}
...
async function main() {
  ...
  const patternRows = AIRLINE_SAFETY_PATTERNS.map((pattern) => ({
  ...
  const graphEdges = AIRLINE_SAFETY_PATTERNS.flatMap(graphEdgesFor);
  ...
  const { count: edgeCount, error: edgeCountError } = await sb
    ...
    .select('id', { count: 'exact', head: true })
    .eq('vertical', 'airline')
    .in('from_node_id', AIRLINE_SAFETY_PATTERNS.map((pattern) => pattern.code));
  if (edgeCountError) throw edgeCountError;

  console.log(`Seeded airline safety/compliance Genome patterns: ${patternCount ?? 0}`);
  console.log(`Seeded airline safety/compliance Genome graph edges: ${edgeCount ?? 0}`);
}
