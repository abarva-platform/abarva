...
const AIRLINE_REVENUE_PATTERNS: AirlineRevenuePatternSeed[] = [
  {
    code: 'A300',
  ...
  return [
    {
      ...
      vertical: 'airline',
      weight: 1,
      evidence: { seeded_by: 'seed-airline-dom01-revenue-mgmt', office_category: pattern.officeCategory },
    ...
    },
    {
      ...
      vertical: 'airline',
      weight: 0.82,
      evidence: { seeded_by: 'seed-airline-dom01-revenue-mgmt', keywords: pattern.keywords },
      source_key: 'skyharbor-air',
    },
  ];
}
...
async function main() {
  ...
  const patternRows = AIRLINE_REVENUE_PATTERNS.map((pattern) => ({
  ...
  const graphEdges = AIRLINE_REVENUE_PATTERNS.flatMap(graphEdgesFor);
  ...
  const { count: edgeCount, error: edgeCountError } = await sb
    ...
    .select('id', { count: 'exact', head: true })
    .eq('vertical', 'airline')
    .in('from_node_id', AIRLINE_REVENUE_PATTERNS.map((pattern) => pattern.code));
  if (edgeCountError) throw edgeCountError;

  console.log(`Seeded airline revenue management Genome patterns: ${patternCount ?? 0}`);
  console.log(`Seeded airline revenue management Genome graph edges: ${edgeCount ?? 0}`);
}
