...
const AIRLINE_DIGITAL_AI_PATTERNS: AirlineDigitalAiPatternSeed[] = [
  {
    code: 'A5100',
...
function graphEdgesFor(pattern: AirlineDigitalAiPatternSeed): Array<Record<string, unknown>> {
  ...
  return [
    {
      ...
      vertical: 'airline',
      weight: 1,
      evidence: { seeded_by: 'seed-airline-dom17-digital-ai', office_category: pattern.officeCategory },
    ...
    },
    {
      ...
      vertical: 'airline',
      weight: 0.82,
      evidence: { seeded_by: 'seed-airline-dom17-digital-ai', keywords: pattern.keywords },
      source_key: 'skyharbor-air',
    },
  ];
}
...
async function main() {
  ...
  const patternRows = AIRLINE_DIGITAL_AI_PATTERNS.map((pattern) => ({
  ...
  const graphEdges = AIRLINE_DIGITAL_AI_PATTERNS.flatMap(graphEdgesFor);
  ...
  const { count: edgeCount, error: edgeCountError } = await sb
    ...
    .select('id', { count: 'exact', head: true })
    .eq('vertical', 'airline')
    .in('from_node_id', AIRLINE_DIGITAL_AI_PATTERNS.map((pattern) => pattern.code));
  if (edgeCountError) throw edgeCountError;

  console.log(`Seeded airline digital AI Genome patterns: ${patternCount ?? 0}`);
  console.log(`Seeded airline digital AI Genome graph edges: ${edgeCount ?? 0}`);
}
