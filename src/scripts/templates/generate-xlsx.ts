...
function writeSheet(wb: ExcelJS.Workbook, t: DimensionTemplate) {
  ...
  ws.getRow(2).values = [
    `Required columns highlighted teal · Template v${TEMPLATE_VERSION}`,
  ];
...
function writeReadme(wb: ExcelJS.Workbook) {
  ...
  const lines: Array<{
    ...
    {
      text: "AbarVa Control Tower · Data Bundle Template",
      bold: true,
      size: 18,
    },
    ...
    { text: "Azure, GCP, Okta/Entra, and Snowflake lives at /tower." },
    { text: "" },
