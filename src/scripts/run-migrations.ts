...
export function parseArgs(argv: string[]): CliFlags {
  const isDry =
    argv.includes("--dry") || process.env.MIGRATION_DRY_RUN === "true";
  const isCi = argv.includes("--ci") || process.env.MIGRATION_CI === "true";
  const markAllApplied =
    argv.includes("--mark-all-applied") ||
    process.env.MIGRATION_MARK_ALL_APPLIED === "true";
  const allowDestructive =
    argv.includes("--allow-destructive") ||
    process.env.MIGRATION_ALLOW_DESTRUCTIVE === "true";
  const forceIdx = argv.indexOf("--force");
  const forceName =
    forceIdx >= 0
      ? (argv[forceIdx + 1] ?? null)
      : (process.env.MIGRATION_FORCE_NAME ?? null);
...
async function main() {
  const { isDry, isCi, markAllApplied, allowDestructive, forceName } =
  ...
  if (!url) {
    ...
    console.error(
      "   Use the Azure/Postgres direct migration connection string for production deploys.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (driftFindings.length > 0) {
      ...
      console.error(
        "   If this drift is intentional and understood, re-record the hash with:",
      );
      console.error(
        "     npx tsx src/scripts/run-migrations.ts --force <name>\n",
      );
      process.exit(1);
    }
    ...
    if (markAllApplied) {
      ...
      console.log(
        `✓  Marked ${all.length} migration${all.length === 1 ? "" : "s"} as applied (${inserted} new rows, ${all.length - inserted} already recorded).`,
      );
      return;
    }
    ...
    if (allFindings.length > 0 && !allowDestructive) {
      ...
      console.error("");
      console.error(
        "   If this is intentional, add the opt-in marker as the FIRST",
      );
      ...
      console.error("");
      console.error(
        "   Then re-run. The marker is a deliberate audit signal — only",
      );
      console.error(
        "   add it after a human review of the destructive change.",
      );
      process.exit(1);
    }
    if (allFindings.length > 0 && allowDestructive) {
      ...
      console.warn(
        "   Use this only for controlled empty-database bootstrap / parallel-run migrations.\n",
      );
      for (const f of allFindings) {
    ...
    if (!lockResult.rows[0]?.locked) {
      ...
      console.error(
        "   Wait for it to finish, or investigate a stuck/crashed prior run.\n",
      );
      process.exit(1);
    }
    ...
    try {
      ...
      for (const filename of pending) {
        ...
        try {
          ...
          console.error(
            `  Fix the error above, then re-run. Applied migrations are tracked; only the failed one + later ones will retry.`,
          );
          process.exit(1);
        }
      }
