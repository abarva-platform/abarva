// src/scripts/setup-data/load-internal-golden-setup-data.ts
//
// Golden Move fixture tenant setup-data loader — `internal-golden`.
//
// Loads the synthetic "AbarVa Golden Health System" dataset (datasets live at
// repo-root `internal-golden-data/`, mirroring the shape of `meridian-data/`)
// into the same 14-segment data-inventory tables the other tenant loaders use.
//
// This tenant exists per docs/architecture/MOVES_OPERATING_MODEL.md's "Golden
// Move" proposal: a permanent, internal-only regression/demo/proof fixture so
// that MEMBER AI ASSIST (a real, disputed Move under active fabrication-
// incident remediation) never has to be touched again for testing. It is not
// a client tenant and must never be treated as one — see
// datasets/tenant-inputs/tenant-input-registry.json's `internal-golden` entry
// (classification: "internal-fixture") and docs/governance/dataset-manifests/
// internal-golden.json.
//
// This file is code/config only. Running it with --apply performs a real
// Supabase mutation and must not be run from a sandboxed environment against
// production credentials — see the release record for this change.
...
const TENANT_KEY = 'internal-golden';
const SOURCE_BASIS = 'internal_fixture_synthetic_generation';
const UPLOADED_BY = 'Golden Move synthetic dataset import · 2026-07-24';
...
const SEGMENTS: SegmentConfig[] = [
  ...
  { familyNumber: 2, segmentId: 'org_structure', segmentName: 'Org structure', dir: '02_org_structure', expectedRecordCount: 36, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 3, segmentId: 'it_landscape', segmentName: 'IT system landscape', dir: '03_it_landscape', expectedRecordCount: 285, expectedFreshnessDays: 90, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 4, segmentId: 'it_financials', segmentName: 'IT financials', dir: '04_it_financials', expectedRecordCount: 199, expectedFreshnessDays: 30, reasoningModes: ['tenant_grounded'] },
  { familyNumber: 5, segmentId: 'kpi_dictionary', segmentName: 'KPI dictionary', dir: '05_kpi_dictionary', expectedRecordCount: 150, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 6, segmentId: 'program_inventory', segmentName: 'Program inventory', dir: '06_program_inventory', expectedRecordCount: 40, expectedFreshnessDays: 14, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 7, segmentId: 'sourcing_artifacts', segmentName: 'Sourcing artifacts', dir: '07_sourcing_artifacts', expectedRecordCount: 44, expectedFreshnessDays: 14, reasoningModes: ['tenant_grounded'] },
  { familyNumber: 8, segmentId: 'program_deliverables', segmentName: 'Program deliverables', dir: '08_program_deliverables', expectedRecordCount: 4, expectedFreshnessDays: 14, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 9, segmentId: 'evidence_ledger', segmentName: 'Evidence ledger', dir: '09_evidence_ledger', expectedRecordCount: 40, expectedFreshnessDays: 30, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 10, segmentId: 'operating_telemetry', segmentName: 'Operating telemetry', dir: '10_operating_telemetry', expectedRecordCount: 31, expectedFreshnessDays: 7, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 11, segmentId: 'vendor_contracts', segmentName: 'Vendor and contract', dir: '11_vendor_contracts', expectedRecordCount: 80, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 12, segmentId: 'compliance', segmentName: 'Compliance and regulatory', dir: '12_compliance', expectedRecordCount: 21, expectedFreshnessDays: 90, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 13, segmentId: 'industry_context', segmentName: 'Industry context', dir: '13_industry_context', expectedRecordCount: 28, expectedFreshnessDays: 30, reasoningModes: ['cross_corpus'] },
  { familyNumber: 14, segmentId: 'cross_program_signals', segmentName: 'Cross-program signals', dir: '14_cross_program_signals', expectedRecordCount: 22, expectedFreshnessDays: 7, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
];
...
function parseArgs() {
  ...
  return {
    sourceRoot: sourceIdx >= 0 ? args[sourceIdx + 1] : path.resolve(process.cwd(), 'internal-golden-data'),
    dryRun,
  };
}
...
function pickId(payload: Record<string, unknown>, fallback: string): string {
  for (const key of ['id', 'record_id', 'system_id', 'tool_id', 'kpi_id', 'contract_id', 'vendor_id', 'template_id']) {
    const value = payload[key];
...
function buildGraph(records: RecordSeed[]): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  addNode({ node_id: 'enterprise:internal-golden', node_type: 'enterprise', label: 'AbarVa Golden Health System', source_segment_id: 'enterprise_profile', source_record_id: 'enterprise_profile:enterprise-profile', source_doc: 'enterprise_profile.md', confidence: 0.9 });
  ...
  for (const record of records) {
    ...
    if (typeof p.id === 'string' && p.id.startsWith('person:')) {
      ...
      addEdge({ edge_id: `enterprise:internal-golden:has-executive:${p.id}`, from_node_id: 'enterprise:internal-golden', to_node_id: p.id, edge_type: 'HAS_EXECUTIVE', source_segment_id: record.segmentId, source_record_id: record.recordId, source_doc: record.sourceDoc, confidence: record.confidence });
    ...
    if (typeof p.system_id === 'string') {
      ...
      if (typeof p.vendor === 'string' && p.vendor) {
        const vendorId = `vendor:golden:${slugify(p.vendor)}`;
      ...
      if (Array.isArray(p.supports_claims_in)) {
        for (const supported of p.supports_claims_in) {
          if (typeof supported === 'string') {
            addEdge({ edge_id: `${p.id}:supports:${supported}`, from_node_id: p.id, to_node_id: supported.startsWith('golden-') ? `program:${supported}` : supported, edge_type: 'SUPPORTS_CLAIM_IN', source_segment_id: record.segmentId, source_record_id: record.recordId, source_doc: record.sourceDoc, confidence: record.confidence });
          }
        }
      }
    }
...
async function ensureClient(client: SupabaseClient) {
  const existing = await client
    .from('clients')
    .select('id')
    .or('name.eq.AbarVa Golden Health System,legal_name.eq.AbarVa Golden Health System (Golden Move fixture)')
    .limit(1)
    .maybeSingle();
  if (existing.error) throw new Error(`Golden Move fixture client lookup failed: ${existing.error.message}`);
  ...
  const inserted = await client
    ...
    .insert({
      ...
      legal_name: 'AbarVa Golden Health System (Golden Move fixture)',
      industry_code: 'healthcare',
    })
    .select('id')
    .single();
  if (inserted.error) throw new Error(`Golden Move fixture client insert failed: ${inserted.error.message}`);
...
async function main() {
  ...
  const runStart = await client.from('data_ingestion_runs').insert({ client_id: clientId, tenant_key: TENANT_KEY, source_label: 'AbarVa Golden Health System synthetic setup dataset (Golden Move fixture)', source_root: sourceRoot, status: 'started' }).select('id').single();
  ...
  try {
    await upsertBatch(client, 'tenant_expected_baselines', SEGMENTS.map((segment) => ({
      ...
      tenant_archetype: 'healthcare-idn',
    ...
    const existingAudit = await client
      ...
      .eq('source_doc', 'internal-golden-synthetic-dataset');
    ...
    const auditRows = SEGMENTS
      ...
      .map((segment) => ({
        ...
        segment_id: segment.segmentId,
        record_id: null,
        after_state: { record_count: bySegment.get(segment.segmentId)?.length ?? 0, source_label: 'AbarVa Golden Health System synthetic setup dataset (Golden Move fixture)' },
        classification_at_action: 'Mixed',
        source_doc: 'internal-golden-synthetic-dataset',
      }));
    if (auditRows.length) {
