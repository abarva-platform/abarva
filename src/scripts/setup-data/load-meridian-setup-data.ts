...
const UPLOADED_BY = 'Meridian synthetic dataset import · 2026-04-29';
...
const SEGMENTS: SegmentConfig[] = [
  ...
  { familyNumber: 2, segmentId: 'org_structure', segmentName: 'Org structure', dir: '02_org_structure', expectedRecordCount: 35, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 3, segmentId: 'it_landscape', segmentName: 'IT system landscape', dir: '03_it_landscape', expectedRecordCount: 110, expectedFreshnessDays: 90, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 4, segmentId: 'it_financials', segmentName: 'IT financials', dir: '04_it_financials', expectedRecordCount: 135, expectedFreshnessDays: 30, reasoningModes: ['tenant_grounded'] },
  ...
  { familyNumber: 7, segmentId: 'sourcing_artifacts', segmentName: 'Sourcing artifacts', dir: '07_sourcing_artifacts', expectedRecordCount: 45, expectedFreshnessDays: 14, reasoningModes: ['tenant_grounded'] },
  ...
  { familyNumber: 10, segmentId: 'operating_telemetry', segmentName: 'Operating telemetry', dir: '10_operating_telemetry', expectedRecordCount: 35, expectedFreshnessDays: 7, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 11, segmentId: 'vendor_contracts', segmentName: 'Vendor and contract', dir: '11_vendor_contracts', expectedRecordCount: 55, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 12, segmentId: 'compliance', segmentName: 'Compliance and regulatory', dir: '12_compliance', expectedRecordCount: 20, expectedFreshnessDays: 90, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
...
function parseArgs() {
  ...
  return {
    sourceRoot: sourceIdx >= 0 ? args[sourceIdx + 1] : path.resolve(process.cwd(), 'meridian-data'),
    dryRun,
  };
}
...
function buildGraph(records: RecordSeed[]): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  addNode({ node_id: 'enterprise:meridian-health', node_type: 'enterprise', label: 'Meridian Health System', source_segment_id: 'enterprise_profile', source_record_id: 'enterprise_profile:enterprise-profile', source_doc: 'enterprise_profile.md', confidence: 0.9 });
  ...
  for (const record of records) {
    ...
    if (typeof p.id === 'string' && p.id.startsWith('person:')) {
      ...
      addEdge({ edge_id: `enterprise:meridian-health:has-executive:${p.id}`, from_node_id: 'enterprise:meridian-health', to_node_id: p.id, edge_type: 'HAS_EXECUTIVE', source_segment_id: record.segmentId, source_record_id: record.recordId, source_doc: record.sourceDoc, confidence: record.confidence });
    ...
    if (typeof p.system_id === 'string') {
      ...
      if (typeof p.vendor === 'string' && p.vendor) {
        const vendorId = `vendor:meridian:${slugify(p.vendor)}`;
      ...
      if (Array.isArray(p.supports_claims_in)) {
        for (const supported of p.supports_claims_in) {
          if (typeof supported === 'string') {
            addEdge({ edge_id: `${p.id}:supports:${supported}`, from_node_id: p.id, to_node_id: supported.startsWith('meridian-') ? `program:${supported}` : supported, edge_type: 'SUPPORTS_CLAIM_IN', source_segment_id: record.segmentId, source_record_id: record.recordId, source_doc: record.sourceDoc, confidence: record.confidence });
          }
        }
      }
    }
...
async function main() {
  ...
  const runStart = await client.from('data_ingestion_runs').insert({ client_id: clientId, tenant_key: TENANT_KEY, source_label: 'Meridian Health synthetic setup dataset', source_root: sourceRoot, status: 'started' }).select('id').single();
  ...
  try {
    ...
    const existingAudit = await client
      ...
      .eq('source_doc', 'meridian-health-synthetic-dataset');
    ...
    const auditRows = SEGMENTS
      ...
      .map((segment) => ({
        ...
        segment_id: segment.segmentId,
        record_id: null,
        after_state: { record_count: bySegment.get(segment.segmentId)?.length ?? 0, source_label: 'Meridian Health synthetic setup dataset' },
        classification_at_action: 'Mixed',
        source_doc: 'meridian-health-synthetic-dataset',
      }));
    if (auditRows.length) {
