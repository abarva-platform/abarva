...
const UPLOADED_BY = 'Apex synthetic dataset import · 2026-04-29';
...
const SEGMENTS: SegmentConfig[] = [
  ...
  { familyNumber: 2, segmentId: 'org_structure', segmentName: 'Org structure', dir: '02_org_structure', expectedRecordCount: 50, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 3, segmentId: 'it_landscape', segmentName: 'IT system landscape', dir: '03_it_landscape', expectedRecordCount: 80, expectedFreshnessDays: 90, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 4, segmentId: 'it_financials', segmentName: 'IT financials', dir: '04_it_financials', expectedRecordCount: 30, expectedFreshnessDays: 30, reasoningModes: ['tenant_grounded'] },
  { familyNumber: 5, segmentId: 'kpi_dictionary', segmentName: 'KPI dictionary', dir: '05_kpi_dictionary', expectedRecordCount: 50, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 6, segmentId: 'program_inventory', segmentName: 'Program inventory', dir: '06_program_inventory', expectedRecordCount: 4, expectedFreshnessDays: 14, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 7, segmentId: 'sourcing_artifacts', segmentName: 'Sourcing artifacts', dir: '07_sourcing_artifacts', expectedRecordCount: 10, expectedFreshnessDays: 14, reasoningModes: ['tenant_grounded'] },
  ...
  { familyNumber: 10, segmentId: 'operating_telemetry', segmentName: 'Operating telemetry', dir: '10_operating_telemetry', expectedRecordCount: 15, expectedFreshnessDays: 7, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 11, segmentId: 'vendor_contracts', segmentName: 'Vendor and contract', dir: '11_vendor_contracts', expectedRecordCount: 28, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 12, segmentId: 'compliance', segmentName: 'Compliance and regulatory', dir: '12_compliance', expectedRecordCount: 10, expectedFreshnessDays: 90, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 13, segmentId: 'industry_context', segmentName: 'Industry context', dir: '13_industry_context', expectedRecordCount: 10, expectedFreshnessDays: 30, reasoningModes: ['cross_corpus'] },
  { familyNumber: 14, segmentId: 'cross_program_signals', segmentName: 'Cross-program signals', dir: '14_cross_program_signals', expectedRecordCount: 12, expectedFreshnessDays: 7, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
];
...
function parseArgs() {
  ...
  return {
    sourceRoot: sourceIdx >= 0 ? args[sourceIdx + 1] : '/private/tmp/nexus-new-datasets-intake/apex/apex-data',
    dryRun,
  };
}
...
function buildGraph(records: RecordSeed[]): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  addNode({ node_id: 'enterprise:apex-retail', node_type: 'enterprise', label: 'Apex Retail Group', source_segment_id: 'enterprise_profile', source_record_id: 'enterprise_profile:enterprise-profile', source_doc: 'enterprise_profile.md', confidence: 0.9 });
  ...
  for (const record of records) {
    ...
    if (typeof p.id === 'string' && p.id.startsWith('person:')) {
      ...
      addEdge({ edge_id: `enterprise:apex-retail:has-executive:${p.id}`, from_node_id: 'enterprise:apex-retail', to_node_id: p.id, edge_type: 'HAS_EXECUTIVE', source_segment_id: record.segmentId, source_record_id: record.recordId, source_doc: record.sourceDoc, confidence: record.confidence });
    ...
    if (typeof p.system_id === 'string') {
      ...
      if (typeof p.vendor === 'string' && p.vendor) {
        const vendorId = `vendor:apex:${slugify(p.vendor)}`;
      ...
      if (typeof p.owner_id === 'string' && p.owner_id) {
        addEdge({ edge_id: `${p.system_id}:owned-by:${p.owner_id}`, from_node_id: p.system_id, to_node_id: p.owner_id, edge_type: 'OWNED_BY', source_segment_id: record.segmentId, source_record_id: record.recordId, source_doc: record.sourceDoc, confidence: record.confidence });
      }
    }
    ...
    if (typeof p.kpi_id === 'string') {
      ...
      if (typeof p.source_system === 'string' && p.source_system) {
        addEdge({ edge_id: `${p.kpi_id}:measured-from:${p.source_system}`, from_node_id: p.kpi_id, to_node_id: p.source_system, edge_type: 'MEASURED_FROM', source_segment_id: record.segmentId, source_record_id: record.recordId, source_doc: record.sourceDoc, confidence: record.confidence });
      }
    }
...
async function ensureClient(client: SupabaseClient) {
  const existing = await client
    ...
    .limit(1)
    .maybeSingle();
  if (existing.error) throw new Error(`Apex client lookup failed: ${existing.error.message}`);
  if (existing.data?.id) return existing.data.id as string;

  const inserted = await client
    .from('clients')
    .insert({ name: 'Apex Retail', legal_name: 'Apex Retail Group', industry_code: 'retail' })
    .select('id')
    .single();
  if (inserted.error) throw new Error(`Apex client insert failed: ${inserted.error.message}`);
...
async function main() {
  ...
  const runStart = await client.from('data_ingestion_runs').insert({ client_id: clientId, tenant_key: TENANT_KEY, source_label: 'Apex Retail synthetic setup dataset', source_root: sourceRoot, status: 'started' }).select('id').single();
  ...
  try {
    ...
    const existingAudit = await client
      ...
      .eq('source_doc', 'apex-retail-synthetic-dataset.zip');
    ...
    const auditRows = SEGMENTS
      ...
      .map((segment) => ({
        ...
        segment_id: segment.segmentId,
        record_id: null,
        after_state: { record_count: bySegment.get(segment.segmentId)?.length ?? 0, source_label: 'Apex Retail synthetic setup dataset' },
        classification_at_action: 'Mixed',
        source_doc: 'apex-retail-synthetic-dataset.zip',
      }));
    if (auditRows.length) {
