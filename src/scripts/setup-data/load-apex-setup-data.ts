...
export const TENANT_KEY = "apex-retail";
export const SOURCE_BASIS = "tenant_admin_upload";
export const UPLOADED_BY = "Apex synthetic dataset import · 2026-04-29";
...
export const SEGMENTS: SegmentConfig[] = [
  {
    ...
    dir: "01_enterprise_profile",
    expectedRecordCount: 1,
    expectedFreshnessDays: 90,
    reasoningModes: ["tenant_grounded", "cross_corpus"],
  },
  {
    familyNumber: 2,
    segmentId: "org_structure",
    segmentName: "Org structure",
    dir: "02_org_structure",
  ...
  {
    familyNumber: 8,
    segmentId: "program_deliverables",
    segmentName: "Program deliverables",
    dir: "08_program_deliverables",
    expectedRecordCount: 4,
    expectedFreshnessDays: 14,
    reasoningModes: ["tenant_grounded", "cross_corpus"],
  },
  ...
  },
  {
    ...
    dir: "13_industry_context",
    expectedRecordCount: 10,
    expectedFreshnessDays: 30,
    reasoningModes: ["cross_corpus"],
  },
  {
    familyNumber: 14,
    segmentId: "cross_program_signals",
    segmentName: "Cross-program signals",
    dir: "14_cross_program_signals",
...
function parseArgs() {
  ...
  return {
    sourceRoot:
      sourceIdx >= 0
        ? args[sourceIdx + 1]
        : "/private/tmp/nexus-new-datasets-intake/apex/apex-data",
    dryRun,
  };
}
...
function textFromPayload(payload: unknown): string {
  ...
  if (payload && typeof payload === "object" && !Array.isArray(payload)) {
    return Object.entries(payload as Record<string, unknown>)
      .map(
        ([key, value]) =>
          `${key}: ${Array.isArray(value) || typeof value === "object" ? JSON.stringify(value) : String(value)}`,
      )
      .join("\n");
  }
...
function makeRecord(params: {
  ...
  const classification =
    (typeof params.payload.data_classification === "string" &&
      params.payload.data_classification) ||
    (typeof params.rootMeta?.data_classification === "string" &&
      params.rootMeta.data_classification) ||
    "Internal";
  const lastReviewed =
    (typeof params.payload.last_updated === "string" &&
      params.payload.last_updated) ||
    (typeof params.payload.last_reviewed === "string" &&
      params.payload.last_reviewed) ||
    (typeof params.rootMeta?.last_updated === "string" &&
      params.rootMeta.last_updated) ||
    "2026-04-29";
  return {
    ...
    confidence:
      confidenceFrom(
        params.payload.confidence ?? params.payload.confidence_in_measurement,
      ) ?? 0.82,
    lastReviewed,
    ...
    for (const file of await collectFiles(segmentDir)) {
      ...
      if (ext === ".md") {
        ...
        csv.data.forEach((row, idx) => {
          records.push(
            makeRecord({
              ...
              payload: {
                ...row,
                _parse_warnings: csv.errors.map((error) => error.message),
              },
        ...
        const parsed = JSON.parse(readFileSync(file, "utf8")) as Record<
          ...
          unknown
        >;
        const rootMeta = Object.fromEntries(
...
export function buildGraph(records: RecordSeed[]): {
  nodes: GraphNode[];
  edges: GraphEdge[];
} {
  ...
  addNode({
    node_id: "enterprise:apex-retail",
    node_type: "enterprise",
    label: "Apex Retail Group",
    source_segment_id: "enterprise_profile",
    source_record_id: "enterprise_profile:enterprise-profile",
  ...
  for (const record of records) {
    ...
    if (typeof p.id === "string" && p.id.startsWith("person:")) {
      ...
      addEdge({
        edge_id: `enterprise:apex-retail:has-executive:${p.id}`,
        from_node_id: "enterprise:apex-retail",
        to_node_id: p.id,
        edge_type: "HAS_EXECUTIVE",
        source_segment_id: record.segmentId,
        source_record_id: record.recordId,
        source_doc: record.sourceDoc,
        confidence: record.confidence,
      ...
      if (typeof p.reports_to === "string" && p.reports_to) {
        addEdge({
          edge_id: `${p.id}:reports-to:${p.reports_to}`,
          from_node_id: p.id,
          to_node_id: p.reports_to,
    ...
    if (typeof p.system_id === "string") {
      addNode({
        node_id: p.system_id,
        node_type: "system",
        label: String(p.name ?? record.title),
        source_segment_id: record.segmentId,
        source_record_id: record.recordId,
        source_doc: record.sourceDoc,
        data_classification: record.dataClassification,
      ...
      if (typeof p.vendor === "string" && p.vendor) {
        ...
        addNode({
          ...
          source_doc: record.sourceDoc,
          confidence: record.confidence,
          properties: { vendor_name: p.vendor },
        });
        addEdge({
          edge_id: `${p.system_id}:licensed-from:${vendorId}`,
          from_node_id: p.system_id,
          to_node_id: vendorId,
          edge_type: "LICENSED_FROM",
          source_segment_id: record.segmentId,
          source_record_id: record.recordId,
          source_doc: record.sourceDoc,
          confidence: record.confidence,
      ...
      if (typeof p.owner_id === "string" && p.owner_id) {
        addEdge({
          edge_id: `${p.system_id}:owned-by:${p.owner_id}`,
          from_node_id: p.system_id,
          to_node_id: p.owner_id,
          edge_type: "OWNED_BY",
          source_segment_id: record.segmentId,
          source_record_id: record.recordId,
          source_doc: record.sourceDoc,
    ...
    if (typeof p.kpi_id === "string") {
      ...
      if (typeof p.source_system === "string" && p.source_system) {
        addEdge({
          edge_id: `${p.kpi_id}:measured-from:${p.source_system}`,
          from_node_id: p.kpi_id,
          to_node_id: p.source_system,
          edge_type: "MEASURED_FROM",
          source_segment_id: record.segmentId,
          source_record_id: record.recordId,
          source_doc: record.sourceDoc,
          confidence: record.confidence,
    ...
    if (record.recordKind === "active_programs" && typeof p.id === "string") {
      ...
      for (const key of ["sponsor_id", "co_sponsor_id", "program_lead_id"]) {
        if (typeof p[key] === "string") {
          addEdge({
            edge_id: `program:${p.id}:${key}:${p[key]}`,
            from_node_id: `program:${p.id}`,
            to_node_id: p[key],
            edge_type: key === "program_lead_id" ? "LED_BY" : "SPONSORED_BY",
            source_segment_id: record.segmentId,
            source_record_id: record.recordId,
            source_doc: record.sourceDoc,
            confidence: record.confidence,
    ...
    if (typeof p.vendor_id === "string") {
      const vendorName =
        typeof p.vendor_name === "string" ? p.vendor_name : record.title;
      addNode({
        node_id: p.vendor_id,
        node_type: "vendor",
        label: vendorName,
      ...
      if (typeof p.relationship_owner === "string") {
        addEdge({
          edge_id: `${p.vendor_id}:relationship-owner:${p.relationship_owner}`,
          from_node_id: p.vendor_id,
          to_node_id: p.relationship_owner,
          edge_type: "RELATIONSHIP_OWNED_BY",
          source_segment_id: record.segmentId,
          source_record_id: record.recordId,
          source_doc: record.sourceDoc,
          confidence: record.confidence,
...
async function ensureClient(client: SupabaseClient) {
  const existing = await client
    ...
    .select("id")
    .or(
      "name.eq.Apex Retail,name.eq.Apex Retail Group,legal_name.eq.Apex Retail Group",
    )
    .limit(1)
    .maybeSingle();
  if (existing.error)
    throw new Error(`Apex client lookup failed: ${existing.error.message}`);
  ...
  const inserted = await client
    ...
    .insert({
      name: "Apex Retail",
      legal_name: "Apex Retail Group",
      industry_code: "retail",
    })
    .select("id")
    .single();
  if (inserted.error)
    throw new Error(`Apex client insert failed: ${inserted.error.message}`);
...
async function main() {
  ...
  const runStart = await client
    ...
    .insert({
      ...
      source_label: "Apex Retail synthetic setup dataset",
  ...
  if (runStart.error)
    throw new Error(
      `data_ingestion_runs insert failed: ${runStart.error.message}`,
    );
  ...
  try {
    ...
    await upsertBatch(
      ...
      SEGMENTS.map((segment) => {
        ...
        const staleCount = segmentRecords.filter(
          (record) => freshState(record.lastReviewed, segment) === "stale",
        ).length;
        const coverage = Math.min(
          100,
          Math.round(
            (segmentRecords.length / segment.expectedRecordCount) * 10000,
          ) / 100,
        );
        return {
          client_id: clientId,
    ...
    if (existingAudit.error)
      throw new Error(
        `data_inventory_audit_log lookup failed: ${existingAudit.error.message}`,
      );
    const auditedSegments = new Set(
      (existingAudit.data ?? []).map((row) => row.segment_id as string),
    );
    const auditRows = SEGMENTS.filter(
      (segment) => !auditedSegments.has(segment.segmentId),
    ).map((segment) => ({
      client_id: clientId,
      tenant_key: TENANT_KEY,
    ...
    if (auditRows.length) {
      ...
      if (auditInsert.error)
        throw new Error(
          `data_inventory_audit_log insert failed: ${auditInsert.error.message}`,
        );
    }
    ...
    const { error: runError } = await client
      ...
      .eq("id", runId);
    if (runError)
      throw new Error(`data_ingestion_runs update failed: ${runError.message}`);
