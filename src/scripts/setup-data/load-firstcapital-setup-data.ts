...
const UPLOADED_BY = 'First Capital synthetic dataset import · 2026-05-10';
...
const SEGMENTS: SegmentConfig[] = [
  ...
  { familyNumber: 2, segmentId: 'org_structure', segmentName: 'Org structure', dir: '02_org_structure', expectedRecordCount: 90, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 3, segmentId: 'it_landscape', segmentName: 'IT system landscape', dir: '03_it_landscape', expectedRecordCount: 75, expectedFreshnessDays: 90, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
  { familyNumber: 4, segmentId: 'it_financials', segmentName: 'IT financials', dir: '04_it_financials', expectedRecordCount: 75, expectedFreshnessDays: 30, reasoningModes: ['tenant_grounded'] },
  ...
  { familyNumber: 11, segmentId: 'vendor_contracts', segmentName: 'Vendor and contract', dir: '11_vendor_contracts', expectedRecordCount: 51, expectedFreshnessDays: 60, reasoningModes: ['tenant_grounded', 'cross_corpus'] },
...
function parseArgs() {
  ...
  return {
    sourceRoot: sourceIdx >= 0 ? args[sourceIdx + 1] : path.resolve(process.cwd(), 'src/scripts/setup-data/firstcapital-data'),
    dryRun,
  };
}
...
function buildGraph(records: RecordSeed[]): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  addNode({ node_id: 'enterprise:first-capital', node_type: 'enterprise', label: 'First Capital Financial', source_segment_id: 'enterprise_profile', source_record_id: 'enterprise_profile:enterprise-profile', source_doc: 'enterprise_profile.md', confidence: 0.9 });
  ...
  for (const record of records) {
    ...
    if (typeof p.system_id === 'string') {
      ...
      if (typeof p.vendor === 'string' && p.vendor) {
        const vendorId = `vendor:firstcapital:${slugify(p.vendor)}`;
...
async function ensureClient(client: SupabaseClient) {
  const existing = await client
    .from('clients')
    .select('id')
    .or('name.eq.First Capital,name.eq.First Capital Financial,legal_name.eq.First Capital Financial Group')
    .limit(1)
    .maybeSingle();
  ...
  const inserted = await client
    .from('clients')
    .insert({ name: 'First Capital', legal_name: 'First Capital Financial Group', industry_code: 'finserv' })
    .select('id')
    .single();
...
async function main() {
  ...
  try {
    ...
    const existingAudit = await client
      ...
      .eq('source_doc', 'first-capital-synthetic-dataset.zip');
    ...
    const auditRows = SEGMENTS
      ...
      .map((segment) => ({
        ...
        source_doc: 'first-capital-synthetic-dataset.zip',
      }));
    if (auditRows.length) {
