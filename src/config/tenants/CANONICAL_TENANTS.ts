...
export const DEFAULT_TENANT_COMPLIANCE_METADATA = {
  ...
  notes: 'No regulated healthcare compliance posture is configured for this tenant.',
...
export const MERIDIAN_PHS_COMPLIANCE_METADATA = {
  ...
  evidencePointer: 'docs/enterprise-context/synthetic/meridian/15-risk-compliance-register.csv',
} as const satisfies TenantComplianceMetadata;
...
export function getTenantComplianceMetadata(key: string): TenantComplianceMetadata {
  return getCanonicalTenant(key)?.compliance ?? DEFAULT_TENANT_COMPLIANCE_METADATA;
}
