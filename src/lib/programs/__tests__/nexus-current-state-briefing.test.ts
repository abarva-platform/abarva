...
const program: ProgramCore = {
  ...
  problemStatement: 'Repeat transfers and repeat contacts are rising across Apex Retail care queues.',
  targetOutcome: 'Reduce repeat-transfer rate by 25% and seven-day repeat contact by 15%.',
...
describe('buildNexusCurrentStateBriefing', () => {
  it('builds a cited CXO current-state brief across org, tech, financials, vendors, and guidance', async () => {
    ...
    expect(briefing.sections.find((section) => section.id === 'technology')?.facts[0]).not.toMatch(/^No /);
    expect(briefing.sections.find((section) => section.id === 'financials')?.facts[0]).not.toMatch(/^No /);
  ...
  it('answers CXO questions from the matching current-state section with citations', async () => {
    ...
    const orgAnswer = briefing.answerQuestion('Who owns the decision rights for this Move?');
    const techAnswer = briefing.answerQuestion('What systems constrain the design?');
    const financeAnswer = briefing.answerQuestion('What financial baseline should Carlos trust?');

    expect(orgAnswer.confidence).not.toBe('low');
    expect(orgAnswer.citationIds.length).toBeGreaterThan(0);
    expect(techAnswer.answer).toMatch(/system|technology|landscape|dependency/i);
    expect(financeAnswer.answer).toMatch(/financial|baseline|value|metric|KPI/i);
  });
