...
function valueRange(program: ProgramCore): string {
  if (program.valueProjectedLowUsd !== null && program.valueProjectedHighUsd !== null) {
...
function mergeSupplementalCorpusDomains(
  ...
  const needsKind = (kind: EnterpriseContextItemKind) => primary.items.every((item) => item.kind !== kind);
  ...
  const citationIds = new Set(primary.citations.map((citation) => citation.evidenceId));
  const supplementalCitations = supplement.citations.filter((citation) => !citationIds.has(citation.evidenceId));
...
function summarizeItems(
  ...
  const facts = items.slice(0, limit).map((item) => `${item.title}: ${item.summary}`);
...
function collectCitations(items: EnterpriseAgentContextItem[]): NexusBriefCitation[] {
...
function deriveGuidance(program: ProgramCore, bundle: EnterpriseAgentContextBundle): string[] {
  ...
  if ((program.currentPhase ?? 0) >= 3) {
    guidance.push('Before P4, Nexus should force an explicit design-to-value trace: root causes, target architecture, operating owner, KPI cadence, and finance assumptions.');
  }
  if (systems.length > 0) {
    guidance.push(`Treat ${systems[0].title} as a design dependency; do not approve architecture without confirming ownership, integration path, and operational constraints.`);
  } else {
    guidance.push('Technology landscape is thin in the broker bundle; ask for system owners and integration evidence before design sign-off.');
  }
  if (vendors.length > 0) {
    guidance.push(`Vendor posture matters: ${vendors[0].title} should be checked for renewal, commercial leverage, and Source handoff implications.`);
  }
  if (financials.length > 0) {
    guidance.push(`Financial shaping should anchor on ${financials[0].title}; validate baseline grain and value attribution before declaring the business case decision-grade.`);
  } else {
    guidance.push('Financial baseline evidence is missing or sparse; Nexus should not let the Move advance on value narrative alone.');
  }
  if (evidence.length === 0) {
    guidance.push('Evidence provenance is sparse; label recommendations as directional until source-backed artifacts are attached.');
  }
...
function answerFromSections(
  ...
  const picked =
    /\b(org|owner|sponsor|decision|who|structure|role|it leadership)\b/.test(q)
      ...
      : /\b(finance|financial|cost|value|baseline|kpi|metric|money|budget)\b/.test(q)
        ? sections.find((s) => s.id === 'financials')
        : /\b(system|systems|tech|technology|architecture|integration|platform|landscape)\b/.test(q)
          ? sections.find((s) => s.id === 'technology')
          : /\b(vendor|contract|renewal|source|sourcing|commercial)\b/.test(q)
            ? sections.find((s) => s.id === 'vendors')
            : /\b(evidence|source|citation|provenance|corpus|knowledge)\b/.test(q)
              ? sections.find((s) => s.id === 'evidence')
              : /\b(program|move|portfolio|dependency|collision|in flight)\b/.test(q)
                ? sections.find((s) => s.id === 'programs')
                : sections.find((s) => s.id === 'guidance');

  const active = picked ?? sections[sections.length - 1];
  const hasSpecificFacts = active.facts.length > 0 && !active.facts[0].startsWith('No ');
  return {
    question,
    ...
    confidence: hasSpecificFacts && active.citationIds.length > 0 ? 'high' : hasSpecificFacts ? 'medium' : 'low',
    missingContext: hasSpecificFacts ? [] : [`More ${active.title.toLowerCase()} evidence is needed.`],
  };
}
...
export async function buildNexusCurrentStateBriefing(
  ...
  const primaryBundle = input.bundle ?? await buildProgramsContextBundleAsync({
  ...
  const guidanceItems = [...systems.slice(0, 1), ...vendors.slice(0, 1), ...financials.slice(0, 1), ...evidence.slice(0, 1)];
  const phaseLabel = input.program.currentPhase !== null
  ...
  const sections: NexusCurrentStateBriefingSection[] = [
    section('org', 'Org Structure & Decision Rights', 'Who owns the Move, who must shape it, and which executives matter.', people, 'No org-structure records are available in the current bundle.', 5),
    section('programs', 'Program & Portfolio State', 'Active Moves, adjacent programs, and cross-program signals Nexus should sequence against.', programs, 'No adjacent program inventory is available in the current bundle.', 5),
    section('technology', 'Technology Landscape', 'Systems and architecture dependencies that constrain the Move design.', systems, 'No system-landscape records are available in the current bundle.', 5),
    section('vendors', 'Vendor & Commercial Posture', 'Contracts, renewal pressure, and Source handoff triggers that may shape the Move.', vendors, 'No vendor-contract records are available in the current bundle.', 5),
    section('financials', 'Financial Baselines & Value Levers', 'Metrics, baselines, and value assumptions Nexus can use to keep the business case honest.', financials, 'No financial baseline records are available in the current bundle.', 5),
    section('evidence', 'Knowledge Corpus & Provenance', 'Evidence records and corpus-backed facts Nexus can cite while advising the CXO.', evidence, 'No evidence-ledger records are available in the current bundle.', 6),
    {
      id: 'guidance',
  ...
  const executiveRead = [
    ...
    systems[0] ? `Primary technology dependency visible to Nexus: ${systems[0].title}.` : 'Technology dependency evidence is sparse.',
    financials[0] ? `Primary financial anchor: ${financials[0].title}.` : 'Financial baseline evidence is sparse.',
  ].join(' ');
  ...
  return {
    ...
    answerQuestion: (question: string) => answerFromSections(question, sections),
  };
}
