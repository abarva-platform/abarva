...
export interface DelCanvasItem {
  ...
  nextAction: string;
  /** Explicit AI-draft marker shown before a user can approve or export. */
  aiDraftLabel: string;
  /** Human-review requirement paired with the AI draft label. */
  editBeforeCommitRequirement: string;
...
export const MOVES_AI_DRAFT_LABEL = "AI Draft";
export const MOVES_EDIT_BEFORE_COMMIT_REQUIREMENT =
  "Edit before commit - human review required before approval, export, or phase advance.";
...
export function buildDeliverablesCanvasView(
  ...
  const rawItems: DelCanvasItem[] = deliverables.map((d) => {
    ...
    return {
      ...
      aiDraftLabel: MOVES_AI_DRAFT_LABEL,
      editBeforeCommitRequirement: MOVES_EDIT_BEFORE_COMMIT_REQUIREMENT,
  ...
  const honestDisclaimer =
    ...
    `state machine and export pipeline are deferred. ${MOVES_EDIT_BEFORE_COMMIT_REQUIREMENT}`;
