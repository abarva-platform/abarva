...
export interface UploadAttachmentResult {
  ...
  /** Discovery Intake (Tier B): the extraction receipt the upload route returns
   *  when `discovery_intake_v2` is on for the tenant. Null/absent otherwise. */
  discovery?: ExtractionReceipt | null;
}
...
export async function uploadAttachment(
  ...
  return new Promise<UploadAttachmentResult | UploadAttachmentError>(
    (resolve) => {
      ...
      xhr.onload = () => {
        ...
        if (status >= 200 && status < 300) {
          try {
            ...
            const attachment = (parsed as { attachment?: AttachmentRecord })
              .attachment;
            if (!attachment) {
              resolve({
                ok: false,
                code: "unknown",
                message:
                  "Upload succeeded but response was missing attachment data.",
                status,
              });
              return;
            }
            const discovery =
              (parsed as { discovery?: ExtractionReceipt | null }).discovery ??
              null;
            resolve({ ok: true, attachment, discovery });
          } catch {
            resolve({
              ok: false,
              code: "unknown",
        ...
        const friendly = (() => {
          ...
          if (code === "sensitive_data_quarantined")
            return (
              detail ||
              "Upload quarantined before storage/indexing because it appears to contain PHI/PII."
            );
