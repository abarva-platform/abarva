...
export function isTowerFactualSpineCandidate(question: string): boolean {
  ...
  return (
    ...
    /\b(weak\s+value\s+evidence|weakest\s+evidence|missing\s+value\s+evidence|missing\s+value\s+proof|evidence\s+gap|evidence\s+improves|hold\s+until\s+evidence|under[- ]?proven)\b/.test(q) ||
