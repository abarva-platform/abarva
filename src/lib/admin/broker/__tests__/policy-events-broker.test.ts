...
function row(overrides: Partial<FakeRow> = {}): FakeRow {
  return {
    ...
    actor_label:
      'actor_label' in overrides ? overrides.actor_label ?? null : 'Admin · CIO',
...
describe('getRecentPolicyEvents', () => {
  ...
  it('NEVER selects prior_policy / new_policy / actor_id (payload-safety)', async () => {
    ...
    expect(columns).toEqual(['id', 'tenant_id', 'actor_label', 'reason', 'created_at']);
  ...
  it('returns [] AND warns when the query throws', async () => {
    selectMock.mockRejectedValue(new Error('relation tenant_policy_audit blocked'));
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
