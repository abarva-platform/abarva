...
describe('getIsolationPosture', () => {
  ...
  it('flags deny / error / error_message / metadata-mismatch as anomalies', async () => {
    selectMock.mockResolvedValue([
      row({ id: 'a', policy_decision: 'deny', decision_reason: 'kernel-only mode' }),
      row({ id: 'b', policy_decision: 'error', decision_reason: 'provider failed', error_message: 'upstream 500' }),
      row({ id: 'c', policy_decision: 'allow', error_message: 'partial result' }),
      row({
        id: 'd',
    ...
    const anomalies = result.recentEvents.filter((e) => e.anomaly).map((e) => e.id);
  ...
  it('maps severity: error→high, deny→med, restricted+allow→med, redact_required→low', async () => {
    ...
    const result = await getIsolationPosture('apex-retail');
    const sevById = Object.fromEntries(result.recentEvents.map((e) => [e.id, e.severity]));
  ...
  it('returns estimated fallback when tenant resolution throws', async () => {
    ...
    expect(warnSpy).toHaveBeenCalled();
    const logged = warnSpy.mock.calls[0]?.[0];
    expect(logged as string).toMatch(/isolation_posture\.client_resolve_failed/);
    warnSpy.mockRestore();
  });
  ...
  it('does NOT request payload-fingerprint columns from ai_egress_audit', async () => {
    ...
    const banned = ['prompt_hash', 'response_hash', 'prompt_snapshot_ref', 'response_snapshot_ref'];
