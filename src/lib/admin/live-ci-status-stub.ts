...
// Documents what GitHub Actions and Azure deployment polling would look like
...
export interface LiveCiStatusStubReport {
   ...
   * "remain deferred", "GitHub Checks", and "Azure deployments" —
...
const AZURE_STUB: CiProviderStub = {
  provider: "azure",
  stubMode: "stub",
  // Documentation string only — not used in functional code:
  wouldPollEndpoint:
    "management.azure.com/subscriptions/{subscriptionId}/providers/Microsoft.App/containerApps",
  requiredEnvVar: "AZURE_STATUS_TOKEN",
  tokenConfigured: false,
  lastStubRefreshedAt: STUB_GENERATED_AT,
  note:
    "Azure Container Apps deployment polling is deferred to PROD7. A real integration would read " +
    "deployment state from Azure Resource Manager using AZURE_STATUS_TOKEN. " +
    "This stub returns a deterministic shape only — no live network call is made.",
};

const PROVIDERS: CiProviderStub[] = [GITHUB_ACTIONS_STUB, AZURE_STUB];
...
const UNIFIED_CONTROL_PLANE_NOTE =
  ...
  "Live CI polling — including GitHub Checks from GitHub Actions and Azure deployments from Azure Container Apps — " +
  "is not true live monitoring at this time. Real-time polling of GitHub Actions workflow runs and " +
  "Azure deployments will remain deferred until PROD7 wires the provider tokens and scheduled " +
