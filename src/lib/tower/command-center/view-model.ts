...
function toAiView(item: TowerMartAiPortfolioItem, n: number): TowerAiView {
  ...
  return {
    ...
    actualSpendYtdUsd: item.actualSpendYtdUsd ?? null,
    forecastSpendUsd: item.forecastSpendUsd ?? null,
    technologyOwnerRole: item.technologyOwnerRole ?? null,
    targetGoLiveDate: item.targetGoLiveDate ?? null,
    realizationStartMonth: item.realizationStartMonth ?? null,
    metricBaselineValue: item.metricBaselineValue ?? null,
    metricTargetValue: item.metricTargetValue ?? null,
    metricUnit: item.metricUnit ?? null,
    benefitRealizationLagMonths: item.benefitRealizationLagMonths ?? null,
