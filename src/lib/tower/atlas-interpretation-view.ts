...
import {
  type DeferredMetric,
  listDeferredMetrics,
} from "@/lib/tower/deferred-metrics";
import {
  type AtlasPatternId,
  selectAtlasPatterns,
...
export interface AtlasInterpretation extends AtlasObservationsView {
  ...
  accountabilityDisclosure: AtlasInterpretationDisclosure;
}
  ...
  return [
    {
      ...
      field: "ai_initiatives.display_id",
      value: initiative.displayId,
    },
    {
      initiativeId: initiative.initiativeId,
      field: "ai_initiatives.name",
      value: initiative.name,
    },
    {
      initiativeId: initiative.initiativeId,
      field: "ai_initiatives.status_flag",
      value: initiative.statusFlag,
    },
    {
      initiativeId: initiative.initiativeId,
      field: "ai_initiatives.confidence_level",
...
function byDisplayId(
  initiatives: ReadonlyArray<AIInitiative>,
): Map<string, AIInitiative> {
  return new Map(
    initiatives.map((initiative) => [initiative.displayId, initiative]),
  );
}
...
function weakestConfidence(
  ...
  if (initiatives.some((initiative) => initiative.confidenceLevel === "LOW"))
    return "LOW";
  if (initiatives.some((initiative) => initiative.confidenceLevel === "MED"))
    return "MED";
  return "HIGH";
}
...
function composeVendorClock(
  input: AtlasReasoningInput,
  number: number,
): AtlasInterpretiveObservation | null {
  ...
  const vendor = input.vendors
    .filter(
      (candidate) =>
        candidate.initiativeDisplayId === pressure.displayId &&
        candidate.renewalDate,
    )
    .sort(
      (a, b) =>
        daysUntil(a.renewalDate ?? "", input.todayIso) -
        daysUntil(b.renewalDate ?? "", input.todayIso),
    )[0];
  ...
  return {
    ...
    citations: [
      ...initiativeCitations(initiative),
      {
        ...
        field: "ai_initiative_vendors.vendor_name",
        value: vendor.vendorName,
      },
      {
        vendorId: vendor.vendorId,
        initiativeId: vendor.initiativeId,
        field: "ai_initiative_vendors.renewal_date",
        value: vendor.renewalDate,
      },
      {
        vendorId: vendor.vendorId,
        initiativeId: vendor.initiativeId,
        field: "ai_initiative_vendors.contract_value_usd",
        value: vendor.contractValueUsd ?? 0,
      },
...
function composeTopPressure(
  ...
  const thinEvidence =
    initiative.confidenceLevel === "LOW"
          ...
          : [
              {
                ...
                href: `/admin/ai-initiatives/${encodeURIComponent(initiative.displayId)}`,
              },
            ],
    confidenceFloor: initiative.confidenceLevel,
    citations: [
      ...initiativeCitations(initiative),
      {
        initiativeId: initiative.initiativeId,
        field: "ai_initiatives.status_summary",
        value: initiative.statusSummary,
      },
      {
        initiativeId: initiative.initiativeId,
        field: "ai_initiatives.committed_annual_usd",
        value: initiative.committedAnnualUsd ?? 0,
      },
      {
        initiativeId: initiative.initiativeId,
        field: "ai_initiatives.measured_value_usd",
        value: initiative.measuredValueUsd ?? 0,
      },
      {
        initiativeId: initiative.initiativeId,
        field: "tower_view.pressure_magnitude",
        value: pressure.magnitudeValue,
      },
      {
        initiativeId: initiative.initiativeId,
        field: "tower_view.pressure_next_action",
        value: pressure.nextAction,
      },
      ...(pressure.type === "dupl"
        ? [
            {
              field: "tower_view.attribution_study_weeks",
              value: 6,
            } satisfies AtlasCitation,
          ]
        : []),
    ],
  };
...
function composeSharedRoot(
  ...
  return {
    ...
    actions: [
      { label: "See programs lagging on value", href: "/programs?lens=value" },
    ],
...
function composeDefend(
  ...
  return {
    ...
    citations: head.flatMap((initiative) => [
      ...
      {
        initiativeId: initiative.initiativeId,
        field: "ai_initiatives.aligned_callout",
        value: String(initiative.alignedCallout),
      },
      {
        initiativeId: initiative.initiativeId,
        field: "ai_initiatives.measured_value_usd",
...
function composeLookAhead(
  ...
  if (bet) {
    const initiative = input.initiatives.find(
      (candidate) => candidate.displayId === bet.displayId,
    );
    if (!initiative) return null;
    return {
      number,
      ...
      citations: [
        ...
        {
          initiativeId: initiative.initiativeId,
          field: "ai_initiatives.stage",
          value: initiative.stage,
        },
        {
          initiativeId: initiative.initiativeId,
          field: "ai_initiatives.stage_detail",
          value: initiative.stageDetail ?? "multi-year strategic bet",
        },
        {
          initiativeId: initiative.initiativeId,
          field: "ai_initiatives.committed_total_usd",
          value:
  ...
  return {
    ...
    actions: [
      { label: "Review renewal calendar", href: "/source", pending: true },
    ],
    confidenceFloor: "MED",
    citations: [
      {
...
function composeHealthy(
  input: AtlasReasoningInput,
  number: number,
): AtlasInterpretiveObservation {
  const scaled = input.initiatives.filter(
    (initiative) => initiative.stage === "scaled",
  );
  const aligned = input.initiatives
...
function buildInterpretationDisclosure(params: {
  observationsCount: number;
  citationsCount: number;
  confidence: AtlasInterpretationConfidence;
  deferredMetricsCount: number;
  citationErrorsCount: number;
}): AtlasInterpretationDisclosure {
  const citationSummary =
    params.citationsCount === 0
      ? "No citations are available; treat this as insufficient support."
      : `${params.citationsCount} citation${params.citationsCount === 1 ? "" : "s"} across ${params.observationsCount} observation${params.observationsCount === 1 ? "" : "s"}.`;
  const confidenceDisclosure =
    params.citationErrorsCount > 0
      ? `Confidence is low because ${params.citationErrorsCount} citation validation issue${params.citationErrorsCount === 1 ? "" : "s"} were found.`
      : `Confidence is ${params.confidence}; it follows the weakest cited initiative confidence and citation validation result.`;
...
export function buildAtlasInterpretation(
  input: AtlasReasoningInput,
): AtlasInterpretation {
  if (input.initiatives.length === 0 && input.vendors.length === 0) {
    return {
      ...
      patternsFired: [],
      citationErrors: [],
      accountabilityDisclosure: buildInterpretationDisclosure({
        observationsCount: 0,
        citationsCount: 0,
        confidence: "low",
        deferredMetricsCount: listDeferredMetrics("tower-cfo").length,
        citationErrorsCount: 0,
      }),
    };
  }

  const deferredMetrics =
    input.deferredMetrics ?? listDeferredMetrics("tower-cfo");
  const selection = selectAtlasPatterns(input);
  const observations: AtlasInterpretiveObservation[] = [];
  const push = (observation: AtlasInterpretiveObservation | null) => {
    if (observation)
      observations.push({ ...observation, number: observations.length + 1 });
  };
  ...
  for (const pattern of selection.secondaryPatterns) {
    if (pattern === "pattern_02_shared_root")
      push(composeSharedRoot(input, observations.length + 1));
    if (pattern === "pattern_03_defend_while_resolving")
      push(composeDefend(input, observations.length + 1));
    if (pattern === "pattern_05_look_ahead")
      push(composeLookAhead(input, observations.length + 1));
  }

  const citations = observations.flatMap(
    (observation) => observation.citations,
  );
  ...
  const confidence: AtlasInterpretationConfidence =
    observations.length === 0 || citationErrors.length > 0
      ? "low"
      : observations.some(
            (observation) => observation.confidenceFloor === "LOW",
          )
        ? "med"
        : "high";
  const patternsFired = [
    selection.leadPattern,
    ...selection.secondaryPatterns.slice(
      0,
  ...
  return {
    ...
    accountabilityDisclosure: buildInterpretationDisclosure({
      ...
      confidence,
      deferredMetricsCount: deferredMetrics.length,
      citationErrorsCount: citationErrors.length,
    }),
  };
}
