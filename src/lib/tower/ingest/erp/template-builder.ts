...
const FINANCIAL_COLUMNS: ColumnSpec[] = [
  { key: 'program_id', label: 'program_id', required: true, type: 'string', description: 'Customer program identifier (Oracle Project ID, SAP WBS, internal code).' },
  { key: 'period_start', label: 'period_start', required: true, type: 'date', description: 'Fiscal period start. YYYY-MM-DD.' },
  { key: 'period_end', label: 'period_end', required: true, type: 'date', description: 'Fiscal period end. YYYY-MM-DD. Must be ≥ period_start.' },
  { key: 'budget_usd', label: 'budget_usd', required: false, type: 'number', description: 'Planned spend, USD. Non-negative.' },
  { key: 'actual_usd', label: 'actual_usd', required: false, type: 'number', description: 'Posted actual spend, USD. Non-negative.' },
  { key: 'capex_usd', label: 'capex_usd', required: false, type: 'number', description: 'Capital-expenditure share of actual. capex+opex ≤ actual.' },
  { key: 'opex_usd', label: 'opex_usd', required: false, type: 'number', description: 'Operating-expenditure share of actual. capex+opex ≤ actual.' },
  { key: 'vendor_id', label: 'vendor_id', required: false, type: 'string', description: 'FK to Vendor Spend sheet vendor_id if present.' },
  { key: 'cost_center', label: 'cost_center', required: false, type: 'string', description: 'Cost-center / profit-center code.' },
  { key: 'gl_account', label: 'gl_account', required: false, type: 'string', description: 'GL natural account code.' },
];

const VENDOR_COLUMNS: ColumnSpec[] = [
  { key: 'vendor_id', label: 'vendor_id', required: true, type: 'string', description: 'Stable vendor master ID from the source ERP.' },
  { key: 'vendor_name', label: 'vendor_name', required: true, type: 'string', description: 'Vendor display name.' },
  { key: 'cost_center', label: 'cost_center', required: false, type: 'string', description: 'Default cost center this vendor rolls to.' },
  { key: 'gl_account', label: 'gl_account', required: false, type: 'string', description: 'Default GL natural account.' },
  { key: 'ttm_spend_usd', label: 'ttm_spend_usd', required: false, type: 'number', description: 'Trailing-twelve-month vendor spend, USD. Non-negative.' },
];

function writeBanner(ws: ExcelJS.Worksheet, columnCount: number, banner: string) {
  ...
  c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: BANNER_FILL } };
...
function writeSchemaSheet(wb: ExcelJS.Workbook) {
  ...
  for (const [sheet, columns] of [
    ...
    for (const col of columns) {
      ws.getRow(rowIdx).values = [sheet, col.label, col.type, col.required ? 'yes' : 'no', col.description];
      rowIdx += 1;
    }
  }
}
...
export async function buildErpWorkbook(options: BuildErpWorkbookOptions = {}): Promise<Buffer> {
