...
const TOPICS: Record<string, TopicConfig> = {
  loaded_context: topic('loaded_context', 'enterprise_profile', ['evidence_sources'], ['V6_01_enterprise_profile.csv', 'V6_13_evidence_sources.csv'], ['Record', 'Industry/model', 'Priority/context'], ['company_name', 'industry', 'strategic_priorities']),
  business_areas: topic('business_areas', 'business_functions', ['org_ownership'], ['V6_02_business_functions.csv'], ['Business area', 'Executive owner', 'Critical processes'], ['function_name', 'executive_owner', 'critical_processes']),
  it_org: topic('it_org', 'org_ownership', ['business_functions'], ['V6_03_org_ownership.csv'], ['IT/domain area', 'Leader role', 'Decision rights'], ['org_unit_name', 'leader_role', 'decision_rights']),
  technology_leaders: topic('technology_leaders', 'org_ownership', ['business_functions'], ['V6_03_org_ownership.csv'], ['Technology domain', 'Leader role', 'Supported ownership'], ['org_unit_name', 'leader_role', 'owned_systems']),
  apps_systems: topic('apps_systems', 'applications_systems', ['relationships', 'data_assets_integrations'], ['V6_05_applications_systems.csv'], ['System', 'Owner', 'Criticality', 'Lifecycle'], ['system_name', 'system_owner', 'criticality', 'lifecycle_status']),
  data_estate: topic('data_estate', 'data_assets_integrations', ['applications_systems'], ['V6_06_data_assets_integrations.csv'], ['Data asset', 'Owner', 'Quality', 'Governance'], ['data_asset_name', 'data_owner', 'quality_score', 'governance_status']),
  integrations: topic('integrations', 'data_assets_integrations', ['relationships'], ['V6_06_data_assets_integrations.csv', 'V6_12_relationships.csv'], ['Object', 'System/relationship', 'Evidence/confidence'], ['data_asset_name', 'system_of_record', 'relationship_confidence']),
  vendors_contracts: topic('vendors_contracts', 'vendors_contracts', ['applications_systems', 'spend_value'], ['V6_07_vendors_contracts.csv'], ['Vendor', 'Service', 'Renewal', 'Contract risk'], ['vendor_name', 'service', 'renewal_date', 'contract_risk']),
  budget_spend: topic('budget_spend', 'spend_value', ['enterprise_profile'], ['V6_08_spend_value.csv', 'V6_01_enterprise_profile.csv'], ['Spend record', 'Amount', 'Type', 'Owner'], ['spend_id', 'amount_usd', 'amount_type', 'owner']),
  ai_footprint: topic('ai_footprint', 'ai_initiatives', ['programs_initiatives'], ['V6_10_ai_initiatives.csv'], ['Use case/tool', 'Users', 'Adoption', 'Readiness'], ['use_case', 'licensed_users', 'active_users', 'data_readiness']),
  ai_initiatives: topic('ai_initiatives', 'ai_initiatives', ['programs_initiatives'], ['V6_10_ai_initiatives.csv', 'V6_09_programs_initiatives.csv'], ['Initiative/use case', 'Measured value', 'Status', 'Decision'], ['use_case', 'measured_value_usd', 'production_status', 'decision_needed']),
  operations_service: topic('operations_service', 'operations_risk_controls', ['applications_systems'], ['V6_11_operations_risk_controls.csv'], ['Process/control', 'Severity/status', 'Affected systems', 'Impact'], ['process', 'severity', 'affected_systems', 'business_impact']),
  risk_controls: topic('risk_controls', 'operations_risk_controls', ['ai_initiatives'], ['V6_11_operations_risk_controls.csv', 'V6_10_ai_initiatives.csv'], ['Risk/control', 'Status', 'Systems/readiness', 'Boundary'], ['process', 'status', 'affected_systems', 'data_readiness']),
  governance_policy: topic('governance_policy', 'metric_definitions', ['operations_risk_controls'], ['V6_14_metric_definitions.csv', 'V6_10_ai_initiatives.csv'], ['Metric/governance item', 'Owner', 'Claim level', 'Forbidden claim'], ['metric_name', 'metric_owner', 'metric_claim_level', 'not_allowed_claims']),
  metrics_outcomes: topic('metrics_outcomes', 'metric_definitions', ['ai_initiatives'], ['V6_14_metric_definitions.csv', 'V6_10_ai_initiatives.csv'], ['Metric/outcome', 'Definition', 'Owner', 'Claim level'], ['metric_name', 'metric_definition', 'metric_owner', 'metric_claim_level']),
  relationships: topic('relationships', 'relationships', ['applications_systems', 'vendors_contracts'], ['V6_12_relationships.csv'], ['From', 'Relationship', 'To', 'Confidence'], ['from_object_family', 'relationship_type', 'to_object_family', 'relationship_confidence']),
  evidence_gaps: topic('evidence_gaps', 'evidence_gaps', ['all_v6_files'], allCoreFiles(), ['Area/file', 'Known gap', 'Count'], ['business_object_family', 'known_gaps', 'confidence']),
  data_thin: topic('data_thin', 'data_thin', ['all_v6_files'], allCoreFiles(), ['Area/file', 'Known gap', 'Count'], ['business_object_family', 'known_gaps', 'confidence']),
  readiness: topic('readiness', 'readiness', ['evidence_sources'], ['V6_01_enterprise_profile.csv', 'V6_13_evidence_sources.csv', 'V6_14_metric_definitions.csv'], ['Area', 'Readiness signal', 'Boundary'], ['business_object_family', 'confidence', 'not_allowed_claims']),
  source_trail: topic('source_trail', 'evidence_sources', ['relationships'], ['V6_13_evidence_sources.csv'], ['Evidence title', 'Type', 'Location', 'Confidence'], ['evidence_title', 'evidence_type', 'source_location', 'evidence_confidence']),
  board_summary: topic('board_summary', 'board_summary', ['enterprise_profile', 'ai_initiatives', 'industry_patterns'], ['V6_01_enterprise_profile.csv', 'V6_10_ai_initiatives.csv', 'V6_15_industry_corpus_patterns.csv'], ['Area', 'Supported signal', 'Boundary'], ['business_object_family', 'record_name', 'not_allowed_claims']),
  handoff_intelligence: topic('handoff_intelligence', 'ai_initiatives', ['industry_patterns', 'expert_lenses'], ['V6_10_ai_initiatives.csv', 'V6_15_industry_corpus_patterns.csv', 'V6_16_expert_lenses.csv'], ['Context', 'Pattern/lens', 'How Intelligence should use it'], ['use_case', 'pattern_name', 'recommended_actions'], 'intelligence', 'Intelligence owns advisory synthesis, options, and pattern-backed leadership recommendations.'),
  handoff_tower: topic('handoff_tower', 'ai_initiatives', ['spend_value', 'operations_risk_controls'], ['V6_08_spend_value.csv', 'V6_10_ai_initiatives.csv', 'V6_11_operations_risk_controls.csv'], ['Context', 'Status/readiness', 'Tower use'], ['use_case', 'scale_hold_stop', 'data_readiness'], 'tower', 'Tower owns portfolio execution, spend, adoption, readiness, and value tracking.'),
  sourcing_relevance: topic('sourcing_relevance', 'vendors_contracts', ['evidence_sources'], ['V6_07_vendors_contracts.csv', 'V6_13_evidence_sources.csv'], ['Vendor/evidence', 'Renewal/type', 'Sourcing relevance'], ['vendor_name', 'renewal_date', 'contract_risk'], 'source', 'Source owns sourcing events, vendor evidence, renewal decisions, and partner tradeoffs.'),
  move_relevance: topic('move_relevance', 'programs_initiatives', ['relationships', 'industry_patterns'], ['V6_09_programs_initiatives.csv', 'V6_12_relationships.csv', 'V6_15_industry_corpus_patterns.csv'], ['Program/pattern', 'Phase/relationship', 'Move relevance'], ['record_name', 'phase', 'relationship_type'], 'moves', 'Moves owns governed change framing, ownership, sequencing, and mobilization.'),
};
...
function topic(intent: string, primaryDimension: string, relatedDimensions: string[], files: string[], tableHeaders: string[], tableColumns: string[], handoffTarget?: string | null, handoffReason?: string): TopicConfig {
  return { intent, primaryDimension, relatedDimensions, files, tableHeaders, tableColumns, handoffTarget, handoffReason };
}
...
function loadV6Dataset(tenantKey: string): V6Dataset {
  ...
  if (!existsSync(manifestPath)) {
    throw new Error(`V6 dataset is not available for ${appClientKey}. Expected ${manifestPath}.`);
  }
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8')) as V6GeneratedManifest;
  ...
  for (const manifestFile of manifest.files.map((item) => item.file)) {
    ...
    const rows = existsSync(sourcePath) ? parseCsv(readFileSync(sourcePath, 'utf8')) : [];
    files[file] = rows;
...
function classifyQuestion(question: string): keyof typeof TOPICS {
  ...
  if (/application|core systems|systems context|apps/.test(q)) return 'apps_systems';
  if (/data|analytics estate/.test(q) && !/data-thin|thin/.test(q)) return 'data_estate';
...
function selectRows(dataset: V6Dataset, config: TopicConfig): Array<Record<string, string>> {
...
function hasUsableRow(row: Record<string, string>, config: TopicConfig): boolean {
  if (config.intent === 'data_thin' || config.intent === 'evidence_gaps') return Boolean(row.known_gaps);
  return config.tableColumns.some((column) => clean(row[column]));
}

function buildTable(rows: Array<Record<string, string>>, config: TopicConfig): V6HomeTable | null {
...
function buildCitations(dataset: V6Dataset, config: TopicConfig, rows: Array<Record<string, string>>): V6HomeCitation[] {
...
function buildGaps(rows: Array<Record<string, string>>, dataset: V6Dataset, config: TopicConfig): V6HomeGap[] {
  const gapCounts = new Map<string, number>();
  for (const row of rows) {
    for (const gap of (row.known_gaps ?? '').split('|')) {
      const value = gap.trim();
      if (value.startsWith('data_thin:')) gapCounts.set(value.replace('data_thin:', ''), (gapCounts.get(value.replace('data_thin:', '')) ?? 0) + 1);
    }
  }
  if (config.intent === 'budget_spend') {
    const amountThin = rows.some((row) => String(row.amount_usd ?? '').startsWith('data_thin:'));
    if (amountThin) gapCounts.set('amount_usd_missing', Math.max(gapCounts.get('amount_usd_missing') ?? 0, 1));
  }
  return [...gapCounts.entries()]
    ...
    .map(([label, count]) => ({
      ...
      remediation: `Load client-approved ${humanize(label).toLowerCase()} evidence into ${dataset.datasetDir}.`,
    }));
}
...
function buildAnswer(args: {
  ...
  const paragraphs = answerParagraphsByTopic({ displayName, dataset, config, topicKey, rows, gaps });
  return {
    mode: 'home_know',
    answerSource: 'v6_dataset_contract',
    ...
    answerBoundary: {
      canAnswer: [
        'Answer from the tenant V6 contract pack only.',
        'Show loaded facts, explicit gaps, evidence families, readiness, and handoff boundaries.',
      ],
      cannotAnswer: [
        'Borrow facts from retired Semantic2/Home KNOW layers.',
        'Use dollar amounts, leaders, systems, vendors, or statuses that are not present in the selected V6 context.',
      ],
...
function answerParagraphsByTopic(args: {
  ...
  const countLine = `${displayName} has ${rows.length} usable ${humanize(config.primaryDimension).toLowerCase()} record${rows.length === 1 ? '' : 's'} in the V6 Home contract pack.`;
  if (topicKey === 'budget_spend') {
    ...
    return [
      usableAmounts.length
        ? `${countLine} Spend records support the listed amount fields below; each amount must stay tied to amount type, owner, and evidence basis. Home should not reuse older budget figures that are not present in the selected V6 context.`
        : `${countLine} The selected spend records mark amount, owner, program, vendor, system, value linkage, and unit economics as data-thin. Home should not reuse older budget figures or invent a spend breakout.`,
      strategicBudget
        ? `The profile-level budget context says ${formatStrategicPriorityList(strategicBudget)}. Treat that as context, not a Tower-grade budget packet.`
  ...
  if (topicKey === 'apps_systems') {
    const named = rows.map((row) => clean(row.system_name)).filter((value) => value && !/application_system record/i.test(value)).slice(0, 6);
    return [
      named.length
        ? `${countLine} Named systems include ${joinList(named)}. Use the table for owner, criticality, and lifecycle status.`
        : `${countLine} The application context area is present, but many records have generic system labels or data-thin ownership fields. Home can describe coverage and caveats, not a polished named application inventory.`,
  ...
  if (topicKey === 'vendors_contracts' || topicKey === 'sourcing_relevance') {
    const vendors = rows.slice(0, 8).map((row) => clean(row.vendor_name)).filter(Boolean);
    return [
  ...
  if (topicKey === 'operations_service') {
    const processes = rows.slice(0, 6).map((row) => clean(row.process) || clean(row.record_name)).filter(Boolean);
    return [
  ...
  if (topicKey === 'technology_leaders' || topicKey === 'it_org') {
    const roles = rows.slice(0, 8).map((row) => `${display(row.org_unit_name)} (${display(row.leader_role)})`);
    return [
  ...
  if (topicKey === 'ai_footprint') {
    const uses = rows.slice(0, 8).map((row) => {
      ...
      return active && licensed ? `${useCase} (${active}/${licensed} active/licensed)` : useCase;
    }).filter(Boolean);
    return [
  ...
  if (topicKey === 'loaded_context') {
    return [
      `${displayName} is using the V6 Home contract pack ${dataset.datasetDir}, with ${dataset.manifest.totals.files} files and ${dataset.manifest.totals.rows} business records.`,
      'Home can safely answer what is loaded, which facts are populated, which fields are data-thin, and which workspace should take over. It cannot borrow from retired Semantic2/Home KNOW layers.',
      gapSentence(gaps),
    ];
  }
  return [
...
function summarizeRows(rows: Array<Record<string, string>>, config: TopicConfig): string {
  ...
  return samples.length
    ...
    : 'The selected V6 evidence areas are present, but the usable fields for this question are data-thin.';
}
...
function branchOptionsFor(config: TopicConfig): V6HomeAnswer['branchOptions'] {
  return config.relatedDimensions.slice(0, 4).map((dimension) => ({
    ...
    summary: `Inspect the related ${humanize(dimension).toLowerCase()} facts and gaps in the V6 contract pack.`,
...
function followUpFor(topicKey: string): string {
  ...
  return 'Which V6 evidence area or evidence gap should aVa inspect next?';
}

function visibleQualityGate(answer: V6HomeAnswer): V6HomeAskResult['proof']['qualityGate'] {
  const text = answer.directAnswer;
  const issues: string[] = [];
  if (/semantic2|enterprise_context_|source rows|raw facts|datasets\/|\.csv\b|\bV6_\d+_/i.test(text)) issues.push('raw_internal_marker');
  if (/as discussed|this session|earlier in this session/i.test(text)) issues.push('session_dependent_answer');
...
function formatStrategicPriorityList(value: string): string {
  const entries = value
    .split('|')
    .map((part) => part.trim())
    .filter(Boolean)
    .map((part) => {
      ...
      if (!raw) return humanize(part);
      return `${humanize(key)} is ${formatBusinessValue(raw.trim())}`;
    });
  return joinList(entries);
}

function formatBusinessValue(value: string): string {
  if (/^\d+(\.\d+)?$/.test(value)) {
    const amount = Number(value);
    if (amount >= 1_000_000_000) return `$${(amount / 1_000_000_000).toFixed(2).replace(/0$/, '').replace(/\.0$/, '')}B`;
...
function countFacts(rows: Array<Record<string, string>>): number {
  return rows.reduce((sum, row) => sum + Object.values(row).filter((value) => Boolean(clean(value))).length, 0);
}

function gapSentence(gaps: V6HomeGap[]): string {
  if (!gaps.length) return 'No specific V6 gap is surfaced for this question, but final decisions still require client validation of source ownership and freshness.';
...
function humanize(value: string): string {
  return value.replace(/^V6_\d+_/, '').replace(/\.csv$/, '').replace(/_/g, ' ').replace(/\b\w/g, (match) => match.toUpperCase());
}
