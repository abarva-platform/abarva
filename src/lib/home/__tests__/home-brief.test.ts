...
describe('buildHomeBrief', () => {
  it('derives real money KPIs from initiatives — never fabricated', () => {
    const brief = buildHomeBrief({
      ...
      firstName: 'Carlos',
      initiatives: [
        initiative({ committedTotalUsd: 240_000_000, measuredValueUsd: 40_000_000 }),
        initiative({ initiativeId: 'i2', name: 'CDP', committedTotalUsd: 120_000_000, measuredValueUsd: 0, statusFlag: 'value_lag' }),
      ],
      approvals: [],
    });
  ...
  it('surfaces a pending approval as the single decision, routing into the workspace', () => {
    const brief = buildHomeBrief({
      ...
      approvals: [approval({ programId: 'p-store-labor', briefSnapshot: { title: 'Store Labor AI pilot' } })],
    });
