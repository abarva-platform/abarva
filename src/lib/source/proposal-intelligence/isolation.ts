...
export function checkVendorIsolation(
  ...
  const violations = objects
    ...
    .map((o) => o.ref);
  return { status: violations.length === 0 ? 'isolated' : 'violation_detected', violations };
}
...
export function isolateBundleForVendor<T extends VendorTaggedObject>(
  ...
  const bundle = objects.filter((o) => o.vendorName === null || o.vendorName === targetVendor);
...
export function buildProposalContextTrace(inp: ProposalTraceInput): SourceProposalContextBundleTrace {
  const isolation = checkVendorIsolation(inp.vendorName, inp.bundleObjects);
  const citationSet = new Set(inp.citations);
  const unsupported = inp.claims.filter((c) => !c.citation || !citationSet.has(c.citation));
  const hash = createHash('sha256')
    .update([inp.vendorName, inp.proposalVersion, ...inp.evidenceUsed, ...inp.pricingInputsUsed].join('|'))
    .digest('hex')
    .slice(0, 16);
  return {
    ...
    excluded_objects_by_reason: {
      ...
      ...(isolation.violations.length > 0 ? { vendor_isolation_excluded: isolation.violations.length } : {}),
    },
