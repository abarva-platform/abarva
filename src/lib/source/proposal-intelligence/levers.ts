...
const text = (rows: ProposalNormalizationRow[], cat: ProposalNormalizationRow['normalizedCategory']) =>
...
const RULES: Rule[] = [
  {
    ...
    matches: (i) => {
      ...
      const bundled = pricing.find((r) => r.deviations.some((d) => /bundl|not broken|no breakout|aggregate/i.test(d)) || r.normalizedAnswer === null);
      return bundled
        ? { issue: `Pricing not provided at tower/line level (${bundled.deviations[0] ?? 'no comparable breakout'}).`, evidence: bundled.evidenceReference ? [bundled.evidenceReference] : [] }
        : null;
    },
  ...
  },
  {
    ...
    matches: (i) => {
      ...
      const cola = com.find((r) => /cola|indexation|cpi|escalat/i.test(`${r.vendorResponseSummary} ${r.assumptions.join(' ')}`));
      return cola && !/cap|capped|fixed/i.test(cola.vendorResponseSummary)
        ? { issue: 'COLA/indexation proposed without a cap.', evidence: cola.evidenceReference ? [cola.evidenceReference] : [] }
        : null;
    },
  ...
  },
  {
    ...
    matches: (i) => {
      ...
      const vague = auto.find((r) => r.deviations.some((d) => /vague|uncommitted|marketing|not committed/i.test(d)) || (r.completeness !== 'complete' && r.vendorResponseSummary.length > 0));
      return vague
        ? { issue: 'Productivity/automation claims are not contractually committed.', evidence: vague.evidenceReference ? [vague.evidenceReference] : [] }
        : null;
    },
  ...
  },
  {
    ...
    matches: (i) => {
      ...
      const weak = sla.find((r) => r.deviations.some((d) => /credit|earn[- ]?back|teeth/i.test(d)));
      return weak ? { issue: 'SLA regime lacks meaningful credits / has easy earn-back.', evidence: weak.evidenceReference ? [weak.evidenceReference] : [] } : null;
    },
  ...
  },
  {
    ...
    matches: (i) => {
      ...
      const onshore = staff.find((r) => r.deviations.some((d) => /onshore|high[- ]cost|location mix/i.test(d)));
      return onshore ? { issue: 'Higher-cost onshore mix than the baseline requires.', evidence: onshore.evidenceReference ? [onshore.evidenceReference] : [] } : null;
    },
  ...
  },
  {
    ...
    matches: (i) => {
      ...
      const fee = t.find((r) => /transition (fee|cost)/i.test(r.vendorResponseSummary) && !/at risk|holdback|milestone/i.test(r.vendorResponseSummary));
      return fee ? { issue: 'Transition fee not at risk / not milestone-gated.', evidence: fee.evidenceReference ? [fee.evidenceReference] : [] } : null;
    },
  ...
  },
  {
    ...
    matches: (i) => {
      const p = text(i.rows, 'pricing_structure').concat(text(i.rows, 'tooling_approach'));
      const dup = p.find((r) => r.deviations.some((d) => /pass[- ]?through|tooling cost|duplicate/i.test(d)));
      return dup ? { issue: 'Opaque or duplicated tooling/pass-through costs.', evidence: dup.evidenceReference ? [dup.evidenceReference] : [] } : null;
    },
  ...
  },
  {
    ...
    matches: (i) => {
      const f = i.findings.find((x) => x.dimension === 'subcontractors' && x.severity !== 'info');
      return f ? { issue: f.finding, evidence: f.evidenceReference ? [f.evidenceReference] : [] } : null;
    },
...
export function buildVendorLevers(input: LeverInput): NegotiationLever[] {
  ...
  for (const rule of RULES) {
    ...
    if (!hit) continue;
    n += 1;
    const evidenced = Boolean(rule.valueShare) && input.annualRunRateUsd !== null && hit.evidence.length > 0;
    levers.push({
      ...
      expectedValueLowUsd: evidenced ? Math.round(input.annualRunRateUsd! * rule.valueShare![0]) : null,
      expectedValueHighUsd: evidenced ? Math.round(input.annualRunRateUsd! * rule.valueShare![1]) : null,
...
export function topLevers(all: NegotiationLever[], n = 10): NegotiationLever[] {
  ...
  return [...all]
    .sort((a, b) => {
      if (prio[a.bafoPriority] !== prio[b.bafoPriority]) return prio[a.bafoPriority] - prio[b.bafoPriority];
