import {
  getSourcePricingNormalizationSeed,
  getSourceVendorResponseSeed,
} from "./mock-seed";
import { getSourceCorpusCommercialTraps } from "./source-corpus-uplift";
import type {
...
function toContext(
  ...
  const responseSeed = getSourceVendorResponseSeed(
    input.event.id,
  ).responses.find((response) => response.vendorId === vendor.vendorId);
  if (!responseSeed) {
    return {
      ...
      pricingTemplateStatus:
        vendor.completenessStatus === "complete" ? "complete" : "missing",
      transitionPlanStatus:
        vendor.responseStatus === "not_started" ? "missing" : "incomplete",
      securityResponseStatus: "missing",
      automationRoadmapStatus:
        vendor.responseStatus === "submitted" ? "complete" : "missing",
      responseStatus: vendor.responseStatus ?? "not_started",
      comparabilityStatus:
        vendor.completenessStatus === "complete"
          ? "comparable"
          : "not_comparable",
    };
  }

  const requiredSections = responseSeed.requiredSections.map((section) =>
    section.toLowerCase(),
  );
  const submittedSections = responseSeed.submittedSections.map((section) =>
    section.toLowerCase(),
  );
  const missingRequired = requiredSections.filter(
    (section) => !submittedSections.includes(section),
  );
  const hasCriticalMissing = missingRequired.some((section) =>
    REQUIRED_FOR_COMPARABLE.has(section),
  );
  ...
  return {
    ...
    comparabilityStatus:
      hasCriticalMissing || responseSeed.pricingTemplateStatus === "missing"
        ? "not_comparable"
        : responseSeed.responseStatus === "blocked" ||
            responseSeed.responseStatus === "rejected"
          ? "blocked"
          : responseSeed.evidenceUsability === "low_confidence" ||
              responseSeed.evidenceUsability === "restricted"
...
function pickCosts(
  ...
  const complianceHoldback = vendor.securityComplianceNotes.some(
    (note) =>
...
function buildCommercialTraps(
  ...
  if (
    context.pricingTemplateStatus === "missing" ||
    context.pricingTemplateStatus === "incomplete"
  ) {
    addTrap(traps, vendor, {
      category: "Pricing template",
      signal: "Pricing template is missing or incomplete.",
      impact:
        "Without comparable line items, deterministic normalization is blocked.",
      recommendation:
        "Request complete pricing workbook before shortlist comparison.",
      severity: "high",
    });
  }

  if (
    context.transitionPlanStatus === "missing" ||
    context.transitionPlanStatus === "incomplete"
  ) {
    addTrap(traps, vendor, {
      ...
      recommendation:
        "Request transition inclusions and runbook handoff commitments.",
      severity: "high",
    });
  }

  if (
    context.securityResponseStatus === "missing" ||
    context.securityResponseStatus === "incomplete"
  ) {
    addTrap(traps, vendor, {
      category: "Security/compliance",
      signal: "Security/compliance response is incomplete.",
      impact:
        "Commercial comparability is reduced by incomplete risk coverage.",
      recommendation:
        "Complete security response and map obligations to scope.",
      severity: "medium",
    });
  }
  ...
  if (vendor.automationProductivityAssumptionPercent > 0) {
    if (!optionals.includes("automation")) {
      addTrap(traps, vendor, {
        ...
        signal:
          "Automation assumption is not mapped to explicit commercial scope.",
        impact: "Savings claims may not be enforceable.",
        recommendation:
          "Attach explicit automation milestones and commitments.",
        severity: "low",
      });
    } else if (
      vendor.automationProductivityAssumptionPercent >= 20 &&
      context.automationRoadmapStatus !== "complete"
    ) {
      addTrap(traps, vendor, {
        category: "Automation trap",
        ...
        recommendation:
          "Request measurable automation commitments before final score.",
        severity: "medium",
      });
    }
  }
  ...
  if (
    ...
    addTrap(traps, vendor, {
      ...
      impact:
        "Transition and support risk should be validated for critical services.",
      recommendation:
        "Request staged transition and retained knowledge transfer details.",
      severity: "low",
    });
  }
  ...
  if (
    exclusions.includes("release") ||
    exclusions.includes("minor enhancement") ||
    exclusions.includes("tooling")
  ) {
    addTrap(traps, vendor, {
      category: "Exclusions",
      ...
      recommendation:
        "Require explicit inclusion or priced exclusion list in evaluation package.",
      severity: "medium",
    });
  }

  if (
    context.evidenceUsability === "low_confidence" ||
    context.evidenceUsability === "restricted"
  ) {
    addTrap(traps, vendor, {
      category: "Evidence",
      signal: `Evidence usability is ${context.evidenceUsability.replace("_", " ")}.`,
      impact:
        "Commercial claims are provisional until stronger evidence is available.",
      recommendation:
        "Request corroborating references and usage evidence before award.",
      severity: "medium",
    });
  }

  if (
    context.responseStatus === "blocked" ||
    ...
    addTrap(traps, vendor, {
      ...
      recommendation:
        "Resolve response blockers in compliance with response SLA.",
      severity: "high",
    });
  }

  traps.push(
    ...getSourceCorpusCommercialTraps({
      id: event?.id,
      name: event?.name,
      vendor,
    }),
...
function buildRatios(
  costs: SourcePricingSourceDataCost,
  ...
  const costPerTicket =
    annualTickets > 0 ? transitionInclusiveYearOneUsd / annualTickets : 0;

  return {
    transitionInclusiveYearOneUsd,
    costPerApplicationUsd:
      assumptions.applicationCount > 0
        ? transitionInclusiveYearOneUsd / assumptions.applicationCount
        : 0,
    costPerTicketUsd: costPerTicket,
    costPerTicketSavingsAtRiskUsd:
      costPerTicket * (assumptions.supportHoursPerWeek / 100),
  };
}
...
function buildYearlyCosts(costs: SourcePricingSourceDataCost): {
  ...
  return {
    ...
    year3: clampMoney(
      year1 * rateMultiplier * rateMultiplier +
        costs.changeOrderExposureUsd * 2,
    ),
  };
}

function resolveRowReadiness(
  context: SourcePricingVendorContext,
  traps: SourceCommercialTrap[],
): {
  ...
  if (
    context.comparabilityStatus === "partially_comparable" ||
    traps.some((trap) => trap.severity === "high")
  ) {
    return {
      readinessStatus: "risk_adjusted",
      comparabilityStatus: traps.some((trap) => trap.severity === "high")
        ? "partially_comparable"
        : "partially_comparable",
    };
  }
...
function buildSnapshot(
  ...
  const traps = buildCommercialTraps(vendor, context, event);
  const { readinessStatus, comparabilityStatus } = resolveRowReadiness(
    context,
    traps,
  );
  ...
  const stewardGateNotes = traps.some(
    (trap) => trap.category === "Response state" || trap.severity === "high",
  )
    ? [
...
function scoreSnapshot(snapshot: SourcePricingVendorSnapshot): number {
  ...
  return clampMoney(
    snapshot.costByYear.year1 + comparabilityPenalty + trapPenalty,
  );
}

function rankSnapshots(
...
function buildSummary(
  ...
  const comparable = snapshots.filter(
    (snapshot) => snapshot.comparabilityStatus === "comparable",
  ).length;
  const partiallyComparable = snapshots.filter(
    (snapshot) => snapshot.comparabilityStatus === "partially_comparable",
  ).length;
  const notComparable = snapshots.filter(
    (snapshot) => snapshot.comparabilityStatus === "not_comparable",
  ).length;
  const blocked = snapshots.filter(
    (snapshot) => snapshot.comparabilityStatus === "blocked",
  ).length;
  const comparableWithoutExclusions = snapshots.filter(
    (snapshot) =>
      snapshot.comparabilityStatus === "comparable" &&
...
function computeReadinessScore(
  ...
  return Math.round(
    ((summary.comparable +
      summary.partiallyComparable * 0.5 +
      summary.comparableWithoutExclusions * 0.25) /
      denominator) *
      100,
  );
}
...
function summarizeMissingInputs(
  ...
  return input.event.dataReadiness
    .filter(
      (row) =>
        row.readinessState === "Missing" ||
        row.evidenceUsability === "not_available",
    )
...
function buildNarrative(
  ...
  if (status === "not_comparable") {
    return (
      `${summary.notComparable} vendor row(s) are not comparable.` +
...
function pickRequiredInputs(input: SourcePricingNormalizationInput): string[] {
  ...
  return required.filter(
    (item) => !missing.some((entry) => entry.startsWith(item)),
  );
}
...
export function getSourcePricingNormalizationNextActions(
  ...
  return [input.recommendedNextAction, ...input.blockers].filter(Boolean);
}
...
export function summarizeSourcePricingNormalization(
  input: SourcePricingNormalization,
): string {
  return (
    `Pricing normalization: ${input.summary.totalVendors} vendors, ${input.summary.comparable} comparable,` +
    ` ${input.summary.partiallyComparable} risk-adjusted, ${input.summary.notComparable} not comparable.`
  );
}

export function normalizeVendorPricing(
  vendor: SourcePricingVendorInput,
): SourcePricingVendorSnapshot {
...
export function buildSourcePricingNormalization(
  ...
  const snapshots = pricingInputs.map((vendor) =>
    buildSnapshot(vendor, toContext(input, vendor), input.event),
  );
  ...
  const uniqueTraps = new Map(
    allTraps.map((trap) => [
      `${trap.vendorId}:${trap.category}:${trap.signal}`,
      trap,
    ]),
  );
  ...
  output.sentinelEvidenceNotes = output.traps
    .filter((trap) => trap.category === "Evidence")
    .map((trap) => `${trap.vendorName}: ${trap.signal}`);
  output.stewardGateNotes =
    output.summary.notComparable > 0 || output.summary.blocked > 0
      ? [
  ...
  output.atlasExecutiveImplication =
    `${output.summary.comparable} comparable row(s),` +
...
export function formatSourcePricingNormalizationAsMarkdown(
  ...
  for (const row of input.comparison) {
    const snapshot = input.snapshots.find(
      (entry) => entry.vendorId === row.vendorId,
    );
