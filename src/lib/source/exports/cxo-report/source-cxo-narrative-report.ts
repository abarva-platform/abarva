...
import type { DealPackArtifact, DealPackInput } from '../deal-pack/stage-sections';
import {
...
export function buildSourceCxoNarrativeReport(input: DealPackInput): SourceCxoNarrativeReport {
  ...
  const slides: SourceCxoSlide[] = [
    {
      ...
      message: 'CXO narrative report for the Source event. The deck tells the executive story; the Deal Pack remains the audit appendix.',
      ...
      metrics: [
        ...
        { label: 'Estimated value', value: valueLabel, note: 'From source event substrate' },
        { label: 'Generated', value: input.generatedAt.slice(0, 10), note: 'Deterministic event report' },
      ],
      notes: ['Use this report for CXO readout; use the Source Deal Pack for detailed audit and artifact review.'],
    },
    {
      id: 'answer',
      kind: 'answer',
      ...
      metrics: [
        { label: 'Verdict', value: decision.verdict, note: decision.confidence, status: decision.status },
        { label: 'Financial impact', value: valueLabel, note: 'Do not treat as approved value unless evidence supports it' },
        { label: 'Current stage', value: stageLabel, note: 'Where the event sits today' },
        { label: 'Open gaps', value: String(missingCount + scaffoldCount), note: `${missingCount} missing · ${scaffoldCount} scaffold`, status: missingCount ? 'warn' : 'neutral' },
      ],
      table: {
        title: 'What the executive is being asked to decide',
        columns: ['Decision area', 'Answer', 'Why it matters'],
        rows: [
          ['Recommended action', decision.answer, 'Keeps the event from becoming activity without a decision.'],
          ['What would change the answer', decision.changeTrigger, 'Forces the expert challenge to be explicit.'],
          ['Next gate', decision.nextStep, 'Turns the deck into operating action.'],
        ],
      },
    ...
    },
    {
      ...
      metrics: [
        { label: 'Artifacts authored', value: `${authoredCount}/${coverage.length}`, note: 'Artifact coverage across Source lifecycle' },
        { label: 'Evidence rows', value: String(evidenceCount), note: 'Rows available in the evidence ledger' },
        { label: 'Gate decisions', value: String(gateCount), note: 'Recorded stage-gate decisions' },
        { label: 'Owner', value: input.eventOwner ?? 'Not recorded — seed gap', note: 'Accountable sponsor / event owner', status: input.eventOwner ? 'neutral' : 'warn' },
      ],
      table: coverageTable(coverage),
      notes: ['A CXO report should not hide lifecycle gaps; incomplete stages are named rather than inferred.'],
    },
    {
      id: 'path',
      kind: 'path',
      title: 'Source path and operating shape',
      message: 'The sourcing path is credible only if demand, approach, market, scope, pricing, evaluation, risk and renewal artifacts line up.',
      ...
      notes: ['This slide is intentionally stage-based: it checks process completeness before recommending commercial action.'],
    },
    {
      id: 'economics',
      kind: 'economics',
      ...
      notes: ['The report never converts missing vendor submissions into claimed savings.'],
    },
    {
      id: 'vendor-field',
      kind: 'vendor-field',
      ...
      notes: ['A CXO should see which options are credible and why the losing paths lose.'],
    },
    {
      id: 'commercial-risk',
    ...
    },
    {
      ...
      notes: ['If the event is not a renewal, this slide still names that the SRM/renewal substrate is not the decision driver.'],
    },
    {
      id: 'evidence',
      kind: 'evidence',
      ...
      metrics: [
        { label: 'Evidence rows', value: String(evidenceCount), note: 'From event evidence ledger' },
        { label: 'Gate decisions', value: String(gateCount), note: 'From stage-gate records' },
        { label: 'Missing artifacts', value: String(missingCount), note: 'Shown as gaps, not inferred', status: missingCount ? 'warn' : 'good' },
        { label: 'Scaffolds', value: String(scaffoldCount), note: 'Generated but not authored/sign-off' },
      ],
      table: evidenceTable(input),
      notes: ['This is the governance slide: it tells the CXO where the deck is strong and where it is provisional.'],
    },
    {
      id: 'asks',
      kind: 'asks',
      ...
      metrics: [
        { label: 'Primary ask', value: decision.verdict, note: decision.answer },
        { label: 'Owner', value: input.eventOwner ?? 'Not recorded — seed gap', note: 'Needs explicit accountability', status: input.eventOwner ? 'neutral' : 'warn' },
        { label: 'Next gate', value: stageLabel, note: 'Move only when evidence supports the gate' },
        { label: 'Appendix', value: 'Source Deal Pack', note: 'Detailed artifact audit remains available' },
      ],
      table: {
        title: 'CXO action list',
        columns: ['Action', 'Owner', 'Condition'],
        rows: [
          [decision.nextStep, input.eventOwner ?? 'Not recorded — seed gap', decision.changeTrigger],
          ['Review detailed Source Deal Pack', 'Sourcing lead', 'Required before external vendor commitment.'],
          ['Close missing evidence / artifact gaps', 'Event owner', `${missingCount + scaffoldCount} open item(s).`],
        ],
      },
...
function artifactKindForCode(code: string): SourceArtifactCoverage['artifactKind'] {
...
function findArtifact(input: DealPackInput, code: string): DealPackArtifact | null {
  return input.stages.flatMap((s) => s.artifacts).find((a) => a.code === code) ?? null;
}

function findStructured(input: DealPackInput, kind: string): DealPackArtifact | null {
  return (
    input.stages
      .flatMap((s) => s.artifacts)
      .find((a) => a.kind === 'structured' && a.structured?.kind === kind) ?? null
  );
}
...
function synthesizeDecision(input: DealPackInput, judgment: SourceJudgment): {
  ...
  if (selection) {
    return {
      ...
      changeTrigger: 'Material legal, pricing or transition exception before signature.',
      status: 'good',
    };
  }
  if (renewal) {
    return {
      ...
      answer: firstHeading(renewal, 'Renewal posture is authored; execute before the notice window closes.'),
      detail: 'Renewal decision is authored and should be converted into negotiation / SRM action.',
      confidence: 'Medium-high — renewal artifact authored',
      nextStep: 'Enact the renewal posture and track SRM/Tower follow-up.',
      changeTrigger: 'Vendor terms, usage, benchmark or alternative changes before signature.',
      status: 'warn',
    };
  }
  if (decision) {
    return {
      ...
      answer: firstHeading(decision, 'Decision brief is authored; close traps before sponsor approval.'),
      detail: 'Decision brief is present, but final selection memo is not yet the controlling artifact.',
      confidence: 'Medium — decision brief authored',
      nextStep: 'Run BAFO / close traps, then prepare selection memo.',
      changeTrigger: 'Unresolved P0 trap, pricing exception or risk clause gap.',
      status: 'warn',
    };
  }
  if (demand) {
    return {
      ...
      answer: firstHeading(demand, 'Demand challenge is authored; choose sourcing path before scope/RFP.'),
      ...
      changeTrigger: 'Demand is invalidated by existing contract, overlap or weak business case.',
      status: 'warn',
    };
  }
  return {
    verdict: 'Recommendation pending',
    answer: 'Recommendation pending — lifecycle artifacts are not sufficiently authored to make a CXO decision.',
    detail: 'The report is usable as a readiness readout, not as a decision recommendation.',
    confidence: 'Low — no authored decision artifact',
    nextStep: 'Author the next stage artifact and close evidence gaps before a CXO readout.',
...
function firstHeading(markdown: string, fallback: string): string {
  ...
  return markdown.split('\n').find((line) => line.trim())?.slice(0, 140) ?? fallback;
}

function coverageTable(coverage: ReadonlyArray<SourceArtifactCoverage>): SourceCxoTable {
  return {
...
function stageMetrics(input: DealPackInput): SourceCxoMetric[] {
  return input.stages.slice(0, 4).map((stage) => ({
    ...
    status: stage.artifacts.some((a) => a.kind !== 'missing') ? 'neutral' : 'warn',
  }));
}

function summarizeStage(artifacts: ReadonlyArray<DealPackArtifact>): string {
  const authored = artifacts.filter((a) => a.kind !== 'missing' && a.bodyIsAuthored).length;
  const scaffold = artifacts.filter((a) => a.kind !== 'missing' && !a.bodyIsAuthored).length;
  const missing = artifacts.filter((a) => a.kind === 'missing').length;
  return `${authored} authored · ${scaffold} scaffold · ${missing} missing`;
}

function buildEconomicsMessage(tco: DealPackArtifact | null, pricing: DealPackArtifact | null, valueLabel: string): string {
  if (tco?.kind === 'structured') return `Economics are grounded by the TCO artifact; estimated value is ${valueLabel}.`;
  if (pricing?.kind === 'structured') return `Pricing comparison is available, but TCO may still need hidden-cost coverage; estimated value is ${valueLabel}.`;
...
function economicsMetrics(tco: DealPackArtifact | null, pricing: DealPackArtifact | null, valueLabel: string): SourceCxoMetric[] {
  return [
    { label: 'Estimated value', value: valueLabel, note: 'From source event substrate' },
    { label: 'TCO artifact', value: statusLabel(tco), note: 'Hidden cost coverage', status: statusMetric(tco) },
    { label: 'Pricing comparison', value: statusLabel(pricing), note: 'Vendor commercial normalization', status: statusMetric(pricing) },
    { label: 'Board claim', value: tco || pricing ? 'Provisional' : 'Not defensible', note: 'Must match evidence depth', status: tco || pricing ? 'warn' : 'bad' },
  ];
}

function economicsTable(tco: DealPackArtifact | null, pricing: DealPackArtifact | null): SourceCxoTable {
  return {
    title: 'Commercial evidence check',
    columns: ['Proof object', 'State', 'CXO implication'],
    rows: [
      ['Pricing comparison', statusLabel(pricing), pricing ? 'Can compare supplier economics.' : 'Cannot claim supplier economics.'],
      ['TCO iceberg', statusLabel(tco), tco ? 'Can discuss hidden cost layers.' : 'Hidden costs must not be implied.'],
      ['Savings claim', tco || pricing ? 'Provisional' : 'Blocked', 'Only claim when pricing/TCO evidence exists.'],
    ],
  };
}

function buildVendorMessage(scorecard: DealPackArtifact | null): string {
  if (scorecard?.kind === 'structured') return 'Vendor recommendation is backed by a scored evaluation artifact.';
...
function vendorMetrics(scorecard: DealPackArtifact | null): SourceCxoMetric[] {
  return [
    { label: 'Scorecard', value: statusLabel(scorecard), note: 'Weighted vendor evaluation', status: statusMetric(scorecard) },
    { label: 'Decision quality', value: scorecard ? 'Comparable' : 'Incomplete', note: 'Do vendors have a common yardstick?' },
    { label: 'Losing logic', value: scorecard ? 'Traceable' : 'Not yet traceable', note: 'Why alternatives lose' },
    { label: 'Bias risk', value: scorecard ? 'Controlled' : 'Open', note: 'Without scorecard, narrative can dominate evidence' },
  ];
}
...
function vendorTable(scorecard: DealPackArtifact | null): SourceCxoTable {
  return {
    ...
    rows: [
      ['Are vendors scored consistently?', statusLabel(scorecard), scorecard ? 'Review weighting.' : 'Complete scorecard.'],
      ['Can we explain losers?', scorecard ? 'Yes, if scorecard is complete.' : 'No.', 'Document losing logic.'],
      ['Can we move to award?', scorecard ? 'Only after traps/BAFO close.' : 'No.', 'Close evaluation artifact.'],
    ],
  };
}

function buildRiskMessage(aiClause: DealPackArtifact | null, vendorRisk: DealPackArtifact | null): string {
  if (aiClause || vendorRisk) return 'Risk is visible enough for executive review, but open clause/risk gaps must control the signature path.';
  return 'Risk is not yet executive-ready; AI clause and vendor risk artifacts are missing or scaffolded.';
}

function riskMetrics(aiClause: DealPackArtifact | null, vendorRisk: DealPackArtifact | null): SourceCxoMetric[] {
  return [
    { label: 'AI clause gap', value: statusLabel(aiClause), note: 'Data/model contract controls', status: statusMetric(aiClause) },
    { label: 'Vendor risk pack', value: statusLabel(vendorRisk), note: 'Operational and supplier risk', status: statusMetric(vendorRisk) },
    { label: 'Signature posture', value: aiClause || vendorRisk ? 'Conditional' : 'Blocked', note: 'Risk controls must gate signature' },
    { label: 'Auditability', value: aiClause || vendorRisk ? 'Partial' : 'Weak', note: 'Risk evidence depth' },
  ];
}

function riskTable(aiClause: DealPackArtifact | null, vendorRisk: DealPackArtifact | null): SourceCxoTable {
  return {
    title: 'Risk controls before commitment',
    columns: ['Risk lane', 'State', 'Decision use'],
    rows: [
      ['AI/data clauses', statusLabel(aiClause), aiClause ? 'Gate contract redlines.' : 'Do not sign AI scope.'],
      ['Vendor operational risk', statusLabel(vendorRisk), vendorRisk ? 'Gate transition and controls.' : 'Complete risk pack.'],
      ['Executive override', 'Allowed only with named risk owner', 'Creates audit trail.'],
    ],
  };
}

function buildRenewalMessage(renewal: DealPackArtifact | null): string {
  if (renewal?.kind === 'structured') return 'Renewal posture is explicit; convert it into negotiation and SRM/Tower actions.';
...
function renewalMetrics(renewal: DealPackArtifact | null): SourceCxoMetric[] {
  return [
    { label: 'Renewal decision', value: statusLabel(renewal), note: 'Stage 7 artifact', status: statusMetric(renewal) },
    { label: 'Auto-renewal trap', value: renewal ? 'Tested' : 'Not tested', note: 'Requires notice-window evidence' },
    { label: 'Negotiation posture', value: renewal ? 'Available' : 'Not recorded', note: 'Commercial stance' },
    { label: 'SRM handoff', value: renewal ? 'Available' : 'Not recorded', note: 'Tower follow-up' },
  ];
}
...
function renewalTable(renewal: DealPackArtifact | null): SourceCxoTable {
  return {
    ...
    rows: [
      ['Is there an explicit posture?', statusLabel(renewal), renewal ? 'Act before notice window.' : 'Do not let contract renew silently.'],
      ['Is negotiation posture named?', renewal ? 'Yes' : 'No', renewal ? 'Use posture in vendor discussion.' : 'Generate renewal decision artifact.'],
      ['Is Tower/SRM handoff present?', renewal ? 'Yes' : 'No', 'Track value after decision.'],
    ],
  };
}
  ...
  return {
    ...
    rows:
      input.evidence.length === 0
        ? [['No evidence rows', 'Not recorded — seed gap', '—', 'Evidence ledger must be loaded before external readout.']]
        : input.evidence.slice(0, 6).map((e) => [
            e.requirementId,
...
function statusMetric(artifact: DealPackArtifact | null): SourceCxoMetric['status'] {
