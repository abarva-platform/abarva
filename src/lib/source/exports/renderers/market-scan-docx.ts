...
export function buildMarketScanDocx(payload: MarketScanPayload): Document {
  const marketScanGovernanceNotice = sourceArtifactGovernanceBanner("ai_draft");
  return new Document({
    ...
    sections: [
      {
        ...
        children: [
          ...
          governanceNoticeParagraph(
            marketScanGovernanceNotice.message,
            marketScanGovernanceNotice.detail,
          ),
          bodyParagraph([
            bodyRun(
                ...
                columns: [
                  {
                    header: "Vendor",
                    widthPercent: 30,
                    extract: () => SEED_GAP_LINE,
                  },
                  {
                    header: "Note",
                    widthPercent: 70,
                    extract: () => "No vendor longlist supplied.",
                  },
                ],
                rows: [{}],
                rowStyle: () => "warning",
            ...
            columns: [
              {
                header: "Capability",
                widthPercent: 50,
                extract: (r) => r.capability,
              },
              {
                header: "M/I/O",
                widthPercent: 10,
                extract: (r) => r.importance,
              },
              {
          ...
          buildMultiColumnTable({
            columns: [
              ...
              },
              {
                header: "Delivery",
                widthPercent: 14,
                extract: (r) => r.delivery,
              },
              {
                header: "Specialization",
                widthPercent: 26,
                extract: (r) => r.specialization,
              },
              {
                header: "Rate USD/hr",
                widthPercent: 18,
