...
export function buildBafoQuestionPackDocx(
  ...
  const bafoQuestionPackGovernanceNotice = sourceArtifactGovernanceBanner(
    "ai_draft",
    { artifactCode: "d22" },
  );
  return new Document({
    ...
    sections: [
      {
        ...
        children: [
          ...
          governanceNoticeParagraph(
            bafoQuestionPackGovernanceNotice.message,
            bafoQuestionPackGovernanceNotice.detail,
          ),
          ...(payload.roundLabel
            ...
            columns: [
              {
                ...
                widthPercent: 10,
                style: "locked",
                extract: (r: { name: string; idx: number }) =>
                  String(r.idx + 1),
              },
              {
                header: "Vendor",
                widthPercent: 90,
                style: "locked",
                extract: (r: { name: string; idx: number }) => r.name,
              },
            ],
          ...
          buildMultiColumnTable({
            columns: [
              ...
              },
              {
                header: "Severity",
                widthPercent: 9,
                extract: (r) => r.severity,
              },
              {
                header: "Question",
                widthPercent: 46,
                extract: (r) => r.question,
              },
              {
                header: "Response format",
                widthPercent: 20,
                extract: (r) => r.responseFormat,
              },
            ],
