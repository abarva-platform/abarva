...
export function buildBafoQuestionPackPdf(
  ...
  return buildStructuredPdfDocument({
    ...
    meta: [
      ...
      ...(payload.issuedBy
        ? [{ label: "Issued by", value: payload.issuedBy }]
        : []),
      { label: "Generated", value: payload.generatedAt },
      ...(payload.roundLabel
        ? [{ label: "BAFO round", value: payload.roundLabel }]
        : []),
    ],
    ...
    body: (
      <>
        <StructuredPdfHeading>
          Vendors invited to this BAFO round
        </StructuredPdfHeading>
        <StructuredPdfNote>
          ...
          columns={[
            {
              header: "#",
              flex: 0.4,
              extract: (r: { name: string; idx: number }) => String(r.idx + 1),
            },
            {
              header: "Vendor",
              flex: 4.6,
