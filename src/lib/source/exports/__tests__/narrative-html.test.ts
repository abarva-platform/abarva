...
describe("buildNarrativeHtml", () => {
  ...
  it("renders the body markdown content", () => {
    ...
    expect(out).toContain('class="source-doc__body"');
  ...
  it("emits business-readable document metadata without raw artifact codes", () => {
    const out = buildNarrativeHtml(makePayload(), RFP_PACK_HTML_CONFIG);
    expect(out).toContain('name="x-source-document"');
    expect(out).toContain('content="RFP Package"');
    expect(out).not.toContain("x-source-artifact-code");
    expect(out).not.toContain("d09_rfp_pack");
  });
