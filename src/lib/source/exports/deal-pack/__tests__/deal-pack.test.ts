...
function makeFileCabinetArtifact(
  ...
  return {
    ...
    description: null,
    fileName: "client-final-rfp.md",
    fileFormat: "md",
    blobContainer: "source-artifacts",
    blobPath: "client-final-rfp.md",
    fileSize: 100,
    version: 2,
    status: "client_final",
    generatedBy: null,
    ...
    clientFinalUploadedBy: "client-user",
    clientFinalUploadedAt: GENERATED_AT,
    clientFinalAcceptedBy: "client-user",
    clientFinalAcceptedAt: GENERATED_AT,
...
describe("Source Deal Pack · assemble-deal-pack", () => {
    ...
    describe(fixture.label, () => {
      ...
      it("renders lifecycle anchors including Stage 10 Transition", async () => {
        ...
        expect(html).toContain(
          "APX-CDP-2026 Q3 2026 data-migration freeze window",
        );
      });
      ...
      it("embeds the tenant name + event code in the topbar + meta tags", async () => {
        ...
        expect(html).toContain(
          `name="x-source-event-code" content="${fixture.ctx.event.code}"`,
        );
      });
    });
  }
  ...
  it("keeps the Apex Retail authored renewal body in the Stage 7 section", async () => {
    const { html } = await assembleDealPack(
      apexRetailFixture().ctx,
      GENERATED_AT,
    );
  ...
  it("uses client-final File Cabinet content instead of a stale generated artifact-state body", async () => {
    const fixture = meridianHealthFixture();
    const staleDraftMarker = "STALE GENERATED RFP DRAFT SHOULD NOT RENDER";
    const clientFinalMarker = "CLIENT FINAL RFP CONTENT APPROVED BY SOURCING";
    const staleState = makeArtifactState(
      "d09_rfp_pack",
      `# RFP package\n\n${staleDraftMarker}`,
      {
        id: "artifact-state-stale-d09",
        bodyGenerationMetadata: PASSED_RFP_QUALITY_METADATA,
      },
    );
    fixture.ctx.artifactStates = [
      makeArtifactState("d05_scope_memo", "# Scope memo\n\nScope is ready."),
      staleState,
    ];

    const { html, input } = await assembleDealPack(fixture.ctx, GENERATED_AT, {
      fileCabinetArtifacts: [
        makeFileCabinetArtifact({
          id: "client-final-d09",
          artifactType: "d09_rfp_pack",
          fileName: "client-final-rfp.md",
          fileFormat: "md",
    ...
    expect(
      input.artifactStates.find(
        (state) => state.artifactCode === "d09_rfp_pack",
      )?.body,
    ).toContain(clientFinalMarker);
    expect(
      input.stages[3].artifacts.find(
        (artifact) => artifact.code === "d09_rfp_pack",
      )?.bodyMarkdown,
    ).toContain(clientFinalMarker);
    expect(html).toContain(clientFinalMarker);
    expect(html).not.toContain(staleDraftMarker);
    expect(html).toContain("Authoritative client-final artifact");
    expect(html).toContain("client-final-d09");
    expect(html).toContain("Superseded Source artifact-state body");
    expect(html).toContain("artifact-state-stale-d09");
  });

  it("never throws on First Capital empty-substrate path; renders a complete document", async () => {
    const { html } = await assembleDealPack(
      firstCapitalFixture().ctx,
      GENERATED_AT,
    );
...
describe("Source Deal Pack · kernel verdict drives the headline", () => {
  ...
  function heldEventCtx(): SourceGenerationContext {
    ...
    return {
      ...
      artifactStates: [
        makeArtifactState("d27_selection_memo", heldSelectionMemo, {
          stage: "selection",
        }),
      ],
