...
export interface DealPackArtifact {
  ...
  bodyMarkdown?: string;
  bodyHtml?: string;
  sourceAuthority?: NarrativeDocxPayload["sourceAuthority"];
...
function synthesizeHeadline(
  ...
  if (selectionMemo) {
    return {
      kind: "decided",
      action: extractFirstHeading(
        selectionMemo,
        "Selection finalized — see Stage 5",
      ),
  ...
  if (renewalDecision) {
    return {
      ...
      action: extractFirstHeading(
        renewalDecision,
        "Renewal posture recorded — see Stage 7",
      ),
      confidence: kernelConfidence,
      financialImpact: valueLabel,
      keyRisk:
        kernelKeyRisk ??
        "Auto-renewal trap if posture not enacted before lock date",
      nextStep:
        "Enact posture per Stage 7 candidate row; flip Final Decision once panel signs.",
    };
  }
  if (decisionBrief) {
    return {
      ...
      action: extractFirstHeading(
        decisionBrief,
        "Recommendation drafted — see Stage 5",
      ),
      confidence: kernelConfidence,
      financialImpact: valueLabel,
  ...
  if (demandChallenge) {
    return {
      ...
      action: extractFirstHeading(
        demandChallenge,
        "Demand challenged — see Stage 0",
      ),
      confidence: kernelConfidence,
      financialImpact: valueLabel,
      keyRisk: kernelKeyRisk ?? "Sourcing without demand-challenge sign-off",
      nextStep:
        "Confirm sourcing approach in Stage 1 before issuing solicitation.",
    };
  }
...
function deriveKeyRisk(judgment: SourceJudgment): string | null {
  ...
  const blockingGap =
    judgment.evidenceGaps.find((g) => g.blocksDecision) ??
    judgment.evidenceGaps[0];
...
function renderToc(input: DealPackInput): string {
  const stageItems = input.stages
    .map(
      (s) =>
        `<li><a href="#${escapeHtml(s.slug)}">Stage ${s.stage} — ${escapeHtml(s.title)}</a></li>`,
    )
    .join("\n");
  return `
  ...
  if (a.kind === "narrative") {
    ...
    return `
      <div class="dp-artifact">
        ${head}
        ${issuedLine}
        ${renderSourceAuthority(a.sourceAuthority)}
...
function renderSourceAuthority(
  authority: DealPackArtifact["sourceAuthority"],
): string {
  if (!authority) return "";
  const fileLabel = authority.fileName ? ` · ${authority.fileName}` : "";
  const versionLabel =
    typeof authority.version === "number" ? ` · v${authority.version}` : "";
  const requestedLabel = authority.requestedArtifactId
    ? ` Replaces requested artifact ${authority.requestedArtifactId}.`
    : "";
  const history = authority.history?.length
    ? `<ul>${authority.history
        .map((item) => {
          const itemFile = item.fileName ? ` · ${item.fileName}` : "";
          const itemVersion =
            typeof item.version === "number" ? ` · v${item.version}` : "";
          return `<li>${escapeHtml(item.label)}: ${escapeHtml(item.artifactId)}${escapeHtml(itemFile)}${escapeHtml(itemVersion)}</li>`;
        })
        .join("")}</ul>`
    : "";
  return `
    <div class="dp-stage__empty">
      <strong>${escapeHtml(authority.label)}</strong> — ${escapeHtml(authority.artifactId)}${escapeHtml(fileLabel)}${escapeHtml(versionLabel)}.${escapeHtml(requestedLabel)}
      ${history}
    </div>
  `;
}
