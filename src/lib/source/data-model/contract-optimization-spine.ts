...
const SOURCE_CONNECTIONS: readonly ContractOptimizationSourceConnection[] = [
  {
    ...
    examples: ['Icertis', 'Ironclad', 'DocuSign CLM', 'Agiloft', 'Conga', 'SharePoint contract library'],
    extract: 'Executed agreement, SOW, order forms, pricing schedules, renewal and termination clauses.',
    evidenceClasses: ['contract_term', 'rate_card', 'renewal', 'approved_agreement'],
    ledgers: ['negotiated_improvement', 'avoided_cost'],
    fields: ['contract_id', 'document_id', 'clause_type', 'effective_date', 'page/span', 'review_state'],
    outcome: 'Proves rights, deadlines, pricing mechanics, benchmark rights, caps, and signed concessions.',
  },
  {
    id: 'procurement',
    sourceSystem: 'Procurement / S2P',
    examples: ['SAP Ariba', 'Coupa', 'Oracle Procurement', 'Workday Strategic Sourcing', 'Zip'],
    extract: 'POs, sourcing events, supplier responses, award summary, approved savings case.',
    evidenceClasses: ['supplier_offer', 'benchmark', 'scope', 'approved_agreement'],
    ledgers: ['negotiated_improvement', 'avoided_cost'],
    fields: ['supplier_id', 'event_id', 'bid_round', 'normalized_price', 'award_state', 'approver_role'],
    outcome: 'Turns leverage into a defendable negotiation plan rather than a generic price ask.',
  },
  {
    id: 'ap_erp',
    sourceSystem: 'AP / ERP / financial subledger',
    examples: ['SAP S/4HANA', 'Oracle Fusion', 'Workday Financials', 'NetSuite', 'Coupa Invoice'],
    extract: 'Invoice lines, payments, PO match, GL coding, credits, disputes, taxes, and pass-throughs.',
    evidenceClasses: ['invoice', 'payment', 'rate_card'],
    ledgers: ['recoverable_leakage', 'avoided_cost'],
    fields: ['invoice_id', 'line_id', 'po_id', 'service_period', 'amount', 'contract_id', 'match_state'],
    outcome: 'Finds duplicate charges, off-contract billing, rate-card variance, and unclaimed credits.',
  },
  {
    id: 'itsm',
    sourceSystem: 'ITSM / service management',
    examples: ['ServiceNow', 'Jira Service Management', 'PagerDuty', 'BMC Helix'],
    extract: 'SLA performance, incident severity, breach logs, service review packs, credit eligibility.',
    evidenceClasses: ['sla', 'service_credit'],
    ledgers: ['recoverable_leakage'],
    fields: ['service_id', 'period', 'target', 'actual', 'breach_count', 'credit_earned', 'credit_claimed'],
    outcome: 'Quantifies service credits and separates operational pain from commercial recoverability.',
  },
  {
    id: 'usage',
    sourceSystem: 'Usage / entitlement / consumption platforms',
    examples: ['Salesforce admin', 'Microsoft 365 admin', 'Snowflake', 'AWS CUR', 'Azure Cost Management', 'GCP Billing'],
    extract: 'Seats, active users, consumption, storage, workload, feature adoption, license assignment.',
    evidenceClasses: ['usage', 'cloud_consumption', 'scope'],
    ledgers: ['avoided_cost'],
    fields: ['product_id', 'sku', 'assigned_qty', 'active_qty', 'period', 'usage_unit', 'cost'],
    outcome: 'Separates shelfware, under-used entitlement, and real demand before negotiating renewal scope.',
  },
  {
    id: 'finance_tower',
    ...
    fields: ['claim_id', 'metric_ref', 'baseline', 'actual', 'currency', 'claim_state', 'approver_role'],
    outcome: 'Promotes value only after finance confirmation; everything else remains potential or workflow-required.',
  },
];
...
export function buildContractOptimizationSpine(input: {
  ...
  const selected = input.contract
    ? candidates.find((candidate) => candidate.contractId === input.contract?.contract_id) ?? null
    : null;
...
function rankCandidates(input: {
  ...
  const leverageByContract = new Map(input.leverageEntries.map((entry) => [entry.contractId, entry]));
  const concentration = computeVendorConcentration(input.contracts);
  const topVendorRefs = new Set(concentration.byVendor.slice(0, 5).map((vendor) => vendor.vendorRef));
  const renewal = computeRenewalExposure(input.contracts, input.asOfDateIso, 180);
  const noticePassed = new Set(renewal.noticeDeadlinePassed.map((row) => row.contract_id));
  const expiring180 = new Set(renewal.expiringWithinWindow.map((row) => row.contract_id));

  const ranked = input.contracts.map((contract) => {
    const leverage = leverageByContract.get(contract.contract_id) ?? null;
    const ledger =
      input.selectedContractId === contract.contract_id ? input.selectedLedger : null;
...
function scoreContract(input: {
  ...
  if (annualValue >= 25_000_000 || input.topVendorRefs.has(contract.vendor_ref)) {
    reasons.push({
      ...
      sourceRef: 'source.contract_360.annual_value + computeVendorConcentration(source.contract_360)',
      tone: 'strong',
  ...
  if (input.noticePassed.has(contract.contract_id) || input.expiring180.has(contract.contract_id)) {
    reasons.push({
      kind: 'decision_timing',
      label: input.noticePassed.has(contract.contract_id) ? 'Notice decision is already late' : 'Renewal decision window is active',
  ...
  if (annualVariance != null && Math.abs(annualVariance) >= Math.max(1_000_000, annualValue * 0.05)) {
    reasons.push({
      ...
      sourceRef: 'source.contract_360.annual_value + source.contract_360.actual_annual_spend',
      tone: 'neutral',
      points: 10,
    });
  }
  ...
  const initiativeDependencies = numberFromDb(contract.initiative_dependency_count);
  if ((criticalApps ?? 0) > 0 || (initiativeDependencies ?? 0) > 0) {
    reasons.push({
      ...
      sourceRef: 'source.contract_application_scope + source.contract_initiative_dependency',
      tone: 'strong',
      points: 8,
    });
  }
  ...
  if (input.ledger) {
    ...
    if (missingCount > 0) {
      reasons.push({
        ...
        detail: 'The opportunity is real enough to work, but missing extracts prevent AbarVa from inventing recoverable leakage or realized value.',
        sourceRef: 'contract optimization ledger evidence_status',
        tone: 'missing',
        points: 4,
      });
    }
    if (input.ledger.realizedValueUsd > 0 || input.ledger.quantifiedLeakageUsd > 0) {
      reasons.push({
        kind: 'value_proof',
  ...
  return {
    ...
    score,
    rank: 0,
    band: score >= 70 ? 'Prime optimization candidate' : score >= 45 ? 'Workable optimization candidate' : score >= 25 ? 'Monitor and enrich evidence' : 'Not a priority yet',
...
function missingEvidenceFor(lines: readonly ContractOptimizationLedgerLine[]): readonly string[] {
  return lines
