...
export function buildContractOptimizationOpportunitySet(
  ...
  const vendorName =
    text(input.overview?.vendor_name) ?? text(input.contract?.vendor_name) ?? null;
  const contractName =
    text(input.overview?.contract_name) ??
    text(input.contract?.contract_name) ??
    null;
  const annualValue =
    number(input.overview?.annual_value_usd) ?? number(input.contract?.annual_value);
  const actualSpend =
  ...
  const sourceRef = (
    ...
    sourceFileReport: text(row.source_file_report) ?? text(row.source_file_name),
    tableName,
    pageSpan,
  ...
  const opportunities =
    baseline.status === 'conflict'
      ...
      : buildReadyOpportunities(input, contractId, baseline, sourceRef, pdfRefs);
  ...
  const potentialNegotiableUsd = sum(
    opportunities
      .filter((opportunity) => opportunity.valueType === 'negotiable_improvement')
      .map((opportunity) => opportunity.amountUsd),
  );
  const financeConfirmedUsd = sum(financeRealizations.map((item) => item.amountUsd));

  const selectedOpportunityId =
    opportunities.find((opportunity) =>
      opportunity.opportunityId.endsWith(':rate-variance'),
    )?.opportunityId ?? opportunities[0]?.opportunityId ?? null;
...
function buildReadyOpportunities(
  ...
  const slaOpportunity = buildSlaOpportunity(input, contractId, sourceRef, pdfRefs);
  ...
  ].filter((item): item is ContractOptimizationOpportunity => Boolean(item)).map(
    (opportunity) => ({
      ...opportunity,
      narrative:
...
function buildConflictOpportunities(
  ...
  const contractId =
    ...
    'contract';
  const refs = [
    ...input.pricingRows.slice(0, 3).map((row) =>
      sourceRef(row, 'source.golden_contract_pricing_schedule'),
    ),
    ...(input.overview
...
function buildRateVarianceOpportunity(
  ...
  const excludedRows = input.invoiceRows.filter(
    (row) => text(row.exception_type) == null || number(row.exception_amount_usd) === 0,
  );
  ...
  const includedLines = rateRows.map((row) => {
    ...
    return {
      lineId: text(row.invoice_line_id) ?? text(row.source_record_id) ?? 'invoice-line',
      ...
      sourceRefs: [
        ...
        ...(pricing ? [sourceRef(pricing, 'source.golden_contract_pricing_schedule')] : []),
        ...pdfRefs.filter((ref) => ref.pageSpan?.includes('pricing')).slice(0, 2),
      ],
    };
  });
  ...
  const pendingLines = pendingRows.slice(0, 4).map((row) => ({
    ...
    inclusionReason: 'Off-contract billing requires coverage review before classification.',
    pricingScheduleRef: text(row.sku_or_service_code),
    contractTermRef: 'doc.extraction:contract.pricing_schedule',
    amendmentRef: null,
    sourceRefs: [sourceRef(row, 'source.golden_contract_invoice_lines')],
  ...
  const calculatedAmount = roundCurrency(sum(includedLines.map((line) => line.amountUsd)));
  ...
  return {
    ...
    blockingGap: null,
    nextAction:
      'Review the included invoice lines, confirm there is no approved exception, then move the recovery ask into Optimize Contract.',
    ...
    approvalState: 'ready_for_procurement_review',
    narrative:
...
function buildInvoiceExceptionOpportunity(
  ...
  const amount = roundCurrency(sum(rows.map((row) => number(row.exception_amount_usd))));
  return {
    ...
    evidenceRefs: [
      ...rows.map((row) => sourceRef(row, 'source.golden_contract_invoice_lines')),
...
function buildSlaOpportunity(
  ...
  const earned = sum(input.slaRows.map((row) => number(row.service_credits_earned_usd)));
  const claimed = sum(input.slaRows.map((row) => number(row.service_credits_claimed_usd)));
  const received = sum(input.slaRows.map((row) => number(row.service_credits_received_usd)));
  ...
  return {
    ...
    evidenceRefs: [
      ...input.slaRows
        .slice(0, 6)
        .map((row) =>
          sourceRef(row, 'source.golden_contract_sla_incident_service_credit_monthly'),
        ),
...
function buildShelfwareOpportunity(
  ...
  const reviewed = input.renewalRows.find(
    (row) =>
      ...
      text(row.finding_or_offer_summary)?.toLowerCase().includes('rationalization') ||
      text(row.event_type)?.toLowerCase().includes('license') ||
      text(row.event_type)?.toLowerCase().includes('true-up'),
  );
  const amount =
    number(reviewed?.estimated_value_usd) ?? number(input.financeRow?.avoided_cost_usd);
  if (amount == null || amount <= 0) return null;
  const usageRefs = input.usageRows
    .slice(0, 6)
    .map((row) => sourceRef(row, 'source.golden_contract_usage_entitlement_monthly'));
  return {
    ...
    evidenceRefs: [
      ...(reviewed
        ? [sourceRef(reviewed, 'source.golden_contract_renewal_negotiation_history')]
        : []),
      ...usageRefs,
      ...pdfRefs.filter((ref) => ref.pageSpan?.includes('scope')).slice(0, 2),
    ],
  ...
  return [
    {
      ...
      sourceRefs: [
        sourceRef(input.financeRow, 'source.golden_contract_finance_value_confirmation'),
      ],
    },
  ];
}

function clauseRefs(rows: readonly Row[]): readonly OpportunitySourceReference[] {
  return rows.map((row) => ({
