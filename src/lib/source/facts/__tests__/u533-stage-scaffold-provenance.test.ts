...
function measureStage(stageKey: string) {
  ...
  return {
    ...
    carriedTaskTitles: view.tasks.map((task) => task.title),
    carriedGateApprover: view.gate.approver,
    carriedGateConfirmLabels: view.gate.confirms.map((c) => c.label),
  ...
  return {
    ...
    summary: {
      ...
      stagesWithScaffoldGate: stages.filter((s) => s.declared?.gate === "scaffold")
        .length,
      distinctCarriedApprovers: [
        ...new Set(stages.map((s) => s.carriedGateApprover)),
      ].sort(),
...
describe("U-533 · what is carried, and what only agrees", () => {
  it("records exactly the five fields that are carried on every armed stage", () => {
    ...
    expect(measurement.summary.factDerivedFields).toEqual(["intel", "waterfall"]);
    expect(measurement.summary.stagesWithScaffoldTasks).toBe(10);
    expect(measurement.summary.stagesWithScaffoldGate).toBe(10);
  });
  ...
  it("finds four exemplars whose approver reads as a person, not a role", () => {
    ...
    expect(measurement.summary.distinctCarriedApprovers).toContain(
...
describe("U-533 · the boundary declares which beats are carried", () => {
  it.each(ARMED_STAGE_KEYS)("%s declares both intake beats as scaffold", (stageKey) => {
    const view = buildFor(stageKey);
  ...
  it("names the exemplar the view was actually built from", () => {
    const view = buildFor("bafo");
    expect(view.beatProvenance?.scaffoldSource).toBe("SAMPLE_BAFO_STAGE");
    expect(liveStageScaffoldFor("bafo")).toBe(SAMPLE_BAFO_STAGE);
  });
