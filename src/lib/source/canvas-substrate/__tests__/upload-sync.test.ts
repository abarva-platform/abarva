...
function fakeDb(seed: { evidence?: Row | null; criteria?: Record<string, Row> }) {
  const writes: Array<{ table: string; op: string; payload: Row; filters: Array<[string, unknown]> }> = [];
  const state = { evidence: seed.evidence ?? null, criteria: seed.criteria ?? {} };
  function builder(table: string) {
    ...
    b.maybeSingle = async () => {
      if (table === 'source_event_evidence_states') return { data: state.evidence, error: null };
...
describe('matchEvidenceRequirementForUpload (filename → canonical requirement)', () => {
  it('matches the SkyHarbor audit files to the right Strategy requirements', () => {
    expect(
      matchEvidenceRequirementForUpload({ stageKey: 'strategy', filename: 'SkyHarbor_Incumbent_AMS_Support_Contract_Baseline.csv' })?.requirementId,
    ).toBe('EVID-SRC-STR-INCUMBENT');
    expect(
      matchEvidenceRequirementForUpload({ stageKey: 'strategy', filename: 'SkyHarbor_Sponsor_Commitment_Memo.txt' })?.requirementId,
    ).toBe('EVID-SRC-STR-SPONSOR-COMMIT');
  });

  it('is stage-scoped ("contract" maps differently in scope) and honest on no match', () => {
    expect(
      matchEvidenceRequirementForUpload({ stageKey: 'scope', filename: 'fy26_support_contract_spend.xlsx' })?.requirementId,
    ).toBe('EVID-SRC-SCOPE-FY-CONTRACT');
    expect(
      matchEvidenceRequirementForUpload({ stageKey: 'scope', filename: 'application_inventory_cmdb.csv' })?.requirementId,
    ).toBe('EVID-SRC-SCOPE-APP-INV');
    expect(matchEvidenceRequirementForUpload({ stageKey: 'strategy', filename: 'holiday_photos.png' })).toBeNull();
  });
});
...
describe('syncUploadToCanvasSubstrate (durable F1 fix)', () => {
  ...
  it('upgrades Not Requested → Parsed, links the artifact, reports minimum honestly', async () => {
    const { db, writes } = fakeDb({ evidence: { current_state: 'Not Requested' } });
    const res = await syncUploadToCanvasSubstrate({ ...base, artifactFamily: 'other' }, db);
  ...
  it('meets minimum when the requirement floor is Loaded', async () => {
    ...
    const res = await syncUploadToCanvasSubstrate(
      { ...base, filename: 'SkyHarbor_Sponsor_Commitment_Memo.txt', artifactFamily: 'other', parsed: false },
      db,
    );
  ...
  it('never downgrades a validated state', async () => {
    const { db, writes } = fakeDb({ evidence: { current_state: 'Usable Evidence' } });
    const res = await syncUploadToCanvasSubstrate({ ...base, artifactFamily: 'other' }, db);
    expect(res.evidence?.newState).toBe('Usable Evidence');
    expect(writes.filter((w) => w.table === 'source_event_evidence_states')).toHaveLength(0);
  });
  ...
  it('ART-* presence criteria auto-meet; appends evidence id; dedups', async () => {
    const { db, writes } = fakeDb({
      criteria: { 'ART-AMS-PLAN-01': { state: 'pending', evidence_artifact_ids: ['old-1'] } },
    });
    const res = await syncUploadToCanvasSubstrate(
      { ...base, filename: 'SkyHarbor Sourcing Strategy v2.docx', artifactFamily: 'sourcing_strategy' },
      db,
    );
    const crit = res.criteria.find((c) => c.criterionId === 'ART-AMS-PLAN-01');
    expect(crit?.autoMet).toBe(true);
    const w = writes.find((x) => x.table === 'source_event_gate_criterion_states');
  ...
  it('non-ART (human/HARD) criteria are linked but never auto-met', async () => {
    const { db, writes } = fakeDb({
      criteria: { 'ART-AMS-PLAN-01': { state: 'met', evidence_artifact_ids: [] } },
    });
    const res = await syncUploadToCanvasSubstrate(
      { ...base, filename: 'strategy.docx', artifactFamily: 'sourcing_strategy' },
      db,
    );
...
describe('inferSourceArtifactFamily (F2 — raw evidence is not a deliverable)', () => {
  it('no longer mislabels the audit files as sourcing_strategy', () => {
    expect(inferSourceArtifactFamily({ stageKey: 'strategy', filename: 'SkyHarbor_Incumbent_AMS_Support_Contract_Baseline.csv' })).toBe('other');
    expect(inferSourceArtifactFamily({ stageKey: 'strategy', filename: 'SkyHarbor_Sponsor_Commitment_Memo.txt' })).toBe('other');
    expect(inferSourceArtifactFamily({ stageKey: 'scope', filename: 'application_inventory_cmdb_export.csv' })).toBe('other');
    expect(inferSourceArtifactFamily({ stageKey: 'scope', filename: 'itsm_ticket_volumes_may.xlsx' })).toBe('other');
  });

  it('keeps real deliverables in their families', () => {
    expect(inferSourceArtifactFamily({ stageKey: 'rfp', filename: 'AMS RFP v1.docx' })).toBe('rfp');
    expect(inferSourceArtifactFamily({ stageKey: 'pricing', filename: 'pricing_workbook.xlsx' })).toBe('pricing_workbook');
    expect(inferSourceArtifactFamily({ stageKey: 'strategy', filename: 'sourcing strategy memo.docx' })).toBe('sourcing_strategy');
    expect(inferSourceArtifactFamily({ stageKey: 'evaluation', filename: 'vendor scorecard.xlsx' })).toBe('scorecard');
  });

  it('stage fallback still applies to unrecognized deliverable-ish names', () => {
    expect(inferSourceArtifactFamily({ stageKey: 'responses', filename: 'acme_reply_final.pdf' })).toBe('proposal');
  });
});
