...
export type SourceValueStateSubtype = 'cost' | 'risk_avoided' | 'value_delivered' | 'commercial_terms';
export type SourceValueAmountBasis = 'projected' | 'tracked' | 'verified';
export type SourceValueDeltaClassification = 'cost_avoided' | 'cost_saved' | 'risk_reduced' | 'value_increased';
...
function chainStep(layer: SourceValueStateLayer): number {
  return layer === 'baseline' ? 1 : layer === 'intervention' ? 2 : layer === 'negotiated' ? 3 : 4;
}

function deltaClass(layer: SourceValueStateLayer): SourceValueDeltaClassification | null {
...
async function latestState(sourceEventId: string, layer: SourceValueStateLayer): Promise<SourceValueStateRow | null> {
  const { data, error } = await getServerSupabase()
    ...
    .limit(1)
    .maybeSingle();
  if (error) throw new Error(`source value state lookup failed: ${error.message}`);
  return (data as SourceValueStateRow | null) ?? null;
}

async function recordState(input: RecordStateInput): Promise<SourceValueStateRow> {
  if (input.stateLayer === 'baseline' && input.evidenceLedgerIds.length < 2) {
    throw new Error('Baseline cannot be recorded without at least 2 Evidence Ledger citations.');
  }
  if (input.stateLayer === 'realized' && (!input.attestedBy || !input.attestedAt)) {
    throw new Error('Realized value cannot be recorded without CFO attestation.');
  }
  ...
  const { data, error } = await getServerSupabase()
    ...
    .select('*')
    .single();
  if (error) throw new Error(`source value state insert failed: ${error.message}`);
  const state = data as SourceValueStateRow;

  const priorStep = input.stateLayer === 'baseline'
    ? null
    : await latestChainState(input.sourceEventId, chainStep(input.stateLayer) - 1);
  ...
  if (chainError) throw new Error(`source value chain upsert failed: ${chainError.message}`);
  return state;
}
...
async function latestChainState(sourceEventId: string, step: number): Promise<SourceValueStateRow | null> {
  ...
  const { data: state, error: stateError } = await getServerSupabase()
    ...
    .eq('id', stateId)
    .maybeSingle();
  if (stateError) throw new Error(`source value chain state row lookup failed: ${stateError.message}`);
  return (state as SourceValueStateRow | null) ?? null;
}

export async function computeBaseline(sourceEventId: string): Promise<SourceValueStateRow> {
  const event = await loadSourceEvent(sourceEventId);
  const existing = await latestState(sourceEventId, 'baseline');
  if (existing) return existing;

  const now = new Date().toISOString();
  const amount = event.estimated_value_usd ?? 0;
  const amountEvidenceId = await recordEvidence({
    ...
    sourceRef: { table: 'source_events', row_id: event.id, field: 'estimated_value_usd' },
  ...
  const scopeEvidenceId = await recordEvidence({
    ...
    sourceRef: { table: 'source_events', row_id: event.id, field: 'scope_description' },
    ...
    notEnoughDataReason: event.scope_description ? null : 'Source event scope is not populated.',
    createdBy: 'source-value-chain',
  });
  ...
  return recordState({
    ...
    methodology: 'Persisted source_events.estimated_value_usd plus scope boundary evidence.',
...
export async function recordIntervention(
  ...
  const evidenceId = await recordEvidence({
    ...
    sourceType: 'derived',
    sourceRef: { table: 'source_events', row_id: event.id, field: 'intervention_trace' },
  ...
  return recordState({
    ...
    amountBasis: 'tracked',
    methodology: 'AbarVa intervention trace: scope refinement, leverage points, hard questions, or BAFO posture.',
...
export async function recordNegotiatedOutcome(
  ...
  const evidenceId = await recordEvidence({
    ...
    sourceRef: { table: 'source_value_states', field: 'manual_negotiated_entry' },
    sourceQuote: terms.join('; ') || null,
    freshnessAt: new Date().toISOString(),
    confidence: terms.length > 0 ? 0.8 : 0.45,
    confidenceBasis: terms.length > 0 ? 'Manual negotiated terms entered by operator.' : 'Manual entry lacks contract extract.',
    ownerRole: event.decision_owner ?? 'VP Procurement',
    notEnoughData: terms.length === 0,
    notEnoughDataReason: terms.length === 0 ? 'Contract document extract or terms are required for high-confidence negotiated value.' : null,
    createdBy: 'source-value-chain',
  });
  return recordState({
    clientId: event.client_key,
    ...
    amountBasis: 'tracked',
    methodology: 'Manual negotiated outcome entry compared to baseline exposure.',
    evidenceLedgerIds: [evidenceId],
    createdBy: 'source-value-chain',
  });
}
...
export async function attestRealized(
  ...
  const event = await loadSourceEvent(sourceEventId);
  if (!attestor.trim()) throw new Error('CFO attestor is required for realized value.');
  const evidenceId = await recordEvidence({
    clientId: event.client_key,
    surface: 'source',
    artifactType: 'value_state',
    ...
    sourceType: 'tenant_record',
    sourceRef: { table: 'source_value_states', field: 'cfo_attestation', period },
    freshnessAt: new Date().toISOString(),
    confidence: 0.9,
    confidenceBasis: 'CFO attestation event recorded in Source value proof loop.',
    ownerRole: 'CFO',
    createdBy: 'source-value-chain',
  });
  return recordState({
...
export async function getValueChain(sourceEventId: string): Promise<SourceValueChainView> {
  ...
  if (chainResult.error) throw new Error(`source value chain load failed: ${chainResult.error.message}`);
  const states = (statesResult.data ?? []) as SourceValueStateRow[];
  const chain = (chainResult.data ?? []) as SourceValueChainRow[];
  const baseline = states.find((state) => state.state_layer === 'baseline')?.amount_usd ?? 0;
  const realized = states.find((state) => state.state_layer === 'realized')?.amount_usd ?? null;
  return {
    states,
    chain,
    cumulativeSavingsUsd: realized === null ? 0 : baseline - realized,
    complete: ['baseline', 'intervention', 'negotiated', 'realized'].every((layer) =>
      states.some((state) => state.state_layer === layer),
    ),
  };
}
...
export async function computeCumulativeSavings(clientId: string, periodDays: number): Promise<number> {
  ...
  if (error) throw new Error(`source value cumulative savings load failed: ${error.message}`);
  const rows = (data ?? []) as Array<Pick<SourceValueStateRow, 'source_event_id' | 'state_layer' | 'amount_usd'>>;
  const byEvent = new Map<string, { baseline?: number; realized?: number }>();
  for (const row of rows) {
    const current = byEvent.get(row.source_event_id) ?? {};
    if (row.state_layer === 'baseline' && row.amount_usd !== null) current.baseline = row.amount_usd;
    if (row.state_layer === 'realized' && row.amount_usd !== null) current.realized = row.amount_usd;
    byEvent.set(row.source_event_id, current);
  }
  let total = 0;
  for (const value of byEvent.values()) {
    if (typeof value.baseline === 'number' && typeof value.realized === 'number') {
