...
const entry = (over: Partial<SourceAppInventoryEntry> = {}): SourceAppInventoryEntry => ({
  appId: 'APP-001',
...
describe('buildAppInventoryPromptBlock', () => {
  it('emits a not-loaded note (no table) when inventory is empty', () => {
    ...
    expect(block).toContain('none loaded for this tenant');
  ...
  it('renders a header count and a markdown table when inventory is present', () => {
    const block = buildAppInventoryPromptBlock([entry(), entry({ appId: 'APP-002', name: 'CRM' })]);
    expect(block).toContain('2 systems already on record');
    expect(block).toContain('| App ID | Name | Tier | Owner | Vendor | Criticality | Notes |');
    expect(block).toContain('| APP-001 | Core Banking Platform | Tier 1 | Jane Doe | Acme | High | Primary ledger |');
    expect(block).toContain('| APP-002 | CRM |');
  });

  it('singularises the header for a single system', () => {
    expect(buildAppInventoryPromptBlock([entry()])).toContain('1 system already on record');
  });

  it('leaves blank cells empty rather than inventing values', () => {
    const block = buildAppInventoryPromptBlock([
      entry({ appId: 'APP-003', name: 'Legacy HR', tier: null, owner: null, vendor: null, criticality: null, notes: null }),
    ]);
  ...
  it('escapes pipes and newlines so a payload value cannot break the table', () => {
    ...
    // The only real row delimiters are the header + separator + one data row.
    expect(block.split('\n').filter((l) => l.startsWith('| APP-004'))).toHaveLength(1);
  });
});
