 ...
 *   in-scope application list from, verbatim.
 */
export function buildAppInventoryPromptBlock(
  ...
  if (!entries || entries.length === 0) {
    return [
      "Enterprise application inventory: (none loaded for this company).",
      "Do not invent applications. Produce the application-list table framework and flag that the inventory is not yet ingested and must be authored or uploaded before scope can lock.",
    ].join("\n");
  }

  const header = `Enterprise application inventory — ${entries.length} system${entries.length === 1 ? "" : "s"} already on record for this company. Pre-fill the in-scope application table directly from these rows, verbatim IDs and names, one row per system. Do not add applications beyond this list; where a Tier or Owner cell is blank, carry it forward as a coverage gap rather than guessing.`;
