...
const STAGE_DRAFT_CONFIG: Partial<Record<SourceStageKey, StageDraftConfig>> = {
  strategy: {
    ...
    draftBody:
      "Sentinel drafts from the event facts: why now, scope, value target, archetype, and rigor. You review before anything external uses it.",
    draftPrimary: "Draft with Sentinel",
...
export function resolveStageNextMove({
  ...
  if (stageNeedsDraft(config.artifactCodes, artifacts)) {
    return {
      ...
      secondaryLabel:
        config.draftPrimary === "Draft with Sentinel"
          ? "Author manually"
