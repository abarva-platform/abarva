...
const contract: CsvRecord = {
  ...
  contract_english_overview:
    "A governed cloud commitment buys platform capacity ahead of usage.",
  ...
  evidence_boundary_summary:
    "Usage, coverage, AP, scope, and opportunities are loaded.",
};
...
describe("buildCloudContractIntelligenceRecords", () => {
  it("keeps native cloud evidence separate and exposes all structured levers", () => {
      ...
      optimizationOpportunities: [
        ...
        row("opp-signal", {
          ...
          negotiation_language:
            "Re-open the discount question after evidence loads.",
    ...
    expect(records[0].education.archetypeKey).toBe("cloud_consumption_commit");
    expect(records[0].reporting.state).toBe("not_declared");
    expect(records[0].reporting.completeness).toBe("unknown");
    expect(
      records[0].baseline.metrics.find(
        (metric) => metric.key === "observed_spend",
      ),
    ).toMatchObject({
      valueNumeric: 100,
      unit: "USD",
    ...
    expect(
      records[0].evidenceLanes.find((lane) => lane.key === "cloud_usage")
        ?.rowCount,
    ).toBe(1);
    expect(
