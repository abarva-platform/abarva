...
function getSeededVendorResponses(
  eventId: string,
): SourceVendorResponseSeed["responses"] {
  const seed = getSourceVendorResponseSeed(eventId);
  return seed.responses;
}

function pickEventSeedResponses(
  event: SourceVendorResponseCompletenessInput["event"],
): SourceVendorResponseSeed["responses"] {
...
function getMissingSections(
  ...
  return required.filter(
    (section) => !submittedSet.has(section.trim().toLowerCase()),
  );
}
...
function buildVendorBlockers(
  ...
  for (const section of missingSections) {
    if (COMPARABILITY_CRITICAL_SECTIONS.includes(section)) {
      blockers.push(
        `${vendor.vendorName}: critical section missing - ${section}.`,
      );
    } else {
  ...
  if (
    ...
    vendor.securityResponseStatus === "incomplete"
  ) {
    blockers.push(
      `${vendor.vendorName}: security and compliance response is incomplete.`,
    );
  }

  if (
    vendor.transitionPlanStatus === "missing" ||
    vendor.transitionPlanStatus === "incomplete"
  ) {
    blockers.push(
      `${vendor.vendorName}: transition plan status is incomplete.`,
    );
  }
...
function isWeakEvidence(
  ...
  return (
    WEAK_EVIDENCE_STATUSES.has(status) ||
    ["low_confidence", "restricted"].includes(usability)
  );
}

function buildCompletenessRecord(
  vendor: SourceVendorResponseSeedInput,
): SourceVendorResponseCompletenessRecord {
  const requiredSections = REQUIRED_RESPONSE_SECTIONS.filter((section) =>
    vendor.requiredSections.includes(section),
  );
  const missingSections = getMissingSections(
    requiredSections,
    vendor.submittedSections,
  );
  const blockers = buildVendorBlockers(vendor, missingSections);

  const hasCriticalMissing = COMPARABILITY_CRITICAL_SECTIONS.some((section) =>
    missingSections.includes(section),
  );
  const hasWeakEvidence = isWeakEvidence(
    vendor.evidenceStatus,
    vendor.evidenceUsability,
  );

  let completenessStatus: SourceVendorCompletenessStatus = "complete";
  let comparabilityStatus: SourceVendorResponseCompletenessRecord["comparabilityStatus"] =
    "comparable";
  ...
  if (
    ...
    if (
      vendor.responseStatus === "not_started" ||
      vendor.responseStatus === "in_progress"
    ) {
      completenessStatus =
        missingSections.length > 0 ? "incomplete" : "partially_complete";
  ...
  if (hasWeakEvidence) {
    rationale.push(
      "Evidence quality is weak and should not be used for direct scoring.",
    );
  }
  ...
  if (completenessStatus === "partially_complete") {
    rationale.push(
      "Response is mostly complete but not yet at full comparability.",
    );
  }
  ...
  const atlasExecutiveImplication =
    hasWeakEvidence || comparabilityStatus !== "comparable"
...
function buildSummary(
  ...
  return {
    ...
    complete: records.filter(
      (record) => record.completenessStatus === "complete",
    ).length,
    partiallyComplete: records.filter(
      (record) => record.completenessStatus === "partially_complete",
    ).length,
    incomplete: records.filter(
      (record) => record.completenessStatus === "incomplete",
    ).length,
    notComparable: records.filter(
      (record) => record.completenessStatus === "not_comparable",
    ).length,
    blocked: records.filter((record) => record.completenessStatus === "blocked")
      .length,
  };
}
...
export function buildSourceVendorResponseCompleteness(
  ...
  const recommendedNextAction =
    comparabilityReadiness !== "complete"
  ...
  return input.records.flatMap((record) => {
    const statusMissing =
      record.missingSections.length > 0
        ? `Missing sections: ${record.missingSections.join(", ")}`
        : null;
    const actionGap =
      record.assumptions.length === 0 || record.exclusions.length === 0
        ? "assumption/exclusion coverage"
        : null;
    return [statusMissing, actionGap].filter((entry) =>
      Boolean(entry),
    ) as string[];
  });
}
...
export function summarizeSourceVendorResponseCompleteness(
  input: SourceVendorResponseCompleteness,
): string {
