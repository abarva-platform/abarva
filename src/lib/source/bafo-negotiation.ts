...
import {
  getSourceCorpusAssumptionLocks,
  getSourceCorpusBafoAsks,
} from "./source-corpus-uplift";
import type { SourcePricingVendorInput } from "./pricing-normalization-types";
import type { SourcePricingVendorSnapshot } from "./pricing-normalization-types";
...
function pickPricingInputs(
  input: SourceBafoNegotiationInput["event"],
): SourcePricingVendorInput[] | undefined {
...
function pickSnapshots(
  ...
  if (
    input.pricingNormalizationSnapshots &&
    input.pricingNormalizationSnapshots.length > 0
  ) {
...
function buildInferencePricingMap(
  ...
  return new Map(
    (input.vendorInferencePricing ?? []).map((entry) => [
      entry.vendorId,
      entry,
    ]),
  );
...
function evaluateVendorReadiness(
  ...
  const missingRequired = REQUIRED_SECTIONS.filter(
    (section) =>
      requiredSet.has(normalizeText(section)) &&
      !submittedSet.has(normalizeText(section)),
  );

  if (
    vendor.responseStatus === "blocked" ||
    vendor.responseStatus === "rejected"
  ) {
    return "blocked";
  }

  if (
    vendor.pricingTemplateStatus === "missing" ||
    vendor.securityResponseStatus === "missing"
  ) {
  ...
  const hasCriticalDataGap =
    vendor.evidenceUsability === "low_confidence" ||
    vendor.evidenceUsability === "restricted";
  if (
...
export function buildVendorNegotiationQuestions(
  ...
  if (
    exclusionFlags.includes("release") ||
    exclusionFlags.includes("minor") ||
    exclusionFlags.includes("tooling")
  ) {
    questions.push(
      buildQuestion(
  ...
  if (
    snapshot &&
    snapshot.commercialTraps.some((trap) => trap.category === "Rate escalation")
  ) {
    questions.push(
      buildQuestion(
  ...
  if (
    vendor.vendorId === "vendor-c" &&
    snapshot &&
    snapshot.commercialTraps.some(
      (trap) =>
...
function buildKeyIssues(
  ...
  if (snapshot.commercialTraps.length > 0) {
    issues.add(
      `${snapshot.commercialTraps.length} commercial trap(s) present.`,
    );
  }
...
function buildRequiredClarifications(
  vendor: SourceVendorResponseSeedInput,
): string[] {
  const output: string[] = [];
  ...
  if (vendor.pricingTemplateStatus !== "complete") {
    output.push(
      "Missing or incomplete pricing template line-items require re-submission.",
    );
  }
  if (vendor.transitionPlanStatus !== "complete") {
    output.push(
      "Transition plan should explicitly cover release, handoff, and support ownership.",
    );
  }
  if (vendor.securityResponseStatus !== "complete") {
    output.push(
      "Security/compliance obligations should include owner, liability, and escalation path.",
    );
  }
  if (vendor.responseRiskLevel === "high") {
    output.push(
      "Risk notes should be signed and explicitly accepted before recommendation lock.",
    );
  }

  if (output.length === 0) {
    output.push(
      "No additional clarifications required for the current BAFO cycle.",
    );
  }
...
function buildRecommendedAsks(
  ...
  if (
    vendor.pricingTemplateStatus === "missing" ||
    vendor.pricingTemplateStatus === "incomplete"
  ) {
    asks.add("Upload complete pricing template (all sections and line-items).");
  }
  if (vendor.transitionPlanStatus !== "complete") {
    asks.add(
      "Upload transition milestones with retained and vendor role split.",
    );
  }
  if (
    vendor.evidenceUsability === "low_confidence" ||
    ...
    asks.add(
      "Supplement claims with measurable references and usage evidence.",
    );
  }
  ...
  if (
    ...
    asks.add(
      "Clarify exclusion language around release support and minor enhancements.",
    );
  }
  if (
    snapshot?.commercialTraps.some(
      (trap) => trap.category === "Security/compliance",
    )
  ) {
    asks.add(
      "Confirm security obligations are contract-ready and evidence-backed.",
    );
  }
  for (const ask of getSourceCorpusBafoAsks({
    id: event?.id,
    name: event?.name,
    vendor,
    snapshotCategories: snapshot?.commercialTraps.map((trap) => trap.category),
  })) {
    asks.add(ask);
  }
  if (asks.size === 1) {
    asks.add(
      "No additional BAFO asks outside current commercial follow-through.",
    );
  }
...
function buildVendorRiskNotes(
  ...
  if (vendor.responseRiskLevel === "high") {
    notes.add(
      "High response risk level indicates elevated commercial uncertainty.",
    );
  }
  if (
    vendor.evidenceUsability === "low_confidence" ||
    ...
    notes.add(
      "Evidence cannot yet be treated as deterministic for recommendation lock.",
    );
  }
  if (snapshot) {
...
function buildBlockers(
  ...
  if (
    ...
    blockers.push(
      `${vendor.vendorName}: response is not in an evaluable state.`,
    );
  }
  if (vendor.pricingTemplateStatus !== "complete") {
    blockers.push(
      `${vendor.vendorName}: pricing template is incomplete or missing.`,
    );
  }
  if (vendor.transitionPlanStatus !== "complete") {
  ...
  if (vendor.securityResponseStatus !== "complete") {
    blockers.push(
      `${vendor.vendorName}: security/compliance response remains incomplete.`,
    );
  }
  if (
    vendor.evidenceUsability === "low_confidence" ||
  ...
  if (questions.length > 2) {
    blockers.push(
      `${vendor.vendorName}: multiple high-priority BAFO actions remain.`,
    );
  }
  return blockers;
}
...
function buildExpectedValueImpact(
  ...
  if (
    snapshot &&
    snapshot.commercialTraps.some((trap) => trap.category === "Automation trap")
  ) {
...
function buildCommercialTrapSummary(
...
function aggregateExclusions(
  vendors: SourceVendorResponseSeedInput[],
  snapshots: SourcePricingVendorSnapshot[],
): string[] {
  ...
  for (const snapshot of snapshots) {
    ...
    for (const optional of snapshot.optionals) {
      if (/release|minor|tooling/i.test(optional)) {
        exclusions.add(
          `Optional item requires explicit decision: ${optional}.`,
        );
      }
    }
  }
...
function toBafoReadiness(
  vendors: SourceVendorBafoReadiness[],
): SourceBafoNegotiationReadiness {
...
function summarizeBlockers(
  vendors: SourceBafoVendorNegotiationPlan[],
): string[] {
...
export function getSourceBafoPriorities(
  plan: SourceBafoNegotiationPlan,
): string[] {
  const priorities = new Set<string>();
  const notComparable = plan.vendorNegotiationPlans.filter(
    (vendor) => vendor.readiness === "not_comparable",
  );
  if (notComparable.length > 0) {
    priorities.add(
      `Close not comparable blockers for ${notComparable.map((vendor) => vendor.vendorName).join(", ")}.`,
    );
  }
  const blocked = plan.vendorNegotiationPlans.filter(
    (vendor) => vendor.readiness === "blocked",
  );
  if (blocked.length > 0) {
    priorities.add(
      `Resolve blocked vendor states before decision lock: ${blocked.map((vendor) => vendor.vendorName).join(", ")}.`,
    );
  }
  const conditional = plan.vendorNegotiationPlans.filter(
    (vendor) => vendor.readiness === "conditional",
  );
  if (conditional.length > 0) {
    priorities.add(
      `Stabilize evidence and pricing conditions for ${conditional.map((vendor) => vendor.vendorName).join(", ")}.`,
    );
  }
  priorities.add(
    "Publish assumption lock list for legal, finance, security, and transition decisions.",
  );
  priorities.add("Collect comparability evidence for BAFO selection set.");
  return Array.from(priorities).slice(0, 5);
}

export function summarizeSourceBafoNegotiation(
  plan: SourceBafoNegotiationPlan,
): string {
  return (
    `BAFO negotiation: ${plan.vendorNegotiationPlans.length} vendors, overall ${plan.overallNegotiationReadiness},` +
    ` ${plan.blockers.length} blockers, next action "${plan.nextAction}".`
  );
}

export function formatSourceBafoNegotiationAsMarkdown(
  plan: SourceBafoNegotiationPlan,
): string {
  const lines = [
  ...
  for (const vendor of plan.vendorNegotiationPlans) {
    ...
    if (vendor.counterClauses.length > 0) {
      ...
      for (const clause of vendor.counterClauses) {
        ...
        lines.push(
          `      Peer provenance: ${clause.peerProvenance.join(", ")}`,
        );
      }
    }
  }
...
export function buildSourceBafoNegotiationPlan(
  input: SourceBafoNegotiationInput,
): SourceBafoNegotiationPlan {
  ...
  const snapshotByVendor = new Map(
    pricingSnapshots.map((snapshot) => [snapshot.vendorId, snapshot]),
  );
  const inferencePricingByVendor = buildInferencePricingMap(input.event);

  const vendorPlans: SourceBafoVendorNegotiationPlan[] = vendorResponses.map(
    (vendor) => {
      ...
      const recommendedAsks = buildRecommendedAsks(
        vendor,
        snapshot,
        input.event,
      );
  ...
  const readiness = toBafoReadiness(
    vendorPlans.map((vendorPlan) => vendorPlan.readiness),
  );
  const blockers = summarizeBlockers(vendorPlans);
  const commercialTrapSummary = buildCommercialTrapSummary(pricingSnapshots);
  const excludedScopeList = aggregateExclusions(
    vendorResponses,
    pricingSnapshots,
  );
  ...
  const assumptions = Array.from(
    new Set([
      ...ASSUMPTION_LOCKS,
      ...getSourceCorpusAssumptionLocks(input.event),
    ]),
  );
...
function tradeoffSummary(
  ...
  const ready = vendorPlans.filter(
    (vendor) => vendor.readiness === "ready",
  ).length;
  const blocked = vendorPlans.filter(
    (vendor) => vendor.readiness === "blocked",
  ).length;
  const compareRisk = snapshots.some((snapshot) =>
    snapshot.commercialTraps.some((trap) => trap.severity === "high"),
  );
  const evidenceWeak = vendorPlans.some(
    (vendor) => vendor.evidenceStatus === "Low Confidence",
  );
