...
export const AMS_MANAGED_SERVICES: SourceEventArchetype = {
  ...
  valueLeverRules: [
    { key: 'AMS.ENHANCEMENT_LEAKAGE', name: 'Enhancement / change-order leakage', category: 'scope_leakage', valueType: 'protected', capabilityRef: 'value_lever_map', whatToWatch: 'Recurring support work billed as enhancement or change-order rather than base run scope.', requiredEvidence: ['ticket_volumes', 'contract_baseline', 'service_tower_scope'], triggerLogic: 'Recurring ticket/enhancement categories appear outside the base run scope, or historical change-order spend is a material share of run cost.', valueBasis: 'Avoidable portion of recurring change-order / enhancement spend converted to base scope or a fixed service catalog.', computation: { inputs: [{ key: 'annual_change_order_spend', label: 'Annual change-order / enhancement spend', unit: 'usd_per_year', source: 'extracted_contract', citationRequired: true }, { key: 'recurring_avoidable_pct', label: 'Share of change-order spend that is recurring/avoidable', unit: 'pct', source: 'enterprise_inventory', citationRequired: true }, { key: 'term_years', label: 'Contract term (years)', unit: 'count', source: 'extracted_vendor', citationRequired: false }], method: 'recurring_avoidable_pct × annual_change_order_spend × term_years', formulaId: 'AVOIDABLE_SPEND_OVER_TERM', rangeMethod: 'Low/high from a conservative vs. observed recurring_avoidable_pct band; no point estimate.', onMissingEvidence: 'insufficient_evidence' }, defaultConfidence: 'med', rfpClause: 'Require vendors to classify recurring support vs enhancement and price a fixed service catalog / unit-rate catalog for recurring items.', evaluationImpact: 'Penalize vague enhancement exclusions and open-ended change-order language.', bafoAsk: 'Convert the recurring change-order categories into a fixed service catalog or capped unit-rate structure.', executiveImplication: 'Unresolved scope ambiguity is the most common source of post-award leakage on AMS deals.', commercialRisk: 'Claiming this value without the classified ticket/change-order data overstates a number the vendor will refute — cite the spend record or render it insufficient.' },
    { key: 'AMS.VOLUME_BAND_PRICING', name: 'Volume-band price flex-down', category: 'pricing', valueType: 'incremental_negotiated', capabilityRef: 'vendor_commercial_posture', whatToWatch: 'Resource-unit / FTE pricing that does not step down as ticket volume falls or automation removes work.', requiredEvidence: ['ticket_volumes', 'run_cost_baseline'], triggerLogic: 'Pricing is fixed-FTE or a single blended unit rate with no volume-band step-down clause.', valueBasis: 'Run-cost reduction from repricing to the actual lower volume band over the term (projected volume decline × unit rate delta).', computation: { inputs: [{ key: 'annual_run_cost', label: 'Annual run cost at current band', unit: 'usd_per_year', source: 'enterprise_inventory', citationRequired: true }, { key: 'projected_volume_decline_pct', label: 'Projected volume decline over term', unit: 'pct', source: 'enterprise_inventory', citationRequired: true }, { key: 'variable_cost_share_pct', label: 'Share of run cost that flexes with volume', unit: 'pct', source: 'extracted_vendor', citationRequired: true }, { key: 'term_years', label: 'Contract term (years)', unit: 'count', source: 'extracted_vendor', citationRequired: false }], method: 'annual_run_cost × variable_cost_share_pct × projected_volume_decline_pct, summed over term_years', formulaId: 'BAND_STEPDOWN_SAVINGS', rangeMethod: 'Low/high from conservative vs expected volume-decline band.', onMissingEvidence: 'insufficient_evidence' }, defaultConfidence: 'med', rfpClause: 'Require a resource-unit pricing schedule with explicit volume bands and step-down thresholds.', evaluationImpact: 'Disqualify pricing with no band step-down; score band granularity.', bafoAsk: 'Add volume-band step-down pricing so cost falls when volumes/automation reduce tickets.', executiveImplication: 'Without band flex-down, the buyer keeps paying peak-volume rates as demand declines.' },
    { key: 'AMS.PRODUCTIVITY_CREDITS', name: 'Productivity / automation not priced back', category: 'productivity', valueType: 'solution_tightening', capabilityRef: 'value_lever_map', whatToWatch: 'Automation or productivity improvements claimed but not converted into committed year-over-year credits.', requiredEvidence: ['ticket_volumes', 'tooling_landscape'], triggerLogic: 'Vendor asserts automation/productivity gains with no credit schedule or gainshare.', valueBasis: 'Committed productivity-credit percentage × the automatable ticket/effort pool per year.', computation: { inputs: [{ key: 'automatable_effort_pool', label: 'Annual automatable effort pool', unit: 'usd_per_year', source: 'should_cost', citationRequired: true }, { key: 'committed_credit_pct', label: 'Committed productivity-credit %', unit: 'pct', source: 'extracted_vendor', citationRequired: true }, { key: 'term_years', label: 'Contract term (years)', unit: 'count', source: 'extracted_vendor', citationRequired: false }], method: 'committed_credit_pct × automatable_effort_pool, ramped over term_years', formulaId: 'PRODUCTIVITY_CREDIT_POOL', rangeMethod: 'Low/high from vendor-committed vs benchmark credit %.', onMissingEvidence: 'insufficient_evidence' }, defaultConfidence: 'med', rfpClause: 'Require a year-by-year productivity/automation commitment with a credit schedule tied to ticket-volume / incident reduction.', evaluationImpact: 'Reward committed credits; treat uncredited claims as evaluation risk.', bafoAsk: 'Add a productivity-credit schedule tied to measured ticket-volume and incident reduction.', executiveImplication: 'Automation savings the vendor keeps are savings the buyer forfeits for the contract term.', commercialRisk: 'A vendor automation claim is not value until it is a credit schedule — count only the committed %, not the marketing claim.' },
    { key: 'AMS.RETAINED_COST', name: 'Retained client / SME cost omitted from TCO', category: 'retained_cost', valueType: 'risk_adjusted', capabilityRef: 'value_waterfall', whatToWatch: 'Governance, vendor-management, and SME effort the buyer keeps in-house, left out of the comparison.', requiredEvidence: ['retained_org_model', 'staffing_baseline'], triggerLogic: 'Proposals compare run cost without the retained-organization and client-side effort each model requires.', valueBasis: 'Difference in retained FTE effort between models, valued at loaded cost, plus reduction commitment over the term.', computation: { inputs: [{ key: 'retained_fte_delta', label: 'Retained FTE difference vs baseline model', unit: 'fte', source: 'extracted_vendor', citationRequired: true }, { key: 'loaded_fte_cost', label: 'Loaded annual cost per retained FTE', unit: 'usd_per_year', source: 'enterprise_inventory', citationRequired: true }, { key: 'term_years', label: 'Contract term (years)', unit: 'count', source: 'extracted_vendor', citationRequired: false }], method: 'retained_fte_delta × loaded_fte_cost × term_years (a TCO normalization, applied as a debit to over-cheap bids)', formulaId: 'RETAINED_EFFORT_DELTA', rangeMethod: 'Low/high from stated vs likely retained-effort delta.', onMissingEvidence: 'insufficient_evidence' }, defaultConfidence: 'low', rfpClause: 'Require vendors to state the retained client effort their model assumes and how it reduces over time.', evaluationImpact: 'Normalize all proposals to include retained cost before ranking on price.', bafoAsk: 'Commit to reducing retained client/SME effort over the term with named milestones.', executiveImplication: 'A vendor can look cheapest on paper while shifting hidden effort onto the buyer.', commercialRisk: 'This is a normalization, not a saving — presenting it as headline value double-counts. Apply it to make bids comparable.' },
    { key: 'AMS.SLA_ECONOMICS', name: 'SLA credit economics', category: 'sla_economics', valueType: 'protected', capabilityRef: 'value_lever_map', whatToWatch: 'SLA credits/remedies that are weak relative to the business criticality of the towers.', requiredEvidence: ['sla_baseline', 'incident_problem_change'], triggerLogic: 'Credit caps are low, or there is no chronic-miss remedy, versus the criticality in the incident/SLA baseline.', valueBasis: 'Value protected = expected credit pool + avoided business impact from chronic-miss remedies (protection, not headline savings).', computation: { inputs: [{ key: 'at_risk_fee_pool', label: 'Annual fee pool exposed to SLA credits', unit: 'usd_per_year', source: 'extracted_vendor', citationRequired: true }, { key: 'credit_cap_pct', label: 'SLA credit cap %', unit: 'pct', source: 'extracted_vendor', citationRequired: true }, { key: 'chronic_miss_rate', label: 'Historical chronic-miss rate', unit: 'pct', source: 'enterprise_inventory', citationRequired: true }], method: 'at_risk_fee_pool × credit_cap_pct × chronic_miss_rate (protected value, not a discount)', formulaId: 'SLA_CREDIT_PROTECTION', rangeMethod: 'Low/high from historical vs stressed miss rate.', onMissingEvidence: 'insufficient_evidence' }, defaultConfidence: 'low', rfpClause: 'Require an SLA/KPI schedule with credits scaled to criticality and a chronic-miss remedy.', evaluationImpact: 'Do not let a low headline price outrank a vendor with materially stronger SLA remedies once normalized.', bafoAsk: 'Increase the credit pool and add a chronic-miss remedy for the critical towers.', executiveImplication: 'Weak SLA economics convert vendor underperformance into the buyer’s operational and financial risk.', commercialRisk: 'Protected value is a risk hedge, not cash — never sum it into a headline savings figure.' },
    { key: 'AMS.TRANSITION_RISK', name: 'Transition fees not milestone-based', category: 'transition_risk', valueType: 'risk_adjusted', capabilityRef: 'vendor_commercial_posture', whatToWatch: 'Transition/KT priced as a lump sum with no fee-at-risk tied to milestones or acceptance.', requiredEvidence: ['transition_constraints'], triggerLogic: 'Transition is under-specified or fees are not tied to KT checkpoints, cutover acceptance, or milestone slippage.', valueBasis: 'Avoided failed/extended-transition cost = probability-weighted transition overrun × transition value at risk.', computation: { inputs: [{ key: 'transition_fee', label: 'Quoted transition fee', unit: 'usd', source: 'extracted_vendor', citationRequired: true }, { key: 'overrun_probability', label: 'Probability of transition overrun', unit: 'pct', source: 'benchmark', citationRequired: true }, { key: 'overrun_cost_multiple', label: 'Overrun cost as a multiple of transition fee', unit: 'ratio', source: 'benchmark', citationRequired: false }], method: 'transition_fee × overrun_cost_multiple × overrun_probability (expected avoided overrun)', formulaId: 'TRANSITION_RISK_EXPOSURE', rangeMethod: 'Low/high from benchmark overrun-probability band.', onMissingEvidence: 'insufficient_evidence' }, defaultConfidence: 'low', rfpClause: 'Require a milestone-based transition plan with KT evidence, cutover acceptance criteria, and fee-at-risk on slippage.', evaluationImpact: 'Do not award full transition score to a generic transition narrative.', bafoAsk: 'Tie transition fees to KT milestones and cutover acceptance, with remedies for slippage.', executiveImplication: 'Transition ambiguity commonly becomes retained-cost exposure and delayed value realization.' },
  ],
};
