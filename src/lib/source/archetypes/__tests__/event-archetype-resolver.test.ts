...
describe("event → archetype resolution (classifier category)", () => {
  ...
  it("resolves data_ai_platform → AI_DATA_PLATFORM and saas_renewal → CONTRACT_RENEWAL", () => {
    expect(
      resolveArchetypeForEvent({ categoryId: "data_ai_platform" }).archetypeId,
    ).toBe("AI_DATA_PLATFORM");
    expect(
      resolveArchetypeForEvent({ categoryId: "saas_renewal" }).archetypeId,
  ...
  it("resolves the newly-shipped archetype categories", () => {
    ...
    expect(
      resolveArchetypeForEvent({ categoryId: "staff_aug_vs_managed_service" })
        .archetypeId,
    ).toBe("STAFF_AUGMENTATION");
    expect(
    ...
    expect(
      resolveArchetypeForEvent({ categoryId: "bpo_contact_centre" })
        .archetypeId,
    ).toBe("CONTACT_CENTER_CX");
    expect(
      resolveArchetypeForEvent({ categoryId: "bpo_shared_services" })
        .archetypeId,
    ).toBe("BPO_SHARED_SERVICES");
  });
});

describe("event → archetype resolution (event_type fallback)", () => {
  it("routes coarse managed-service fallback to the managed-services playbook", () => {
    const r = resolveArchetypeForEvent({ eventType: "managed_service" });
    expect(r.resolved).toBe(true);
    expect(r.archetypeId).toBe("AMS_MANAGED_SERVICES");
    expect(r.archetype?.name).toMatch(/managed services/i);
    expect(r.source).toBe("event_type_fallback");
  });

  it("keeps renewal events on the renewal playbook when the governed category says renewal", () => {
    const r = resolveArchetypeForEvent({
      categoryId: "saas_renewal",
      eventType: "managed_service",
    });
    expect(r.resolved).toBe(true);
    expect(r.archetypeId).toBe("CONTRACT_RENEWAL");
    expect(r.archetype?.name).toBe("Contract Renewal / Renegotiation");
    expect(r.source).toBe("classifier_category");
  });
  ...
  it("refuses for an ambiguous/legacy event_type with no confident archetype", () => {
    expect(resolveArchetypeForEvent({ eventType: "other" }).resolved).toBe(
      false,
    );
    expect(resolveArchetypeForEvent({ eventType: "staffing" }).resolved).toBe(
      false,
    );
  });
