...
import { buildSourceEvidenceReadiness, type EvidenceStateMap } from '../evidence-readiness';
...
export function runSkyHarborAmsScenario(): SkyHarborScenarioResult {
  ...
  const sc = shouldCostModel(archetype, {
    components: [
      { value: 6_200_000, citation: 'ev:run_cost_baseline#labor', label: 'labor' },
      { value: 1_100_000, citation: 'ev:run_cost_baseline#tooling', label: 'tooling' },
      { value: 900_000, citation: 'ev:run_cost_baseline#shift', label: 'shift' },
    ],
  ...
  const tco = normalizeProposals(archetype, [
    { vendor: 'NorthOps', lines: [
      { value: 7_400_000, citation: 'p:northops#base', label: 'base' },
      { value: 700_000, citation: 'p:northops#transition', label: 'transition' },
    ] },
    { vendor: 'SkyManage', lines: [
  ...
  candidates.push(
    { ...agentReadyCandidate('vendor_proposal'), id: 'sky-northops', citations: ['p:northops#base', 'p:northops#transition'] },
    { ...agentReadyCandidate('vendor_proposal'), id: 'sky-skymanage', citations: ['p:skymanage#base'] },
  );
  // Inject a cross-tenant candidate to prove the fence holds end-to-end.
  candidates.push({ ...agentReadyCandidate('ticket_volumes'), id: 'leak', client_key: 'apex-retail' });
  ...
  const grounded = buildGroundedSourceAnswer({
    ...
    candidates,
    draftClaims: [
      { text: `Independent should-cost is $${(sc.totalShouldCost / 1e6).toFixed(1)}M/yr`, backedByCitation: 'ev:run_cost_baseline#skyharbor' },
      { text: 'On a normalized basis NorthOps is the true low bidder despite a lower SkyManage headline', backedByCitation: 'p:northops#base' },
    ],
  });
  ...
  const narrativeMarkdown = renderNarrative({
    ...
    scopeBlockers: scopeGate?.blockers.map((b) => `${b.family} (${b.reason})`) ?? [],
...
function renderNarrative(x: {
  ...
  L.push('');
  L.push(`**Archetype resolved:** ${x.archetypeName}  |  **Tenant:** ${x.envelope.tenantResolved}  |  **Confidence:** ${x.envelope.confidence}`);
  L.push('');
  L.push('## 1 · Evidence readiness');
  L.push(`Overall: **${x.readinessVerdict}**. Required families still missing: ${x.missingRequired.join(', ') || 'none'}.`);
  L.push(`Scope-gate blockers: ${x.scopeBlockers.join('; ') || 'none'}.`);
  L.push('');
  L.push('## 2 · Event-specific RFP');
  L.push(x.rfpMd);
  L.push('## 3 · Should-cost (independent baseline)');
  L.push(`Total: **${m(x.shouldCost.totalShouldCost)}/yr** (confidence ${x.shouldCost.confidence}; cited ${x.shouldCost.citations.join(', ')}).`);
  if (x.shouldCost.glidePathYear2) L.push(`Year-2 after 8% productivity glide-path: **${m(x.shouldCost.glidePathYear2)}**.`);
  L.push('');
  L.push('## 4 · Proposal TCO normalization');
  for (const v of x.tco.vendors) {
    L.push(`- **${v.vendor}** — stated ${m(v.statedTotal)}, normalized **${m(v.normalizedTotal)}**${v.excludedComponents.length ? ` (added back: ${v.excludedComponents.join(', ')})` : ''}`);
  }
  L.push(`On a true apples-to-apples basis the low bidder is **${x.tco.bestVendor}**.`);
  L.push('');
  ...
  L.push('```');
  if (x.envelope.unsupportedClaims.length === 0) L.push('_No unsupported claims. Cross-tenant evidence was fenced. Missing evidence is named, not hidden._');
  return L.join('\n');
}
