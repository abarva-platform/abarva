...
const evt = (over: Record<string, unknown>) => ({
  id: 'e1', event_code: 'SKYH-X-2026', event_name: 'Event', current_stage_key: 'strategy',
  lifecycle_state: 'active', estimated_value_usd: 11000000, ...over,
});
...
describe('buildApprovalsInbox', () => {
  it('lists pending intake approvals with a direct link to the approval page', () => {
    const inbox = buildApprovalsInbox({
      pendingEvents: [evt({ id: 'p1', lifecycle_state: 'waiting_on_client', event_name: 'AMS Consolidation' })],
      activeEvents: [],
      criterionRows: [],
    });
