...
const baseRow: SourceEventRow = {
  ...
  trigger_description:
    "Second attempt at AMS consolidation after prior sourcing failure.",
...
describe("Source persisted event row mapping", () => {
  ...
  it("coerces pg NUMERIC strings before computing value fields", () => {
    const summary = sourceEventRowToSummary(
      {
        ...baseRow,
    ...
    expect(summary.projectedValueUsd).toBe(35_000_000);
  });
  ...
  it("maps a persisted source_events row into a full event canvas detail", () => {
    ...
    expect(detail.stages.find((stage) => stage.key === "scope")?.status).toBe(
      "active",
    );
  ...
  it("marks the current stage as needing approval when the event is waiting on client approval", () => {
    ...
    expect(
      detail.alerts.some((alert) => alert.id.endsWith(":approval-needed")),
    ).toBe(true);
  });
});
