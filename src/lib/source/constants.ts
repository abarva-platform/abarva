...
export const SOURCE_LEGACY_STAGE_ALIASES: Partial<
  Record<SourceStageKey | string, SourceStageKey>
> = {
  intake: "strategy",
  sourcing_strategy: "strategy",
  rfp_rfi_package: "rfp",
...
export function isCanonicalSourceStageKey(
  value: unknown,
): value is SourceStageKey {
  return (
    typeof value === "string" &&
    (SOURCE_STAGE_ORDER as readonly string[]).includes(value)
  );
}
...
export interface SourceStagePresentation {
  key: SourceStageKey;
  number: number;
  label: string;
}

export function sourceStagePresentationFor(
  stageKey: unknown,
): SourceStagePresentation {
  const key = normalizeSourceStageKey(stageKey) ?? "strategy";
  const index = SOURCE_STAGE_ORDER.indexOf(key);
  return {
    key,
...
export const SOURCE_LIFECYCLE_STATUS_LABELS: Record<
  SourceLifecycleStatus,
  string
> = {
  active: "Active",
...
export const SOURCE_ARTIFACT_STATUS_LABELS: Record<
  SourceArtifactStatus,
  string
> = {
  not_started: "Not Started",
...
export const SOURCE_SCORECARD_LIFECYCLE_LABELS: Record<
  ScorecardLifecycleState,
  string
> = {
...
export const SOURCE_FOUNDATIONS: SourcePlatformFoundation[] = [
  {
    ...
    summary:
      "Tenant-scoped patterns, evidence, and applicability signals that ground sourcing decisions.",
  },
  {
    key: "agent_fabric",
    label: "Agent Fabric",
    summary:
      "Sentinel-led workflow orchestration with specialist rails and cited, fail-honest responses.",
  },
  {
    key: "artifact_studio",
    label: "Artifact Studio",
    summary:
      "Structured scorecards, decision packets, and approval-grade artifacts generated from the event spine.",
  },
  {
    key: "control_tower",
    label: "Control Tower",
    summary:
      "Portfolio pressure, adoption, risk, cost, and contradiction context available to every event.",
  },
  {
    key: "value_ledger",
    label: "Value Ledger",
    summary:
      "Projected and realized value tracked as first-class records, not chat-only narrative.",
  },
];
...
export const SOURCE_REUSE_MAP = {
  ...
  agentRails: ["NexusProgramRail.tsx", "AtlasRail.tsx", "NexusPanel.tsx"],
  workspace: ["ModuleWorkspace.tsx", "EngagementConsole.tsx"],
  evidence: [
  ...
  workflow: ["types.db.ts", "types.ui.ts", "governance.ts", "quality-gates.ts"],
} as const;
