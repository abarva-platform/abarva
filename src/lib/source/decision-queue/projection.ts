...
function bool(payload: Record<string, unknown>, ...keys: string[]): boolean {
  for (const key of keys) {
    ...
    if (typeof v === 'string') {
      ...
      if (['true', 'yes', 'y', 'auto', 'auto-renew'].includes(lower)) return true;
...
function isoDate(payload: Record<string, unknown>, ...keys: string[]): string | null {
...
function rate01(payload: Record<string, unknown>, ...keys: string[]): number | null {
...
export function projectVendorContract(
  ...
  const vendorName =
    str(p, 'vendor_name', 'vendor', 'supplier', 'supplier_name') ?? record.title;
  const product =
...
export function projectFinancialLine(
  ...
  const category =
    str(p, 'category', 'capability', 'domain', 'cost_category', 'spend_category') ??
    record.title;
  return {
