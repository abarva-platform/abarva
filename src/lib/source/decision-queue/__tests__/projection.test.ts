...
function raw(payload: Record<string, unknown>, overrides: Partial<RawTenantRecord> = {}): RawTenantRecord {
  return {
    recordId: 'vc:test',
...
describe('projectVendorContract', () => {
  ...
  it('accepts utilization given as a 0..100 percentage', () => {
    const projected = projectVendorContract(raw({ vendor_name: 'V', utilization_rate: 42 }));
    expect(projected!.utilizationRate).toBeCloseTo(0.42);
  });

  it('accepts alternate field names (renewal_or_expiry, auto_renewal)', () => {
    const projected = projectVendorContract(
      raw({ vendor: 'V', renewal_or_expiry: '2026-09-01', auto_renewal: 'yes' }),
    );
  ...
  it('leaves unparseable monetary fields as null rather than guessing', () => {
    const projected = projectVendorContract(raw({ vendor_name: 'V', annual_spend_usd: 'tbd' }));
...
describe('projectFinancialLine', () => {
  ...
  it('returns null when neither a budget nor a benchmark is present', () => {
    const projected = projectFinancialLine(raw({ category: 'crm' }, { recordId: 'fin:empty' }));
    expect(projected).toBeNull();
  });
});
