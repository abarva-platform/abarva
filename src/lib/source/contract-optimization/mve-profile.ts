...
function staffingGap(input: ContractOptimizationInput): ContractOptimizationFinding | null {
  ...
  const committedFte = input.staffingCommitments.reduce((sum, row) => sum + row.committedFte, 0);
  const observedFte = input.staffingCommitments.reduce((sum, row) => sum + row.observedFte, 0);
...
function changeOrderExposure(input: ContractOptimizationInput): ContractOptimizationFinding | null {
  const risky = input.changeOrders.filter(
    (line) => !line.catalogMapped || line.approvalEvidence !== "complete" || line.recurring,
  );
  ...
  return {
    ...
    evidenceLabels: [...new Set(risky.map((line) => evidenceLabel(input, line.evidenceId)))],
    severity: exposure > input.currentAnnualRunRateUsd * 0.03 ? "high" : "medium",
  ...
  return findings.map((finding, index) => {
    ...
    if (finding.category === "scope_change_order_exposure") {
      return {
        ...
        valueBasis: finding.estimatedAnnualImpactUsd ? "evidenced" : "opportunity_to_test",
        annualImpactLowUsd: finding.estimatedAnnualImpactUsd ? Math.round(finding.estimatedAnnualImpactUsd * 0.4) : null,
        annualImpactHighUsd: finding.estimatedAnnualImpactUsd,
      };
    }
    return {
...
export function buildContractOptimizationMveProfile(
  ...
  const clientToComplete = [
    ...
    input.changeOrders.length ? null : "Load change-order, amendment, and approval history.",
  ...
  return {
    ...
    minimumViableExtractionAreas: [
      ...
      area("Change orders and amendments", [...new Set(input.changeOrders.map((line) => evidenceLabel(input, line.evidenceId)))], "Separates approved scope growth from leakage, recurring exceptions, and renewal baseline creep."),
...
export function buildSkyHarborAmsExistingContractInput(
  overrides: Partial<Pick<ContractOptimizationInput, "sourceEventId" | "tenantKey">> = {},
): ContractOptimizationInput {
  return {
    ...
    evidenceRefs: [
      ...
      { evidenceId: "E-CHANGE", role: "change_order_log", title: "Change-order and amendment ledger", sourceType: "synthetic_demo", reference: "Change-order ledger FY26 rows 1-9", syntheticDemoLabel: "Synthetic demo evidence" },
      { evidenceId: "E-GOV", role: "governance_minutes", title: "Supplier QBR and cure-item minutes", sourceType: "synthetic_demo", reference: "QBR minutes FY26 Q2-Q4", syntheticDemoLabel: "Synthetic demo evidence" },
    ],
    terms: [
    ...
    changeOrders: [
      { requestId: "CO-0261", category: "airport operations app hypercare", amountUsd: 186_000, recurring: false, catalogMapped: true, approvalEvidence: "complete", evidenceId: "E-CHANGE" },
      { requestId: "CO-0274", category: "premium weekend coverage", amountUsd: 312_000, recurring: true, catalogMapped: false, approvalEvidence: "partial", evidenceId: "E-CHANGE" },
      { requestId: "CO-0288", category: "integration incident bridge support", amountUsd: 428_000, recurring: true, catalogMapped: false, approvalEvidence: "partial", evidenceId: "E-CHANGE" },
      { requestId: "CO-0302", category: "finance close stabilization", amountUsd: 144_000, recurring: false, catalogMapped: false, approvalEvidence: "missing", evidenceId: "E-CHANGE" },
      { requestId: "CO-0310", category: "tooling pass-through uplift", amountUsd: 268_000, recurring: true, catalogMapped: false, approvalEvidence: "partial", evidenceId: "E-CHANGE" },
    ],
  };
}
