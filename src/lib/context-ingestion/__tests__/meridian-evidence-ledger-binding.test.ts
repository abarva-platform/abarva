...
describe("Meridian evidence ledger binding", () => {
  it("maps evidence-register rows into citation-grade evidence ledger inputs", () => {
    const inputs = buildMeridianEvidenceLedgerInputs({
      ...
      rows: [
        {
          citation_key: "Meridian-STARS-2026",
    ...
    expect(inputs).toEqual([
      expect.objectContaining({
        ...
        sourceRef: expect.objectContaining({
          citation_key: "Meridian-STARS-2026",
          source_doc: "meridian-evidence-register.csv",
          source_row: 2,
          usable_by_surface: ["moves", "admin", "tower"],
        }),
      }),
