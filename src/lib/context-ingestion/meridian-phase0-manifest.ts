export type MeridianManifestSeverity = "error" | "warning";

export type MeridianManifestObjectType =
  | "evidence_item"
  | "uploaded_artifact"
  | "workload_record"
  | "rate_card_row"
...
export interface MeridianManifestIssue {
  severity: MeridianManifestSeverity;
  objectType: MeridianManifestObjectType | "manifest";
  objectId: string | null;
  field: string;
  message: string;
}

export interface MeridianEvidenceItem {
  citationKey: string;
  title: string;
...
export interface MeridianUploadedArtifact {
  artifactId: string;
  displayName: string;
  artifactType: string;
  phase: string;
  ...
  sensitivity: MeridianEvidenceItem["sensitivity"];
  sourceEvidenceIds: string[];
}

export interface MeridianWorkloadRecord {
  workloadId: string;
  workloadName: string;
  domain: string;
...
export interface MeridianRateCardRow {
  rateCardId: string;
  role: string;
...
export interface MeridianGateCriterion {
  gateId: string;
  phase: string;
  criterion: string;
...
export interface MeridianApprovalRecord {
  approvalId: string;
  artifactId: string;
  approverName: string;
  role: string;
...
export interface MeridianPhase0Manifest {
  manifestId: string;
  tenantKey: string;
  clientName: string;
  generatedAt: string;
  evidenceItems: MeridianEvidenceItem[];
  uploadedArtifacts: MeridianUploadedArtifact[];
  workloadRecords: MeridianWorkloadRecord[];
  rateCardRows: MeridianRateCardRow[];
  gateCriteria: MeridianGateCriterion[];
  approvalRecords: MeridianApprovalRecord[];
}

export interface MeridianManifestValidationResult {
  valid: boolean;
  readyForStageAdvance: boolean;
  issues: MeridianManifestIssue[];
  counts: Record<MeridianManifestObjectType, number>;
}
...
function pushRequiredStringIssue(
  issues: MeridianManifestIssue[],
  objectType: MeridianManifestIssue["objectType"],
  objectId: string | null,
  field: string,
  value: unknown,
): void {
...
function addCountIssues(
  issues: MeridianManifestIssue[],
  manifest: MeridianPhase0Manifest,
): void {
  const required: Array<[MeridianManifestObjectType, number]> = [
  ...
  for (const [objectType, count] of required) {
    ...
    issues.push({
      ...
      objectId: null,
      field: objectType,
      message: `At least one ${objectType} is required before the Meridian demo can advance beyond setup.`,
    });
  }
}

export function validateMeridianPhase0Manifest(
  manifest: MeridianPhase0Manifest,
): MeridianManifestValidationResult {
  const issues: MeridianManifestIssue[] = [];
  ...
  for (const rate of manifest.rateCardRows) {
    ...
    if (rate.utilizationAssumption <= 0 || rate.utilizationAssumption > 1) {
      issues.push({
        ...
        message:
          "utilizationAssumption must be greater than zero and no more than one.",
      });
    }
  }
