import type { ContextDimension, ContextDimensionFamily, UploadedFileFormat } from './types';
...
export const UNIVERSAL_CONTEXT_TEMPLATES: ContextTemplateDefinition[] = [
  // ── Family 1 — Enterprise & Operating Model ──────────────────────────────
  {
    ...
    requiredFields: ['tenant_key', 'company_name', 'revenue_usd', 'employees', 'fiscal_year_end'],
    optionalFields: ['countries', 'business_units', 'it_budget_usd', 'debt_usd', 'industry_code'],
  ...
  },
  {
    ...
    requiredFields: ['function_id', 'function_name', 'executive_owner_role', 'head_count'],
    optionalFields: ['cost_center', 'p_and_l_owner', 'revenue_attributed_usd', 'ai_maturity'],
  ...
  },
  {
    ...
    acceptedFormats: ['csv', 'xlsx', 'json'],
    requiredFields: ['team_id', 'team_name', 'it_owner_role', 'domain', 'head_count'],
    optionalFields: ['budget_usd', 'onshore_pct', 'offshore_pct', 'gcc_location', 'vendor_dependency'],
    ownerRole: 'CIO',
    refreshCadence: 'quarterly',
    loadOrder: 2,
    unlocks: UNIVERSAL_UNLOCKS,
  },
  {
    ...
    requiredFields: ['capability_id', 'capability_name', 'business_function', 'maturity_level'],
    optionalFields: ['value_stream', 'owner_role', 'key_system_ids', 'ai_opportunity'],
  ...
  // ── Family 2 — Technology Estate ─────────────────────────────────────────
  {
    ...
    acceptedFormats: ['csv', 'xlsx'],
    requiredFields: ['app_id', 'name', 'vendor', 'category', 'criticality', 'it_owner_team', 'lifecycle_stage'],
    optionalFields: ['run_cost_usd', 'deployment', 'primary_dataclass', 'integration_count', 'ai_eligibility_score', 'notes'],
    ownerRole: 'VP Enterprise Architecture',
  ...
  },
  {
    ...
    acceptedFormats: ['csv', 'xlsx', 'json'],
    requiredFields: ['mapping_id', 'app_id', 'function_id', 'support_type', 'primary_flag'],
    optionalFields: ['module', 'user_count', 'process_area', 'criticality_override'],
    ownerRole: 'VP Enterprise Architecture',
    refreshCadence: 'monthly',
    loadOrder: 4,
    unlocks: ['System-to-function graph edges become traversable', ...UNIVERSAL_UNLOCKS],
  },
  {
    ...
    acceptedFormats: ['csv', 'xlsx', 'json'],
    requiredFields: ['resource_id', 'resource_type', 'provider', 'region', 'criticality', 'owner_team'],
    optionalFields: ['monthly_cost_usd', 'environment', 'retirement_date', 'dr_tier', 'compliance_scope'],
  ...
  },
  {
    ...
    optionalFields: ['peak_value', 'growth_rate_pct', 'capacity_pct', 'threshold_alert'],
  ...
  // ── Family 3 — Data & Connectivity ───────────────────────────────────────
  {
    ...
    acceptedFormats: ['csv', 'xlsx'],
    requiredFields: ['data_product_id', 'name', 'source_system', 'owner_role', 'data_class', 'refresh_sla'],
    optionalFields: ['phi_flag', 'pii_flag', 'quality_score', 'consumers', 'lineage_documented'],
  ...
  },
  {
    ...
    requiredFields: ['edge_id', 'source_app_id', 'target_app_id', 'integration_type', 'direction'],
    optionalFields: ['latency_sla', 'daily_volume', 'failure_impact', 'kill_blocker_flag', 'middleware'],
    ownerRole: 'VP Enterprise Architecture',
    refreshCadence: 'monthly',
    loadOrder: 5,
    unlocks: ['Integration topology graph edges become traversable', ...UNIVERSAL_UNLOCKS],
  },
  // ── Family 4 — Financial & Commercial ────────────────────────────────────
  {
    ...
    acceptedFormats: ['csv', 'xlsx'],
    requiredFields: ['vendor_id', 'vendor_name', 'contract_id', 'annual_value_usd', 'renewal_date', 'primary_system_id'],
    optionalFields: ['auto_renew', 'notice_days', 'exit_terms', 'ai_clauses', 'data_rights', 'benchmark_present'],
    ownerRole: 'VP Procurement',
    refreshCadence: 'monthly',
    loadOrder: 6,
    unlocks: ['Vendor-to-system edges enable renewal risk traversal', ...UNIVERSAL_UNLOCKS],
  },
  {
    ...
    acceptedFormats: ['csv', 'xlsx'],
    requiredFields: ['budget_line_id', 'category', 'owner_team', 'annual_budget_usd', 'fiscal_year'],
    optionalFields: ['capex_usd', 'opex_usd', 'labor_usd', 'vendor_usd', 'actuals_usd', 'variance_pct'],
    ownerRole: 'CFO / CIO',
    refreshCadence: 'quarterly',
    loadOrder: 6,
    unlocks: UNIVERSAL_UNLOCKS,
  ...
  // ── Family 5 — Execution & Operations ────────────────────────────────────
  {
    ...
    acceptedFormats: ['csv', 'xlsx', 'json'],
    requiredFields: ['initiative_id', 'title', 'status', 'sponsor_role', 'committed_usd', 'posture'],
    optionalFields: ['projected_value_usd', 'linked_app_ids', 'target_date', 'phase', 'vendor', 'benefit_claim'],
  ...
  },
  {
    ...
    requiredFields: ['record_id', 'record_type', 'system_id', 'severity', 'opened_at'],
    optionalFields: ['closed_at', 'root_cause', 'mttr_hours', 'sla_breached', 'customer_impact'],
  ...
  },
  {
    ...
    acceptedFormats: ['csv', 'xlsx'],
    requiredFields: ['kpi_id', 'kpi_name', 'owner_role', 'baseline_value', 'current_value', 'target_value', 'unit'],
    optionalFields: ['initiative_id', 'measurement_date', 'evidence_source', 'confidence', 'ai_attribution'],
    ownerRole: 'CFO / CIO',
    refreshCadence: 'monthly',
    loadOrder: 8,
    unlocks: UNIVERSAL_UNLOCKS,
  ...
  // ── Family 6 — Governance, AI & Evidence ─────────────────────────────────
  {
    ...
    acceptedFormats: ['csv', 'xlsx'],
    requiredFields: ['control_id', 'control_area', 'system_id', 'status', 'owner_role'],
    optionalFields: ['regulation', 'last_audit_date', 'finding_count', 'remediation_date', 'examiner_note'],
  ...
  },
  {
    ...
    acceptedFormats: ['csv', 'xlsx'],
    requiredFields: ['tool_id', 'tool_name', 'vendor', 'owner_role', 'workflow', 'risk_classification'],
    optionalFields: ['model_name', 'regulated_workflow_flag', 'monthly_spend_usd', 'active_users', 'approval_status'],
  ...
  // ── Dimension 19 — Personas & Workforce ──────────────────────────────────
  {
    ...
    acceptedFormats: ['csv', 'xlsx'],
    requiredFields: ['persona_id', 'persona_name', 'function_id', 'head_count', 'primary_tools'],
    optionalFields: ['ai_adoption_pct', 'time_on_ai_tasks_pct', 'baseline_throughput', 'current_throughput', 'skill_level', 'work_pattern'],
    ownerRole: 'CHRO / CIO',
    refreshCadence: 'quarterly',
    loadOrder: 2,
    unlocks: [
...
export const MERIDIAN_CONTEXT_TEMPLATES: ContextTemplateDefinition[] = [
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['app_id', 'name', 'vendor', 'category', 'clinical_criticality', 'run_cost_fy25_usd'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['module', 'owning_service_line', 'optimization_state', 'workflow_risk'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['payer', 'service_line', 'volume', 'denial_rate_pct', 'automation_readiness'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['denial_category', 'payer', 'monthly_cases', 'recoverable_usd', 'root_cause'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['specialty', 'vendor', 'clinicians', 'note_time_delta_min', 'adoption_pct'],
    optionalFields: [],
    ownerRole: 'CMIO',
    refreshCadence: 'weekly',
    unlocks: MERIDIAN_UNLOCKS,
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['model_id', 'use_case', 'risk_class', 'clinical_owner', 'validation_status'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['control_id', 'control_area', 'system', 'phi_touch', 'status'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['vendor', 'contract_id', 'baa_status', 'ai_clause_status', 'renewal_date'],
    optionalFields: [],
    ownerRole: 'VP Procurement',
    refreshCadence: 'monthly',
    unlocks: MERIDIAN_UNLOCKS,
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['service_line', 'net_revenue_usd', 'direct_margin_pct', 'avoidable_cost_usd'],
    optionalFields: [],
    ownerRole: 'CFO FP&A',
    refreshCadence: 'monthly',
    unlocks: MERIDIAN_UNLOCKS,
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['unit', 'open_shifts', 'premium_labor_usd', 'forecast_accuracy_pct'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['queue', 'call_volume', 'abandon_rate_pct', 'digital_deflection_pct'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['modality', 'algorithm', 'study_volume', 'turnaround_delta_min', 'safety_review_state'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['requirement', 'system', 'deadline', 'readiness_state', 'blocker'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['team', 'deploy_frequency', 'lead_time_hours', 'change_failure_rate_pct'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['event_id', 'system', 'severity', 'clinical_impact', 'root_cause'],
    optionalFields: [],
    ownerRole: 'VP IT Operations',
    refreshCadence: 'weekly',
    unlocks: MERIDIAN_UNLOCKS,
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['contract', 'covered_lives', 'risk_pool_usd', 'quality_gap', 'ai_use_case'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['panel', 'patients', 'risk_signal', 'intervention_owner', 'outcome_metric'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['data_product', 'source_system', 'phi_class', 'refresh_sla', 'quality_score'],
    optionalFields: [],
    ownerRole: 'Chief Data Officer',
    refreshCadence: 'monthly',
    unlocks: MERIDIAN_UNLOCKS,
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['journey', 'channel', 'conversion_rate_pct', 'dropoff_reason', 'owner'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['category', 'annual_spend_usd', 'shortage_risk', 'ai_opportunity', 'owner'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['decision_id', 'use_case', 'decision', 'conditions', 'review_date'],
    optionalFields: [],
  ...
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['system', 'downtime_tier', 'rto_hours', 'clinical_workaround', 'last_drill_date'],
    optionalFields: [],
    ownerRole: 'CISO',
    refreshCadence: 'quarterly',
    unlocks: MERIDIAN_UNLOCKS,
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['unit', 'patient_days', 'acuity_index', 'nurse_hours_per_patient_day', 'burnout_signal'],
    optionalFields: [],
    ownerRole: 'Chief Nursing Officer',
    refreshCadence: 'weekly',
    unlocks: MERIDIAN_UNLOCKS,
  },
  {
    ...
    acceptedFormats: ['csv'],
    requiredFields: ['tool', 'owner', 'workflow', 'phi_access', 'monthly_spend_usd'],
    optionalFields: [],
    ownerRole: 'AI Governance Lead',
    refreshCadence: 'monthly',
    unlocks: MERIDIAN_UNLOCKS,
...
export function getTemplatesForTenant(tenantKey?: string | null): ContextTemplateDefinition[] {
...
export function getTemplatesForTenantWithFallback(tenantKey?: string | null): ContextTemplateDefinition[] {
...
export function getTemplateById(
  ...
  return tenantTemplates.find((template) => template.id === id)
...
export function getTemplateForDimension(
  ...
  const tenantTemplates = getTemplatesForTenant(options.tenantKey);
  return tenantTemplates.find((template) => template.dimension === dimension)
    ?? NORTHSTAR_CONTEXT_TEMPLATES.find((template) => template.dimension === dimension)
    ?? MERIDIAN_CONTEXT_TEMPLATES.find((template) => template.dimension === dimension)
    ?? null;
}
