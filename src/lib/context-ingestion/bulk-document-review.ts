...
export async function readBulkDocumentReviewArtifact(args: {
  ...
  const bytes = await getObjectStorageAdapter().download(args.bucket, args.path);
  const parsed = JSON.parse(bytes.toString("utf8")) as BulkDocumentReviewArtifact;
  if (
...
export async function commitApprovedBulkDocumentReviewArtifact(args: {
  ...
  const approvedCandidates = args.artifact.candidates.filter(
    (candidate) => approvedIds.has(candidate.candidateId) && !rejectedIds.has(candidate.candidateId),
  );
  ...
  if (error) throw new Error(`bulk_document_review_commit_failed:${error.message}`);
