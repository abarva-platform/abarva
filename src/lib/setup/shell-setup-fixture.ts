...
const SETUP_CONNECTOR_DETAILS: ConnectorDetail[] = [
  ...
  },
  {
    ...
    status: 'disconnected',
    statusNote: 'Blocked until Setup W4 live OAuth boundary',
    vendor: 'Microsoft',
    authType: 'OAuth 2.0',
    authMethod: 'oauth',
    endpoint: 'https://graph.microsoft.com/v1.0',
    connectorType: 'Productivity',
    description: 'Microsoft Graph contract for users, groups, calendar, mail metadata, and usage-report evidence. This W3 fixture is not live.',
    errorCode: 'GRAPH_OAUTH_NOT_CONFIGURED',
    errorMessage: 'Microsoft Graph configure/test is blocked until Setup W4 implements tenant admin consent, token storage, and steward validation.',
    errorTime: 'Apr 26 · 12:00 UTC',
    syncFrequency: 'Blocked until W4',
    dataFlows: [
      { direction: 'inbound', description: 'Users and groups -> Access evidence', lastSynced: 'Not configured' },
      { direction: 'inbound', description: 'Calendar metadata -> Program operating cadence', lastSynced: 'Not configured' },
      { direction: 'inbound', description: 'Mail metadata and usage reports -> Governance review triggers', lastSynced: 'Not configured' },
    ],
    agentQuote: 'Microsoft Graph is registered as a W3 contract stub only. Required scopes are documented and masked, but no OAuth grant, token, or Graph pull exists.',
    actions: [
      { letter: 'A', text: 'Review required scopes', detail: 'Masked W3 contract: User.Read.All, Group.Read.All, Directory.Read.All, Calendars.Read, Mail.ReadBasic.All, Reports.Read.All' },
      { letter: 'B', text: 'Configure Graph OAuth', detail: 'Blocked until Setup W4 owns admin consent and token storage' },
      { letter: 'C', text: 'Test Graph connection', detail: 'Blocked until Setup W4 provides live OAuth and steward validation' },
    ],
    reconnectProfile: {
      summary: 'Microsoft Graph OAuth is explicitly out of scope for Setup W3. This surface records the W4 boundary and does not open a live consent flow.',
      estimate: 'Blocked until Setup W4',
      callToAction: 'Review W4 OAuth boundary',
      successMessage: 'Graph boundary reviewed. Live Microsoft Graph verification remains blocked until W4.',
      steps: [
        'Confirm tenant admin owner and least-privilege scope set',
        'Document app registration, redirect URI, and consent requirements',
        'Prepare secure token storage and audit-event requirements for W4',
        'Run live Graph configure/test only after W4 implements the OAuth boundary',
      ],
    },
  },
  {
...
export const SETUP_INDEX_VIEW = {
  ...
  agentQuote:
    '10 typed connector profiles are registered. ServiceNow is the only degraded connector. Microsoft Graph, PostgreSQL, and Anthropic remain disconnected, and every card now calls out whether the profile is seeded or live.',
  ...
  actions: [
    ...
    {
      letter: 'B' as const,
      text: 'Review Microsoft Graph W4 boundary',
      detail: 'Graph configure/test is blocked until live OAuth is explicitly implemented',
    },
    {
      letter: 'C' as const,
