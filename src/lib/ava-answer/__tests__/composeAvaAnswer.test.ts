...
describe("composeAvaAnswer", () => {
  ...
  it("shapes Home KNOW bridge prose for legacy matrix consumers without changing directAnswer", () => {
    const prose =
      "Apex Retail Group's data analytics context is strong enough to answer as a current-state dossier, not a fragment lookup. Applications/systems in dossier: 170; Vendors/contracts in dossier: 100; Integrations/interfaces in dossier: 260; Data products/platform records in dossier: 125; Operational/service signals in dossier: 12. Operationally, the useful signal is how the primary binder connects to adjacent dimensions: relationship paths are thin, so the safe answer should emphasize loaded facts and call out the missing joins. No blocking gap is visible in the assembled dossier, though final operating choices still need client validation.";
    const answer = composeHomeKnowAvaAnswer({
      mode: "KNOW",
      tenantKey: "apex-retail",
      question: "What is loaded?",
      intent: "lookup",
      ...
      prose,
      dimensionsUsed: ["data_analytics_estate"],
      facts: [],
      tables: [
        {
          id: "coverage-table",
          ...
          columns: [{ key: "dimension", label: "Dimension" }],
          rows: [{ dimension: "Data & Analytics Estate" }],
          citationIds: ["c1"],
        },
      ],
      charts: [],
      ...
      citations: [
        {
          ...
          excerpt: "Loaded context coverage",
          confidence: "high",
        },
      ],
      handoff: null,
    ...
    expect(answer.directAnswer).toBe(prose);
    expect(answer.prose?.split(/\n{2,}/).length).toBeGreaterThanOrEqual(3);
