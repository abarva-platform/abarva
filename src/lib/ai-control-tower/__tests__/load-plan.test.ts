...
describe("AI Control Tower load plan", () => {
  it("maps the canonical template package into substrate rows and context rows", () => {
    ...
    expect(
      plan.diagnostics.filter((diagnostic) => diagnostic.level === "error"),
    ).toEqual([]);
    ...
    expect(plan.rowCounts.ai_control_sources).toBe(2);
    expect(plan.rowCounts.ai_control_initiatives).toBe(1);
    expect(plan.rowCounts.ai_control_context_facts).toBe(2);
    expect(plan.sourceKeyById["SRC-M365-COPILOT-MAY"]).toBe(
      "SRC-M365-COPILOT-MAY",
    );
  });
  ...
  it("derives actions from evidence instead of requiring an action sheet", () => {
    ...
    expect(
      plan.derivedActions.every(
        (action) => action.payload.source === "system_derived",
      ),
    ).toBe(true);
    expect(
      plan.derivedActions.some(
        (action) =>
          action.payload.derivation_rule === "risk_severity_or_governance_gate",
      ),
    ).toBe(true);
    expect(
      plan.derivedActions.some(
        (action) =>
          action.payload.derivation_rule ===
          "benefit_not_realized_or_low_confidence",
      ),
    ).toBe(true);
    expect(
      plan.derivedActions.some(
        (action) =>
          action.payload.derivation_rule === "low_adoption_with_material_spend",
      ),
    ).toBe(true);
    expect(
      plan.derivedActions.some(
        (action) => action.payload.derivation_rule === "renewal_inside_90_days",
      ),
    ).toBe(true);
    expect(plan.tableRows.ai_control_actions).toHaveLength(
      plan.derivedActions.length,
    );
  });
  ...
  it("keeps missing required source data as a parse diagnostic, not a false load claim", () => {
    ...
    expect(
      plan.diagnostics.some(
        (diagnostic) => diagnostic.code === "missing_required_sheet_rows",
      ),
    ).toBe(true);
