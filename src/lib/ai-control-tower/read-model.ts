...
function normalizeFunctionName(value: unknown): string {
  ...
  const functionLabels: Record<string, string> = {
    'FCF-BF-RETAIL': 'Retail Banking',
    'FCF-BF-COMMBK': 'Commercial Banking',
    'FCF-BF-OPS': 'Operations',
    'FCF-BF-RISK': 'Risk & Compliance',
    'FCF-BF-DIGITAL': 'Digital Banking',
    'FCF-BF-WEALTH': 'Wealth & Private Banking',
    'FCF-BF-TECH': 'Technology',
    'FCF-BF-FINANCE': 'Finance & Accounting',
    'FCF-BF-HR': 'Human Resources',
    'FCF-BF-TREASURY': 'Treasury',
  };
  const label = functionLabels[raw.toUpperCase()];
  ...
  if (cleaned === 'Hr') return 'Human Resources';
  if (cleaned === 'Commbk') return 'Commercial Banking';
  if (cleaned === 'Digital') return 'Digital Banking';
  if (cleaned === 'Retail') return 'Retail Banking';
  if (cleaned === 'Wealth') return 'Wealth & Private Banking';
...
function evidenceState(value: unknown): AiControlTowerEvidenceStatus {
  ...
  if (state === 'missing' || state === 'rejected' || state === 'none' || state === 'no_evidence') return 'missing';
  return 'committed';
}
...
async function readFirstCapitalFallbackRows(): Promise<AiControlTowerReadRows> {
  try {
    const [
      ...
      parseCsv('datasets/first-capital-financial-synthetic-v2/family-5-execution-operations/F13_initiatives-portfolio.csv'),
      ...
      parseCsv('datasets/first-capital-financial-synthetic-v2/family-2-technology-estate/F05_applications-systems.csv'),
    ]);

    const appsById = firstByKey(apps, 'app_id');
    const appDetailsFor = (value: unknown): JsonRecord[] =>
      arrayFromUnknown(value)
        .map((appId) => appsById.get(appId))
        .filter((row): row is JsonRecord => Boolean(row));
    const joinedAppNames = (appRows: JsonRecord[]): string =>
      [...new Set(appRows.map((app) => text(app.name)).filter(Boolean))].join(' | ');
    const joinedAppVendors = (appRows: JsonRecord[]): string =>
      [...new Set(appRows.map((app) => text(app.vendor)).filter(Boolean))].join(' | ');
    const functionFromApps = (appRows: JsonRecord[]): string =>
      normalizeFunctionName(appRows.find((app) => text(app.business_function))?.business_function);

    const initiativeRows = initiatives.map((row) => ({
      ...(() => {
        const linkedApps = appDetailsFor(row.linked_app_ids);
        return {
          appRows: linkedApps,
          appVendor: joinedAppVendors(linkedApps),
          appNames: joinedAppNames(linkedApps),
          appFunction: functionFromApps(linkedApps),
        };
      })(),
      initiative_key: text(row.initiative_id ?? row.id),
      initiative_name: text(row.title ?? row.initiative_name ?? row.name),
      category: text(row.type ?? row.category ?? row.theme, 'AI initiative'),
      stage: text(row.stage ?? row.status),
      function_name: normalizeFunctionName(row.business_function),
      ...
      vendor: text(row.vendor) || joinedAppVendors(appDetailsFor(row.linked_app_ids)),
      tool_or_system: text(row.system_id ?? row.tool_or_system) || joinedAppNames(appDetailsFor(row.linked_app_ids)),
    ...
    const benefitRows = benefits.map((row) => ({
      ...
      promised_annual_value_usd: num(row.committed_roi_usd || row.projected_roi_usd),
      target_annual_value_usd: num(row.projected_roi_usd),
    ...
    const spendRows = spend.map((row) => ({
      ...(() => {
        const initiative = initiativeRows.find((item) => text(item.initiative_key) === text(row.initiative_id));
        return { initiative };
      })(),
      spend_key: text(row.initiative_id),
      initiative_key: text(row.initiative_id),
      vendor: text(initiativeRows.find((item) => text(item.initiative_key) === text(row.initiative_id))?.vendor, 'Mixed'),
    ...
    const riskRows = [
      ...risks.map((row) => ({
        ...(() => {
          const linkedApps = appDetailsFor(row.linked_system_id);
          const explicitFunction = text(row.business_function);
          return {
            function_name: explicitFunction ? normalizeFunctionName(explicitFunction) : functionFromApps(linkedApps),
          };
        })(),
...
function buildRisks(rows: JsonRecord[], initiatives: AiControlTowerInitiativeRead[]): AiControlTowerRiskRead[] {
  ...
  return rows.map((row) => {
    ...
    const rowFunction = normalizeFunctionName(row.function_name);
    const payloadFunction = normalizeFunctionName(payload.business_function);
    const initiativeFunction = normalizeFunctionName(initiativeByKey.get(initiativeId ?? '')?.functionName);
    const functionName = rowFunction !== 'Unassigned'
      ? rowFunction
      : payloadFunction !== 'Unassigned'
        ? payloadFunction
        : initiativeFunction;
    return {
