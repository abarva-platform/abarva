...
function sampleRow(overrides: Partial<EngagementPortfolioRow> = {}): EngagementPortfolioRow {
  return {
    id: 'eng-1',
    client_id: 'client-1',
...
describe('supabaseProgramsReadAdapter', () => {
  ...
  it('applies the RBAC allowlist as an `in` filter when provided', async () => {
    ...
    expect(calls.some((c) => c.method === 'in' && c.args[0] === 'id')).toBe(true);
  });
  ...
  it('throws on a genuine query error (route handles the 500)', async () => {
    const builder: Record<string, unknown> = {};
    for (const m of ['select', 'eq', 'is', 'order', 'in']) builder[m] = () => builder;
    builder.limit = () => Promise.resolve({ data: null, error: { message: 'boom' } });
    const client = { from: () => builder } as unknown as SupabaseClient;
    const adapter = createSupabaseProgramsReadAdapter(() => client);
    await expect(
      adapter.getProgramPortfolioRows({ clientId: 'c', allowedProgramIds: null, limit: 10 }),
