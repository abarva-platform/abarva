...
function fakeSupabase(opts: { rowFor?: RowFor; errFor?: ErrFor } = {}): {
  ...
  function builder(
    ...
    calls.push(call);
    const result = () => ({
      data: rowFor(table, op),
      error: errFor(table, op),
    });
    ...
    from(table: string) {
      return {
        insert: (body: Record<string, unknown>) =>
          builder(table, "insert", body),
        upsert: (body: Record<string, unknown>) =>
          builder(table, "upsert", body),
        update: (body: Record<string, unknown>) =>
          builder(table, "update", body),
        select() {
          ...
          const chain: Record<string, unknown> = {
            ...
            maybeSingle() {
              return Promise.resolve({
                data: rowFor(table, "select"),
                error: errFor(table, "select"),
              });
            },
          };
          void call;
...
function fakeTxSession(
  ...
  return { session, statements, paramSets, state };
}
...
describe("programs runAdvancePhase", () => {
  ...
  it("supabase: a snapshot insert error is surfaced as ok:false", async () => {
    const { client } = fakeSupabase({
      errFor: (t) =>
        t === "phase_snapshots" ? { message: "rls denied" } : null,
    });
    const adapter = createSupabaseProgramsWriteAdapter(() => client);
    const res = await adapter.runAdvancePhase(ADVANCE_INPUT);
...
describe("programs runDraftModuleDeliverable", () => {
  it("supabase: creates a new deliverable + version when none exists", async () => {
    ...
    expect(
      calls.find((c) => c.table === "deliverable_types")?.body,
    ).toMatchObject({
      type_key: "p2_package",
  ...
  it("supabase: a version insert error is surfaced as ok:false", async () => {
    const { client } = fakeSupabase({
      rowFor: (t, op) =>
        t === "deliverables_v2" && op === "select" ? null : { id: "x" },
      errFor: (t) =>
        t === "deliverable_versions" ? { message: "bad insert" } : null,
    });
    const adapter = createSupabaseProgramsWriteAdapter(() => client);
    const res = await adapter.runDraftModuleDeliverable(DRAFT_INPUT);
    expect(res.ok).toBe(false);
  ...
  it("azure: runs the upsert + version insert in ONE transaction", async () => {
    ...
    expect(
      tx.statements.some((s) => s.includes("INSERT INTO deliverables_v2")),
    ).toBe(true);
    expect(
...
describe("source-artifacts write adapter", () => {
  ...
  it("supabase: a DB error is surfaced as ok:false", async () => {
    const { client } = fakeSupabase({
      errFor: () => ({ message: "unique violation" }),
    });
    const adapter = createSupabaseSourceArtifactsWriteAdapter(() => client);
  ...
  it("azure: serializes JSONB source_artifacts metadata columns", async () => {
    const tx = fakeTxSession(() => [{ id: "art-jsonb" }]);
    const adapter = createAzureSourceArtifactsWriteAdapter(tx.session);
    const res = await adapter.insertArtifact(
      {
        ...ARTIFACT_COLS,
        disclosure_classification: {
          sensitivity: "client_final",
          visibleTo: ["buyer_team"],
        },
        evidence_families_used: ["vendor_response", "pricing_workbook"],
        missing_inputs: ["final legal markup"],
        client_complete_items: ["executive summary accepted"],
        assumptions: ["client final supersedes draft"],
        client_final_change_summary: {
          summary: "Client redlines accepted",
          sourceGeneratedArtifactId: "art-generated",
        },
      },
      "id",
    );
    ...
    expect(res.ok).toBe(true);
    expect(tx.statements[0]).toContain("disclosure_classification");
    expect(tx.statements[0]).toContain("disclosure_classification,");
    expect(tx.statements[0]).toContain("::jsonb");

    const sql = tx.statements[0];
    const params = tx.paramSets[0];
    const columns = sql
      .match(/source_artifacts \(([^)]+)\)/)?.[1]
      .split(", ")
    ...
    expect(paramFor("evidence_families_used")).toBe(
      JSON.stringify(["vendor_response", "pricing_workbook"]),
    );
    expect(paramFor("missing_inputs")).toBe(
      JSON.stringify(["final legal markup"]),
    );
    expect(paramFor("client_complete_items")).toBe(
      JSON.stringify(["executive summary accepted"]),
    );
    expect(paramFor("assumptions")).toBe(
      JSON.stringify(["client final supersedes draft"]),
    );
    expect(paramFor("client_final_change_summary")).toBe(
      JSON.stringify({
        summary: "Client redlines accepted",
        sourceGeneratedArtifactId: "art-generated",
      }),
