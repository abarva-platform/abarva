...
export const TOWER_SUBSURFACE_DEFINITIONS = [
  {
    ...
    label: "Shadow AI",
    description:
      "Unsanctioned tools, unmanaged spend, and PHI or customer-data exposure signals.",
  },
  {
    slug: "vendors",
    label: "Vendors",
    description:
      "Vendor concentration, overlap, renewal clocks, and rationalization candidates.",
  },
  {
    slug: "regulatory",
    label: "Regulatory",
    description:
      "Policy obligations, audit posture, and governance controls that shape execution risk.",
  },
  {
    slug: "council",
    label: "Council",
    description:
      "Decision bodies, sponsor ownership, escalation rights, and phase-gate operating cadence.",
  },
  {
    slug: "models",
    label: "Models",
    description:
      "Model inventory, deployment posture, evaluation state, and guardrail coverage.",
  },
] as const;

export type TowerSubsurfaceSlug =
  (typeof TOWER_SUBSURFACE_DEFINITIONS)[number]["slug"];
...
export function buildCanonicalRouteLinks(): RouteLinkRecord[] {
  ...
  for (const tenant of plan.tenants) {
    pushLink(
      links,
      tenantDashboardPath(tenant),
      tenantProgramsPath(tenant),
      "tenant_programs",
      "Programs",
    );
    pushLink(
    ...
    for (const surface of TOWER_SUBSURFACE_DEFINITIONS) {
      pushLink(
        links,
        tenantTowerPath(tenant),
        tenantTowerSubsurfacePath(tenant, surface.slug),
        "tower_subsurface",
        surface.label,
      );
      pushLink(
    ...
    for (const pattern of patterns) {
      pushLink(
        links,
        tenantPatternPath(tenant, pattern.slug),
        patternRouteFor(pattern.slug),
        "global_pattern",
        pattern.name,
      );
    }
    ...
    for (const program of tenant.programs) {
      pushLink(
        links,
        tenantProgramsPath(tenant),
        tenantProgramPath(tenant, program),
        "program_index",
        program.name,
      );
      pushLink(
      ...
      for (const phase of SPEC_PHASES) {
        pushLink(
          links,
          tenantProgramPath(tenant, program),
          tenantProgramPhasePath(tenant, program, phase),
          "phase_summary",
          `Phase ${phase}`,
        );
      }
      ...
      for (const deliverable of program.deliverables) {
        const deliverablePath = tenantDeliverablePath(
          tenant,
          program,
          deliverable,
        );
        ...
        pushLink(
          links,
          tenantProgramPhasePath(tenant, program, deliverable.phaseSpec),
          deliverablePath,
          "phase_deliverable",
          deliverable.deliverableCode,
        );
...
function addProgramRecords(
  records: CanonicalRouteRecord[],
