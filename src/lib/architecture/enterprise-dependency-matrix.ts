...
const ROW_FRONTEND_HOSTING: CapabilityRow = {
  ...
  perTarget: {
    ...
    azure_vnet: {
      defaultOption:
        "Azure Container Apps managed environment in a private VNet",
    ...
    aws_private: {
      ...
      readiness: "deferred",
      notes: "Forward slice; no AWS lab blueprint or IaC starter exists today.",
      followUp:
        "Author the AWS private deployment blueprint slice and a parallel AWS IaC starter.",
    },
    on_prem_air_gapped: {
      defaultOption:
        "Customer-operated Kubernetes cluster running the AbarVa container image",
...
const ROW_COMPUTE: CapabilityRow = {
  ...
  perTarget: {
    vercel_saas: {
      defaultOption:
        "Vercel Functions (Node / Edge runtimes) on AbarVa-owned project",
    ...
    azure_vnet: {
      defaultOption:
        "Azure Container Apps with managed identity inside a private VNet",
    ...
    aws_private: {
      defaultOption:
        "AWS Fargate task in a private VPC fronted by an internal ALB",
      ...
      followUp: "Author the AWS deployment blueprint and IaC starter.",
    },
    on_prem_air_gapped: {
      defaultOption:
        "Customer-operated Kubernetes node pool running the AbarVa container image",
      fallbackOption:
        "Customer-operated container host with the AbarVa runtime image",
      readiness: "deferred",
      notes:
...
const ROW_POSTGRES: CapabilityRow = {
  ...
  perTarget: {
    vercel_saas: {
      defaultOption:
        "AbarVa-owned managed Postgres pool with row-level security",
    ...
    azure_vnet: {
      defaultOption:
        "Azure Database for PostgreSQL Flexible Server inside the VNet via private endpoint",
    ...
    aws_private: {
      ...
      readiness: "deferred",
      notes: "AWS lab path is forward; no blueprint or IaC starter exists.",
      followUp:
    ...
    on_prem_air_gapped: {
      ...
      fallbackOption:
        "Customer-operated Postgres in a managed Kubernetes operator",
      readiness: "deferred",
      notes:
...
const ROW_BLOB: CapabilityRow = {
  ...
  perTarget: {
    vercel_saas: {
      defaultOption:
        "AbarVa-owned managed blob bucket bound to the SaaS project",
    ...
    azure_vnet: {
      defaultOption:
        "Azure Storage Account with private endpoint and public network access disabled",
      fallbackOption:
        "Azure Storage Account with managed identity and firewall rules",
      readiness: "preview",
      notes:
    ...
    gcp_vpc: {
      defaultOption:
        "GCS bucket with VPC Service Controls and uniform bucket-level access",
      fallbackOption: "GCS bucket with private Service Connect access",
      readiness: "deferred",
      notes: "GCP lab path is forward; no IaC starter for GCS exists yet.",
      followUp:
        "Author a GCS private-access IaC starter mirroring CLOUD5 shape.",
    },
    aws_private: {
      defaultOption:
        "AWS S3 bucket accessed only through a VPC endpoint with bucket policy enforcement",
      fallbackOption:
        "AWS S3 bucket with KMS-managed keys and a private ALB-fronted gateway",
      readiness: "deferred",
      notes: "AWS lab path is forward; no IaC starter for S3 exists yet.",
      followUp: "Author an S3 VPC-endpoint IaC starter mirroring CLOUD5 shape.",
    },
    on_prem_air_gapped: {
      defaultOption:
        "Customer-operated S3-compatible object store (e.g., MinIO) on hardened VMs",
      fallbackOption:
        "Customer-operated CIFS / NFS share for artifact retention",
      readiness: "deferred",
      notes:
        "Air-gapped customers operate object storage themselves; AbarVa documents the artifact contract only.",
      followUp: "Author the air-gapped object-storage operator runbook.",
    },
  },
};
...
const ROW_SECRETS: CapabilityRow = {
  ...
  perTarget: {
    ...
    azure_vnet: {
      defaultOption:
        "Azure Key Vault with private endpoint and managed-identity access",
      fallbackOption:
        "Azure Key Vault with firewall rules and a tenant-bound app role",
      readiness: "preview",
      notes:
    ...
    gcp_vpc: {
      defaultOption:
        "GCP Secret Manager with VPC Service Controls and per-secret IAM",
      fallbackOption:
        "GCP Secret Manager bound to a workload-identity service account",
      readiness: "deferred",
      notes: "GCP lab path is forward; no IaC starter exists yet.",
      followUp: "Author a Secret Manager IaC starter mirroring CLOUD5 shape.",
    },
    aws_private: {
      defaultOption:
        "AWS Secrets Manager with VPC endpoint and KMS-managed keys",
      fallbackOption: "AWS Systems Manager Parameter Store with KMS encryption",
      readiness: "deferred",
      notes: "AWS lab path is forward; no IaC starter exists yet.",
      followUp: "Author a Secrets Manager IaC starter mirroring CLOUD5 shape.",
    },
    on_prem_air_gapped: {
      defaultOption: "Customer-operated HashiCorp Vault on hardened VMs",
      fallbackOption:
        "Customer-operated sealed-secrets controller in Kubernetes",
      readiness: "deferred",
      notes:
        "Air-gapped customers operate the secret store themselves; AbarVa documents the secret contract only.",
      followUp: "Author the air-gapped secret-rotation operator runbook.",
    },
  },
};
...
const ROW_IDP: CapabilityRow = {
  ...
  perTarget: {
    ...
    azure_vnet: {
      defaultOption:
        "Microsoft Entra ID (SAML / OIDC) federated to the customer Clerk tenant",
      fallbackOption:
        "Microsoft Entra ID with B2B guest invitations during pilot",
      readiness: "preview",
      notes:
    ...
    gcp_vpc: {
      defaultOption:
        "Google Workspace SAML federated to the customer Clerk tenant",
      fallbackOption: "Google Cloud Identity OIDC federation",
      readiness: "deferred",
      notes: "GCP federation runbook is forward; no automation exists.",
      followUp: "Author the Workspace federation runbook.",
    },
    aws_private: {
      defaultOption:
        "AWS IAM Identity Center (SSO) federated to the customer Clerk tenant",
      fallbackOption: "Customer Okta tenant federated via SAML",
      readiness: "deferred",
      notes: "AWS federation runbook is forward; no automation exists.",
      followUp: "Author the IAM Identity Center federation runbook.",
    },
    on_prem_air_gapped: {
      defaultOption:
        "Customer-operated Keycloak (OIDC / SAML) federated to AbarVa runtime",
      ...
      followUp: "Author the air-gapped IdP federation operator runbook.",
    },
  },
};
...
const ROW_OBSERVABILITY: CapabilityRow = {
  ...
  perTarget: {
    vercel_saas: {
      defaultOption:
        "Vercel runtime logs plus AbarVa-owned PostHog project for product analytics",
      fallbackOption:
        "Azure Container Apps revision logs streamed to a managed log lane",
      readiness: "preview",
      notes:
    ...
    azure_vnet: {
      defaultOption:
        "Azure Log Analytics workspace bound to the Container Apps environment",
      fallbackOption:
        "Azure Monitor with Application Insights bound to the Container App",
      readiness: "preview",
      notes:
    ...
    gcp_vpc: {
      defaultOption:
        "GCP Cloud Logging plus Cloud Monitoring scoped to the project",
      fallbackOption: "GCP Cloud Trace bound to the Cloud Run service",
      readiness: "deferred",
      notes: "GCP lab path is forward; no IaC starter exists.",
      followUp: "Author a Cloud Logging IaC starter mirroring CLOUD5 shape.",
    },
    aws_private: {
      defaultOption:
        "AWS CloudWatch Logs plus CloudWatch Metrics scoped to the VPC",
      fallbackOption: "AWS X-Ray traces bound to the Fargate task",
      readiness: "deferred",
      notes: "AWS lab path is forward; no IaC starter exists.",
      followUp: "Author a CloudWatch IaC starter mirroring CLOUD5 shape.",
    },
    on_prem_air_gapped: {
      ...
      followUp: "Author the air-gapped observability operator runbook.",
    },
  },
};
...
const ROW_VECTOR: CapabilityRow = {
  ...
  perTarget: {
    vercel_saas: {
      defaultOption:
        "AbarVa-owned managed pgvector index inside the SaaS Postgres pool",
      fallbackOption:
        "AbarVa-owned managed pgvector index with per-tenant namespace filter",
      readiness: "preview",
      notes:
    ...
    azure_vnet: {
      defaultOption:
        "pgvector extension in the per-tenant Azure Postgres Flexible Server",
      fallbackOption:
        "Azure AI Search index inside the VNet via private endpoint",
      readiness: "deferred",
      notes: "No vector IaC starter is committed for Azure today.",
      followUp:
    ...
    gcp_vpc: {
      ...
      readiness: "deferred",
      notes: "GCP vector path is forward; no IaC starter exists.",
      followUp: "Author a Cloud SQL pgvector IaC starter when CLOUD3 lands.",
    },
    aws_private: {
      ...
      readiness: "deferred",
      notes: "AWS vector path is forward; no IaC starter exists.",
      followUp:
        "Author an RDS pgvector IaC starter when the AWS blueprint lands.",
    },
    on_prem_air_gapped: {
      defaultOption:
        "pgvector extension in the customer-operated Postgres cluster",
      fallbackOption:
        "Customer-operated Qdrant / Milvus instance on hardened VMs",
      readiness: "deferred",
      notes:
        "Air-gapped vector path is customer-operated; AbarVa documents the retrieval contract only.",
      followUp: "Author the air-gapped vector operator runbook.",
    },
  },
};
...
const ROW_GRAPH: CapabilityRow = {
  ...
  perTarget: {
    vercel_saas: {
      defaultOption:
        "Postgres-backed graph tables with tenant-scoped traversal helpers",
      fallbackOption:
        "Postgres recursive CTE traversal with a tenant predicate",
      readiness: "preview",
      notes:
    ...
    azure_vnet: {
      defaultOption:
        "Postgres-backed graph tables in the per-tenant Azure Postgres",
      fallbackOption: "Azure Cosmos DB for Apache Gremlin inside the VNet",
      readiness: "deferred",
      notes: "No graph IaC starter is committed for Azure today.",
      followUp:
    ...
    gcp_vpc: {
      ...
      readiness: "deferred",
      notes: "GCP graph path is forward; no IaC starter exists.",
      followUp: "Author a Cloud SQL graph IaC starter when CLOUD3 lands.",
    },
    aws_private: {
      defaultOption:
        "Postgres-backed graph tables in the per-tenant RDS Postgres",
      fallbackOption: "AWS Neptune cluster inside the VPC",
      readiness: "deferred",
      notes: "AWS graph path is forward; no IaC starter exists.",
      followUp: "Author an RDS graph IaC starter when the AWS blueprint lands.",
    },
    on_prem_air_gapped: {
      defaultOption:
        "Postgres-backed graph tables in the customer-operated Postgres cluster",
      fallbackOption:
        "Customer-operated Neo4j / JanusGraph instance on hardened VMs",
      readiness: "deferred",
      notes:
        "Air-gapped graph path is customer-operated; AbarVa documents the traversal contract only.",
      followUp: "Author the air-gapped graph operator runbook.",
    },
  },
};
...
const ROW_MODEL_GATEWAY: CapabilityRow = {
  ...
  perTarget: {
    vercel_saas: {
      defaultOption:
        "AbarVa-owned model gateway with per-tenant routing keys and audit destination",
      fallbackOption:
        "AbarVa-owned gateway with row-scoped audit ledger and tenant-scoped allowlist",
      readiness: "preview",
      notes:
    ...
    azure_vnet: {
      defaultOption:
        "Customer-hosted gateway image inside the Container Apps environment with per-tenant policy",
      fallbackOption:
        "Azure API Management as a gateway proxy with managed-identity outbound",
      readiness: "deferred",
      notes: "No customer-hosted gateway IaC is committed today.",
      followUp:
        "Document the customer-hosted gateway runbook for the CLOUD5 starter.",
    },
    gcp_vpc: {
      defaultOption:
        "Customer-hosted gateway image inside Cloud Run with per-tenant policy",
      fallbackOption: "GCP API Gateway as a proxy with VPC egress control",
      readiness: "deferred",
      notes: "GCP gateway path is forward; no IaC starter exists.",
      followUp: "Author the customer-hosted gateway runbook for GCP.",
    },
    aws_private: {
      defaultOption:
        "Customer-hosted gateway image inside Fargate with per-tenant policy",
      fallbackOption: "AWS API Gateway with PrivateLink as a proxy",
      readiness: "deferred",
      notes: "AWS gateway path is forward; no IaC starter exists.",
      followUp: "Author the customer-hosted gateway runbook for AWS.",
    },
    on_prem_air_gapped: {
      defaultOption:
        "Customer-operated gateway image bound to a customer-approved local model lane",
      fallbackOption:
        "Customer-operated gateway with a deterministic offline simulator lane",
...
const ROW_CI_CD: CapabilityRow = {
  ...
  perTarget: {
    vercel_saas: {
      defaultOption:
        "GitHub Actions building the Next.js app and Vercel preview / production deploys",
      ...
      followUp:
        "PROD4 must add secure GitHub / Azure deployment ingestion before any readiness promotion to production_deployment.",
    },
    azure_vnet: {
      defaultOption:
        "GitHub Actions running az bicep build + az deployment group what-if against the customer subscription",
      fallbackOption:
        "Azure DevOps Pipelines targeting the customer subscription with managed identity",
      readiness: "preview",
      notes:
    ...
    gcp_vpc: {
      defaultOption:
        "GitHub Actions invoking gcloud with workload-identity federation against the customer project",
      fallbackOption:
        "Google Cloud Build pipeline targeting the customer project",
      readiness: "deferred",
      notes: "GCP CI / CD path is forward; no pipeline is committed.",
      followUp:
        "Author a workload-identity GitHub Actions pipeline for CLOUD3.",
    },
    aws_private: {
      defaultOption:
        "GitHub Actions invoking aws CLI with OIDC federation against the customer account",
      fallbackOption:
        "AWS CodePipeline targeting the customer account with IAM role assumption",
      readiness: "deferred",
      notes: "AWS CI / CD path is forward; no pipeline is committed.",
      followUp:
        "Author an OIDC-federated GitHub Actions pipeline for the AWS blueprint.",
    },
    on_prem_air_gapped: {
      defaultOption:
        "Customer-operated CI runner inside the air-gapped network with offline image transfer",
      fallbackOption:
        "Customer-operated GitLab runner with image promotion via signed bundle",
      readiness: "deferred",
      notes:
