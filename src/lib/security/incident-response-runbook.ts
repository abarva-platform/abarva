...
const RESPONSE_STEPS: ReadonlyArray<IncidentResponseStep> = [
  // ── DETECT ──────────────────────────────────────────────────────────
  {
    ...
    escalateIf:
      "Alert not acknowledged within SLA: SEV1=5min, SEV2=15min, SEV3=30min, SEV4=4h.",
  ...
  },
  {
    ...
    escalateIf:
      "Multiple tenants affected or data exfiltration suspected → escalate to SEV1.",
  ...
  // ── CONTAIN ─────────────────────────────────────────────────────────
  {
    ...
    escalateIf:
      "Feature flag toggle does not reduce error rate → proceed to rollback.",
  ...
  },
  {
    ...
    escalateIf:
      "Rollback does not resolve the issue → this may be a data or infrastructure issue, not a code issue.",
  ...
  },
  {
    ...
    escalateIf:
      "Data exfiltration confirmed → escalate to legal-counsel immediately.",
  ...
  },
  {
    ...
    escalateIf:
      "Evidence of unauthorised cross-tenant data access → notify affected tenants.",
  ...
  // ── RECOVER ─────────────────────────────────────────────────────────
  {
    ...
    actions: [
      ...
      "Monitor Azure Container Apps revision logs during rollout.",
  ...
  },
  {
    ...
    escalateIf:
      "Restoration fails or data loss exceeds expected window → escalate to Supabase support.",
...
export function getStepsBySeverity(
  ...
  return RESPONSE_STEPS.filter((s) =>
    s.applicableSeverities.includes(severity),
  );
}
