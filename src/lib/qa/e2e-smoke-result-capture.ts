...
export function buildSmokeResultFixture(): SmokeResultCapture {
  const routeResults: RouteResult[] = [
    ...
    },
    {
      ...
      readinessImpact: "none",
      nextValidationAction: "Verify live refresh shows correct CI/Azure status",
    },
    {
      id: "QA19-R04",
    ...
    },
    {
      ...
      nextValidationAction:
        "Verify all preview routes accessible without Clerk",
    },
    {
      id: "QA19-R11",
      route: "/tower",
      ...
      blocker: null,
      notes:
        "Control Tower index renders (was /preview/tower — shell removed per Tower audit)",
      readinessImpact: "none",
  ...
  const deferredRoutes = routeResults.filter(
    (r) => r.status === "deferred",
  ).length;
  const blockedRoutes = routeResults.filter(
    (r) => r.status === "blocked",
  ).length;
...
export function validateSmokeResultCapture(
  capture: SmokeResultCapture,
): string[] {
  const errors: string[] = [];
