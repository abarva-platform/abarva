...
export function buildProductionReadinessPageVerificationReport(): ProductionReadinessPageVerificationReport {
  ...
  const deploymentStatusPath =
    "src/app/api/admin/production-readiness/deployment-status/route.ts";
  const liveRefreshPath =
    "src/app/api/admin/production-readiness/refresh/route.ts";
  ...
  const pages: PageVerificationRecord[] = [
    {
      ...
      description:
        "Admin production readiness tracker page — deterministic read model surface for Steward.",
      status: adminPageStatus,
      filePath: adminPageStatus === "present" ? adminPagePath : adminPagePath,
      caveat:
        "This page renders a static manifest read model. Live CI/Azure status requires external polling and is not reflected here.",
    ...
    },
    {
      ...
      description:
        "API route that returns the production readiness JSON manifest response.",
    ...
    },
    {
      ...
      filePath:
        deploymentStatusStatus === "present"
    ...
    },
    {
      ...
      description:
        "Live refresh route for on-demand manifest reload (from PROD3).",
      status:
        liveRefreshStatus === "present"
          ? "present"
          : ("deferred" as PageVerificationStatus),
      filePath: liveRefreshStatus === "present" ? liveRefreshPath : "deferred",
      caveat:
  ...
  try {
    ...
    const topLevelKeys = Object.keys(manifest).filter(
      ...
      for (const comp of components as Array<Record<string, unknown>>) {
        ...
        if (comp.dimensions && typeof comp.dimensions === "object") {
          for (const val of Object.values(
            comp.dimensions as Record<string, unknown>,
          )) {
            if (val === "production_ready") {
              ...
              const hasNotes =
                Array.isArray(comp.notes) &&
                (comp.notes as unknown[]).length > 0;
              if (!hasNotes) {
                hasNoFalseProductionReady = false;
              }
            }
  ...
  const manifestVerification: ReadinessManifestVerification = {
    ...
    staticManifestNote:
      "production-readiness.json is a static manifest. Live CI/Azure status requires external polling and is not reflected here.",
  };
  ...
  const verificationCaveats: string[] = [
    "Live CI/Azure deployment status is not available without external API polling.",
