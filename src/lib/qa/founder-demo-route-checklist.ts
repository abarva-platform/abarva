...
const DEMO_ROUTES: readonly DemoRoute[] = Object.freeze([
  {
    route: "/home",
    purpose:
      "Executive entry point — establishes the agentic operating model and surfaces the platform mission before any tenant context is loaded.",
    primaryAgent: "Nexus",
    demoTalkingPoint:
    ...
    readinessCaveat:
      "None — route is fully rendered and design-canon compliant.",
    fallbackIfBlocked:
      "Show a screenshot of the home screen captured during the 2026-04-26 smoke run.",
    validationStatus: "ready",
  },
  {
    route: "/platform/admin",
    purpose:
      "Platform administration hub — lets the operator see all tenant accounts, agent health, and global system controls in one place.",
    ...
    fallbackIfBlocked:
      "Navigate to /platform/admin/production-readiness as a substitute to demonstrate Steward governance depth.",
    validationStatus: "ready",
  },
  {
    route: "/platform/admin/production-readiness",
    purpose:
      "Production readiness tracker — gives the operator a live, component-by-component view of platform readiness across ten dimensions before any go-live decision.",
    primaryAgent: "Steward",
    demoTalkingPoint:
    ...
    readinessCaveat:
      "Live CI and Azure polling tokens are absent in the demo environment; liveStatus is correctly reported as unavailable rather than fabricated.",
    fallbackIfBlocked:
      "Show the static production-readiness.json manifest in docs/build/ to demonstrate the canonical readiness schema.",
    validationStatus: "ready",
  },
  {
    route: "/platform/admin/build-progress",
    purpose:
      "Build progress tracker — shows the founder exactly which of the eleven build waves have merged, which slices are complete, and what the next action is.",
    primaryAgent: "Steward",
    demoTalkingPoint:
    ...
    readinessCaveat:
      "None — reads from the deterministic build-waves.json manifest; no live GitHub or Azure polling.",
    fallbackIfBlocked:
      "Open docs/build/build-waves.json directly and narrate the completed-wave list.",
    validationStatus: "ready",
  },
  {
    route: "/tenant/apex-retail/programs",
    purpose:
      "Programs list for the Apex Retail demo tenant — shows four active AI programs across Contact Center AI, CDP, Store Associate Productivity, and Demand Forecasting.",
    primaryAgent: "Nexus",
    demoTalkingPoint:
    ...
    readinessCaveat:
      "None — programs render from the Apex Retail seed data installed in the demo environment.",
    fallbackIfBlocked:
      "Show the seed data in src/lib/programs/ to demonstrate program structure and fields.",
    validationStatus: "ready",
  },
  {
    route: "/tenant/apex-retail/programs/[programSlug]",
    purpose:
      "Program detail page — deep-dives a single program showing phase timeline, deliverables, evidence panel, and agent recommendations.",
    primaryAgent: "Nexus",
    demoTalkingPoint:
    ...
    readinessCaveat:
      "Content is seeded data only; live program-state ingestion, real deliverable generation, and workshop mode execution are deferred.",
    fallbackIfBlocked:
      "Remain on the programs list and walk the summary cards in detail rather than drilling into a specific program.",
  ...
  },
  {
    ...
    purpose:
      "Solution Intelligence canvas — Sentinel's active-pattern detection surface showing which AI solution patterns are active, which are at risk, and which are emerging across the tenant.",
    ...
    readinessCaveat:
      "None — Intelligence canvas renders with the full active-pattern seed and the SentinelPatternRail is installed on the tenant pattern pages.",
    fallbackIfBlocked:
      "Walk the Intelligence section of the production-readiness tracker to describe the validated contract.",
    validationStatus: "ready",
  },
  {
    route: "/tenant/apex-retail/intelligence/patterns/[patternKey]",
    purpose:
      "Pattern detail page — drill-down into a specific Sentinel detection showing evidence sources, affected programs, severity trend, and recommended remediation actions.",
    primaryAgent: "Sentinel",
    demoTalkingPoint:
    ...
    readinessCaveat:
      "Pattern detail content is seeded data only; live detection ingestion, real-time severity scoring, and automated remediation dispatch are deferred.",
    fallbackIfBlocked:
      "Show the active pattern list on the Intelligence canvas without drilling into a specific pattern, and narrate the evidence model.",
    validationStatus: "partial",
  },
  {
    route: "/source",
    purpose:
      "Source platform entry — the data-ingestion and event-governance surface where operators configure what data flows into Nexus, Atlas, Sentinel, and Steward.",
    primaryAgent: "Steward",
    demoTalkingPoint:
    ...
    readinessCaveat:
      "Source surfaces are partially implemented; admin/setup readiness contract, data readiness panel, and event canvas are code-complete but live event ingestion and authenticated route smoke automation are deferred.",
    fallbackIfBlocked:
      "Describe the data trust model (L0–L4 sharing levels and the five-state trust ladder) using the TRUST1 documentation in docs/build/slices/.",
    validationStatus: "partial",
  },
  {
    route: "/source/events",
    purpose:
      "Event canvas — shows the live stream of data events flowing into the platform, filterable by source, type, trust level, and processing status.",
    primaryAgent: "Steward",
    demoTalkingPoint:
    ...
    readinessCaveat:
      "Event canvas renders from seed data; live event stream ingestion, real-time filtering, and authenticated persona-walk automation are deferred.",
    fallbackIfBlocked:
      "Show the Source admin/setup readiness contract documentation to explain the event data governance model without a live feed.",
    validationStatus: "partial",
  },
] as const);
