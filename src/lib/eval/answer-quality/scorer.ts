...
export function scoreAnswer(answer: string, context: AnswerQualityContext): AnswerQualityScore {
  ...
  if (!normalized) {
    return failAll('clarity', 'Empty answer.', 'Return a concise, decision-grade answer.', dimensions);
  }
  ...
  if (longParagraph) {
    penalize(dimensions, violations, 'clarity', 45, trimWords(longParagraph, 18), 'Split long prose into scannable decision bullets.');
  }

  if (!/[.!?]$/.test(normalized)) {
    penalize(dimensions, violations, 'clarity', 15, trimWords(normalized, 12), 'End the answer with a complete sentence.');
  }

  for (const pattern of RAW_ID_PATTERNS) {
    const excerpt = firstMatch(normalized, pattern.re);
    if (excerpt) {
      penalize(dimensions, violations, 'noRawIds', 100, excerpt, `Replace the raw ${pattern.name} with a human-readable label.`);
    }
  }
  ...
  const vagueAction = VAGUE_ACTION_PATTERNS.map((pattern) => firstMatch(normalized, pattern)).find(Boolean);
  if (vagueAction) {
    penalize(
      dimensions,
      violations,
  ...
  if (hasAction && !/\b(?:by|owner|open|assign|approve|decide|validate|escalate|pause|route)\b/i.test(normalized)) {
    penalize(
      dimensions,
      violations,
      'realNextMove',
  ...
  const overall = Math.round(
    ANSWER_QUALITY_DIMENSIONS.reduce((sum, dimension) => sum + dimensions[dimension] * DIMENSION_WEIGHT, 0) / 100,
  );
  ...
  return {
    ...
    dimensions,
    violations,
    gatePassed: overall >= ANSWER_QUALITY_PASS_THRESHOLD && violations.length === 0,
  };
}
