...
function inferCanonicalClientKeyFromEmail(
  ...
  if (
    ...
    return "apexretail";
  if (
    normalized.endsWith("@first-capital.example.com") ||
    normalized.includes("+arcturus@abarva.com")
  )
    return "arcturus";
  return null;
}
...
function defaultDataClasses(
  canViewFinancialData: boolean,
): Pick<UserProgramAccessPolicy, "allowedDataClasses" | "deniedDataClasses"> {
...
function buildPolicy(args: {
  ...
  return {
    ...
    programScope: none
      ? "assigned_programs_only"
      : args.programIdsAllowed === null
        ? "all_client_programs"
        : "assigned_programs_only",
    ...
    canGenerateDeliverables: none
      ? false
      : (args.canGenerateDeliverables ?? true),
    canPublishDeliverables: none
      ? false
      : (args.canPublishDeliverables ?? admin),
...
export async function loadUserProgramAccessPolicy(
  ...
  const explicitProgramScopedMembership =
    membership?.access_level === "program_member" ||
    membership?.access_level === "program_viewer";
  ...
  return buildPolicy({
    ...
    canCreatePrograms:
      !none && (Boolean(membership?.can_create_programs) || admin),
    canApproveGates:
      !none &&
...
export async function allowedProgramIdsForUser(
  ctx: TenancyCtx,
): Promise<string[] | null> {
...
export async function canReadProgram(
  ctx: TenancyCtx,
  programId: string,
): Promise<boolean> {
...
export function formatUserProgramAccessPolicyForPrompt(
