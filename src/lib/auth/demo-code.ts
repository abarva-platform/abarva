...
export const DEMO_CODE_ALLOWED_EMAILS = [...CANONICAL_AUTH_EMAILS] as const;

const DEMO_CODE_ALLOWED_EMAIL_SET = new Set<string>(DEMO_CODE_ALLOWED_EMAILS);

export function normalizeDemoCodeEmail(
...
export function isDemoCodeEmail(email: string | null | undefined): boolean {
  ...
  return (
    DEMO_CODE_ALLOWED_EMAIL_SET.has(normalized) ||
    isLaunchApprovedEmail(normalized)
  );
}
