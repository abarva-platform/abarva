...
export const STATIC_LAUNCH_ACCESS_PROFILES = [
  { email: 'admin@abarva.ai', role: 'admin', label: 'AbarVa admin' },
...
const STATIC_PROFILE_BY_EMAIL: ReadonlyMap<string, LaunchAccessProfile> = new Map(
  STATIC_LAUNCH_ACCESS_PROFILES.map((profile) => [profile.email, profile]),
);

export function getStaticLaunchAccessProfile(email: string | null | undefined): LaunchAccessProfile | null {
  return STATIC_PROFILE_BY_EMAIL.get(normalizeLaunchEmail(email)) ?? null;
}

export function isStaticLaunchApprovedEmail(email: string | null | undefined): boolean {
