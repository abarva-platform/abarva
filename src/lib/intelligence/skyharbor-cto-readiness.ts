...
export function composeSkyHarborCtoAnswer(
  ...
  const boardGap =
    /evidence gap|evidence gaps|board decision|board guidance|before.*board|board.*before|board.*evidence/.test(
      lower,
    );
  ...
  let pointOfView =
    "Airline Demo should fund IROPS readiness before autonomous scale.";
  if (fund)
    pointOfView =
      "Airline Demo CTO should fund the IROPS data foundation, event backbone, and AI governance gate before broad autonomous recovery scale.";
  if (systems)
    pointOfView =
      "Airline Demo IROPS depends on a small set of operationally critical systems, and the weak spot is the certified data path across them.";
  if (data)
    pointOfView =
      "Airline Demo should certify crew legality, flight status, PNR/reservation events, operational event store, and recovery decision history first.";
  if (value)
    pointOfView =
      "Airline Demo can claim a directional value story today, but Finance signoff is required before board use.";
  if (controls)
    pointOfView =
      "Airline Demo should treat model-risk tiering, human-in-loop control, and operational evidence as the gates that block autonomous scale.";
...
function composeSkyHarborBoardGapAnswer(
  ...
  return [
    "Airline Demo IROPS AI case is not board-ready yet; it is planning-grade and strong enough to fund readiness.",
    [
  ...
  const branch: DecisionBranch = {
    ...
    choices: [
      ...
      },
      {
        ...
        description:
          "Route missing evidence to the named owner where the V6 packet identifies one.",
      },
    ],
    customAllowed: true,
    rawBlock: "",
...
function readV6File(root: string, file: string): V6Record[] {
  ...
  if (!existsSync(filePath))
    throw new Error(`Missing SkyHarbor V6 file: ${filePath}`);
