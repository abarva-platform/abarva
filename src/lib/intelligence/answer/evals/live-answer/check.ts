...
const NEXT_MOVE_CUE =
  /\b(next step|recommend|start by|pilot|run a|stand up|prioritize|sequence|validate|baseline|workshop|assess|pull(?:s)? the data|pulled data|pull that data|reporting workbench|reporting environment|analytics workbench|source system|focus first|faster win|decision-grade|first thing to fix|before we get to tactics|in order|triage|what i can give you)\b/i;
