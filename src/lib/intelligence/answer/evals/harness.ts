import { scoreAnswer, type AnswerQualityScore } from "@/lib/eval/answer-quality/scorer";
...
function emptyBundle(question: GoldenQuestion, generatedAt: string): ContextBundle {
  return {
    ...
    warnings: [],
    infoTags: ["Deterministic W5.1 eval bundle; no live tenant data or model call."],
  };
}

async function deterministicAnswer(question: GoldenQuestion, generatedAt: string): Promise<AgentAnswer> {
  const routing = routeQuestion({ query: question.query, industry: question.industry });
    ...
    recommendedActions: [
      {
        ...
        rationale: "Golden eval answers prove the deterministic path before any tenant-facing rollout.",
      },
    ],
  });
}
...
export async function runAgentAnswerEvalHarness(
  ...
  const answerRunner = options.answerRunner ?? ((question) => deterministicAnswer(question, generatedAt));
  const golden = runGoldenEval();
  const goldenById = new Map(golden.results.map((result) => [result.id, result]));

  const results = await Promise.all(
    GOLDEN_QUESTIONS.map(async (question) => {
      const answer = await answerRunner(question);
      const answerQuality = scoreAnswer(answer.prose, {
        questionId: question.id,
        tenantKey: "eval",
        surface: answer.surface,
      });
      const goldenResult = goldenById.get(question.id);
      if (!goldenResult) throw new Error(`Missing golden result for ${question.id}`);
      const notes = [
        ...goldenResult.notes,
      ...
      return {
        ...
        routedTo: answer.expertId,
        golden: goldenResult,
        answer,
        answerQuality,
        pass: goldenResult.pass && answerQuality.gatePassed && !answer.crossTenantBlocked,
        notes,
      };
    }),
  );
