...
export function gateExpertPack(pack: ExpertPack): GateResult {
  ...
  if (pack === null || typeof pack !== "object") {
    return {
      ...
      blockerCount: 1,
      concernCount: 0,
      findings: [{ severity: "blocker", rule: "structure.pack", detail: "pack is not an object." }],
    };
  }
  ...
  if (!pack.diagnostics) blocker("structure.diagnostics", "pack.diagnostics is missing.");
  ...
  >) {
    ...
    if (counts[key] < min) {
      blocker(
        `depth.${key}`,
        `${key} has ${counts[key]} (minimum ${min}).`,
      );
    }
  }

  // ── Identity coherence (blocker) ───────────────────────────────────────────
  if (!pack.identity) blocker("structure.identity", "pack.identity is missing.");
  const id = pack.identity ?? ({} as ExpertPack["identity"]);
  if (id.kind === "industry-function") {
    if (!id.industry) blocker("identity.industry", "industry-function expert is missing `industry`.");
    if (!id.functionKey) blocker("identity.functionKey", "industry-function expert is missing `functionKey`.");
  } else if (id.kind === "cross-cutting-domain") {
    if (!id.crossCuttingDomain) blocker("identity.crossCuttingDomain", "cross-cutting expert is missing `crossCuttingDomain`.");
  }
  if (!id.scopeNote?.trim()) blocker("identity.scopeNote", "scopeNote is empty — an expert must declare its boundary.");

  // ── Honesty: every AI archetype has a value mechanism (blocker) ────────────
  const operatingMetrics = Array.isArray(domain.operatingMetrics) ? domain.operatingMetrics : [];
  const metricKeys = new Set(operatingMetrics.map((m) => m.key));
  const archetypes = Array.isArray(domain.aiUseCaseArchetypes) ? domain.aiUseCaseArchetypes : [];
  for (const a of archetypes) {
    if (!a.valueMechanism?.trim()) {
      blocker("honesty.value-mechanism", `AI archetype "${a.name}" has no valueMechanism — auto-reject rule.`);
    }
    if (!a.dataDependencies?.length) {
      concern("honesty.data-dependencies", `AI archetype "${a.name}" lists no dataDependencies.`);
    }
    ...
    if (!Array.isArray(a.metricsMoved)) {
      blocker("integrity.metrics-moved", `AI archetype "${a.name}" is missing metricsMoved.`);
    } else {
      for (const mk of a.metricsMoved) {
        if (!metricKeys.has(mk)) {
          blocker("integrity.metric-ref", `AI archetype "${a.name}" references unknown metric key "${mk}".`);
        }
      }
    }
  }
  ...
  const checkRange = (where: string, r: { low: number; high: number; label: string }) => {
    if (r.label !== "planning-range") {
      blocker("honesty.benchmark-label", `${where}: benchmark must be labelled "planning-range" (got "${r.label}").`);
    }
    if (r.low > r.high) {
      blocker("integrity.benchmark-range", `${where}: benchmark low (${r.low}) > high (${r.high}).`);
    }
  };
  ...
  for (const rs of Array.isArray(domain.referenceSolutionPatterns) ? domain.referenceSolutionPatterns : []) {
    if (!rs.humanAccountabilityPoint?.trim()) {
      blocker("governance.accountability", `Reference solution "${rs.name}" has no humanAccountabilityPoint.`);
    }
  }

  // ── Evidence rules present and non-trivial (blocker) ───────────────────────
  if (Object.keys(pack.evidenceRules?.requiredEvidenceByClaimType ?? {}).length === 0) {
    blocker("evidence.rules", "evidenceRules.requiredEvidenceByClaimType is empty.");
  }
  if (!pack.evidenceRules?.citationStandard?.trim()) {
    blocker("evidence.citation-standard", "evidenceRules.citationStandard is empty.");
  }

  // ── Confident-synthesis requires hedge language (blocker) ──────────────────
  if (!pack.hedgeRules?.whenToHedge?.length) {
    blocker("doctrine.hedge", "hedgeRules.whenToHedge is empty — confident synthesis still requires hedges.");
  }
  if (!pack.hedgeRules?.inferenceLanguage?.length) {
    concern("doctrine.inference-language", "hedgeRules.inferenceLanguage is empty — no 'industry pattern, not your data' phrasing.");
  }
  ...
  for (const r of Array.isArray(pack.outputRecipes) ? pack.outputRecipes : []) {
    if (r.exhibitKind === "chart") {
      if (!r.chartKind) {
        blocker("recipe.chart-kind", `Output recipe "${r.questionPattern}" is a chart but has no chartKind.`);
      } else if (!isValidChartKind(r.chartKind)) {
        blocker("recipe.chart-kind", `Output recipe "${r.questionPattern}" has unknown chartKind "${r.chartKind}".`);
      }
    }
  }
  ...
  if (!sm) {
    blocker("success.model", "successModel is missing — the CXO stuck-point (odds/adoption/ROI) is unanswered.");
  } else {
    if (!BANDS.has(sm.probabilityOfSuccess)) blocker("success.probability", `probabilityOfSuccess must be low|medium|high (got "${sm.probabilityOfSuccess}").`);
    if (!BANDS.has(sm.adoptionReadiness)) blocker("success.adoption", `adoptionReadiness must be low|medium|high (got "${sm.adoptionReadiness}").`);
    if (!BANDS.has(sm.roiClarity)) blocker("success.roi-clarity", `roiClarity must be low|medium|high (got "${sm.roiClarity}").`);
    if (!sm.successDrivers?.length) blocker("success.drivers", "successModel.successDrivers is empty — 'what makes it work' is unanswered.");
    if (!sm.failureDrivers?.length) blocker("success.failure-drivers", "successModel.failureDrivers is empty — the honest failure list is missing.");
    if (!sm.adoptionCurve?.trim()) concern("success.adoption-curve", "successModel.adoptionCurve is empty — adoption shape is unstated.");
    if (!sm.roiClarityBasis?.trim()) concern("success.roi-basis", "successModel.roiClarityBasis is empty — why ROI clarity sits where it does is unstated.");
  }

  // ── Provenance per AI-gate decision (concern/blocker) ──────────────────────
  if (pack.provenance?.reviewTier !== "ai-gate") {
    blocker("provenance.review-tier", `reviewTier must be "ai-gate" (got "${pack.provenance?.reviewTier}").`);
  }
  if (!pack.provenance?.asOf?.trim()) {
    concern("provenance.as-of", "provenance.asOf is empty — freshness is unknown.");
  }
