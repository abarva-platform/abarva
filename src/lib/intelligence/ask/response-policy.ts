...
- MANDATORY when the user asks for a value/complexity, readiness/value, 2x2, quadrant, or priority matrix — emit a compact chart-ready GFM table with item, value, complexity/readiness, and basis columns. Use high/medium/low when exact numeric values are not available and state that the view is directional. Do not merely describe the chart in prose; the table is the chart payload.
