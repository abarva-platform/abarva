...
const STRATEGY_TO_MOVES_EXECUTION_RE =
  /\b(run (?:this|it|that)?\s*(?:through|in|as)\s+(?:moves?|a move)|moves?\s+(?:portfolio\s+)?sprint|moves?\s+model|correct\s+moves?\s+model|canonical\s+phases?|phase[-\s]?gate|phase\s+gates?|phase[-\s]?sequencing|p0\s*(?:[-–—/]?\s*)p5|p0\s*,?\s*p1|p0\b.*\bp5\b|p0[-–—/]p5|8[-\s]?week(?:s)?\s+(?:plan|sprint|roadmap)|by phases?|phase plan|through execution|strategy through execution|execution handoff|connect\b.*\bmoves\b.*\btower\b|handoff\b.*\bmoves\b.*\btower\b|executive council|top\s+\d+\s+(?:ai\s+)?(?:bets?|moves?|initiatives?|use cases?)|business case|solution approach|implementation plan|how (?:would|do|should) we execute|what would the plan look like|create (?:a\s+)?(?:data\s*&?\s*ai|ai|technology|digital)\s+strategy|ai strategy(?:\s+with)?\s+(?:top\s+)?bets?|roadmap|transformation sprint|funding decision package|investment case)\b/i;
