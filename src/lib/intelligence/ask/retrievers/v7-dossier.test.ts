...
function fakeSession(seenRunQueries: string[] = []): SessionRunner {
  return async (fn) =>
    fn(async <R>(sql: string, params: unknown[] = []) => {
      if (
        sql.includes("from intelligence_v7.current_tenant_pack_runs") ||
        sql.includes("from intelligence_v7.tenant_pack_runs")
      ) {
      ...
      if (sql.includes("from intelligence_v7.business_records")) {
        const dimensions = Array.isArray(params[2])
          ? new Set(params[2] as string[])
          : null;
        const rows = [
          {
            ...
            values_json: {
              ...
              decision_needed:
                "Validate source quality, control boundary, and business value before scaling.",
            },
          },
          {
            ...
            values_json: {
              ...
              known_business_pain_points:
                "Manual handoffs, fragmented reporting, and source ownership gaps requiring client validation.",
              ai_opportunity_areas:
                "Legal and Compliance summarization, exception triage, forecasting, and guided workflow assistance",
            },
          },
          {
            ...
            values_json: {
              ...
              signals:
                "aging systems; manual workflows; source quality gaps; owner gaps; control pressure",
              recommended_actions:
                "Use as question guide and fit-check against tenant facts before narrative.",
            },
          },
          {
          ...
          },
          {
            ...
            values_json: {
              ...
              role_in_function:
                "Supports agent-assist routing, claims-status lookup, and escalation.",
              criticality_to_function: "high",
              process_supported: "member service intake and resolution",
              data_exchanged:
                "member identity, claim status, prior authorization status, escalation reason",
            },
          },
          {
            record_name: "Member service escalation queue",
            dimension_key: "v7_11_operations_risk_controls",
            source_file: "V7_11_operations_risk_controls.csv",
            source_artifact_name: "V7_11_operations_risk_controls.csv",
            source_validation_status: "synthetic_demo",
            values_json: {
              process_control_name: "Member service escalation queue",
              process: "member service intake and escalation",
              volume: "high",
              cycle_time: "variable",
              risk_category: "member experience and claims-status accuracy",
              status: "evidence owner required",
              business_impact:
                "Agent assist needs escalation and claims-status controls before scale.",
            },
          },
          {
            ...
            values_json: {
              ...
              business_services_supported:
                "member services, claims analytics, care management reporting",
              data_platform_role:
                "target foundation for AI-ready curated data products",
              operational_constraints:
                "lineage and freshness evidence not fully loaded",
            },
          },
        ];
        return rows.filter(
          (row) => !dimensions || dimensions.has(row.dimension_key),
        ) as R[];
      }
...
describe("retrieveV7DossierSources", () => {
  it("builds a Lakeshore active context dossier for Intelligence synthesis without V-version language", async () => {
    ...
    expect(result.sources.map((source) => source.id)).toContain(
      "context-dimension:ai-initiatives",
    );
    expect(result.sources.map((source) => source.id)).toContain(
      "context-dimension:business-functions",
    );
    expect(result.sources.map((source) => source.id)).toContain(
      "context-dimension:industry-and-market-patterns",
    );
    const sourcePacket = result.sources
      .map((source) => `${source.name}\n${source.id}\n${source.detail}`)
      .join("\n");
    expect(sourcePacket).toMatch(/Shared HR exception triage/i);
    expect(sourcePacket).toMatch(
      /demo-depth planning context until client validated/i,
    );
    expect(sourcePacket).not.toMatch(
      /\bV7\b|v7_|intelligence_v7|V7_|substrate/i,
    );
  });
  ...
  it("selects system and data-estate evidence for analytics and reporting estate questions", async () => {
    ...
    expect(result.sources.map((source) => source.id)).toContain(
      "context-dimension:applications-and-systems",
    );
    expect(result.sources.map((source) => source.id)).toContain(
      "context-dimension:data-assets-and-integrations",
    );
  ...
  it("selects current-state systems and data context for agent-assist questions", async () => {
    ...
    expect(result.sources.map((source) => source.id)).toContain(
      "context-dimension:operations-risk-and-controls",
    );
    expect(result.sources.map((source) => source.id)).toContain(
      "context-dimension:function-system-data-vendor-bridge",
    );
    expect(result.sources.map((source) => source.id)).toContain(
      "context-dimension:infrastructure-and-cloud-estate",
    );
    const packet = result.sources
      .map((source) => `${source.name}\n${source.detail}`)
      .join("\n");
    ...
    expect(packet).toMatch(/Member service escalation queue/i);
  ...
  it("fails closed when the v7 schema is unavailable", async () => {
    const session: SessionRunner = async () => {
      throw new Error(
        "relation intelligence_v7.tenant_pack_runs does not exist",
      );
    };

    await expect(
      retrieveV7DossierSources("What is loaded?", {
        tenantInventoryKey: "lakeshore",
        session,
      }),
