...
const DIMENSION_LABELS: Record<string, string> = {
  ...
  v7_19_service_tower_managed_services_scope:
    "Service tower and managed services scope",
  ...
  try {
    return await session(async (run) => {
      ...
      const sources: AskSource[] = [
        ...
        ...dimensions
          .map((dimension) =>
            buildDimensionSource(dimension, grouped.get(dimension) ?? []),
          )
...
function resolveV7TenantKey(opts: V7RetrievalOptions): string | null {
  ...
  if (!raw) return null;
  const appClientKey =
    appClientKeyForTenant(raw) ?? raw.toLowerCase().replace(/[^a-z0-9]+/g, "");
  const profile = tenantProfileForClientKey(appClientKey);
  return (
    V7_TENANT_BY_APP_CLIENT[profile.appClientKey] ??
    V7_TENANT_BY_APP_CLIENT[appClientKey] ??
    null
  );
}
...
function selectDimensions(query: string): string[] {
  ...
  if (
    /\b(ai|automation|copilot|agent|model|scale|hold|stop|fund|prioriti[sz]e|investment|portfolio|readiness)\b/.test(
      normalized,
    )
  ) {
  ...
  if (
    /\b(hr|human resources|finance|fp&a|treasury|kyriba|legal|procurement|shared services?|back[-\s]?office|function|process)\b/.test(
      normalized,
    )
  ) {
    selected.add("v7_03_org_ownership");
  ...
  if (
    /\b(industry|benchmark|market|peer|trend|pattern|case study|external)\b/.test(
      normalized,
    )
  ) {
  ...
  if (
    /\b(evidence|proof|source|validated|confidence|claim|board|audit|governance|control)\b/.test(
      normalized,
    )
  ) {
  ...
  return Array.from(selected).slice(0, 16);
}
...
function buildDimensionSource(
  ...
  const records = rows
    .slice(0, 8)
    .map((row) => summarizeRecord(dimension, row))
    .filter(Boolean);
  if (records.length === 0) return null;

  return {
    type:
      dimension === "v7_15_industry_market_knowledge_patterns"
        ? "PATTERN"
        : dimension === "v7_23_external_benchmark_market_corpus"
          ? "BENCHMARK"
          : "TENANT",
    name: label,
    id: `context-dimension:${slugify(label)}`,
    confidence:
      dimension === "v7_23_external_benchmark_market_corpus" ? 0.76 : 0.84,
    detail: [
...
function summarizeRecord(dimension: string, row: V7RecordRow): string {
  ...
  const source = [
    ...
    row.source_validation_status
      ? `validation: ${row.source_validation_status}`
      : null,
  ]
    .filter(Boolean)
    .join(", ");
