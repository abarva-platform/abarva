...
function validateAnswer(
  ...
  const namedLeadershipCount = Number(
    dossier.rollups.namedLeadershipCount ?? 0,
  );
  const packet = dossier.composerPacket;
  const primaryLoadedSections = packet.sections.filter(
    (section) =>
      section.dimensionFamily === packet.primaryDimension &&
      section.recordCount > 0,
  );
  const adjacentLoadedSections = packet.sections.filter(
    (section) =>
      packet.relatedDimensions.includes(section.dimensionFamily) &&
      section.recordCount > 0,
  );

  if (
    namedLeadershipCount > 0 &&
    /\bcannot be (characterized|identified)\b/.test(lower)
  ) {
    issues.push("answer_contradicts_loaded_named_leadership");
  }
  if (packet.sections.length === 0)
    issues.push("composer_packet_missing_sections");
  if (primaryLoadedSections.length === 0)
    issues.push("composer_packet_missing_primary_dimension_context");
  if (adjacentLoadedSections.length === 0)
    issues.push("composer_packet_missing_adjacent_dimension_context");
  if (packet.citations.length === 0)
    issues.push("composer_packet_missing_citations");
  if (packet.artifactPlan.length === 0)
    issues.push("composer_packet_missing_artifact_plan");
  if (!packet.answerBoundary.canAnswer.length)
    issues.push("composer_packet_missing_answer_boundary");
  if (/missing source support/.test(lower))
    issues.push("answer_leads_with_engine_gap_language");
  if (/\brows\b/.test(lower)) issues.push("answer_uses_row_count_language");
  if (/\braw id\b|semantic packet|\bpacket\b|home_know|debug\b/.test(lower))
    issues.push("answer_leaks_internal_language");
  if (answer.length < 180)
    issues.push("answer_too_thin_for_consultant_grade_response");
  return issues;
}

function dimensionLabel(value: string): string {
  const labels: Record<string, string> = {
    ai_value_governance: "AI and automation",
    application_systems: "technology systems",
    budget_financials: "budget and financial",
    data_analytics: "data and analytics",
    operations_process: "operations and process",
    organization_leadership: "organization and leadership",
    risk_compliance: "risk and control",
    source_moves_tower: "Source, Moves, and Tower",
    vendor_contracts: "vendor and contract",
  };
...
function topMetricPhrase(dossier: UniversalDimensionDossier): string {
  ...
  if (primarySections.length === 0) {
    return "The available business context supports only a directional answer because the primary evidence area is still thin.";
  }
  return `The strongest business context is in ${primarySections.join(", ")}, with adjacent context used to explain dependencies and boundaries`;
}
...
function relationshipPhrase(dossier: UniversalDimensionDossier): string {
  ...
  if (paths.length === 0)
    return "Operating connections are thin, so the safe answer should emphasize what is known and call out the missing joins.";
...
function gapPhrase(dossier: UniversalDimensionDossier): string {
  if (dossier.gaps.length === 0)
    return "The current business context does not show a specific gap for this question, though final operating choices still need client validation.";
  return `The main missing pieces are ${dossier.gaps
    .slice(0, 3)
...
function questionFocusPhrase(dossier: UniversalDimensionDossier): string {
  ...
  if (
    /\bownership gap|owner gap|ownership gaps|lack.*owner|missing.*owner\b/.test(
      q,
    )
  ) {
  ...
  if (
    /\bintegration|interface|connected|most connected|relationship|dependency|dependencies\b/.test(
      q,
    )
  ) {
  ...
  if (
    /\bcybersecurity|cyber|security\b/.test(q) &&
    /\bwho|lead|leader|budget|spend|cost\b/.test(q)
  ) {
  ...
  if (
    /\banalytics platform|analytics platforms|analytics tool|analytics tools|tools\b/.test(
      q,
    )
  ) {
...
function externalTenantMention(
  dossier: UniversalDimensionDossier,
): string | null {
  ...
  const currentClient = ALL_CLIENTS.find((client) => {
    const aliases = [
      ...
      client.name,
      client.shortName,
      ...(CLIENT_KEY_TO_DB_NAME[client.id] ?? []),
    ]
      .map((alias) => alias.toLowerCase())
      .filter(Boolean);
    return (
      aliases.includes(currentKey) ||
      aliases.includes(currentLabel.toLowerCase())
    );
  });
  const currentAliases = currentClient
    ? [
        ...
        ...(CLIENT_KEY_TO_DB_NAME[currentClient.id] ?? []),
      ]
        .map((alias) => alias.toLowerCase())
        .filter(Boolean)
...
function isGapQuestion(dossier: UniversalDimensionDossier): boolean {
  return /\b(gap|gaps|missing|not loaded|thin|incomplete|coverage)\b/i.test(
    dossier.route.question,
  );
}

function composeGapAnswer(dossier: UniversalDimensionDossier): string {
  const label = tenantLabel(dossier.tenantKey);
  const loadedSections = dossier.sections.filter(
    (section) => section.recordCount > 0,
  );
  const primarySections = loadedSections.filter(
    (section) => section.dimensionFamily === dossier.route.primaryDimension,
  );
  const adjacentSections = loadedSections.filter(
    (section) => section.dimensionFamily !== dossier.route.primaryDimension,
  );
  const gapText = dossier.gaps
    .slice(0, 4)
    .map((gap) => gap.label.toLowerCase().replace(/[.]+$/g, ""));
  const gapLabels =
...
function composeGenericKnowAnswer(dossier: UniversalDimensionDossier): string {
  ...
  return `${boundaryLead}${possessive(label)} ${dimension} picture is strong enough for a current-state executive read. ${focus ? `${focus} ` : ""}${topMetricPhrase(dossier)}. Operationally, the useful signal is how this topic connects to adjacent business areas: ${relationshipPhrase(dossier)} ${gapPhrase(dossier)}${handoff}`;
}
...
function composeOrganizationAnswer(dossier: UniversalDimensionDossier): string {
  ...
  const appOwnershipCount = Number(
    dossier.rollups.applicationOwnershipCount ?? 0,
  );
...
export function composeDossierAnswer(
  dossier: UniversalDimensionDossier,
): DossierAnswer {
  const directAnswer = isGapQuestion(dossier)
