...
const TENANT_ALIAS_PROFILES: readonly TenantAliasProfile[] = [
  ...
  },
  {
    ...
    canonicalKey: 'lakeshore-industries',
    brokerKey: 'lakeshore-industries',
    displayName: 'Lakeshore Holdings',
    industryCode: CLIENT_KEY_TO_INDUSTRY_CODE.lakeshore,
    aliases: ['lakeshore', 'lakeshore-holdings', 'lakeshore holdings', 'lakeshore-industries', 'lakeshore industries'],
  },
] as const;
...
export const TENANT_KEY_ALIASES: Readonly<Record<string, string>> = Object.freeze(
  Object.fromEntries(
    TENANT_ALIAS_PROFILES.flatMap((profile) =>
      Array.from(new Set([profile.appClientKey, profile.brokerKey, ...profile.aliases]))
        .filter((alias) => normalizeTenantAlias(alias) !== profile.canonicalKey)
        .map((alias) => [normalizeTenantAlias(alias), profile.canonicalKey] as const),
    ),
  ),
);
...
export function resolveTenantAlias(value: string | null | undefined): TenantAliasProfile | null {
  if (!value) return null;
  return PROFILE_BY_ALIAS.get(normalizeTenantAlias(value)) ?? null;
}

export function tenantProfileForClientKey(clientKey: ClientKey): TenantAliasProfile {
  return PROFILE_BY_APP_KEY.get(clientKey) ?? PROFILE_BY_APP_KEY.get(DEFAULT_CLIENT_KEY)!;
}

export function appClientKeyForTenant(value: string | null | undefined): ClientKey | null {
...
export function brokerTenantKey(value: string | null | undefined): string | null {
  const profile = resolveTenantAlias(value);
  return profile?.brokerKey ?? value ?? null;
}

export function tenantAliasesFor(value: string | ClientKey | null | undefined): string[] {
  const profile = resolveTenantAlias(value);
...
export function canonicalTenantDisplayName(value: string | null | undefined, fallbackName?: string | null): string {
  const profile = resolveTenantAlias(value);
  if (profile) {
    return canonicalClientDisplayName({ key: profile.appClientKey, name: fallbackName }) ?? profile.displayName;
  }
  const option = getClientOption(value);
  return canonicalClientDisplayName({ key: option.id, name: fallbackName }) ?? option.name;
}

export function tenantIndustryCode(value: string | null | undefined): string | null {
  const profile = resolveTenantAlias(value);
...
export function isLegacyTenantAlias(value: unknown): value is keyof typeof TENANT_KEY_ALIASES {
  return typeof value === 'string' && Object.prototype.hasOwnProperty.call(TENANT_KEY_ALIASES, normalizeTenantAlias(value));
}
