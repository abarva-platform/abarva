...
function candidateClientKey(value: string | null | undefined): ClientKey | null {
  return appClientKeyForTenant(value) ?? (isClientKey(value) ? value : null);
}

function firstResolvedCandidate(
  candidates: Array<{ value: string | null | undefined; source: TenantResolutionSource }>,
...
export async function resolveTenant(input: ResolveTenantInput = {}): Promise<CanonicalTenant> {
  ...
  const requested = input.requestedClient ?? input.surfaceClientKey ?? input.surfaceActiveClient ?? null;
  const explicitRequestProvided = Boolean(requested?.trim());
  const resolved = isLockedRole
    ? firstResolvedCandidate([
      { value: explicitTenantEmail ? inferredFromEmail : null, source: 'email' },
  ...
  if (!resolved && !allowFallback) {
    if (explicitRequestProvided) {
      throw new TenantResolutionError('unknown_tenant', `Unknown tenant alias: ${requested}`);
    }
    throw new TenantResolutionError('missing_tenant', 'No tenant could be resolved from request, session, cookie, or email.');
  }
  ...
  return {
    ...
    displayName: canonicalTenantDisplayName(appClientKey, clientRow?.name) ?? profile.displayName,
    industryCode: clientRow?.industry_code?.trim() || tenantIndustryCode(appClientKey),
