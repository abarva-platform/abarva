...
export function resolveCitationRow(row: EvidenceLedgerRow): ResolvedCitation {
  return {
    ...
    freshness: row.freshness_at,
    confidenceLabel: confidenceLabel(row),
    sourceType: row.source_type,
  };
}

export function confidenceLabel(row: Pick<EvidenceLedgerRow, 'confidence' | 'not_enough_data_flag'>): ResolvedCitation['confidenceLabel'] {
...
function humanCitationText(row: EvidenceLedgerRow): string {
  ...
  if (row.source_type === 'tenant_record') {
    ...
    const rowId = readRef(ref, 'row_id') ?? readRef(ref, 'id') ?? row.artifact_ref;
  ...
  if (row.source_type === 'corpus_pattern') {
    const patternId = readRef(ref, 'corpus_pattern_id') ?? readRef(ref, 'pattern_id') ?? 'pattern';
  ...
  if (row.source_type === 'live_telemetry') {
    const metric = readRef(ref, 'metric') ?? readRef(ref, 'field') ?? 'telemetry';
