...
describe("Evidence Ledger v1", () => {
  ...
  it("serializes pg Date freshness values before building citation text", () => {
    const citation = resolveCitationRow({
      id: "ledger-date",
      client_id: "apexretail",
      surface: "source",
      ...
      claim_text: "BlueYonder pricing is incomplete.",
      source_type: "tenant_record",
      source_ref: { table: "intake", row_id: "scope" },
      source_quote: null,
      freshness_at: new Date("2026-05-30T13:25:00.000Z") as unknown as string,
      confidence: 0.93,
      confidence_basis: "tenant intake field captured during sourcing scope",
      owner_role: "VP Procurement",
      not_enough_data_flag: false,
    ...
    expect(citation.freshness).toBe("2026-05-30T13:25:00.000Z");
    expect(citation.humanText).toContain("refreshed 2026-05-30");
  });

  it("labels no-evidence rows as insufficient", () => {
    expect(
      confidenceLabel({ confidence: 0.99, not_enough_data_flag: true }),
    ).toBe("insufficient");
  });
