export interface DesignBriefTemplateSection {
  key: string;
  title: string;
  required: boolean;
  description: string;
...
export interface DesignBriefRubricCriterion {
  criterion: string;
  rationale: string;
  severity: 'blocker' | 'major' | 'minor';
}

export const designBriefTemplateStructure = {
  sections: [
    {
      ...
      example_completed:
        'Design an AI-assisted operating model that reduces associate task friction without introducing new shadow workflows or store-manager burden.',
    },
    {
      ...
      example_completed:
        'The irreversible decision is whether task orchestration becomes the system of engagement or remains a thin layer on top of existing store tools.',
    },
    {
      ...
      example_completed:
        'Architecture: workflow orchestration layer, associate mobile surface, manager exception console, analytics telemetry feed, and governance controls for policy updates.',
    },
    {
      ...
      example_completed:
        'Options considered: extend existing WFM stack, deploy a workflow-orchestration vendor, or build a thin custom layer around existing mobile tooling.',
    },
    {
      ...
      example_completed:
        'Recommendation: workflow-orchestration vendor with limited custom extensions. Tradeoff: faster time to value versus less full-stack control.',
    },
    {
      ...
      example_completed:
        'Data flows from task systems, store telemetry, and labor planning into orchestration; control points include policy updates, exception routing, and KPI threshold changes.',
    },
    {
      ...
      example_completed:
        'Dependencies: mobile-device readiness, store-ops enablement owner, labor-policy review, and API access to the tasking backbone.',
    },
    {
      ...
      example_completed:
        'Approve the preferred design path and pilot vendor approach; open questions are rollout cadence, field support coverage, and store-format exceptions.',
    },
  ],
  format: 'markdown',
  rendering_notes:
    'Optimize for design sign-off. Keep the brief commercially sharp, explicit about tradeoffs, and free of generic architecture filler.',
} satisfies {
  sections: DesignBriefTemplateSection[];
  format: 'markdown';
  rendering_notes: string;
};

export const designBriefQualityRubric: DesignBriefRubricCriterion[] = [
  {
...
export const designBriefGenerationPromptTemplate = `You are drafting a Design Brief in Markdown.
...
export const designBriefDeliverableType = {
  type_key: 'design_brief',
  title: 'Design Brief',
  description: 'Phase 2 design brief covering solution shape, options, tradeoffs, data flow, and the decision to lock before Execute',
  ...
  template_structure: designBriefTemplateStructure,
  ...
  quality_rubric: designBriefQualityRubric,
  generation_prompt_template: designBriefGenerationPromptTemplate,
