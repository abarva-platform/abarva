export interface DiagnosticTemplateSection {
  key: string;
  title: string;
  required: boolean;
  description: string;
...
export const diagnosticCharterTemplateStructure = {
  sections: [
    {
      ...
      example_completed:
        'Store labor is being consumed by low-value inventory checks, while same-store sales pressure requires associates to spend more time with customers.',
    },
    {
      ...
      example_completed:
        'Sponsor: Priya Patel, VP Store Operations. Decision needed: approve AI-assisted store workflows for a 60-store pilot with labor and change-management funding.',
    },
    {
      ...
      example_completed:
        'Baseline: 23 minutes average daily task-search time per associate, 61% on-time replenishment completion, 2.8% same-store sales drag in pilot cohort stores.',
    },
    {
      ...
      example_completed:
        'Confirmed: fragmented tasking tools create duplicate work. Hypothesis: store-level exceptions are underreported because managers normalize workarounds.',
    },
    {
      ...
      example_completed:
        'Evidence includes sponsor intake turns, two active operating-model patterns, and three peer programs that reduced time-to-task completion after workflow unification.',
    },
    {
      key: 'first_win_scope',
      ...
      example_completed:
        'First win: receiving, replenishment, and exception handling workflows for 60 stores across two formats, excluding full labor-scheduling redesign.',
    },
    {
      ...
      example_completed:
        'Move forward only if the team can show a 15%+ task-time reduction path, clear systems ownership, and sponsor agreement on pilot stores and baseline metrics.',
    },
    {
      ...
      example_completed:
        'Approve the pilot design brief, named pilot stores, and baseline metric pack; unresolved items are labor-allocation guardrails and field enablement ownership.',
    },
  ],
  format: 'markdown',
  rendering_notes:
    'Keep the document concise and sponsor-readable. Mark uncertain points as hypotheses, not facts, and preserve section order exactly.',
} satisfies {
...
export const diagnosticCharterQualityRubric: DiagnosticRubricCriterion[] = [
  {
...
export const diagnosticCharterGenerationPromptTemplate = `You are drafting a Diagnostic Charter in Markdown.
...
export const diagnosticCharterDeliverableType = {
  type_key: 'diagnostic_charter',
  title: 'Diagnostic Charter',
  description: 'Phase 1 diagnostic charter with baselines, hypotheses, bounded first-win scope, and explicit design gate decision',
  ...
  template_structure: diagnosticCharterTemplateStructure,
  required_data_inputs: {
    ...
    topic: ['topic_key', 'failure_modes', 'phase_playbook'],
    graph: ['active_patterns', 'peer_decisions'],
  },
  quality_rubric: diagnosticCharterQualityRubric,
  generation_prompt_template: diagnosticCharterGenerationPromptTemplate,
