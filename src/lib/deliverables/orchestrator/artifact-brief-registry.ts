...
const AMS_RFP_BRIEF: DeliverableArtifactBrief = {
  ...
  decisionToSupport: 'Approve issuance of the RFP and the scope/evaluation/commercial model it commits the client to.',
  recommendedStructure: [
    { key: 'exec_overview', title: 'Executive Overview & Objectives', intent: 'Frame the sourcing objective, drivers, and the decision sought.', groundingMode: 'mixed', expectedEvidenceFamilies: [], expertLatitude: 'Sharpen the strategic rationale and the value at stake.' },
    { key: 'scope_service_towers', title: 'Scope & Service Towers', intent: 'Define in-scope towers, services, and boundaries.', groundingMode: 'governed_facts', expectedEvidenceFamilies: ['service_tower_scope', 'application_inventory'], expertLatitude: 'Structure the tower taxonomy to enterprise norms.' },
    { key: 'current_environment', title: 'Current Environment', intent: 'Describe the application/infrastructure estate vendors will support.', groundingMode: 'governed_facts', expectedEvidenceFamilies: ['application_inventory', 'tooling_landscape'], expertLatitude: 'Organize the estate so bidders can size effort.' },
    { key: 'current_state_baseline', title: 'Current-State Baseline (Volumes)', intent: 'Provide demand baseline (tickets/incidents/changes) for sizing.', groundingMode: 'governed_facts', expectedEvidenceFamilies: ['ticket_volumes', 'incident_problem_change'], expertLatitude: 'Present demand in a way that supports resource-unit pricing.' },
    { key: 'service_requirements', title: 'Service Requirements', intent: 'State functional/operational service requirements per tower.', groundingMode: 'mixed', expectedEvidenceFamilies: ['service_tower_scope'], expertLatitude: 'Bring standard AMS service-requirement structure.' },
    { key: 'sla_kpi', title: 'SLA / KPI Schedule', intent: 'Define service levels, measurement windows, and credits.', groundingMode: 'governed_facts', expectedEvidenceFamilies: ['sla_baseline'], expertLatitude: 'Design a defensible SLA/credit framework.' },
    { key: 'transition', title: 'Transition & Cutover', intent: 'Define transition approach, KT, parallel run, exit-of-incumbent.', groundingMode: 'mixed', expectedEvidenceFamilies: ['transition_constraints'], expertLatitude: 'Provide a standard transition methodology with gates.' },
    { key: 'security_compliance', title: 'Security & Compliance', intent: 'State security, data, and compliance obligations.', groundingMode: 'client_to_complete', expectedEvidenceFamilies: ['security_requirements'], expertLatitude: 'Draft standard control families; client confirms specifics.' },
    { key: 'governance_retained_org', title: 'Governance & Retained Organization', intent: 'Define governance model and retained-org responsibilities.', groundingMode: 'mixed', expectedEvidenceFamilies: ['retained_org_model', 'staffing_baseline'], expertLatitude: 'Bring a standard retained-org / vendor-management model.' },
    { key: 'pricing_commercial', title: 'Pricing & Commercial Model', intent: 'Define the commercial model and the pricing response template.', groundingMode: 'mixed', expectedEvidenceFamilies: ['staffing_baseline', 'run_cost_baseline'], expertLatitude: 'Design a resource-unit + productivity-glidepath template; do NOT disclose incumbent spend.' },
    { key: 'vendor_response_instructions', title: 'Instructions to Bidders', intent: 'Tell vendors exactly how to respond.', groundingMode: 'expert_template', expectedEvidenceFamilies: [], expertLatitude: 'Standard, complete instructions-to-bidders.' },
    { key: 'evaluation_criteria', title: 'Evaluation Criteria & Weights', intent: 'State how proposals will be scored.', groundingMode: 'client_to_complete', expectedEvidenceFamilies: ['evaluation_weights'], expertLatitude: 'Propose a weighted model; client confirms final weights.' },
    { key: 'assumptions_dependencies', title: 'Assumptions & Dependencies', intent: 'State assumptions bidders may rely on.', groundingMode: 'assumption_driven', expectedEvidenceFamilies: [], expertLatitude: 'Capture standard assumptions/dependencies.' },
    { key: 'contracting_terms', title: 'Contracting & Commercial Terms', intent: 'Frame the contracting approach and key terms.', groundingMode: 'client_to_complete', expectedEvidenceFamilies: ['legal_terms'], expertLatitude: 'Draft standard term headings; legal completes positions.' },
  ],
  requiredSections: [
    'exec_overview', 'scope_service_towers', 'current_environment', 'service_requirements',
    'sla_kpi', 'transition', 'pricing_commercial', 'vendor_response_instructions', 'evaluation_criteria',
  ],
  optionalSections: ['current_state_baseline', 'security_compliance', 'governance_retained_org', 'assumptions_dependencies', 'contracting_terms'],
  expectedExhibits: [
    { key: 'tower_scope_map', title: 'Service Tower Scope Map', kind: 'matrix', purpose: 'Towers × services × in/out of scope.', preferredFormat: 'xlsx' },
    { key: 'transition_timeline', title: 'Transition Timeline', kind: 'timeline', purpose: 'Phased transition with gates.', preferredFormat: 'pptx' },
    { key: 'evaluation_model', title: 'Evaluation Model', kind: 'matrix', purpose: 'Criteria × weights × scoring method.', preferredFormat: 'xlsx' },
  ],
  expectedTables: [
    { key: 'application_inventory', title: 'Application Inventory', columns: ['App', 'Criticality', 'Business function', 'Tower'], groundingMode: 'governed_facts', moveToExcelIfWide: true },
    { key: 'sla_schedule', title: 'SLA / KPI Schedule', columns: ['Service', 'Metric', 'Target', 'Window', 'Credit'], groundingMode: 'governed_facts', moveToExcelIfWide: true },
    { key: 'pricing_template', title: 'Pricing Response Template', columns: ['Tower', 'Resource unit', 'Rate', 'Volume', 'Annual'], groundingMode: 'expert_template', moveToExcelIfWide: true },
  ...
  requiredPlaceholders: ['security_specifics', 'evaluation_final_weights', 'legal_positions'],
  requiredClientDecisions: ['evaluation_final_weights', 'legal_positions', 'procurement_timeline'],
...
function key(module: string, archetype: string, deliverableType: string): string {
  return `${module}::${archetype}::${deliverableType}`;
}

const BY_KEY = new Map(REGISTRY.map((b) => [key(b.module, b.useCaseArchetype, b.deliverableType), b]));

/** Module-level default so an un-briefed deliverable still gets a sound consulting baseline. */
function defaultBrief(req: DeliverableIntelligenceRequest): DeliverableArtifactBrief {
  const moduleStructure: Record<DeliverableModule, { key: string; title: string }[]> = {
    moves: [
  ...
  return {
    ...
    recommendedStructure: sections.map((s) => ({
      ...
      groundingMode: /current_state|evidence|baseline|signal/.test(s.key) ? 'governed_facts' : 'mixed',
      expectedEvidenceFamilies: [],
      expertLatitude: 'Bring expert structure and synthesis; add sections if they improve the artifact.',
    })),
    requiredSections: sections.slice(0, Math.min(5, sections.length)).map((s) => s.key),
...
export function hasDedicatedBrief(module: string, archetype: string, deliverableType: string): boolean {
