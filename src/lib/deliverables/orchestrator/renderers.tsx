...
import { PDF_COLORS, PDF_FONTS, PDF_STYLES } from '@/lib/exports-shared/pdf-base';
...
const DOC_STATUS_BODY =
  ...
  DOC_STATUS_STEPS.map((step, i) => `(${i + 1}) ${step.replace(/\.$/, '')}`).join('; ') +
  `. ${DOC_STATUS_CAVEAT}`;
...
function pdfLightTable(columns: string[], rows: string[][]): ReactElement {
  return (
    <PdfView style={{ marginVertical: 8 }}>
      ...
      >
        {columns.map((c, i) => (
          <PdfView key={i} style={{ flex: 1, paddingHorizontal: 6, paddingVertical: 4 }}>
            <PdfText
              style={{
                fontSize: 7,
      ...
      {rows.map((row, ri) => (
        ...
        >
          {columns.map((_, i) => (
            <PdfView key={i} style={{ flex: 1, paddingHorizontal: 6, paddingVertical: 4 }}>
              <PdfText style={{ fontSize: 9, color: PDF_COLORS.HEADER }}>{row[i] ?? ''}</PdfText>
            </PdfView>
          ))}
        </PdfView>
      ))}
...
function tableToPdf(table: RenderableTable): ReactElement {
  if (table.rows.length === 0) {
    return (
      <PdfText style={{ ...PDF_STYLES.body, fontFamily: PDF_FONTS.BODY_ITALIC, color: PDF_COLORS.MUTED }}>
...
function exhibitToPdfBlock(exhibit: RenderableExhibit, index: number): ReactElement {
  const svg = withXmlns(resolveSvgTokens(exhibitSvg(exhibit, index)));
  ...
  try {
    ...
    console.error('[renderDeliverablePdf] exhibit rasterisation failed', exhibit.key, err);
  }
  return (
    <PdfView key={exhibit.key} wrap={false} style={{ marginBottom: 10 }}>
      <PdfText style={PDF_STYLES.h3}>{exhibit.title}</PdfText>
      {imageNode}
      <PdfText style={{ ...PDF_STYLES.body, fontFamily: PDF_FONTS.BODY_ITALIC, color: PDF_COLORS.MUTED, fontSize: 8 }}>
        {exhibit.description}
      </PdfText>
    </PdfView>
  );
...
export function renderDeliverablePdf(doc: RenderableDeliverable): ReactElement<PdfDocumentProps> {
  const inDocTables = doc.tables.filter((t) => t.targetFormat !== 'xlsx');
  ...
  return (
    ...
    >
      <PdfPage size="LETTER" style={PDF_STYLES.page} wrap>
        <PdfText style={PDF_STYLES.eyebrow}>AbarVa · Board-grade deliverable</PdfText>
        <PdfText style={PDF_STYLES.title}>{doc.title}</PdfText>
        {doc.subtitle ? (
          <PdfText style={{ ...PDF_STYLES.meta, fontSize: 12 }}>{doc.subtitle}</PdfText>
        ) : null}
        ...
        {doc.exhibits.length > 0 ? (
          <PdfView style={{ marginTop: 10 }}>
            ...
            {doc.exhibits.map((exhibit, index) => exhibitToPdfBlock(exhibit, index))}
          </PdfView>
        ) : null}
        ...
        {doc.clientCompleteChecklist.length > 0 ? (
          <PdfView style={{ marginTop: 10 }}>
            <PdfText style={PDF_STYLES.h1}>Client-to-Complete Checklist</PdfText>
        ...
        <PdfText
          ...
          render={() => `Confidential — ${doc.clientDisplayName} · ${DOC_STATUS_FOOTER}`}
          fixed
        />
      </PdfPage>
    </PdfDocument>
