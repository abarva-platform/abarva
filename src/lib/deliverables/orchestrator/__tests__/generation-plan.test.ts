...
function planWith(
  sections: DeliverableGenerationPlan["sectionPlan"],
): DeliverableGenerationPlan {
  return {
...
describe("sanitizeGenerationPlan", () => {
  ...
  it("keeps a section grounded by a valid citation/placeholder/assumption as-is", () => {
    const plan = planWith([
      ...
      },
      {
        ...
        title: "B",
        groundingMode: "governed_facts",
        evidenceCitations: [99],
        assumptionsUsed: [],
        placeholders: ["CLIENT TO COMPLETE"],
        rationale: "",
      },
    ] as DeliverableGenerationPlan["sectionPlan"]);
    sanitizeGenerationPlan(plan, amsRfpRequest());
  ...
  it("drops forbidden later-phase sections but keeps the decision-sections", () => {
    ...
    expect(plan.sectionPlan.map((s) => s.key)).toEqual([
      "exec_summary",
      "value_hypothesis",
    ]);
    expect(plan.tableAndExhibitPlan.map((t) => t.key)).toEqual([
      "risk_register",
    ]);
  });
...
describe("MOVES_CHARTER structure (phase discipline)", () => {
  ...
  it("drops extra charter sections before drafting when the architect freelances", () => {
    const structure = getDeliverableStructure("moves", "charter")!;
    const plan = planWith([
      ...structure.sections.map((section) => ({
        key: section.key,
        title: section.title,
        groundingMode: "expert_template" as const,
        evidenceCitations: [],
        assumptionsUsed: [],
        placeholders: [],
      ...
      })),
      {
        ...
        assumptionsUsed: [],
        placeholders: [],
        rationale: "freelanced section that belongs to P3",
      },
    ]);
    sanitizeGenerationPlan(plan, amsRfpRequest(), {
      recommendedStructure: structure.sections,
