...
const PHASE_NUMBERS = [1, 2, 3, 4, 5] as SpecPhaseNumber[];
...
export function buildSeedDeliverableRenderModel(context: Required<SeedRouteContext>): DeliverableRenderModel {
  ...
  const table = buildEvidenceTable(tenant, program, deliverable);
  const crossLinks = buildCrossLinks(tenant, program, deliverable, evidence);
...
function buildCrossLinks(
  ...
  if (previous) {
    links.push({ className: 'previous_next', label: 'Previous in phase', title: previous.title, href: tenantDeliverablePath(tenant, program, previous) });
  }
  if (next) {
    links.push({ className: 'previous_next', label: 'Next in phase', title: next.title, href: tenantDeliverablePath(tenant, program, next) });
  }
