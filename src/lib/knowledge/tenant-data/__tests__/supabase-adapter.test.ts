...
jest.mock("@/lib/data-plane/postgresCompat", () => ({
  ...
  runtimePostgresPoolConfig: jest.fn(
...
describe("SupabaseTenantDataAdapter.listSegments", () => {
  ...
  it("falls back to server-only Postgres when a private schema is not exposed through PostgREST", async () => {
    ...
    stagedResults = [
      {
        data: null,
        error: { message: "Invalid schema: client_apex_retail_private" },
      },
    ];
...
describe("SupabaseTenantDataAdapter.listRecords", () => {
  it("maps rows, lowercases classification, and threads tenant + segment filters", async () => {
    ...
    const out = await makeAdapter().listRecords(
      "apex-retail",
      "kpi_dictionary",
      {
  ...
  it("falls back to public records when the lab private schema is not created yet", async () => {
    ...
    stagedResults = [
      {
        ...
        error: {
          message:
            'relation "client_apex_retail_private.data_inventory_records" does not exist',
        },
      },
      {
        data: [
    ...
    const out = await makeAdapter().listRecords("apex-retail", "it_landscape", {
      limit: 6,
    });
...
describe("SupabaseTenantDataAdapter.getRecord", () => {
  it("returns the mapped record when found", async () => {
    ...
    const out = await makeAdapter().getRecord(
      "apex-retail",
  ...
  it("throws on Supabase error", async () => {
    ...
    await expect(makeAdapter().getRecord("apex-retail", "x")).rejects.toThrow(
      /getRecord failed/,
    );
  });
});
...
describe("SupabaseTenantDataAdapter graph methods (TD-3-WIRE delegation)", () => {
  ...
  it("getGraphNeighborhood delegates to GraphTraversal.getNeighborhood with opts", async () => {
    ...
    const adapter = makeAdapter();
    await adapter.getGraphNeighborhood(
      "apex-retail",
...
describe("SupabaseTenantDataAdapter.listContextChunks", () => {
  ...
  it("uses an `in` filter when recordIds is provided", async () => {
    stagedResults = [{ data: [], error: null }];
    await makeAdapter().chunksByRecord(
      "apex-retail",
      "program_inventory:apex:cdp",
    );
    ...
    expect(calls[0].filters).toEqual([
      { op: "eq", column: "tenant_key", value: "apex-retail" },
      {
        ...
        value: ["program_inventory:apex:cdp"],
      },
    ]);
  });
});
...
describe("SupabaseTenantDataAdapter.chunksByKeyword", () => {
  ...
  it("falls back to public chunks when the lab private schema is not created yet", async () => {
    ...
    const out = await makeAdapter().chunksByKeyword(
      "apex-retail",
      ["SAP S/4"],
      10,
    );
...
describe("SupabaseTenantDataAdapter.chunksByVector", () => {
  it("throws when queryVector is empty", async () => {
    const adapter = makeAdapter();
    await expect(adapter.chunksByVector("apex-retail", [])).rejects.toThrow(
      /non-empty number\[\]/,
    );
    expect(mockPgQuery).not.toHaveBeenCalled();
  });

  it("throws when DATABASE_URL is absent because pgvector runs through Postgres SQL", async () => {
    const adapter = makeAdapter();
    await expect(
  ...
  it("returns ContextChunks ordered by Postgres pgvector score with vectorScore populated", async () => {
    ...
    expect(sql).toContain("embedding_status = 'embedded'");
    expect(sql).toContain("embedding_vector is not null");
    expect(sql).toContain("order by embedding_vector <=> $2::vector");
    expect(params).toEqual(["apex-retail", "[0.1,0.2]", 5]);
    expect(calls).toHaveLength(0);
  });
  ...
  it("clamps limit to MAX_VECTOR_LIMIT (50) and defaults to 10", async () => {
    ...
    expect(mockPgQuery.mock.calls[0][1][2]).toBe(50);

    await adapter.chunksByVector("apex-retail", [0.1]);
    expect(mockPgQuery.mock.calls[1][1][2]).toBe(10);
  });

  it("passes tenant key through to pgvector SQL so tenant A never sees tenant B chunks", async () => {
    process.env.DATABASE_URL = "postgres://example";
    mockPgQuery.mockResolvedValue({ rows: [] });
    const adapter = makeAdapter();
    ...
    expect(mockPgQuery.mock.calls[0][1][0]).toBe("tenant-a");
    expect(mockPgQuery.mock.calls[1][1][0]).toBe("tenant-b");
  });

  it("maps legacy transient statuses defensively while exposing DB-native terminal statuses", async () => {
    process.env.DATABASE_URL = "postgres://example";
    mockPgQuery.mockResolvedValueOnce({
      rows: [
        {
          tenant_key: "apex-retail",
          chunk_id: "chunk:apex:failed:0",
          source_segment_id: "kpi_dictionary",
          source_record_id: "kpi_dictionary:apex:failed",
          source_doc: "kpi_dictionary.csv",
          source_path: "05_kpi_dictionary/kpi_dictionary.csv",
          chunk_index: 0,
          chunk_text: "failed chunk",
          token_count: 30,
    ...
    const out = await makeAdapter().chunksByVector("apex-retail", [0.1]);
    expect(out[0].embeddingStatus).toBe("failed");
  });
});
...
describe("SupabaseTenantDataAdapter.hasPersistedData", () => {
  it("returns true when a segment row exists", async () => {
    stagedResults = [
      { data: [{ segment_id: "enterprise_profile" }], error: null },
    ];
    await expect(makeAdapter().hasPersistedData("apex-retail")).resolves.toBe(
      true,
    );
