...
export function selectTenantEnterpriseSegments(query: string): SegmentId[] {
  ...
  if (
    /\b(technology|technologies|tech|system|systems|platform|cloud|data|analytics|reporting|report|dashboard|warehouse|lakehouse|data\s+lake|data\s+platform|data\s+estate|bi|business\s+intelligence|tableau|cognos|power\s+bi|clarity|caboodle|informatica|reltio|\betl\b|\belt\b|pipeline|ml|ai|vendor|vendors?|supplier|suppliers?|contract|contracts?|renewal|renewals?|application|applications|apps?|criticality|portfolio|sap|s\/4|s4|erp|ibm|mainframe|aws|z\s+workloads?|departure|crew|irops|revenue\s+accounting|baggage|cargo|dora|mttr|lead\s+time|deploy\s+frequency|change\s+failure|sdlc|cobol|edp|snowflake|databricks|cyber|security\s+stack|ai\s+tooling)\b/.test(
      normalized,
    )
  ) {
