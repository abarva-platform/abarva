...
const flagMock = isFeatureEnabled as unknown as jest.MockedFunction<typeof isFeatureEnabled>;
  ...
  return {
    embeddings: {
      create: jest.fn(async () => ({
        data: [{ embedding: new Array<number>(EMBED_DIM).fill(0.001), index: 0 }],
...
describe('Broker · Azure AI Search dispatch', () => {
  ...
  it('routes through Azure when the flag is on; pgvector is not called', async () => {
    ...
    const azureRetriever = jest.fn<ReturnType<AzureSearchTenantContextRetriever>, Parameters<AzureSearchTenantContextRetriever>>(async (args) => [{
