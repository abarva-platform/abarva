  ...
  createDefaultSession:
    ...
    async (
      ...
      const run = async (sql: string): Promise<unknown[]> => {
        ...
        let rows = [
          ...(fakeTables?.[table] ??
            (table === "clients" ? [{ id: "northstar-client" }] : [])),
        ];
...
function makeEmptyAdapter(): TenantDataAdapter {
  return {
    ...
    chunksByVector: () =>
      Promise.reject(new Error("Vector retrieval not enabled.")),
...
describe("tenant enterprise context retrieval", () => {
    ...
    fakeAdapter = {
      ...
      listContextChunks: (_tenantKey, opts) => {
        ...
        return Promise.resolve(
          chunks.filter((item) =>
            requested.has(item.sourceSegmentId as SegmentId),
          ),
        );
      },
    };
  ...
  it("selects both org structure and IT financials for leadership plus budget questions", () => {
    ...
    ).toEqual(expect.arrayContaining(["org_structure", "it_financials"]));
  });

  it("treats direct-report questions as tenant org-structure questions", () => {
    expect(
      selectTenantEnterpriseSegments("Who are my direct reports?"),
    ).toEqual(expect.arrayContaining(["org_structure"]));
  });

  it("routes data-analytics and reporting-tool questions to the IT landscape so loaded systems are named", () => {
    for (const query of [
      "tell me about my current data analytics platform situation",
      "what are the technologies currently in place for data analytics and what is used for reporting?",
      "what BI and reporting tools do we run — Tableau, Cognos, Power BI?",
      "what is our data warehouse and lakehouse estate?",
    ]) {
      expect(isTenantEnterpriseQuestion(query)).toBe(true);
      expect(selectTenantEnterpriseSegments(query)).toEqual(
        expect.arrayContaining(["it_landscape"]),
      );
    }
  });
  ...
  it("treats C-level business leader questions as tenant org-structure questions", () => {
    expect(
      selectTenantEnterpriseSegments("Who is my C level leaders in business?"),
    ).toEqual(expect.arrayContaining(["org_structure"]));
  });
  ...
  it("normalizes SkyHarbor app aliases to the loaded skyharbor-air tenant key before chunk lookup", async () => {
    ...
    fakeAdapter = {
      ...
      listContextChunks: (tenantKey, opts) => {
        ...
        const requested = new Set(opts?.segmentIds ?? []);
        if (tenantKey !== "skyharbor-air" || !requested.has("it_financials"))
          return Promise.resolve([]);
        return Promise.resolve([
          chunk(
            "it_financials",
  ...
  it("adds a graph-backed direct-reports source for the active profile across clients", async () => {
    const active = person(
      "person:meridian:anita-krishnamurthy",
    ...
    const cmio = person(
      "person:meridian:jennifer-wexler",
      "Dr. Jennifer Wexler",
      {
    ...
    fakeAdapter = {
      ...fakeAdapter,
      listGraphNodes: (_tenantKey, kind) =>
        Promise.resolve(kind === "person" ? [active, cmio, infra] : []),
      listGraphEdgesForNode: (_tenantKey, nodeId, direction) => {
        if (nodeId !== active.nodeId || direction !== "incoming")
          return Promise.resolve([]);
        return Promise.resolve([
    ...
    const sources = await retrieveTenantEnterpriseSources(
      ...
      {
        activePersonGraphNodeId: active.nodeId,
        activePersonDisplayName: active.title,
      },
    );
    const directReports = sources.find((source) =>
      source.id.includes(":direct_reports:"),
    );

    expect(directReports?.detail).toContain(
      "This is an in-domain tenant org-structure lookup.",
    );
  ...
  it("falls back to org-structure chunks when graph nodes are unavailable", async () => {
    ...
    const sources = await retrieveTenantEnterpriseSources(
      ...
      "Who are my direct reports?",
      {
        activePersonGraphNodeId: "person:meridian:anita-krishnamurthy",
        activePersonDisplayName: "Dr. Anita Krishnamurthy",
      },
    );
    const directReports = sources.find((source) =>
      source.id.includes(":direct_reports:"),
    ...
    expect(directReports?.detail).toContain(
      "Linda Howard — VP Enterprise Architecture — scope: shared",
    );
    expect(directReports?.detail).toContain(
      "Wei Zhang — VP Infrastructure & Cloud — scope: shared",
    );
  });
  ...
  it("adds a parsed C-level business leader source from executive org chunks", async () => {
    ...
    const sources = await retrieveTenantEnterpriseSources(
      "meridian-health",
      "Who is my C level leaders in business?",
    );
    const cLevel = sources.find(
      (source) => source.id === "meridian-health:c_level_business_leaders",
    );

    expect(cLevel?.detail).toContain(
      "Dr. Elaine Morales — President & Chief Executive Officer",
    );
    expect(cLevel?.detail).toContain("David Park — Chief Financial Officer");
    expect(cLevel?.detail).not.toContain(
  ...
  it("normalizes legacy Apex aliases before chunks reach the Sentinel prompt", async () => {
    fakeAdapter = {
      ...
      listContextChunks: (_tenantKey, opts) => {
        ...
        return Promise.resolve([
          chunk(
            "enterprise_profile",
            "Asterline Retail Group is the active tenant. Asterline Retail operates specialty banners.",
          ),
        ]);
      },
    };
    ...
    const sources = await retrieveTenantEnterpriseSources(
      "apex-retail",
      "What do you know about my company profile?",
    );
    const detail = sources.map((source) => source.detail).join("\n");
    ...
    fakeAdapter = {
      ...
      listContextChunks: (_tenantKey, opts) => {
        ...
        return Promise.resolve([
          chunk(
            "enterprise_profile",
            "Meridian Health and First Capital Financial are canonical demo labels.",
          ),
        ]);
      },
    };
  ...
  it("adds Northstar structured application rows before Sentinel reaches for industry-typical clinical systems", async () => {
    ...
    expect(detail).toContain(
      "Answer app-portfolio and criticality questions from these rows",
    );
  ...
  it("adds SkyHarbor DORA scorecard rows for engineering productivity questions", async () => {
      ...
      enterprise_context_chunks: [
        {
          ...
          chunk_text:
            "SkyHarbor record in S09_ENGINEERING_PRODUCTIVITY. Key facts: scorecard_id=SHA-DORA-001; lead_time_for_change_hours=8; deploy_frequency_per_week=25; MTTR_hours=1.5; change_failure_rate_pct=4.",
        },
        {
          ...
          source_doc: "source_uploads/dora_productivity_baseline.csv",
          chunk_text:
            "SkyHarbor record in S09_ENGINEERING_PRODUCTIVITY. Key facts: scorecard_id=SHA-DORA-003; lead_time_for_change_hours=80; deploy_frequency_per_week=3; MTTR_hours=12; change_failure_rate_pct=18.",
        },
      ],
    });
    ...
    expect(sources.map((source) => source.id)).toContain(
      "skyharbor-air:structured:engineering_productivity",
    );
    expect(detail).toContain(
      "SHA-DORA-001 · Mobile Digital · lead_time 8h · deploy_frequency 25/week",
    );
    expect(detail).toContain(
      "SHA-DORA-003 · Mainframe Core · lead_time 80h · deploy_frequency 3/week",
    );
    expect(detail).toContain(
      "Use these exact scorecard IDs and metrics before saying DORA",
    );
  });
  ...
  it("returns explicit 0.99 structured facts for top-app criticality questions", async () => {
    ...
    const sources = await retrieveTenantStructuredFacts(
      "northstar-clinical",
      "Top 5 apps by criticality, name them",
    );

    expect(sources).toHaveLength(1);
    expect(sources[0]?.confidence).toBe(0.99);
    expect(sources[0]?.detail).toContain("NST-APP-234");
    expect(sources[0]?.detail).toContain(
      "Do not substitute industry-typical provider EHR",
    );
  });
  ...
  it("returns explicit 0.99 structured facts for vendor renewals in the next six months", async () => {
    ...
    const sources = await retrieveTenantStructuredFacts(
      "northstar-clinical",
      "Name 3 most-exposed vendor renewals in the next 6 months",
    );
    const detail = sources.map((source) => source.detail).join("\n");
  ...
  it("does not fabricate structured rows for no-match queries", async () => {
    ...
    await expect(
      retrieveTenantStructuredFacts(
        "northstar-clinical",
        "How should we think about AI strategy?",
      ),
    ).resolves.toEqual([]);
  });
