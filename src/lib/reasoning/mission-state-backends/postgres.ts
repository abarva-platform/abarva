/**
 * Postgres mission-state backend.
 *
 * Persists mission completion / dismissal entries to the
 * `reasoning_mission_states` table (migration 20260428190000) using the `pg`
 * driver with a connection string from `DATABASE_URL`. Wired by
 * `mission-state-init.ts` only when both
 ...
 *
 * Contract notes (from {@link MissionStateBackend}):
 ...
 *   - `write` is an UPSERT keyed on `mission_id` because the in-memory
 *     mutation happens first and idempotent re-marks need to win the row.
 *
...
function defaultExecutorFactory(): PostgresSqlExecutor {
  ...
  if (!url) {
    throw new Error('[mission-state-backends/postgres] DATABASE_URL is required');
  }
...
const UPSERT_SQL = `
  INSERT INTO reasoning_mission_states (
    mission_id, status, timestamp, note
  ) VALUES (
    $1, $2, $3, $4
  )
...
const DELETE_SQL = `
  DELETE FROM reasoning_mission_states
   WHERE mission_id = $1
`;

export class PostgresMissionStateBackend implements MissionStateBackend {
  ...
  async write(state: {
    ...
    try {
      ...
      console.warn(
        '[mission-state-backends/postgres] write failed',
  ...
  async clear(id: string): Promise<void> {
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        '[mission-state-backends/postgres] clear failed',
        err instanceof Error ? err.message : String(err),
      );
    }
  }
...
export function createPostgresMissionStateBackend(
  factory?: PostgresSqlExecutorFactory,
): PostgresMissionStateBackend {
  return new PostgresMissionStateBackend(factory);
}
