import type { AgentGroundingAgent, AgentGroundingCase, AgentGroundingTenant } from './types';
...
const AGENT_LANES: Record<AgentGroundingAgent, string> = {
  sentinel: 'Sentinel explains industry context, corpus pattern context, and tenant-safe reasoning.',
  atlas: 'Atlas explains Tower portfolio confidence, value commitment, risks, gaps, and next moves in executive language.',
  nexus: 'Nexus shapes initiatives into fundable Moves with a corpus pattern, gates, value hypotheses, and unsafe-to-fund conditions.',
  source: 'Source explains vendor, sourcing, BAFO, contract, audit, validation, and value-protection decisions with corpus pattern context.',
  steward: 'Steward explains readiness, loaded context, governed uploader status, and what agents may safely answer next.',
};
...
const TENANT_CONTEXT: Record<AgentGroundingTenant, string> = {
  ...
  'first-capital': [
    'First Capital Financial is the banking tenant.',
    'Known context includes lending, digital banking, examiner expectations, model risk, NIM, and banking compliance.',
    'Do not borrow retail, healthcare, or airline facts when scoped to First Capital.',
  ].join(' '),
};
...
export function buildOpenAiGroundingMessages(testCase: AgentGroundingCase): OpenAiGroundingMessage[] {
  return [
    {
      role: 'system',
      content: [
...
function buildOutputChecklist(testCase: AgentGroundingCase): string[] {
  ...
  if (testCase.expected.requiresCorpusContext) {
    checklist.push('Include a line that starts "Pattern lens / industry context:" and uses the word "pattern".');
  }
  ...
  if (testCase.expected.requiresDataGap) {
    checklist.push('Include a line that starts "Status: not loaded / cannot verify from current context. Missing:".');
  }
