...
describe("agent response shape", () => {
  it("removes raw markdown emphasis without losing readable text", () => {
    expect(
      stripChatMarkdownFormatting(
        "**Demand forecasting** is highest. __Missing__: SKU margins.",
      ),
    ).toBe("Demand forecasting is highest. Missing: SKU margins.");
  });
  ...
  it("Strategic Moves Originate (legacy /strategic-moves/new) passes Brief B prose through unchanged", () => {
    ...
    expect(shaped).not.toMatch(
      /^\s*- (?:Why|Evidence|Missing|Next|Question|Choose):/m,
    );
  ...
  it("scrubs internal evidence plumbing terms from Tower copy", () => {
    ...
    expect(shaped).not.toMatch(
      /substrate|canonical value pattern|retrieved corpus chunk/i,
    );
  ...
  describe("Intelligence surface — Brief A prose preservation (INT-VOICE.STRAT-2026-05-10e)", () => {
    ...
    it("scrubs internal retrieval language and ids from visible advisor prose", () => {
      ...
      expect(shaped).not.toMatch(
        /worldview corpus|worldview:W|sig:fcfi|fcfi-data-gov-2026/i,
      );
  ...
  it("only strips raw markdown during streaming and preserves full content until final compaction", () => {
    const streaming = shapeStreamingAgentTextForSurface(
      "/tower",
      "**Apex Retail** has three signals. More text is still arriving.",
    );
  ...
  it("preserves word boundaries when compacting model HTML markup", () => {
    ...
    expect(shaped).not.toContain(
      "choose an action, compare options, or ask for evidence",
    );
  });
  ...
  it("preserves word boundaries during streaming markup repair", () => {
    ...
    expect(streaming).toBe(
      "clinical AI and active clinical work scored at 87 and is in-flight at 82.",
    );
  });
  ...
  it("removes raw use-case ids during streaming", () => {
    ...
    expect(streaming).toBe(
      "the cited pattern is relevant when the data is available.",
    );
  });
  ...
  describe("Source surface — Brief C prose preservation (VOICE.STRAT-2026-05-10f)", () => {
    ...
    it('does not promote a Brief C "I don\'t have visibility into..." caveat into a "- Missing:" bullet', () => {
      ...
      expect(shaped).not.toMatch(/^\s*- Evidence:/m);
      expect(shaped).toContain(
        "I don't have visibility into Apex's current AI tooling spend",
      );
    });

    it("keeps an interior recommendation paragraph when Source prose exceeds the shared budget", () => {
      const raw = [
        "Three credible vendor paths are worth a real shortlist, and I would keep the comparison intact.",
        "",
        "Northstar Analytics is the strongest first option because its assortment planning references are deep, its specialty-retail implementation pattern is mature, and the operating model maps cleanly to a multi-banner environment. The trade-off is a heavier implementation motion, so I would validate internal capacity before treating it as the default.",
        "",
        "Beacon Intelligence is the credible second option. It is not as broad as Northstar, but the evidence suggests a partner that will tune harder around the merchandising workflow, which matters if the business wants a vendor that will adapt rather than impose a generic planning model. This is the paragraph a paragraph budget must never silently remove.",
        "",
        "Cedar Forecasting is the credible third option. It combines demand forecasting and assortment signals well enough to belong in the competitive bid, but I would not make it the sole finalist without deeper reference checks.",
        "",
        "I would drop the horizontal platform suites for this motion because they solve too much outside the specific assortment problem and would make the evaluation less precise.",
      ].join("\n");
      ...
      expect(shaped).toContain(
        "Northstar Analytics is the strongest first option",
      );
      expect(shaped).toContain(
        "Beacon Intelligence is the credible second option",
      );
      expect(shaped).toContain(
        "Cedar Forecasting is the credible third option",
      );
      expect(shaped).toContain("I would drop the horizontal platform suites");
      expect(shaped.indexOf("Northstar Analytics")).toBeLessThan(
        shaped.indexOf("Beacon Intelligence"),
      );
      expect(shaped.indexOf("Beacon Intelligence")).toBeLessThan(
        shaped.indexOf("Cedar Forecasting"),
      );
    });
  });
  ...
  describe("Strategic Moves surface — Brief B prose preservation (VOICE.STRAT-2026-05-10f)", () => {
    it("does not inject the structured-bullet template into a Brief B Sentinel-handoff pickup", () => {
      ...
      expect(shaped).toContain(
        "Picking up from your Intelligence conversation",
      );
    ...
    it("passes through on programs-detail surface (Move detail / phase / evidence)", () => {
      const raw =
        "I'd push back on advancing to charter — your sponsor structure isn't right yet, and three peer cases stalled in months 6-9. High confidence on this one.";
      const shaped = shapeAgentResponseForSurface("programs-detail", raw);

      expect(shaped).not.toMatch(
        /^\s*- (?:Evidence|Missing|Next|Question|Why|Choose):/m,
      );
  ...
  describe("shouldCompactSurface — guard against expert-posture surface compaction", () => {
    ...
    describe("Atlas CXO-quality audit invariants", () => {
      ...
      it("never duplicates the same sentence in both Evidence: and Missing: slots (collision fix)", () => {
        ...
        const evidenceLine = shaped
          .split("\n")
          .find((line) => line.startsWith("- Evidence:"));
        const missingLine = shaped
          .split("\n")
        ...
        if (evidenceLine && missingLine) {
          const evidenceBody = evidenceLine
            .replace(/^- Evidence:\s*/, "")
            .trim();
      ...
      it('never produces a naked "Xth percentile" — formatPercentile requires metric + cohort + sample size', () => {
        ...
        expect(labeled).toBe(
          "18th percentile · adoption · Retail $10B–$50B cohort · n=7",
        );
      ...
      it("formatPercentile produces correct ordinal suffixes (1st/2nd/3rd/4th/11th/21st)", () => {
        ...
        expect(formatPercentile({ ...ctx, value: 1 })).toContain(
          "1st percentile",
        );
        expect(formatPercentile({ ...ctx, value: 2 })).toContain(
          "2nd percentile",
        );
        expect(formatPercentile({ ...ctx, value: 3 })).toContain(
          "3rd percentile",
        );
        expect(formatPercentile({ ...ctx, value: 4 })).toContain(
          "4th percentile",
        );
        expect(formatPercentile({ ...ctx, value: 11 })).toContain(
          "11th percentile",
        );
        expect(formatPercentile({ ...ctx, value: 12 })).toContain(
          "12th percentile",
        );
        expect(formatPercentile({ ...ctx, value: 13 })).toContain(
          "13th percentile",
        );
        expect(formatPercentile({ ...ctx, value: 21 })).toContain(
          "21st percentile",
        );
        expect(formatPercentile({ ...ctx, value: 42 })).toContain(
          "42nd percentile",
        );
      });
