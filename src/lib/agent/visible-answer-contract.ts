...
export const VISIBLE_ANSWER_CONTRACT_PROMPT = [
  ...
  '- No session-history phrases such as "same answer", "same answer as before", "same answer as the last four turns", "last three times", "earlier turns", or "this session"; answer the current question as if it stands alone.',
...
const VISIBLE_ANSWER_BANNED_PATTERNS: Array<{ id: string; pattern: RegExp }> = [
  ...
  },
  {
    ...
    pattern:
      /\b(?:same answer(?: as(?: before| the last\s+\w+\s+(?:turns?|times))?)?|answer (?:is|hasn'?t changed).*last\s+\w+\s+(?:turns?|times)|last\s+\w+\s+(?:turns?|times)|earlier turns|previous conversation|this session)\b/i,
  },
  {
