...
| 412-app portfolio CSV supporting source | supporting | 412 | repo_supporting_evidence_path | Supporting source only. It is a useful in-scope application subset, but it has fewer rows than the selected estate and is packaged so deployed proof can evaluate it without a local Downloads dependency. |
