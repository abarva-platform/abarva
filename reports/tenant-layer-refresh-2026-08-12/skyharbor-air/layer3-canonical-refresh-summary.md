...
| Unregistered files in active root | 10 |

## Required checks

| Check | Result | Detail |
| --- | --- | --- |
| Every canonical object has declared identity | `FAIL` | 10 file(s) in the active root are not declared in the template contract: 04_applications_systems.xlsx; 05_data_assets_integrations.xlsx; 06_infrastructure_platforms.xlsx; 09_programs_initiatives.xlsx; 19_data_analytics_platform_maturity.csv; 19_data_analytics_platform_maturity.xlsx; SA08_AI_Benefits_Realization_Usage_Ledger.csv; SA09_AI_Tool_Usage_Feed.csv; SA10_AI_Value_Interview_Evidence.csv; SA11_AI_KPI_Operational_Outcome_Feed.csv |
...
| Relationship rows use canonical relationship types | `NOT_VERIFIED` | 12_relationships carries 3071 row(s); the canonical relationship dictionary lives in intelligence_v6.relationship_types and was not read (no data-plane access in this lane). |
...
| Money, counts, and metrics are deterministic and not model-invented | `FAIL` | 21 conflicting or divergent claim(s); see claim-reconciliation-matrix.csv. |
| Duplicate/conflicting claims are blocked pending reconciliation | `PASS` | 21 claim(s) recorded as blocked in the governed package blocked_claims.csv. |
...
| `02_org_ownership.csv` | `02_org_ownership.csv` | yes | 38 | 0 |
...
| `12_relationships.csv` | `12_relationships.csv` | yes | 3071 | 0 |
