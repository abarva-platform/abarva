...
| Source | 45% | partially_migrated_legacy_routes_fenced | Required for Knowledge handoff; not certified end-to-end | Source fixture views and event-instance fixtures remain in code | Source write seams are tenant-guarded; legacy fixture-specific routes are regression-proven unavailable for the governed tenant where directly targeted | Certify remaining Source runtime routes and migrate/disable operational state paths that are not yet proven on the Airline data plane. |
