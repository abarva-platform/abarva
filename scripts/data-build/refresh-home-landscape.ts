...
/**
 * The canonical types Home owns — the enterprise landscape. Ownership is "who answers questions
 * about this", so application_system sits here even though Source also reads it for contract
 * scope: Home owns the estate, Source borrows it.
 */
const HOME_DIMENSIONS: Array<{ objectType: string; key: string; label: string }> = [
  { objectType: "tenant_profile", key: "enterprise_profile", label: "Enterprise profile" },
  { objectType: "business_function", key: "business_functions", label: "Business functions" },
  { objectType: "org_owner", key: "org_ownership", label: "Organisation and ownership" },
  { objectType: "workforce_role", key: "workforce_roles", label: "Workforce and roles" },
  { objectType: "application_system", key: "applications", label: "Applications and systems" },
  { objectType: "data_asset_or_integration", key: "data_assets", label: "Data assets and integrations" },
  { objectType: "infrastructure_platform", key: "infrastructure", label: "Infrastructure platforms" },
  { objectType: "platform_maturity_assessment", key: "platform_maturity", label: "Platform maturity" },
  { objectType: "risk_or_control", key: "risks_controls", label: "Risks and controls" },
];

const GENERATOR_VERSION = "home-landscape-projector/v1";
        ...
        for (const [i, d] of dims.entries()) {
          await client.query(
            ...
            [
              ...
              JSON.stringify({ buildVersion: BUILD_VERSION, inputSourceVersion: INPUT_SOURCE }),
              i,
            ],
          );
