...
on conflict (client_id, solution) do update set
  name = excluded.name,
...
on conflict (engagement_id, module_key) do update set
...
function build(args) {
  ...
  const sql = [
    ...
    "-- Idempotent upserts only; does not delete existing Move records.",
    "begin;",
  ...
  const summary = {
    ...
    proof_checks: {
      ...
      idempotent_upserts: /on conflict/i.test(sql),
