...
"""Adapt the SP04 data/BI/ETL source family into ECL context rows.

Local proof only. The source family is intentionally summarized at
function-platform-technology-workload grain; it captures volumetrics without
asking a client to enumerate every report, ETL package, stored procedure, or
script as a separate row.
"""
...
DEFAULT_OUT_DIR = ROOT / "outputs/ecl-client-intake-data-bi-etl-proof"
DEFAULT_TENANT_KEY = "meridian-health"
DEFAULT_ASSESSMENT_ID = "client-intake-data-bi-etl-proof"
DEFAULT_SOURCE_DATE = "2026-08-23"
FAMILY = "SP04_Data_BI_ETL"
METRICS = [
    {
        "metric_key": "workload_count",
        "metric_name": "Workload count",
        "definition": "Count of BI, analytics, ETL, report, script, or data workload items at the declared segment grain.",
        "unit": "count",
        "directionality": "neutral",
        "cadence": "point_in_time",
        "aggregation_rule": "sum",
        "source_field": "workload_count",
    },
    {
        "metric_key": "active_user_count",
        "metric_name": "Active user count",
        "definition": "Active users recorded for the function-platform-technology-workload segment.",
        "unit": "users",
        "directionality": "neutral",
        "cadence": "point_in_time",
        "aggregation_rule": "sum",
        "source_field": "active_user_count",
    },
    {
        "metric_key": "data_volume_tb",
        "metric_name": "Data volume",
        "definition": "Approximate data volume in terabytes for the segment.",
        "unit": "TB",
        "directionality": "neutral",
        "cadence": "point_in_time",
        "aggregation_rule": "sum",
        "source_field": "data_volume_tb",
    },
]
...
def stable_uuid(*parts: object) -> str:
    ...
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"abarva:ecl-client-data-bi-etl:{seed}"))
...
def build_sql(
    *,
    ...
    source_file_rows = [
        {
            ...
            "tenant_key": sql_text(tenant_key),
            "assessment_id": sql_text(assessment_id),
            "source_type": sql_text("bi"),
            "origin": sql_text(origin),
            "source_owner": sql_text("Data, analytics, BI, and ETL platform owners"),
    ...
    for index, row in enumerate(rows, start=1):
        ...
        source_record_rows.append(
            {
                ...
                "parse_notes": sql_text("client_data_bi_etl_adapter"),
            }
        )
        ...
        object_rows_by_key.setdefault(
            ...
            object_row(
                ...
                attrs={"adapter": "client_intake_data_bi_etl", "review_state": row.get("review_state")},
            ),
        )

        platform_key = f"{row['platform_name']}::{row['technology_name']}::{row['workload_type']}"
        platform_id = stable_uuid("object", tenant_key, assessment_id, "data_platform", platform_key)
        platforms.add(platform_key)
        governance_states.add(row.get("governance_state") or "unknown")
        workload_types.add(row.get("workload_type") or "unknown")
        object_rows_by_key.setdefault(
            ("data_platform", platform_key),
            object_row(
        ...
        relationship_key = (platform_id, "USED_BY", function_id)
        relationship_rows_by_key.setdefault(
            relationship_key,
            {
                ...
                "tenant_key": sql_text(tenant_key),
                "assessment_id": sql_text(assessment_id),
                "from_object_id": sql_text(platform_id),
                "relationship_type": sql_text("USED_BY"),
                "to_object_id": sql_text(function_id),
                "direction_label": sql_text("used by"),
                "source_record_id": sql_text(source_record_id),
                "basis": sql_text("source_recorded"),
                "value_state": sql_text("known"),
    ...
    load_sql_path = out_dir / "client_intake_data_bi_etl_ecl_load.sql"
    summary_path = out_dir / "client_intake_data_bi_etl_summary.json"
    ...
    summary = {
        ...
        "scope": "SP04 data/BI/ETL volumetric adapter; summarized segment counts only",
    }
