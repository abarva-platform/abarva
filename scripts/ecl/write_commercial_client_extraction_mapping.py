...
EXTRACT_GUIDE: dict[str, dict[str, str]] = {
    ...
    "source_vendor_response_tracker.csv": {
        "source_owner": "Strategic sourcing lead or procurement event owner",
        "system_to_pull_from": "Sourcing event workspace, RFP tracker, Ariba event, Coupa sourcing, or controlled response log",
        "one_row_represents": "One submitted vendor response for one sourcing event",
        "why_needed": "Lets Source Compare rank submitted responses from response facts instead of market benchmarks or incumbent contract data.",
        "product_consumers": "Source Compare; Source Events; Tower action context; commercial cubes",
        "required_join_keys": "sourcing_event_key, response_id, contract_id, response_vendor_id",
        "do_not_collect": "Do not collect confidential bidder attachments or legal negotiation notes in this tracker.",
        "acceptable_blanks": "Recommended position may be blank before sourcing review; response_id, vendor, status, price, and rank cannot be blank for Compare.",
        "quality_gate": "Every Compare row must have a submitted response, bidder vendor, baseline price, response price, scorecard row, and source record.",
        "example_export": "Sourcing-event response tracker with response ID, bidder, response status, price, baseline delta, exception count, rank, and review state.",
    },
    "source_pricing_response_lines.csv": {
        "source_owner": "Sourcing analyst or commercial finance",
        "system_to_pull_from": "Pricing response workbook, event bid sheet, Coupa/Ariba pricing grid, or commercial finance normalization file",
        "one_row_represents": "One vendor response price line for one service tower",
        "why_needed": "Supports price-delta and exception analysis without collapsing a response into one opaque total.",
        "product_consumers": "Source Compare; Source Value; Tower commercial review context",
        "required_join_keys": "sourcing_event_key, response_id, contract_id, response_vendor_id, service_tower_id",
        "do_not_collect": "Do not include named staff rates, compensation details, or privileged negotiation commentary.",
        "acceptable_blanks": "Unit of measure may be standardised by the adapter; price and baseline fields cannot be blank.",
        "quality_gate": "Response-line totals must reconcile to the response tracker annual price and baseline price.",
        "example_export": "Pricing grid with vendor response, service tower, annual price, baseline annual price, delta, unit of measure, and review state.",
    },
    "source_evaluation_scorecard.csv": {
        "source_owner": "Sourcing evaluation committee or procurement event owner",
        "system_to_pull_from": "RFP scorecard, evaluation committee worksheet, procurement event scoring export",
        "one_row_represents": "One scored vendor response for one sourcing event",
        "why_needed": "Separates ranked evaluation facts from narrative recommendation and prevents Source from awarding without review state.",
        "product_consumers": "Source Compare; Source Approvals; Tower action context",
        "required_join_keys": "sourcing_event_key, response_id, contract_id, response_vendor_id",
        "do_not_collect": "Do not include named evaluator comments, privileged legal advice, or HR-sensitive rationale.",
        "acceptable_blanks": "Comments may be blank; commercial, delivery, risk, overall score, and rank are required once a response is scored.",
        "quality_gate": "Every scored response must resolve to a submitted response tracker row and bidder vendor; ranks must be unique within event.",
        "example_export": "Scorecard export with commercial, delivery, risk, overall score, rank, scorecard basis, and review state.",
    },
}
