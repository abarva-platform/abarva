...
"""Adapt the SP13 data-flows/integrations source family into ECL context rows.

Local proof only. The source family is source-endpoint to target-endpoint
grain. It preserves partial flow details and unresolved platform references
instead of converting missing intake fields into zeros or invented topology.
"""
...
DEFAULT_OUT_DIR = ROOT / "outputs/ecl-client-intake-data-flows-proof"
DEFAULT_TENANT_KEY = "meridian-health"
DEFAULT_ASSESSMENT_ID = "client-intake-data-flows-proof"
DEFAULT_SOURCE_DATE = "2026-08-23"
FAMILY = "SP13_Data_Flows_Integrations"
...
def stable_uuid(*parts: object) -> str:
    ...
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"abarva:ecl-client-data-flows:{seed}"))
...
def row_is_partial(row: dict[str, str]) -> bool:
    return (
        ...
        or row.get("landing_layer") == "unknown"
        or row.get("consumption_layer") == "unknown"
    )
...
def parse_notes_for(row: dict[str, str]) -> str:
    ...
    if row.get("landing_layer") == "unknown" or row.get("consumption_layer") == "unknown":
        return "layer_classification_unknown"
    if row.get("review_state") in {"needs_follow_up", "partial"}:
        return "needs_follow_up"
    return "client_data_flow_adapter"
...
def build_sql(
    *,
    ...
    source_file_rows = [
        {
            ...
            "tenant_key": sql_text(tenant_key),
            "assessment_id": sql_text(assessment_id),
            "source_type": sql_text("etl"),
            "origin": sql_text(origin),
            "source_owner": sql_text("Integration, interoperability, data engineering, and platform owners"),
            ...
            "metadata_json": sql_json(
                {
                    "adapter": "client_intake_data_flows",
                    "source_room_family": FAMILY,
                    "row_grain": manifest_row["row_grain"],
                    "lookup_families": [CMDB_FAMILY, INFRA_FAMILY],
                    "collection_guidance": "one_row_per_declared_flow_or_integration_not_full_lineage_dump",
                }
            ),
        }
    ]
    ...
    integration_patterns: set[str] = set()
    cadences: set[str] = set()
    landing_layers: set[str] = set()
    consumption_layers: set[str] = set()
    regulated_flow_count = 0
    partial_rows = 0
    known_gap_rows = 0
    unknown_layer_rows = 0

    def add_function(function_name: str, source_record_id: str, row_review_state: str | None) -> str:
        ...
        object_rows_by_key.setdefault(
            ...
            object_row(
                ...
                value_state="known",
                row_review_state=row_review_state,
                partial=False,
                attrs={"adapter": "client_intake_data_flows", "review_state": row_review_state},
            ),
        )
        return function_id

    def add_endpoint(ref: str, source_record_id: str, row: dict[str, str]) -> str:
        object_type = endpoint_type(ref)
        partial = False
        ...
        if object_type == "application":
            ...
            if app:
                display_name = app.get("application_name") or ref
                business_domain = app.get("business_function")
                attrs.update(
                    {
                        "lookup_resolved": True,
                        "vendor_name": app.get("vendor_name"),
                        "application_domain": app.get("application_domain"),
                        "application_subdomain": app.get("application_subdomain"),
                    }
                )
            else:
                partial = True
                ...
                attrs["lookup_resolved"] = False
        elif object_type == "data_platform":
            platform_refs.add(ref)
            platform = platform_lookup.get(ref)
            if platform:
                display_name = platform.get("platform_name") or ref
                business_domain = platform.get("business_function")
                attrs.update(
                    {
                        "lookup_resolved": True,
                        "platform_type": platform.get("platform_type"),
                        "hosting_location": platform.get("hosting_location"),
                    }
                )
            else:
                partial = True
        ...
        object_id = stable_uuid("object", tenant_key, assessment_id, object_type, ref)
        object_rows_by_key.setdefault(
            (object_type, ref),
            object_row(
                object_id=object_id,
                ...
                source_record_id=source_record_id,
                basis="source_recorded" if lookup_resolved else "unknown",
                value_state="known" if lookup_resolved else "unknown",
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if row.get("regulated_data_flag") == "yes":
            regulated_flow_count += 1
        integration_patterns.add(row.get("integration_pattern") or "unknown")
        cadences.add(row.get("cadence") or "unknown")
        landing_layers.add(row.get("landing_layer") or "unknown")
        consumption_layers.add(row.get("consumption_layer") or "unknown")
        ...
        add_function(row["source_function"], source_record_id, row.get("review_state"))
        add_function(row["target_function"], source_record_id, row.get("review_state"))
        source_id = add_endpoint(row["source_object_ref"], source_record_id, row)
        target_id = add_endpoint(row["target_object_ref"], source_record_id, row)
        relationship_rows.append(
            {
                "id": sql_text(stable_uuid("relationship", tenant_key, assessment_id, FAMILY, row["flow_id"])),
                "tenant_key": sql_text(tenant_key),
                "assessment_id": sql_text(assessment_id),
                "from_object_id": sql_text(source_id),
                "relationship_type": sql_text("INTEGRATES_WITH"),
                "to_object_id": sql_text(target_id),
                "direction_label": sql_text("integrates with"),
                "source_record_id": sql_text(source_record_id),
                ...
                "attributes_json": sql_json(
                    {
                        ...
                        "target_object_ref": row.get("target_object_ref"),
                        "source_function": row.get("source_function"),
                        "target_function": row.get("target_function"),
                        "integration_pattern": row.get("integration_pattern"),
                        "landing_layer": row.get("landing_layer"),
                        "consumption_layer": row.get("consumption_layer"),
                        "cadence": row.get("cadence"),
                        "regulated_data_flag": row.get("regulated_data_flag"),
                        "interface_owner": row.get("interface_owner"),
                        "source_basis": row.get("source_basis"),
                        "source_review_state": row.get("review_state"),
                    }
                ),
    ...
    load_sql_path = out_dir / "client_intake_data_flows_ecl_load.sql"
    summary_path = out_dir / "client_intake_data_flows_summary.json"
    ...
    summary = {
        ...
        "endpoint_object": len(application_refs) + len(platform_refs),
        "application_reference": len(application_refs),
        "data_platform_reference": len(platform_refs),
        ...
        "regulated_flow_count": regulated_flow_count,
        "integration_patterns": sorted(integration_patterns),
        "cadences": sorted(cadences),
        "landing_layers": sorted(landing_layers),
        "consumption_layers": sorted(consumption_layers),
        "load_sql": load_sql_path.as_posix(),
        "summary_json": summary_path.as_posix(),
        "scope": "SP13 data-flows/integrations adapter; source-target endpoint grain with partial flow detail preserved",
    }
