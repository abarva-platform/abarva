...
function useCaseObjectId(row) {
  return `AI_USE_CASE:${row.business_case_id}`;
}

export async function validateAiBusinessCaseFieldSurvival({
  ...
  const cube = await readCsv(packageDir, contract.layers.ai_case_cube);
  ...
  const badLineage = lineageRows.filter(
    (row) =>
      ...
      row.canonical_object_id !== useCaseObjectId({ business_case_id: row.source_record_id }),
  );
  record(
    "layer_2_row_lineage",
    ...
    missingLineage.length || badLineage.length
      ...
      : `${lineageRows.length} source rows emitted as canonical AI use cases`,
  );
  ...
  for (const check of contract.relationship_checks) {
    ...
    for (const row of rel.rows) {
      if (
        ...
        counts.set(row.to_object_id, (counts.get(row.to_object_id) ?? 0) + 1);
      }
    }
    const mismatches = [];
    for (const row of layer3.rows) {
      const expected = Number(check.expected_per_case);
      const actual = counts.get(row.canonical_ai_use_case_id) ?? 0;
      if (expected !== actual) {
        mismatches.push(`${row[contract.row_key]} expected ${expected}, got ${actual}`);
      }
    }
    record(
      "layer_3_case_project_relationship_counts",
      mismatches.length === 0,
      mismatches.length ? mismatches.join("; ") : "each case has one implementing project",
    );
  }
...
async function main() {
  ...
  const result = await validateAiBusinessCaseFieldSurvival({
    packageDir,
    contractPath,
  });
