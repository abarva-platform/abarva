...
const migration = fs.readFileSync(
  path.join(root, 'supabase/migrations/20260524173000_source_value_proof_loop_v1.sql'),
  'utf8',
);
assert.match(migration, /CREATE TABLE IF NOT EXISTS public\.source_value_states/);
assert.match(migration, /CREATE TABLE IF NOT EXISTS public\.source_value_chain/);
assert.match(migration, /state_layer IN \('baseline', 'intervention', 'negotiated', 'realized'\)/);
assert.match(migration, /COALESCE\(array_length\(evidence_ledger_ids, 1\), 0\) >= 2/);
assert.match(migration, /state_layer <> 'realized'/);
assert.match(migration, /prevent_realized_source_value_state_update/);
assert.match(migration, /ALTER TABLE public\.source_value_states ENABLE ROW LEVEL SECURITY/);
...
assert.match(service, /Baseline cannot be recorded without at least 2 Evidence Ledger citations/);
assert.match(service, /Realized value cannot be recorded without CFO attestation/);
...
const towerRollup = fs.readFileSync(
  path.join(root, 'src/app/(maestro)/tower/source-portfolio-value/page.tsx'),
  'utf8',
);
assert.match(towerRollup, /Source Portfolio Value/);
assert.match(towerRollup, /CFO attestation queue/);
...
assert.match(sourceCanvas, /Value Proof/);
