...
const EXPECTED_MIGRATION_SHA256 = "af244c31c59697094750955eac278ffcbc90a8a89abf7c8f004d96e91d8017ef";
...
async function main() {
  ...
  try {
    ...
    const forceReapply =
      ...
      process.env.MERIDIAN_HEALTH_LAYER2_SCHEMA_FORCE_REAPPLY === "true";
...
async function schemaReadback(client) {
  ...
  for (const table of SOURCE_TABLES) {
    const count = await client.query(
      `SELECT count(*)::int AS row_count FROM ${quoteIdent(DATABASE_SCHEMA)}.${quoteIdent(table)} WHERE tenant_key=$1 AND test_namespace=$2`,
      [TENANT_KEY, TEST_NAMESPACE],
    );
    sourceRowCounts.push({ table_name: table, row_count: Number(count.rows[0].row_count) });
  }
  const layer2RowCounts = [];
  for (const table of LAYER2_TABLES) {
    const count = await client.query(
      `SELECT count(*)::int AS row_count FROM ${quoteIdent(DATABASE_SCHEMA)}.${quoteIdent(table)} WHERE tenant_key=$1 AND test_namespace=$2`,
      [TENANT_KEY, TEST_NAMESPACE],
    );
    layer2RowCounts.push({ table_name: table, row_count: Number(count.rows[0].row_count) });
  }
  const skyharborRows = [];
  for (const table of LAYER2_TABLES) {
    const count = await client.query(
  ...
  return {
    ...
    layer2_row_counts: layer2RowCounts,
...
function schemaDefects(migrationLedger, readback) {
  ...
  for (const [table, expected] of Object.entries(SOURCE_VOLUME_COUNTS)) {
    const actual = Number(readback.source_row_counts.find((row) => row.table_name === table)?.row_count || 0);
...
- Layer 2 tables: ${proof.readback.tables.length}
