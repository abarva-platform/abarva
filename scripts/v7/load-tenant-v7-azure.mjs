...
const payloadFile = process.env.V7_PAYLOAD_FILE || '';
if (!payloadUrl && !payloadFile) throw new Error('V7_PAYLOAD_URL or V7_PAYLOAD_FILE is required');
...
async function applyEntityReadModel(client) {
  const sql = fs.readFileSync(entitySqlFile, 'utf8');
  await q(client, sql);
}
...
try {
  ...
  const entityCoverage = await q(client, `
    ...
    where tenant_key = any($2::text[]) and contract_version=$1
    order by entity_name, dimension_key
  `, [payload.contractVersion, payload.tenantPacks.map((tenant) => tenant.tenantKey)]);
  ...
  const proofDir = process.env.V7_LOAD_PROOF_DIR;
  if (proofDir) {
    fs.mkdirSync(proofDir, { recursive: true });
    fs.writeFileSync(`${proofDir}/v7-load-summary.json`, `${JSON.stringify({ ok: true, summary }, null, 2)}\n`);
  }
