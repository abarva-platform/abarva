-- Enforce Source cloud-consumption Layer 4 control-action readiness.
--
-- Some environments can have the cloud canonical optimization spine loaded
-- while consumption.sourcing_opportunity_v1 still uses the older legacy
-- readiness mapping. Replacing the view here is forward-only and idempotent:
-- canonical control actions remain control blockers, priced opportunities
-- remain finance-confirmation candidates, and legacy rows are suppressed when
-- the canonical spine has the same opportunity ID.
