-- Restore Source structured contract-evidence persistence when historical
-- migration ledger rows exist but the physical tables are absent.
...
CREATE INDEX IF NOT EXISTS idx_source_contract_evidence_rows_period
  ON public.source_contract_evidence_rows
    (tenant_key, source_event_id, evidence_family, period_start, period_end);
...
GRANT SELECT, INSERT, DELETE ON public.source_contract_evidence_rows TO service_role;
GRANT SELECT, INSERT, DELETE ON public.source_contract_evidence_metrics TO service_role;

COMMENT ON TABLE public.source_contract_evidence_manifests IS
  'Source structured evidence pack manifests for contract optimization.';
COMMENT ON TABLE public.source_contract_evidence_rows IS
  'Tenant-scoped sourcing evidence extracts linked to an accepted manifest.';
COMMENT ON TABLE public.source_contract_evidence_metrics IS
  'Deterministic Source evidence rollups derived from structured rows.';
