...
COMMENT ON POLICY phs_demo_delete ON foundation_v2_phs_demo.canonical_promotion_decisions IS
  'Allows the restricted PHS writer to replay Layer 3 after a failed semantic identity gate; rows remain fenced by tenant, namespace and source release.';
COMMENT ON POLICY phs_demo_delete ON foundation_v2_phs_demo.gate_results IS
  'Allows the restricted PHS writer to clear Layer 3 gate rows during an approved same-release replay.';
