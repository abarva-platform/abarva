-- Migration 014 · Portfolio seed
-- Adds Anand as Maestro, James Park + Maria Delgado as sponsors,
-- Arcturus Wealth Platform + Apex Retail HR ERP as active engagements.
-- Idempotent via ON CONFLICT (graph_node_id) DO NOTHING.
...
VALUES
  ('person_anand_sundaram', 'Anand Sundaram', 'anand+clerk_test@abarva.com',
   'maestro', 'AbarVa', 'frequent_collaborator',
   '{"title": "Founder", "cxo_function": "Strategy", "primary_focus": "Building AbarVa. Running all client engagements. Product + sales + delivery."}'::jsonb),
  ('person_james_park', 'James Park', 'james.park@firstcapital.example',
   'sponsor_cxo', 'First Capital Financial', 'returning_recent',
   '{"title": "CTO", "cxo_function": "IT", "primary_focus": "Wealth platform modernization across the CIO organization."}'::jsonb),
  ('person_maria_delgado', 'Maria Delgado', 'maria.delgado@apexretail.example',
   'sponsor_cxo', 'Apex Retail Group', 'first_meeting',
   '{"title": "CHRO", "cxo_function": "HR", "primary_focus": "HR ERP replacement and back-office rationalization."}'::jsonb)
ON CONFLICT (graph_node_id) DO NOTHING;

-- ── Engagements ──────────────────────────────────────────────────────────
-- Arcturus · Phase 3 · gate awaiting review
...
SELECT
  ...
  'FINSERV', 'MIDDLE_OFFICE', 'OPTIMISE', 'wealth_platform_modernization',
  s.id, m.id, 3, 'active',
...
SELECT
  ...
  'RETAIL', 'BACK_OFFICE', 'OPTIMISE', 'hr_erp_replacement',
  s.id, m.id, 2, 'active',
...
-- Backfill maestro on Meridian (if still null)
UPDATE engagements
