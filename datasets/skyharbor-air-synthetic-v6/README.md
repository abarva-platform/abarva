# Airline Demo V6 Synthetic Intelligence Pack

Generated: 2026-06-28T20:37:04.244Z

This pack was generated from the existing V4 tenant pack into the shared Enterprise Intelligence V6 contract.

## Contract Rules

- Every tenant has the same V6 template files.
- Every V6 file has the same headers as the template pack.
- Every column is documented in `V6_BUSINESS_METADATA_DICTIONARY.csv`.
- Missing facts are explicit `data_thin:*` values and are also summarized in `known_gaps`.
- Industry corpus and expert lenses are advisory context, not tenant fact.

## Summary

- Files: 16
- Rows: 14342
- Data-thin cells: 66490

| File | Family | Rows | Data-Thin Cells |
| --- | --- | ---: | ---: |
| `templates/V6_01_enterprise_profile.csv` | enterprise_profile | 1 | 2 |
| `templates/V6_02_business_functions.csv` | business_function | 55 | 110 |
| `templates/V6_03_org_ownership.csv` | org_ownership | 66 | 276 |
| `templates/V6_04_workforce_personas.csv` | workforce_persona | 18 | 36 |
| `templates/V6_05_applications_systems.csv` | application_system | 944 | 8276 |
| `templates/V6_06_data_assets_integrations.csv` | data_asset_integration | 2220 | 17760 |
| `templates/V6_07_vendors_contracts.csv` | vendor_contract | 320 | 2560 |
| `templates/V6_08_spend_value.csv` | spend_value | 483 | 2533 |
| `templates/V6_09_programs_initiatives.csv` | program_initiative | 300 | 3360 |
| `templates/V6_10_ai_initiatives.csv` | ai_initiative | 347 | 4574 |
| `templates/V6_11_operations_risk_controls.csv` | operations_risk_control | 1510 | 6421 |
| `templates/V6_12_relationships.csv` | relationship | 7198 | 17907 |
| `templates/V6_13_evidence_sources.csv` | evidence_source | 80 | 320 |
| `templates/V6_14_metric_definitions.csv` | metric_definition | 306 | 972 |
| `templates/V6_15_industry_corpus_patterns.csv` | industry_corpus_pattern | 489 | 1343 |
| `templates/V6_16_expert_lenses.csv` | expert_lens | 5 | 40 |
