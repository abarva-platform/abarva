...
test.describe('Source critical path (SkyHarbor)', () => {
  ...
  test('Approvals inbox renders with plain-English items and one action per row', async ({ page }) => {
    await page.goto('/source/approvals');
    await expect(page.getByRole('heading', { name: /waiting on you|Nothing waiting/i })).toBeVisible();
    // every listed item carries exactly one primary action link
    const rows = page.locator('a', { hasText: /Review & approve|Approve now|Review & decide/ });
    if ((await rows.count()) > 0) {
      await expect(rows.first()).toHaveAttribute('href', /\/source\/events\//);
    }
  });

  test('Event canvas renders the stage rail and workspace tabs', async ({ page }) => {
    await page.goto(`/source/events/${SKYH_EVENT}?stage=strategy`);
    await expect(page.getByText('Strategy', { exact: false }).first()).toBeVisible();
  ...
  test('Upload document affordance exists on the EVENT DOCUMENTS shelf (not just the chat paperclip)', async ({ page }) => {
    await page.goto(`/source/events/${SKYH_EVENT}?stage=strategy`);
    await expect(page.getByTestId('source-upload-event-document')).toBeVisible();
  });

  test('Gate decision route refuses approve-with-gaps without a rationale (422)', async ({ page }) => {
    await page.goto('/source/approvals'); // ensures session cookies on context
    const res = await page.request.post(`/api/v1/source/events/${SKYH_EVENT}/gate-decision`, {
      data: {
        stageKey: 'rfp_design',
        satisfiedRequirementKeys: ['rfp_sections_drafted', 'pricing_template'],
  ...
  test('Evidence tab shows the seven-state ramp legend (honesty surface)', async ({ page }) => {
    await page.goto(`/source/events/${SKYH_EVENT}?stage=strategy`);
    await page.getByRole('tab', { name: /Evidence/ }).first().click();
    await expect(page.getByText(/Usable\s*Evidence/i).first()).toBeVisible();
    await expect(page.getByText(/Not Requested/i).first()).toBeVisible();
  });
});
