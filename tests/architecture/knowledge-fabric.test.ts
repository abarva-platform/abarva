...
describe("knowledge fabric", () => {
  ...
  it("dry-runs indexCorpus across 207 primitives without mutating stores", () => {
    delete process.env.KNOWLEDGE_FABRIC_WRITES_ENABLED;
    const primitives = corpusToPrimitives();
    const result = indexCorpus();

    expect(primitives).toHaveLength(207);
    expect(result).toEqual(
      expect.objectContaining({
        corpusSize: 207,
        dryRun: true,
        writesEnabled: false,
        attemptedWrites: 1035,
        writtenWrites: 0,
      }),
    );
    expect(result.results).toHaveLength(1035);
