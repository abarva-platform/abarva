...
describe("knowledge fabric", () => {
  ...
  it("dry-runs indexCorpus across 205 primitives without mutating stores", () => {
    delete process.env.KNOWLEDGE_FABRIC_WRITES_ENABLED;
    const primitives = corpusToPrimitives();
    const result = indexCorpus();

    expect(primitives).toHaveLength(205);
    expect(result).toEqual(
      expect.objectContaining({
        corpusSize: 205,
        dryRun: true,
        writesEnabled: false,
        attemptedWrites: 1025,
        writtenWrites: 0,
      }),
    );
    expect(result.results).toHaveLength(1025);
