...
describe("knowledge fabric", () => {
  ...
  it("dry-runs indexCorpus across 209 primitives without mutating stores", () => {
    delete process.env.KNOWLEDGE_FABRIC_WRITES_ENABLED;
    const primitives = corpusToPrimitives();
    const result = indexCorpus();

    expect(primitives).toHaveLength(209);
    expect(result).toEqual(
      expect.objectContaining({
        corpusSize: 209,
        dryRun: true,
        writesEnabled: false,
        attemptedWrites: 1045,
        writtenWrites: 0,
      }),
    );
    expect(result.results).toHaveLength(1045);
