...
describe("knowledge fabric", () => {
  ...
  it("dry-runs indexCorpus across 208 primitives without mutating stores", () => {
    delete process.env.KNOWLEDGE_FABRIC_WRITES_ENABLED;
    const primitives = corpusToPrimitives();
    const result = indexCorpus();

    expect(primitives).toHaveLength(208);
    expect(result).toEqual(
      expect.objectContaining({
        corpusSize: 208,
        dryRun: true,
        writesEnabled: false,
        attemptedWrites: 1040,
        writtenWrites: 0,
      }),
    );
    expect(result.results).toHaveLength(1040);
