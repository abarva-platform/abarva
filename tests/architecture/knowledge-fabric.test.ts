...
describe("knowledge fabric", () => {
  ...
  it("dry-runs indexCorpus across 206 primitives without mutating stores", () => {
    delete process.env.KNOWLEDGE_FABRIC_WRITES_ENABLED;
    const primitives = corpusToPrimitives();
    const result = indexCorpus();

    expect(primitives).toHaveLength(206);
    expect(result).toEqual(
      expect.objectContaining({
        corpusSize: 206,
        dryRun: true,
        writesEnabled: false,
        attemptedWrites: 1030,
        writtenWrites: 0,
      }),
    );
    expect(result.results).toHaveLength(1030);
