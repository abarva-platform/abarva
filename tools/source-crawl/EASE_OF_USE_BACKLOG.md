...
| A2 | `walk-canvas` and `promote-stage` need an event URL pasted from the previous `create-event` run. | Copy-paste between runs is the canonical AI-tool tax. | Default to "the most recent eventUrl from `runs/`" when `--event` is missing. |
| A3 | No way to chain scenarios. | The natural flow is *create event → fill artifacts → promote* — that's one workflow, not three commands. | `npm run scenario:e2e` that runs `create-event → walk-canvas → promote-stage` end-to-end with a single `result.json`. |
| A4 | Output is technical (JSONL / CSV / per-run audit log). | A founder skimming the run wants "did it work, what did we see, where are the screenshots" — not 800-line JSON. | Emit a `runs/<ts>/summary.md` per run with embedded thumbnails. |
...
|---|---|---|---|
| B1 | **No `Submit deliverable` button on artifact cards.** DocumentTab renders the template markdown via `<pre>` — read-only. | The single biggest "how do I actually do my job?" question for a sourcing lead. Every demo I run, the user looks at the artifact and asks "where do I fill it in?" | Inline editor on the artifact card with `Mark complete` button. The substrate already has `artifact_states` — wire the button to flip status to `complete`. |
...
| B6 | `/source/new` intake doesn't autosave. Dropping out loses the form state. | "I closed the tab and lost my work" is unrecoverable today. | Persist intake state to localStorage on every change; restore on mount. |
