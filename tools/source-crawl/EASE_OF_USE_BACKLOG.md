...
| ~~B5~~ | ~~No onboarding flow for first-time users.~~ **CLOSED** — three-step tour activated via `?tour=1` on `/source`, `/source/new`, and `/source/events/{id}`. Step 1 introduces the portfolio + links to the intake; step 2 explains the intake form with an "awaiting your action" hint; step 3 explains the canvas (artifacts + Mark complete + Gate tab + Promote). Skip / Done dismissal persists `onboarded=true` to localStorage. Empty-state copy on `/source` carries a "Take the tour" entry link that re-arms the flow. | n/a | n/a |
